  (function () {
    ...
    function renderMemberAlbumHome(ov) {
      ...
      var cnt = ov.querySelector('.mah-c');
      if (cnt) cnt.textContent = memAlbum.length + ' 张';
      if (!memAlbum.length) {
      ...
      body.innerHTML = '<div class="mah-grid">' + memAlbum.map(function (a, i) {
        return '<div class="mah-cell" data-i="' + i + '">' +
          '<img src="' + a.src + '" alt="">' +
          (a.note ? '<div class="mah-note">' + escHtml(a.note) + '</div>' : '') +
          '<button type="button" class="mah-del" data-del="' + i + '"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><path d="M6 6l12 12M18 6L6 18"/></svg></button>' +
          '</div>';
      }).join('') + '</div>';
    }
    /* v184：全屏相册（点图标入口进来的界面） */
    function openMemberAlbumHome() {
      ...
      ov.innerHTML = '<div class="mah-head"><button type="button" class="mah-back"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 5l-7 7 7 7"/></svg></button>' +
        '<span class="mah-t">角色相册</span><span class="mah-c">' + memAlbum.length + ' 张</span></div>' +
        '<div class="mah-body"></div>' +
        '<div class="mah-tip">点图看大图 · 右上角 ✕ 删图 · 长按图片可保存</div>';
      ...
      ov.addEventListener('click', function (e) {
        var del = e.target.closest ? e.target.closest('.mah-del') : null;
        if (del) {
          e.stopPropagation();
        ...
        if (cell) openMemberAlbum(parseInt(cell.getAttribute('data-i'), 10) || 0);
      });
    }
