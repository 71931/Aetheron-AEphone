  (function () {
    ...
    function memberEditBody(m) {
      ...
      /* v183：角色相册——测试生图生成的图自动收进来，可全屏翻阅 / 下载 */
      form += '<div class="mem-album-wrap" id="memAlbumWrap">' + memberAlbumHtml() + '</div>';
      form += '</div>';
      ...
      form += '<div class="field"><label>角色ID</label>' +
        ...
        '<div class="mem-cm-row" id="memCardModeRow">' +
        '<button type="button" class="mem-cm-btn' + (memCard.mode === 'all' ? ' on' : '') + '" data-mact="cardmode" data-val="all">全部公开</button>' +
        '<button type="button" class="mem-cm-btn' + (memCard.mode === 'part' ? ' on' : '') + '" data-mact="cardmode" data-val="part">部分公开</button>' +
        '<button type="button" class="mem-cm-btn' + (memCard.mode === 'secret' ? ' on' : '') + '" data-mact="cardmode" data-val="secret">仅私密</button>' +
        '</div>' +
    ...
    function memberBindEdit() {
      ...
      if (qpv) qpv.addEventListener('click', function (e) {
        ...
        openQrViewer(memQrText, { avatar: memAvatar, name: memberQrPreviewName() });
      });
      // 头像点击
    ...
    function qrDrawCard(q, cell, img, name, sub) {
      try {
        ...
        qrRoundRect(ctx, 0, 0, W, W, Math.round(cell * 6)); ctx.fill();
        ctx.fillStyle = '#17171b';
        for (var r = 0; r < n; r++) {
          for (var c = 0; c < n; c++) {
            if (q.isDark(r, c)) ctx.fillRect(pad + c * cell, pad + r * cell, cell, cell);
          }
        }
        /* 中心角色头像：白底圆 + 圆裁头像；头像跟随角色，没头像就用首字 */
        var cx = pad + qz / 2, cy = pad + qz / 2;
        var rr = qz * 0.16;
        ctx.fillStyle = '#ffffff';
      ...
      qrAvatarImage(opt.avatar, function (img) {
        ...
        for (var i = 0; i < levels.length; i++) {
          try {
            ...
            q.make();
            png = qrDrawCard(q, cell, levels[i][1] ? img : null, name, sub);
          } catch (e) { png = ''; }
          if (png) break;
        }
    ...
    function memberQrSync() {
      ...
      qrPrettyPng(memQrText, { avatar: memAvatar, name: memberQrPreviewName(), cell: 5, sub: '扫一扫 · 加我为好友' }, function (png) {
    ...
    function memberQrBigPng(text, opt, cb) {
      ...
      opt = opt || {};
      qrPrettyPng(text, { avatar: opt.avatar, name: opt.name, cell: 9, sub: '扫一扫 · 加我为好友' }, function (png) { cb(png, String(text || '')); });
    }
    ...
    function renderMemberAlbumHome(ov) {
      ...
      if (!memAlbum.length) {
        body.innerHTML = '<div class="mah-empty">相册还是空的<br>回角色编辑页点「测试生图」，生成的图会自动收进来</div>';
        if (st.sel) st.sel = [];
        return;
      }
    ...
    function chatRolePickAlbumOne() {
      ...
      if (!list.length) { toast('相册还是空的：先去「名单 → 角色 → 相册」存几张'); return; }
    ...
    function memberAiNpc() {
      ...
      memberAiAsk('你是世界观设定助手，输出严格 JSON。', user, function (txt, err) {
        ...
        toast('已生成 ' + _addCnt + ' 个 NPC，人设已按十段模板自动填充');
      });
    }
    ...
    function memberNpcListBody() {
      ...
      html += '<div class="mem-field-foot"><span>点「添加 NPC」可选自建，或让 AI 按主角色世界观生成；AI 生成的人设会按十段模板自动填充</span></div>';
      html += '</div></div>';
      return html;
    }
    ...
    function memberNpcEditBody() {
      ...
      html += '<div class="field"><div class="chat-cfg-tip" style="line-height:1.7">十段模板：' + escHtml(MEM_NPC_TPL.split('\n').join('；')) + '</div>';
      html += '<textarea class="chat-mini-input" id="npcLore" rows="9" style="margin-top:8px;resize:vertical" placeholder="点下方「AI 生成人设」，按主角色世界观自动补齐十段">' + escHtml(d.lore || '') + '</textarea>';
      html += '<button type="button" class="primary-btn wb-btn" id="npcAiBtn" style="width:100%;margin-top:8px">AI 生成人设（按十段模板）</button>';
      html += '</div>';
      html += '</div>';
    ...
    function memberNpcEditBind() {
      ...
      if (ai) ai.addEventListener('click', memberNpcAiLore);
      var ta = document.getElementById('npcLore');
      if (ta) ta.addEventListener('input', function () { if (memNpcDraft) memNpcDraft.lore = ta.value; });
      memberEditScroll.onclick = function (e) {
    ...
    /* v198：UI 上已撤掉 AI 生成入口，函数保留（内部仍按十段模板认知 NPC 人设结构）。
       v199：按用户要求把「AI 生成」入口恢复回来（名单页「AI 生成 NPC」+ 编辑页「AI 生成人设」）。
       顺便修掉 v197 的 bug：原来是「先写入 draft.lore，再 memberNpcSyncForm()」，
       而 syncForm 会拿 DOM 里还没更新的 textarea 旧值回写 draft.lore，把刚生成的人设又清成空。 */
    function memberNpcAiLore() {
      if (!memNpcDraft) return;
      memberNpcSyncForm();
      var mainLore = ((memRoleFormCache || {}).lore || '');
      if (!String(mainLore).trim()) { toast('主角色「角色身世」为空，先回去填写再用 AI 生成'); return; }
      toast('AI 正在生成 NPC 人设…');
      var user = '这是主角色的完整人设：\n' + memberNpcMainText() +
        '\n\n请为下面这个 NPC 写一份完整人设，严格按十段结构输出：\n' + MEM_NPC_TPL.replace(/\n/g, '\n') +
        '\nNPC 名字：' + (memNpcDraft.name || '未命名') +
        '\nNPC 简介：' + (memNpcDraft.bio || '（未填）') +
        '\n与主角色的关系：' + (memNpcDraft.rel || '（未填）') +
        '\n\n要求：每段 2~4 句，中文，贴合主角色世界观与关系；不要写解释、不要加前言，直接输出十段正文。';
      memberAiAsk('你是世界观与角色设定助手。', user, function (txt, err) {
        if (err || !txt) { toast(err || 'AI 生成失败'); return; }
        if (memNpcDraft) {
          memberNpcSyncForm();                     /* v198：先收拢 DOM 旧值 */
          memNpcDraft.lore = String(txt).trim();   /* 再写入新生成的人设，避免被覆盖回空 */
          memberNpcRender();
        }
        toast('已生成 NPC 人设');
      });
    ...
    var TIP_TEXT = {
      cardmode: ['扫码能看到什么', '<div class="tips-row"><b>全部公开</b>：资料全显示</div><div class="tips-row"><b>部分公开</b>：只显示你勾选的那几项</div><div class="tips-row"><b>仅私密</b>：只显示名字，看不到人设细节，但仍然能加 TA 聊天</div>'],
    ...
    function chatQrRenderCells() {
      ...
      chatDetailBody.querySelectorAll('.chat-qr-card').forEach(function (cell) {
        ...
        qrPrettyPng(key, { avatar: m.qrAvatar, name: m.qrName, cell: 6 }, function (png) {
          if (!png) return;
    ...
    function chatVoiceApplyMember(conv) {
      try {
        ...
        if (s.roleIdentity && typeof s.roleIdentity === 'object') {
          ...
          s.roleIdentity = { name: conv.name || '', avatar: '', sex: m.gender === 'male' ? '男' : (m.gender === 'female' ? '女' : ''), prompt: _rp, remark: '' };
          _roleChanged = true;
        }
