  (function () {
    ...
    function npcLoreExtractFromText(txt) {
      ...
      if (s >= 0 && e > s) {
        try { return npcLoreMapToArr(JSON.parse(t.slice(s, e + 1))); } catch (e1) {}
      }
      return npcLoreSplit(t);
    }
    ...
    function memberNpcListBody() {
      ...
      }).join('') : '<div class="mem-npc-empty">人设库还是空的，点下面的 ＋ 创建第一个 NPC</div>';
      var html = '<div class="npc-edit">';
      html += '<div class="group-card form-card">';
      html += '<div class="mem-npc-list">' + rows + '</div>';
      /* v201：整页只留一个加号，点它才创建人设 */
      html += '<div class="npc-plus-only"><button type="button" class="npc-plus-big" id="npcPlusBtn" data-mact="npcadd" aria-label="创建 NPC">' +
        ...
        '<span class="npc-plus-tip">点 ＋ 创建人设</span></div>';
      html += '</div></div>';
      return html;
    }
    ...
    function npcAiRun(mode, cur, attempt) {
      ...
      var user = '这是主角色的完整人设：\n' + memberNpcMainText() +
        ...
        '，值为对应段落的文字（150~220 字），不要输出任何解释、不要多余文字。\n';
      var _sub = {};
    ...
    function chatRoleStatusOf(conv) {
      ...
      if (!st || typeof st !== 'object') { st = conv.settings.roleStatus = { affinity: null, fields: null, ts: 0, aiTs: 0 }; }
    ...
    function chatRoleStatusEnsure(conv) {
      ...
      if (!st.fields || typeof st.fields !== 'object') { st.fields = chatRoleStatusLocal(conv); st.ts = Date.now(); }
      return st;
    }
    ...
    function chatRoleStatusRender(busy) {
      ...
      var html = '';
      for (var i = 0; i < ROLE_STATUS_DEF.length; i++) {
        html += '<div class="role-st-row"><span class="role-st-k">' + ROLE_STATUS_DEF[i][1] + '</span><span class="role-st-v">' + escHtml(String((st.fields && st.fields[ROLE_STATUS_DEF[i][0]]) || '—')) + '</span></div>';
      }
      html += '<div class="role-st-row role-st-aff"><span class="role-st-k">对你好感度</span>' +
        '<span class="role-st-v"><span class="role-st-bar"><i style="width:' + aff + '%"></i></span>' +
        '<b>' + aff + '</b> · ' + lv.label + '<em>' + escHtml(lv.desc) + '</em></span></div>';
      var src = '跟随「' + (chatCurrentConv.name || 'TA') + '」· ' + (st.aiTs ? ('上次重算 ' + new Date(st.aiTs).toLocaleTimeString()) : '本地即时推断');
    ...
    function chatRoleStatusEl() {
      ...
      el.innerHTML = '<div class="role-st-mask"></div><div class="role-st-box">' +
        '<div class="role-st-head"><span class="role-st-title">角色状态</span><span class="role-st-sub" id="roleStatusName"></span></div>' +
