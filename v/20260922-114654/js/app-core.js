  (function () {
    ...
    /* ===== v201：报错说人话 =====
       接口、浏览器、TTS 抛出来的英文报错统一翻成中文，并把原文附在后面，方便对着报错改配置。 */
    var ERR_ZH_MAP = [
      [/failed to fetch|networkerror|network error|load failed|network request failed|err_connection|econnrefused|econnreset/i, '网络请求发不出去：断网、接口地址写错，或被浏览器跨域（CORS）拦了'],
      [/err_internet_disconnected|offline|enotfound/i, '设备离线或域名解析失败，连不上这个接口'],
      [/timeout|timed out|etimedout|deadline exceeded|abort/i, '请求超时或中途被中断，接口太久没有返回'],
      [/mixed content/i, '页面是 https，接口地址却是 http，被浏览器直接拦了'],
      [/ssl|certificate|tls|err_cert/i, 'HTTPS 证书校验失败'],
      [/unauthorized|invalid api key|invalid_api_key|incorrect api key|\b401\b/i, '密钥无效或未授权（HTTP 401）：检查 API Key 有没有填错、过期'],
      [/forbidden|\b403\b/i, '被服务器拒绝（HTTP 403）：密钥没这个权限，或触发地区/风控限制'],
      [/\b404\b|model not found|no such model|not_found|does not exist/i, '接口或模型不存在（HTTP 404）：检查 Base URL 末尾与模型名'],
      [/\b429\b|rate limit|too many requests|ratelimit|tpm|rpm/i, '触发限流（HTTP 429）：请求太频繁或额度用尽，等一会儿再试'],
      [/\b400\b|bad request|invalid request|invalid_parameter/i, '请求参数有误（HTTP 400）：模型名、消息格式或参数不被接口接受'],
      [/\b5\d\d\b|server error|bad gateway|service unavailable|internal error|overloaded/i, '接口服务端故障（HTTP 5xx）：不是你这边的问题，稍后重试'],
      [/quota|insufficient|balance|billing|欠费|exceeded your current/i, '账号余额或调用额度不足'],
      [/content filter|content_filter|content_policy|safety|flagged|审核/i, '内容被平台安全策略（审核）拦截'],
      [/context length|maximum context|too many tokens|token limit|max_tokens|context_length/i, '上下文太长，超出这个模型的长度上限'],
      [/unexpected token|invalid json|json parse|not valid json/i, '接口返回的不是合法 JSON：多半被网关、登录页或验证页拦了'],
      [/no choices|choices is empty|empty completion/i, '模型返回了空内容（空回）'],
      [/unsupported|not support/i, '当前模型不支持这种请求方式（例如不支持图片或语音）']
    ];
    /* 单条字符串翻译：命中则「中文（原文：xxx）」，没命中且是中文就原样返回 */
    function errZhOne(raw) {
      var s = String(raw == null ? '' : raw).trim();
      if (!s) return '';
      for (var i = 0; i < ERR_ZH_MAP.length; i++) {
        if (ERR_ZH_MAP[i][0].test(s)) {
          return /[\u4e00-\u9fa5]/.test(s) ? s : (ERR_ZH_MAP[i][1] + '（原文：' + s.slice(0, 160) + '）');
        }
      }
      return /[\u4e00-\u9fa5]/.test(s) ? s : ('未识别的报错（原文：' + s.slice(0, 160) + '）');
    }
    function errZh(e) {
      var raw = '';
      if (e && typeof e === 'object') {
        raw = String(e.message || e.errMsg || e.statusText || e.code || '');
        if (e.status && raw.indexOf(String(e.status)) < 0) raw = String(e.status) + ' ' + raw;
      } else raw = String(e == null ? '' : e);
      return errZhOne(raw) || '未知错误（没有任何错误信息）';
    }
    /* 空回：接口通了但没吐出正文时的统一说法 */
    function errEmptyReply(detail) {
      return '空回：模型返回的内容为空' + (detail ? '｜' + String(detail).slice(0, 160) : '') +
        '｜常见原因：被安全策略拦截 / 上下文太长 / 该模型不擅长这条指令；换句话或换个模型再试';
    }
    ...
    // ===== MCP 应用（v214：服务卡片 + 掌心窗 + 按 Char 授权） =====
    var MCP_GRANTS_KEY = 'ins-mcp-grants';
    var mcpOverlay = document.getElementById('mcpOverlay');
    var mcpServiceList = document.getElementById('mcpServiceList');
    var mcpPeekOverlay = document.getElementById('mcpPeekOverlay');
    var mcpPeekBody = document.getElementById('mcpPeekBody');
    var mcpCharOverlay = document.getElementById('mcpCharOverlay');
    var mcpCharList = document.getElementById('mcpCharList');
    var mcpPickBuf = {};

    var MCP_SVC_ICON = '<svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="5"/><path d="M12 17.2s-4.1-2.7-4.1-5.5A2.7 2.7 0 0 1 12 9.3a2.7 2.7 0 0 1 4.1 2.4c0 2.8-4.1 5.5-4.1 5.5z"/></svg>';

    var PEEK_TOOL_GROUPS = [
      ['看手机（状态 / 截图 / 读屏）', ['linjian_status', 'peek_screen', 'latest_screen', 'get_life_state', 'get_senses_state', 'get_phone_state', 'get_screen_nodes']],
      ['动手（点击 / 输入 / 开 App）', ['open_app', 'tap_text', 'input_text', 'send_phone_command', 'run_sequence', 'run_preset', 'phone_home', 'phone_back', 'phone_recents', 'phone_screen_off', 'list_known_apps', 'save_known_app']],
      ['提醒（通知 / 闹钟 / 天气）', ['send_notification', 'set_alarm', 'get_weather_state', 'send_weather_notification']],
      ['日历 / 共同窗语', ['get_guardian_calendar', 'list_guardian_days', 'add_guardian_day', 'update_guardian_day', 'delete_guardian_day', 'get_window_whisper', 'set_window_whisper']],
      ['TA 的日记', ['create_diary_book', 'list_diary_books', 'rename_diary_book', 'write_diary_entry', 'list_diary_entries', 'read_diary_entry', 'search_diary_entries', 'update_diary_entry', 'delete_diary_entry', 'add_diary_annotation', 'list_diary_annotations']],
      ['归电 / 主动关心', ['get_guidian_state', 'set_guidian_config', 'trigger_guidian', 'mark_guidian_returned', 'get_care_policy', 'set_care_policy', 'active_care_check', 'care_action', 'record_care_event', 'get_care_history']],
      ['专注 / 门禁 / 屏幕休息', ['get_focus_status', 'start_focus_mode', 'end_focus_mode', 'set_focus_plan', 'reply_focus_request', 'screen_break_app', 'end_screen_break', 'extend_screen_break', 'temporary_screen_break_release', 'get_screen_break_state', 'list_screen_break_apps', 'add_screen_break_app']],
      ['到访 / 行动记录', ['record_visit', 'get_last_visit', 'get_visit_history', 'get_visit_stats', 'get_activity_events', 'add_activity_event', 'get_companion_actions']],
      ['小金库 / 外卖助手', ['wallet_takeout_action', 'get_takeout_state', 'suggest_takeout_options', 'create_takeout_plan', 'open_takeout_link', 'prepare_takeout_checkout']]
    ];
    var PEEK_TOOL_COUNT = (function () {
      var n = 0;
      for (var gi = 0; gi < PEEK_TOOL_GROUPS.length; gi++) n += PEEK_TOOL_GROUPS[gi][1].length;
      return n;
    })();

    var MCP_PEEK_INFO = [
      ['服务名', '掌心窗 公开版（linjian-peek-public）'],
      ['版本', 'v0.3.8.8'],
      ['传输', 'MCP over HTTP · JSON-RPC'],
      ['部署', 'Render 一键部署（server + mcp 两个服务）'],
      ['令牌', 'LINJIAN_TOKEN · 手机端和这里填同一个'],
      ['设备 ID', 'android-phone'],
      ['工具数', PEEK_TOOL_COUNT + ' 个']
    ];

    var MCP_PEEK_STEPS = [
      ['① 云端起服务（一次性，约 5 分钟）',
       '点下面的链接，用 GitHub 登录并授权（会跳到 Render 的部署页，照着点最下面那个按钮就行）。它会建两个服务：server（手机连它）+ mcp（我连它）。建好后在 mcp 服务里找 LINJIAN_TOKEN 这个值——没有的话先回上面点「生成一个 Token」，三处都填同一个。',
       'https://render.com/deploy?repo=https://github.com/linzhi-524/linjian-peek-public'],
      ['② 手机装 App 并连上',
       '装公开版 APK（仓库 Actions 里的构建产物）。开 App → 设置页填：服务器地址 = server 域名、Token = 上一步那个、设备 ID = android-phone；再去系统里开无障碍、通知使用权、使用情况访问，回 App 点启动。',
       'https://github.com/linzhi-524/linjian-peek-public/actions'],
      ['③ 回到这里填我的地址',
       '把 mcp 服务的公网地址填到上面的通道地址（形如 https://xxxx.onrender.com/mcp），Token 填同一个 LINJIAN_TOKEN，点「保存」再点「测连通」。',
       'https://你的-mcp-域名/mcp']
    ];

    var MCP_PEEK_TIPS = [
      '两个域名别搞混：手机端填 server 域名；这里（MCP）填 mcp 域名，结尾加 /mcp。',
      'Token 只有一个：LINJIAN_TOKEN。手机端、mcp 服务、这里，三处必须是同一个。',
      'Render 免费实例会休眠，第一次点「测连通」可能要等 30~60 秒冷启动，别急着判死刑。',
      '截图、读屏、点击、输入、门禁、息屏都是敏感能力：只在本人手机、本人服务上用，别把 Token 给陌生客户端。',
      '手机端 App 要活着（别被系统杀后台），否则我这边只能读到服务器上的旧状态。',
      '删日记、删日记本这类不可逆动作，MCP 要求 confirm=true 才会执行，站点这边不会替你点头。'
    ];

    function mcpGrants() {
      var o = null;
      try { o = JSON.parse(dbGet(MCP_GRANTS_KEY)); } catch (e) {}
      if (!o || typeof o !== 'object') o = {};
      if (!Array.isArray(o.peek)) o.peek = [];
      return o;
    }
    function mcpGrantedPeek() { return mcpGrants().peek; }
    function mcpSaveGrants(list) { try { dbSet(MCP_GRANTS_KEY, JSON.stringify({ peek: list })); } catch (e) {} }

    /* 「我的聊天」里的角色：会话优先（显示的就是聊天里那个名字），没会话的联系人补上 */
    function mcpCharEntries() {
      var out = [], seen = {};
      for (var i = 0; i < chatConvs.length; i++) {
        var c = chatConvs[i];
        if (c.hidden) continue;
        var key = String(c.contactId || c.id);
        if (seen[key]) continue;
        seen[key] = 1;
        out.push({ id: key, name: convDisplayName(c) || c.name || '', color: c.color || '#7c5cff',
          avatar: (c.settings && c.settings.roleIdentity && c.settings.roleIdentity.avatar) || '' });
      }
      for (var j = 0; j < chatContacts.length; j++) {
        var ct = chatContacts[j];
        if (!ct || seen[String(ct.id)]) continue;
        seen[String(ct.id)] = 1;
        var cv = contactConvOf(ct);
        out.push({ id: String(ct.id), name: (cv ? convDisplayName(cv) : ct.name) || ct.name || '', color: ct.color || '#ffcc00',
          avatar: (cv && cv.settings && cv.settings.roleIdentity && cv.settings.roleIdentity.avatar) || '' });
      }
      return out;
    }

    function mcpCopy(txt) {
      var t = String(txt || '');
      function done() { if (typeof toast === 'function') toast('已复制：' + (t.length > 16 ? t.slice(0, 16) + '…' : t)); }
      try {
      ...
      } catch (e) { if (typeof toast === 'function') toast('复制失败，长按手动选吧'); }
    }

    /* escHtml 不管引号，属性值要自己转 */
    function mcpAttr(x) { return escHtml(x).replace(/"/g, '&quot;').replace(/'/g, '&#39;'); }

    var MCP_NEXT = '<svg viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"/></svg>';

    function mcpRenderServices() {
      var names = mcpGrantedPeek().map(function (x) { return x.name; });
      mcpServiceList.innerHTML =
        '<div class="settings-item mcp-svc" id="mcpSvcPeek">' +
          '<div class="item-icon">' + MCP_SVC_ICON + '</div>' +
          '<div class="item-body">' +
            '<div class="item-title">掌心窗</div>' +
            '<div class="item-sub">云端 MCP · ' + PEEK_TOOL_COUNT + ' 个工具 · 连着你的安卓手机</div>' +
            '<div class="item-sub mcp-svc-grant">' + (names.length ? ('已授权 ' + names.length + ' 个 Char：' + escHtml(names.join('、'))) : '还没授权给任何 Char') + '</div>' +
          '</div>' +
          '<div class="item-arrow">' + MCP_NEXT + '</div>' +
        '</div>';
      var el = document.getElementById('mcpSvcPeek');
      if (el) el.addEventListener('click', openMcpPeek);
    }

    function mcpRenderPeek() {
      var names = mcpGrantedPeek().map(function (x) { return x.name; });
      var html = '';
      html += '<div class="mcp-topbar" id="mcpPeekCharBar">' +
                '<div class="mcp-topbar-l"><b>选择 Char 使用</b><span>勾上谁，谁在聊天里就能看到并使用这台手机</span></div>' +
                '<div class="mcp-topbar-r"><span class="mcp-badge">' + (names.length ? ('已授权 ' + names.length + ' 个') : '未授权') + '</span>' + MCP_NEXT + '</div>' +
              '</div>';
      if (names.length) html += '<div class="mcp-grantees">已授权：' + escHtml(names.join('、')) + '</div>';

      html += '<div class="mcp-sec"><div class="mcp-sec-t">服务信息</div><div class="group-card">';
      MCP_PEEK_INFO.forEach(function (kv) {
        html += '<div class="mcp-kv"><span class="mcp-k">' + escHtml(kv[0]) + '</span><span class="mcp-v">' + escHtml(kv[1]) + '</span></div>';
      });
      html += '</div></div>';

      /* v214：通道地址（站点连云端 MCP，全靠这一栏） */
      var _gwv = peekGw();
      var _gwst = mcpGwState || (_gwv.url ? ('已保存：' + _gwv.url) : '还没填：先按下面三步把通道接上');
      html += '<div class="mcp-sec"><div class="mcp-sec-t">通道地址（站点连云端 MCP）</div><div class="group-card mcp-gw">' +
        '<div class="mcp-gw-tip">掌心窗是「手机 App + 云端 server + 云端 mcp」三件套：手机把状态和截图推到云端，云端再把能力开给我。站点只要拿到 <b>mcp 服务</b>的公网地址就能用，手机在哪儿都行。</div>' +
        '<div class="mcp-gw-bad">别填错：这里要的是 <b>mcp 服务</b>的地址（结尾 <code>/mcp</code>），不是 server 服务的地址——两个是不同的域名。</div>' +
        '<input class="mcp-input" id="mcpGwUrl" type="text" inputmode="url" placeholder="https://xxxx.onrender.com/mcp" value="' + mcpAttr(_gwv.url) + '">' +
        '<input class="mcp-input" id="mcpGwToken" type="text" placeholder="LINJIAN_TOKEN（手机端填的同一个）" value="' + mcpAttr(_gwv.token) + '">' +
        '<div class="mcp-gw-row"><button class="mcp-btn" id="mcpGwSave" type="button">保存</button><button class="mcp-btn ghost" id="mcpGwTest" type="button">测连通</button><button class="mcp-btn ghost" id="mcpGwAuto" type="button">从云端取最新</button></div>' +
        '<div class="mcp-gw-state" id="mcpGwState">' + escHtml(_gwst) + '</div>' +
        '<div class="mcp-gw-tip">地址和 Token 存在这台手机的浏览器本地（localStorage）。云端隧道偶尔会换域名——换了我这边会更新到云端，你点「从云端取最新」或直接聊一句，站点会自己接上，不用你去找地址。</div>' +
        '</div></div>';

      /* v215：主动模式（TA 主动，不用你先发消息） */
      var _pv = peekProCfg();
      html += '<div class="mcp-sec"><div class="mcp-sec-t">主动模式（TA 主动，不用你先发消息）</div><div class="group-card mcp-gw">' +
        '<div class="mcp-gw-tip">开了以后：聊天窗停在<b>已授权角色</b>的单聊里，TA 会隔一段时间自己看一眼你手机（电量 / 当前在干嘛 / 今天玩了多久），惦记着就<b>主动发消息</b>给你——不用你先开口。</div>' +
        '<div style="display:flex;align-items:center;gap:10px;margin:12px 0 8px">' +
          '<span style="flex:0 0 auto;font-size:13px">开关</span>' +
          '<button class="mcp-btn' + (_pv.on ? '' : ' ghost') + '" id="mcpProToggle" type="button" style="margin:0">' + (_pv.on ? '已开启' : '已关闭') + '</button>' +
        '</div>' +
        '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">' +
          '<span style="flex:0 0 auto;font-size:13px">多久看一眼</span>' +
          [['10', '10 分钟'], ['30', '30 分钟'], ['60', '1 小时']].map(function (f) {
            return '<button class="mcp-chip mcp-pro-f" data-mins="' + f[0] + '" type="button" style="cursor:pointer;' + (String(_pv.mins) === f[0] ? 'outline:2px solid #ff3b6b;font-weight:600' : 'opacity:.55') + '">' + f[1] + '</button>';
          }).join('') +
        '</div>' +
        '<div style="margin:4px 0 6px"><button class="mcp-btn ghost" id="mcpProNow" type="button" style="margin:0">现在试一次（看 TA 会不会主动开口）</button></div>' +
        '<div class="mcp-gw-tip">' + (names.length ? ('当前已授权：' + escHtml(names.join('、')) + '。') : '还没勾角色：先去上面「选择 Char 使用」勾一个，TA 才有手机可看。') + '只在站点这个页面开着时工作（网页拿不到后台常驻权限）；读不到手机状态时 TA 不会硬编，宁可不说。</div>' +
        '</div></div>';

      html += '<div class="mcp-sec"><div class="mcp-sec-t">三步接上（一次就够）</div>';
      MCP_PEEK_STEPS.forEach(function (it) {
        html += '<div class="mcp-code" data-copy="' + mcpAttr(it[2]) + '"><span class="mcp-code-t">' + escHtml(it[0]) + '</span><br>' + escHtml(it[1]) +
                '<button class="mcp-copy" type="button">复制</button></div>';
      });
      html += '<a class="mcp-dl2" href="../../tools/peek-help.html" target="_blank" rel="noopener">手机上看得懂的三步说明 / 常见报错</a>';
      html += '<button class="mcp-btn ghost" id="mcpProToken" type="button" style="margin-top:8px;width:100%">不想自己想 Token？点我生成一个（三处填同一个）</button>';
      html += '</div>';

      html += '<div class="mcp-sec"><div class="mcp-sec-t">' + PEEK_TOOL_COUNT + ' 个 MCP 工具（能用什么）</div><div class="group-card mcp-tips" style="padding:10px 14px">';
      PEEK_TOOL_GROUPS.forEach(function (g) {
        html += '<div class="peek-tg"><div class="peek-tg-t">' + escHtml(g[0]) + '</div><div class="mcp-chips">';
        g[1].forEach(function (t) { html += '<span class="mcp-chip">' + escHtml(t) + '</span>'; });
        html += '</div></div>';
      });
      html += '</div></div>';

      html += '<div class="mcp-sec"><div class="mcp-sec-t">踩坑 & 规矩</div><div class="group-card mcp-tips">';
      MCP_PEEK_TIPS.forEach(function (t) { html += '<div class="mcp-tip">' + escHtml(t) + '</div>'; });
      html += '</div></div>';

      mcpPeekBody.innerHTML = html;
      var bar = document.getElementById('mcpPeekCharBar');
      if (bar) bar.addEventListener('click', openMcpCharPick);
      mcpPeekBody.querySelectorAll('[data-copy]').forEach(function (el) {
        el.addEventListener('click', function (e) {
          e.stopPropagation();
          mcpCopy(el.getAttribute('data-copy'));
        });
      });
      mcpGwWire();
      peekProWire();
    ...
    function mcpRenderCharList() {
      var list = mcpCharEntries();
      if (!list.length) { mcpCharList.innerHTML = '<div class="chat-empty">还没有角色，先去「聊天」里建一个</div>'; return; }
      mcpCharList.innerHTML = list.map(function (e) {
        var on = !!mcpPickBuf[String(e.id)];
        var av = e.avatar
          ? '<div class="chat-avatar" style="background-image:url(' + e.avatar + ');background-size:cover;background-position:center"></div>'
          : '<div class="chat-avatar" style="background:' + (e.color || '#7c5cff') + '">' + escHtml(String(e.name || '?').slice(0, 1)) + '</div>';
        return '<div class="mcp-char-item' + (on ? ' on' : '') + '" data-cid="' + mcpAttr(String(e.id)) + '">' + av +
               '<div class="chat-conv-info"><div class="chat-conv-name">' + escHtml(e.name || '') + '</div>' +
               '<div class="mcp-char-sub">' + (on ? '已授权 · 聊天里能用掌心窗' : '未授权') + '</div></div>' +
               '<span class="mcp-char-tick">✓</span></div>';
      }).join('');
      mcpCharList.querySelectorAll('.mcp-char-item').forEach(function (el, i) {
        el.addEventListener('click', function () {
          var id = String(el.getAttribute('data-cid'));
          if (mcpPickBuf[id]) { delete mcpPickBuf[id]; }
          else {
            var e2 = list[i] || {};
            mcpPickBuf[id] = { id: id, name: el.querySelector('.chat-conv-name') ? el.querySelector('.chat-conv-name').textContent : (e2.name || ''), avatar: e2.avatar || '' };
          }
          mcpRenderCharList();
        });
      });
    ...
    function openMcpPeek() {
      mcpRenderPeek();
      mcpPeekBody.scrollTop = 0;
      mcpPeekOverlay.classList.add('open');
    }
    function openMcpCharPick() {
      mcpPickBuf = {};
      mcpGrantedPeek().forEach(function (g) { mcpPickBuf[String(g.id)] = { id: String(g.id), name: g.name || '', avatar: g.avatar || '' }; });
      mcpRenderCharList();
      mcpCharOverlay.classList.add('open');
    }

    if (mcpOverlay) {
      document.getElementById('mcpBack').addEventListener('click', function () { mcpOverlay.classList.remove('open'); });
      mcpOverlay.addEventListener('click', function (e) { if (e.target === mcpOverlay) mcpOverlay.classList.remove('open'); });
      document.getElementById('mcpPeekBack').addEventListener('click', function () { mcpPeekOverlay.classList.remove('open'); });
      mcpPeekOverlay.addEventListener('click', function (e) { if (e.target === mcpPeekOverlay) mcpPeekOverlay.classList.remove('open'); });
      document.getElementById('mcpCharBack').addEventListener('click', function () { mcpCharOverlay.classList.remove('open'); });
      document.getElementById('mcpCharSave').addEventListener('click', function () {
        var list = [];
        for (var k in mcpPickBuf) if (mcpPickBuf.hasOwnProperty(k)) list.push(mcpPickBuf[k]);
        mcpSaveGrants(list);
        mcpCharOverlay.classList.remove('open');
        mcpRenderServices();
        mcpRenderPeek();
        if (typeof toast === 'function') toast(list.length ? ('已授权 ' + list.length + ' 个 Char') : '已清空授权');
        /* 刚授权完就先摸一次手机，别等用户开口 */
        if (list.length) { try { peekAutoPull(chatCurrentConv); } catch (eWarm) {} }
      });
    }

    /* ===== v214：掌心窗通道（云端 MCP，真读状态） ===== */
    var PEEK_GW_KEY = 'ins-peek-gateway';
    var mcpGwTesting = false;
    var mcpGwState = '';
    var peekStateBuf = {};     /* convKey -> { ts, text } */
    var peekInflight = {};
    var peekLastPull = 0;
    var peekInputTimer = null;

    /* ===== v215：掌心窗主动模式（TA 主动来看你手机，不用你先发消息） ===== */
    var PEEK_PRO_KEY = 'ins-peek-proactive';
    var peekProHint = '';        /* 本轮主动触发标记：注入提示词后即被消费清空 */
    var peekProLast = 0;         /* 上次 TA 主动开口的时间 */
    function peekProCfg() {
      var o = { on: false, mins: 10, onAt: 0 };
      try {
        var raw = localStorage.getItem(PEEK_PRO_KEY);
        if (raw) {
          var j = JSON.parse(raw);
          if (j && typeof j === 'object') {
            o.on = !!j.on;
            o.mins = parseInt(j.mins, 10) || 10;
            o.onAt = parseInt(j.onAt, 10) || 0;
          }
        }
      } catch (e) {}
      if (o.mins < 5) o.mins = 5;
      if (o.mins > 180) o.mins = 180;
      return o;
    }
    function peekProSave(on, mins) {
      var c = peekProCfg();
      var m = parseInt(mins, 10) || c.mins || 10;
      var onAt = on ? (c.on ? (c.onAt || Date.now()) : Date.now()) : c.onAt;
      try { localStorage.setItem(PEEK_PRO_KEY, JSON.stringify({ on: !!on, mins: m, onAt: onAt || 0 })); } catch (e) {}
    }
    /* 这个聊天窗里的角色，是不是被授权用掌心窗 */
    function peekProGranted(conv) {
      if (!conv) return false;
      var list = mcpGrants().peek || [];
      var mid = String(conv.contactId || '');
      var vid = String(conv.id || '');
      for (var i = 0; i < list.length; i++) {
        var gid = String(list[i].id || '');
        if (gid && gid !== 'undefined' && (gid === mid || gid === vid)) return true;
      }
      return false;
    }
    function peekProFreqLabel(m) { return m >= 60 ? (Math.round(m / 60) + ' 小时') : (m + ' 分钟'); }
    /* 每 30 秒被叫一次；到点先真读一次手机，读到了才让 TA 开口。返回 true 表示这轮已消费 */
    function peekProRun(force) {
      var c = peekProCfg();
      if (!c.on && !force) return false;
      if (!peekGw().url) return false;
      if (chatAiBusy) return false;
      if (!chatCurrentConv) return false;
      if (!chatDetailOverlay.classList.contains('open')) return false;
      if (!chatFindApi()) return false;
      if (!peekProGranted(chatCurrentConv)) return false;
      var s = chatCurrentConv.settings || {};
      if (s.blocked) return false;
      var last = Math.max(peekProLast, (s.auto && s.auto.last) || 0, (s.bgAct && s.bgAct.last) || 0, c.onAt || 0);
      if (!force && last && (Date.now() - last) < c.mins * 60 * 1000) return false;
      var convRef = chatCurrentConv;
      peekProHint = '【这次是你自己主动的】不是对方来找你，是你自己惦记着，刚看了一眼 TA 的手机。请以你看到的实时状态为由头，用你的身份和口吻主动说一句（1~2 句，短、口语、像真人发微信；别说什么「我看了你的状态」这种后台话，也别把电量、App 名当报表念出来）。';
      peekRefresh(convRef, function (rec) {
        if (!rec || convRef !== chatCurrentConv || !peekProGranted(convRef)) { peekProHint = ''; return; }
        peekProLast = Date.now();
        try { aiReply(true); } catch (eProA) { peekProHint = ''; }
      });
      return true;
    }
    function peekMakeToken() {
      var a = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
      var out = '';
      try {
        var buf = new Uint8Array(32);
        (window.crypto || window.msCrypto).getRandomValues(buf);
        for (var i = 0; i < buf.length; i++) out += a.charAt(buf[i] % a.length);
      } catch (e) {
        for (var j = 0; j < 32; j++) out += a.charAt(Math.floor(Math.random() * a.length));
      }
      return out;
    }
    function peekProWire() {
      var tg = document.getElementById('mcpProToggle');
      if (tg) tg.addEventListener('click', function (e) {
        e.stopPropagation();
        var c = peekProCfg();
        peekProSave(!c.on, c.mins);
        mcpRenderPeek();
        if (typeof toast === 'function') toast(!c.on ? '主动模式已开启' : '主动模式已关闭');
      });
      mcpPeekBody.querySelectorAll('.mcp-pro-f').forEach(function (b) {
        b.addEventListener('click', function (e) {
          e.stopPropagation();
          var c = peekProCfg();
          peekProSave(c.on, parseInt(b.getAttribute('data-mins'), 10));
          mcpRenderPeek();
          if (typeof toast === 'function') toast('主动频率：' + peekProFreqLabel(peekProCfg().mins) + '看一眼');
        });
      });
      var nowB = document.getElementById('mcpProNow');
      if (nowB) nowB.addEventListener('click', function (e) {
        e.stopPropagation();
        if (peekProRun(true)) {
          if (typeof toast === 'function') toast('已让 TA 去看一眼手机，等 TA 发消息');
        } else if (typeof toast === 'function') {
          toast('没触发：确认通道地址已填、这个角色已授权、聊天窗开着、聊天API 配好');
        }
      });
      var gen = document.getElementById('mcpProToken');
      if (gen) gen.addEventListener('click', function (e) {
        e.stopPropagation();
        mcpCopy(peekMakeToken());
        if (typeof toast === 'function') toast('已生成并复制一个 Token，三处填同一个');
      });
    }

    function peekGw() {
      var o = null;
      try { o = JSON.parse(dbGet(PEEK_GW_KEY) || '{}'); } catch (e) { o = null; }
      if (!o || typeof o !== 'object') o = {};
      return { url: String(o.url || '').trim().replace(/\/+$/, ''), token: String(o.token || '').trim() };
    }
    function peekGwSave(url, token) {
      dbSet(PEEK_GW_KEY, JSON.stringify({ url: String(url || '').trim().replace(/\/+$/, ''), token: String(token || '').trim() }));
    }
    function peekErrBrief(e) { return String((e && (e.message || e.name)) || e || '未知错误').slice(0, 90); }
    /* 用户可能只填域名，也可能把 /mcp 一起填上——这里统一补成 /mcp */
    function peekMcpUrl(url) {
      var u = String(url || '').replace(/\/+$/, '');
      if (!u) return '';
      if (/\/(mcp|sse|mcp-wallet)$/.test(u)) return u;
      return u + '/mcp';
    }
    function peekBaseUrl(url) {
      return String(url || '').replace(/\/+$/, '').replace(/\/(mcp|sse|mcp-wallet)$/, '');
    }
    /* 站点是 https（GitHub Pages）时，浏览器不让它请求 http 的地址（混合内容会被直接拦掉）。
       本机 localhost 例外。掌心窗云端部署天然是 https，只有局域网自建才走 http。 */
    function peekMixedBlocked(url) {
      try {
        if (location.protocol !== 'https:') return false;
        if (!/^http:\/\//i.test(String(url || ''))) return false;
        return !/^http:\/\/(localhost|127\.0\.0\.1|\[::1\])(:\d+)?/i.test(String(url || ''));
      } catch (e) { return false; }
    }

    /* ---------- MCP over HTTP：initialize → tools/call ---------- */
    function peekMcpText(r) {
      try {
        var c = r && r.content;
        if (!c) return '';
        if (typeof c === 'string') return c;
        var out = [];
        for (var i = 0; i < c.length; i++) { if (c[i] && c[i].type === 'text') out.push(String(c[i].text || '')); }
        return out.join('\n');
      } catch (e) { return ''; }
    }
    function peekMcpParse(body) {
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        if (ln.indexOf('data:') === 0) {
          ...
          try { var j = JSON.parse(payload); if (j && (j.result || j.error)) last = j; } catch (e2) {}
        }
      }
      return last;
    }
    function peekMcpPost(url, headers, payload) {
      return fetch(url, { method: 'POST', headers: headers, body: JSON.stringify(payload) });
    }
    function peekMcpCall(url, token, tool, args, cb) {
      var h = { 'Content-Type': 'application/json', 'Accept': 'application/json, text/event-stream' };
      if (token) h['Authorization'] = 'Bearer ' + token;
      var sid = '';
      var init = { jsonrpc: '2.0', id: 1, method: 'initialize', params: { protocolVersion: '2024-11-05', capabilities: {}, clientInfo: { name: 'AEphone', version: '214' } } };
      peekMcpPost(url, h, init).then(function (res) {
        sid = res.headers.get('mcp-session-id') || '';
        if (res.status === 401 || res.status === 403) throw new Error('令牌不对（HTTP ' + res.status + '），Token 是不是和手机端填的不是同一个？');
        return res.text();
      }).then(function () {
        var h2 = { 'Content-Type': h['Content-Type'], 'Accept': h['Accept'] };
        if (token) h2['Authorization'] = h['Authorization'];
        if (sid) h2['mcp-session-id'] = sid;
        peekMcpPost(url, h2, { jsonrpc: '2.0', method: 'notifications/initialized' }).catch(function () {});
        return peekMcpPost(url, h2, { jsonrpc: '2.0', id: 2, method: 'tools/call', params: { name: tool, arguments: args || {} } }).then(function (r2) { return r2.text(); });
      }).then(function (body) {
        var j = peekMcpParse(body);
        if (!j) { cb({ ok: false, err: '服务端没回能看懂的 JSON（地址对不对？结尾要是 /mcp）' }); return; }
        if (j.error) { cb({ ok: false, err: String(j.error.message || j.error.code || 'MCP 报错') }); return; }
        var r = j.result || {};
        if (r.isError) { cb({ ok: false, err: peekMcpText(r) || '工具返回错误' }); return; }
        cb({ ok: true, text: peekMcpText(r), raw: r });
      }).catch(function (e) {
        cb({ ok: false, err: '连不上：' + peekErrBrief(e) + '（Render 免费实例冷启动要等 30~60 秒）' });
      });
    }
    /* ===== v217：通道地址自愈 =====
       免费隧道域名会换、会过期，用户不该为这个反复手填地址。
       站点在 tools/peek-endpoint.json 留一份「当前可用通道」；连不上时自动换成它，
       换成功能了就把结果写回本地配置，用户侧零操作。 */
    var PEEK_EP_PATH = 'tools/peek-endpoint.json';
    var peekEpCache = null;
    var peekEpTryAt = 0;
    function peekUrlDead(err) {
      var s = String(err || '');
      return /连不上|Failed to fetch|NetworkError|Load failed|没回能看懂的 JSON|fetch/i.test(s);
    }
    function peekEndpointPull(cb) {
      cb = cb || function () {};
      var now = Date.now();
      if (peekEpCache && (now - peekEpTryAt) < 60 * 1000) { cb(peekEpCache); return; }
      peekEpTryAt = now;
      fetch(PEEK_EP_PATH + '?t=' + now, { cache: 'no-store' }).then(function (r) {
        return r && r.ok ? r.json() : null;
      }).then(function (j) {
        peekEpCache = (j && typeof j === 'object' && j.mcp) ? j : null;
        cb(peekEpCache);
      }).catch(function () { cb(peekEpCache); });
    }
    function peekAutoSwitch(on) {
      var cur = peekGw();
      var tok = (on && on.token) || cur.token || '';
      try { peekGwSave(on.mcp, tok); } catch (e) {}
      mcpGwState = '通道地址已自动更新：' + on.mcp;
      var st = document.getElementById('mcpGwState'); if (st) st.textContent = mcpGwState;
      var iu = document.getElementById('mcpGwUrl'); if (iu) iu.value = on.mcp;
      var it = document.getElementById('mcpGwToken'); if (it && tok) it.value = tok;
      if (typeof chatCurrentConv !== 'undefined' && chatCurrentConv) {
        peekSysInsert(chatCurrentConv, '掌心窗 · 通道地址已自动换到最新，重新接上了。');
      }
    }
    function peekCall(tool, args, cb) {
      cb = cb || function () {};
      var g = peekGw();
      var run = function (url, token, retried) {
        if (!url) { cb({ ok: false, err: '还没填通道地址：MCP → 掌心窗 → 通道地址' }); return; }
        if (peekMixedBlocked(url)) { cb({ ok: false, err: '这条地址是 http 的，站点是 https，浏览器不让连（混合内容会被拦）。云端部署请用 https 的 mcp 地址。' }); return; }
        peekMcpCall(peekMcpUrl(url), token, tool, args, function (r) {
          if (r && r.ok) { if (retried) peekAutoSwitch({ mcp: url, token: token }); cb(r); return; }
          if (retried || !peekUrlDead(r && r.err)) { cb(r); return; }
          peekEndpointPull(function (ep) {
            if (!ep || !ep.mcp || peekBaseUrl(ep.mcp) === peekBaseUrl(url)) { cb(r); return; }
            run(ep.mcp, ep.token || token, true);
          });
        });
      };
      if (g.url) { run(g.url, g.token, false); return; }
      peekEndpointPull(function (ep) {
        if (ep && ep.mcp) { run(ep.mcp, ep.token, true); return; }
        cb({ ok: false, err: '还没填通道地址：MCP → 掌心窗 → 通道地址' });
      });
    }
    /* 测连通：先看 /health，再真调一次 linjian_status */
    function peekHealth(cb) {
      var g = peekGw();
      if (!g.url) { cb({ ok: false, err: '还没填通道地址' }); return; }
      if (peekMixedBlocked(g.url)) { cb({ ok: false, err: 'http 地址会被浏览器拦，换 https 的 mcp 地址' }); return; }
      var base = peekBaseUrl(g.url);
      fetch(base + '/health', { method: 'GET' }).then(function (r) { return r.text(); }).then(function (t) {
        var j = null;
        try { j = JSON.parse(t); } catch (e) {}
        var head = j
          ? ('health ok=' + (j.ok === undefined ? '?' : j.ok) + ' · has_url=' + (j.has_url === undefined ? '?' : j.has_url) + ' · has_token=' + (j.has_token === undefined ? '?' : j.has_token))
          : 'health 没回 JSON';
        peekCall('linjian_status', {}, function (r2) {
          if (r2.ok) cb({ ok: true, text: head + ' ｜ linjian_status：' + String(r2.text || '').replace(/\s+/g, ' ').slice(0, 90) });
          else cb({ ok: false, err: head + ' ｜ 但 linjian_status 没通：' + r2.err });
        });
      }).catch(function (e) { cb({ ok: false, err: '连不上 health：' + peekErrBrief(e) }); });
    }

    /* ---------- 真实状态：读一次、缓存、注入 ---------- */
    function peekStateBrief(text) {
    ...
    function peekConvKey(conv) { return String((conv && (conv.id || conv.contactId)) || ''); }
    function peekCacheGet(conv) {
      var k = peekConvKey(conv);
      return k ? (peekStateBuf[k] || null) : null;
    }
    function peekRefresh(conv, cb) {
      cb = cb || function () {};
      var k = peekConvKey(conv);
      if (!k) { cb(null); return; }
      if (!peekGw().url) { cb(null); return; }
      if (peekInflight[k]) { peekInflight[k].push(cb); return; }
      peekInflight[k] = [cb];
      peekCall('get_life_state', {}, function (r) {
        var waiters = peekInflight[k] || [];
        delete peekInflight[k];
        var rec = null;
        if (r && r.ok) {
          var brief = peekStateBrief(r.text);
          if (brief) { rec = { ts: Date.now(), text: brief }; peekStateBuf[k] = rec; }
        }
        for (var i = 0; i < waiters.length; i++) { try { waiters[i](rec, r); } catch (e) {} }
      });
    }
    /* 静默预热：一分钟最多一次，已有 90 秒内的缓存就不折腾 */
    function peekAutoPull(conv) {
      if (!conv || !peekGw().url) return;
      var now = Date.now();
      if (now - peekLastPull < 60 * 1000) return;
      var hit = peekCacheGet(conv);
      if (hit && (now - hit.ts) < 90 * 1000) return;
      peekLastPull = now;
      peekRefresh(conv, function () {});
    }

    /* ---------- 状态卡片 ---------- */
    function peekStateCardHtml(txt, tag, sub) {
      return '<div class="peek-card"><div class="peek-card-h"><span class="peek-card-tag">' + escHtml(tag || '掌心窗') + '</span>' +
        '<span class="peek-card-sub">' + escHtml(sub || '真实读取') + '</span></div>' +
        '<div class="peek-card-desc">' + escHtml(String(txt || '')) + '</div></div>';
    }
    function peekWarnCardHtml(title, sub) {
      return '<div class="peek-card peek-card-warn"><div class="peek-card-h"><span class="peek-card-tag">掌心窗</span></div>' +
        '<div class="peek-card-t">' + escHtml(title || '') + '</div>' +
        '<div class="peek-card-m">' + escHtml(String(sub || '').slice(0, 180)) + '</div></div>';
    }
    function peekInsert(conv, role, htmlTxt) {
      if (!conv || !htmlTxt) return;
      addChatMsg(role, { type: 'html', text: htmlTxt }, conv);
    }
    /* v216：掌心窗 / MCP 运行提示统一走「界面中央系统小字」，不挂任何一方气泡 */
    function peekSysInsert(conv, txt) {
      if (!conv || !txt) return;
      addChatMsg('sys', { type: 'sys', text: String(txt) }, conv);
    }
    function peekDecodeEnt(x) {
      return String(x == null ? '' : x)
        .replace(/&lt;/g, '<').replace(/&gt;/g, '>')
        .replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&amp;/g, '&');
    }
    /* 把 v216 之前遗留在会话里的 peek-card 气泡，迁移成居中系统小字 */
    function peekMigrateConv(conv) {
      ...
      for (var i = 0; i < conv.messages.length; i++) {
        ...
        var t = String(m.text || '');
        if (t.indexOf('peek-card') < 0) continue;
        var txt = '';
        var desc = t.match(/peek-card-desc">([\s\S]*?)<\/div>/);
        if (desc) {
          txt = '掌心窗 · 已读取手机状态：' + peekDecodeEnt(desc[1]);
        } else {
          var tt = t.match(/peek-card-t">([\s\S]*?)<\/div>/);
          var mm2 = t.match(/peek-card-m">([\s\S]*?)<\/div>/);
          txt = '掌心窗 · ' + peekDecodeEnt(tt ? tt[1] : '提示') + (mm2 ? ('：' + peekDecodeEnt(mm2[1])) : '');
        }
        m.type = 'sys';
        m.role = 'sys';
        m.text = txt;
        if (m.plain) delete m.plain;
        n++;
      }
      return n;
    }
    /* 用户在打听手机上的事 */
    function peekAskWanted(t) {
      var s = String(t || '');
      if (!s) return false;
      return /掌心窗|我手机|手机状态|看我手机|看看我手机|我在干嘛|我在干什么|我在做什么|屏幕上是什么|屏幕上是|你在看什么|你看到什么|我手机快没电|还有多少电/.test(s);
    }
    function peekOnSend(text, conv) {
      if (!conv) return;
      var g = peekGw();
      if (!peekAskWanted(text)) { if (g.url) peekAutoPull(conv); return; }
      if (!g.url) {
        peekSysInsert(conv, '掌心窗 · 通道还没接上。去「MCP → 掌心窗 → 通道地址」把云端 MCP 地址和 Token 填上，我才能真看到你手机。');
        return;
      }
      var hit = peekCacheGet(conv);
      if (hit && (Date.now() - hit.ts) < 60 * 1000) {
        peekSysInsert(conv, '掌心窗 · 已读取手机状态：' + hit.text);
        return;
      }
      peekRefresh(conv, function (rec, r) {
        if (rec) peekSysInsert(conv, '掌心窗 · 已读取手机状态：' + rec.text);
        else peekSysInsert(conv, '掌心窗 · 这次没读到手机状态：' + ((r && r.err) || '手机端 App 可能没开着，或者通道没通。'));
      });
    }
    /* 输入框里刚提到手机就先悄悄读一次，等你按发送时卡片是现成的 */
    function peekOnInput(text) {
      if (peekInputTimer) clearTimeout(peekInputTimer);
      var conv = chatCurrentConv;
      peekInputTimer = setTimeout(function () {
        if (!conv || !peekAskWanted(text) || !peekGw().url) return;
        peekRefresh(conv, function () {});
      }, 600);
    }
    ...
    /* ---------- MCP 详情页：通道地址栏 ---------- */
    function mcpGwWire() {
      var u = document.getElementById('mcpGwUrl');
      var t = document.getElementById('mcpGwToken');
      var sv = document.getElementById('mcpGwSave');
      var ts = document.getElementById('mcpGwTest');
      var au = document.getElementById('mcpGwAuto');
      var st = document.getElementById('mcpGwState');
      if (st && mcpGwState) st.textContent = mcpGwState;
      if (u) u.addEventListener('click', function (e) { e.stopPropagation(); });
      if (t) t.addEventListener('click', function (e) { e.stopPropagation(); });
      if (sv) sv.addEventListener('click', function (e) {
        e.stopPropagation();
        peekGwSave(u ? u.value : '', t ? t.value : '');
        mcpGwState = peekGw().url ? ('已保存：' + peekGw().url) : '通道地址已清空';
        if (st) st.textContent = mcpGwState;
        if (typeof toast === 'function') toast('通道地址已保存');
      });
      if (ts) ts.addEventListener('click', function (e) {
        e.stopPropagation();
        if (mcpGwTesting) return;
        mcpGwTesting = true;
        peekGwSave(u ? u.value : '', t ? t.value : '');
        mcpGwState = '正在测……（Render 冷启动可能要等 30~60 秒）';
        if (st) st.textContent = mcpGwState;
        peekHealth(function (r) {
          mcpGwTesting = false;
          mcpGwState = r.ok ? ('已连通：' + r.text) : ('没通：' + r.err);
          if (st) st.textContent = mcpGwState;
        });
      });
      /* v217：一键取云端最新通道 */
      if (au) au.addEventListener('click', function (e) {
        e.stopPropagation();
        mcpGwState = '正在取云端最新通道……';
        if (st) st.textContent = mcpGwState;
        peekEpCache = null;
        peekEpTryAt = 0;
        peekEndpointPull(function (ep) {
          if (!ep || !ep.mcp) {
            mcpGwState = '云端暂时没有可用通道记录（通道没开的时候就是空的）';
            if (st) st.textContent = mcpGwState;
            return;
          }
          if (u) u.value = ep.mcp;
          if (t && ep.token) t.value = ep.token;
          peekAutoSwitch(ep);
          mcpGwState = '已取到最新通道：' + ep.mcp + '（更新于 ' + (ep.updated || '未知') + '）';
          if (st) st.textContent = mcpGwState;
          if (typeof toast === 'function') toast('已取到云端最新通道');
          peekHealth(function (r) {
            mcpGwState = (r.ok ? ('已连通：' + r.text) : ('没通：' + r.err)) + ' ｜ 通道：' + ep.mcp;
            if (st) st.textContent = mcpGwState;
          });
        });
      });
    ...
    function memberNorm(m) {
      ...
      var n = {
        ...
        voiceId: String(m.voiceId != null ? m.voiceId : ((m.voice && m.voice.voiceId) || '')).trim(),
        voice: (function (v0, id0) {
          var v = (v0 && typeof v0 === 'object') ? v0 : {};
          return {
            enabled: v.enabled !== false,
            voiceId: String(v.voiceId != null ? v.voiceId : (id0 || '')).trim(),
            lang: String(v.lang || 'zh-CN').trim() || 'zh-CN',
            langLabel: String(v.langLabel || '国语/普通话 (Chinese)').trim(),
            speed: (v.speed != null && !isNaN(Number(v.speed))) ? Number(v.speed) : 1,
            freq: String(v.freq || '中等').trim() || '中等'
          };
        })(m.voice, m.voiceId),
        ...
        lookPromptId: String(m.lookPromptId != null ? m.lookPromptId : '').trim(),
        /* v183：角色相册（测试生图的成品图集合，随角色卡一起存） */
        album: Array.isArray(m.album) ? m.album.map(function (x) {
          if (typeof x === 'string') return { id: memberUid(), src: String(x), ts: 0, note: '', savedBy: 'ta' };
          x = x || {};
          var _as = String(x.src || '');
          if (!_as) return null;
          return { id: String(x.id || memberUid()), src: _as, ts: Number(x.ts) || 0, note: String(x.note || '').slice(0, 60), savedBy: (x.savedBy === 'me' ? 'me' : 'ta') };
        }).filter(function (x) { return x; }) : [],
        npcs: Array.isArray(m.npcs) ? m.npcs.map(function (x) {
          x = x || {};
          return { id: x.id || memberUid(), name: String(x.name || '').trim(), bio: String(x.bio || '').trim(), rel: String(x.rel || '').trim(), add: !!x.add, avatar: String(x.avatar || ''), gender: String(x.gender || ''), masterId: String(x.masterId || ''), masterName: String(x.masterName || ''), lore: String(x.lore || ''), link: (x.link === 'low' || x.link === 'high') ? x.link : 'mid' /* v209：关联度要跟着 NPC 一起存 */ };
        }) : [],
        ...
        roleAdd: !!m.roleAdd,
        npcAdd: !!m.npcAdd,
        /* v187：角色名片公开范围（二维码扫出来能看到什么） */
        card: (function (c0) {
          var c = (c0 && typeof c0 === 'object') ? c0 : {};
          var f = (c.fields && typeof c.fields === 'object') ? c.fields : {};
          var mode = String(c.mode || 'all');
          if (['all', 'part', 'secret'].indexOf(mode) < 0) mode = 'all';
          return {
            mode: mode,
            fields: {
              netName: f.netName !== false, gender: f.gender !== false, lore: f.lore !== false,
              look: f.look !== false, voice: f.voice !== false, alts: !!f.alts, npcs: f.npcs !== false
            }
          };
        })(m.card),
        /* v187：角色绑定的世界书（从「设置 → 世界书」里勾选，聊天时随该角色注入） */
        wbList: Array.isArray(m.wbList) ? m.wbList.map(function (x) { return String(x || '').trim(); }).filter(function (x) { return x; }) : []
      };
    ...
    function memberAvatarHtml(m, size) {
      var sz = size || 56;
      if (m.avatar) return '<img class="mem-ava-img" src="' + m.avatar + '" alt="" style="width:' + sz + 'px;height:' + sz + 'px">';
    ...
    var memNpcAdd = false;
    var memQrText = '';
    var MEM_CARD_DEF = { mode: 'all', fields: { netName: true, gender: true, lore: true, look: true, voice: true, alts: false, npcs: true } };
    var memCard = { mode: 'all', fields: { netName: true, gender: true, lore: true, look: true, voice: true, alts: false, npcs: true } };
    var memWbList = [];
    var memVoiceFreq = '中等';
    var memTempUid = '';
    var memDirty = false;
    ...
    // v176：去掉老版 01-09 序号，改由「区块标题」组织信息层级
    function memFieldLabel(n, label, extra) {
      return '<label>' + label + (extra || '') + '</label>';
    }

    var MEM_ICO = {
      basic: '<path d="M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8z"/><path d="M4.5 20c.8-3.4 3.6-5.2 7.5-5.2s6.7 1.8 7.5 5.2"/>',
      lore: '<path d="M4 5.5A2.5 2.5 0 0 1 6.5 3H19v15H6.5A2.5 2.5 0 0 0 4 20.5z"/><path d="M8 7.5h7M8 11h5"/>',
      voice: '<path d="M11 5L6 9H3v6h3l5 4z"/><path d="M15.5 8.5a5 5 0 0 1 0 7"/><path d="M18.5 5.5a9 9 0 0 1 0 13"/>',
      look: '<rect x="3" y="3" width="18" height="18" rx="2.5"/><circle cx="9" cy="9" r="2"/><path d="M21 15.5l-4.5-4.5L7 20.5"/>',
      npc: '<path d="M9.5 11a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7z"/><path d="M2.5 20c.7-3 3.1-4.6 7-4.6s6.3 1.6 7 4.6"/><path d="M17.5 5.6a3 3 0 1 0 0 5.5M18.5 13.4c2.3.4 3.8 2 4.3 4.4"/>',
      rel: '<circle cx="12" cy="5" r="2.6"/><circle cx="5" cy="19" r="2.6"/><circle cx="19" cy="19" r="2.6"/><path d="M12 7.6v3.9M11.8 11.9l-5 4.6M12.2 11.9l5 4.6"/>',
      alt: '<path d="M20.5 13.3L13.3 20.5a2 2 0 0 1-2.8 0L3.5 13.5V3.5h10l7 7a2 2 0 0 1 0 2.8z"/><circle cx="8" cy="8" r="1.4"/>',
      idc: '<rect x="3" y="4.5" width="18" height="15" rx="2.5"/><circle cx="9" cy="11" r="2.2"/><path d="M5.6 16.8c.5-1.8 1.8-2.7 3.4-2.7s2.9.9 3.4 2.7M14.8 9.5h4M14.8 13h3"/>'
    };
    function memSec(ico, title, sub) {
      return '<div class="mem-sec-head"><span class="mem-sec-ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">' + (MEM_ICO[ico] || '') + '</svg></span><span class="mem-sec-tx"><b>' + title + '</b>' + (sub ? '<i>' + sub + '</i>' : '') + '</span></div>';
    }
    ...
    function memberEditBody(m) {
      ...
      form += memSec('basic', '基础资料', '角色对外展示的名字与性别');
      ...
      /* ② 人设设定 */
      form += '<div class="form-card member-card">';
      form += memSec('lore', '人设设定', '身世越完整，AI 网名与 NPC 越准');
      form += '<div class="field">' + memFieldLabel(4, '角色身世') + '<textarea id="memLore" rows="7" placeholder="在这里写下完整角色内容：成长经历、性格、身份、习惯、秘密…">' + escHtml(m ? m.lore : '') + '</textarea>' +
        ...
        '<input type="file" id="memLoreFile" accept=".txt,.md,.markdown,.json,.docx,.doc,text/plain,application/json,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" hidden>' +
        '</div></div>';
      form += '</div>';

      /* ③ 角色音色（含试听） */
      form += '<div class="form-card member-card">';
      form += memSec('voice', '角色音色', '这个角色说话用的 MiniMax 音色');
      form += '<div class="field">' + memFieldLabel(5, '音色 ID') +
        '<div class="mem-voice-row">' +
          '<input type="text" id="memVoiceId" value="' + escHtml(m ? m.voiceId : '') + '" placeholder="MiniMax voice_id，例如 female-shaonv_mei">' +
          '<button type="button" class="mem-voice-test" id="memVoiceTest" data-mact="voicetest"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M11 5L6 9H3v6h3l5 4z"/><path d="M15.5 8.5a5 5 0 0 1 0 7"/><path d="M18.5 5.5a9 9 0 0 1 0 13"/></svg><span>试听</span></button>' +
        '</div>' +
        '<div class="mem-field-foot"><span>填 MiniMax 的 voice_id，点「试听」用该音色念一句示例台词</span></div>' +
        '</div>';
      form += '</div>';

      /* ④ 形象与锁脸 */
      form += '<div class="form-card member-card">';
      form += memSec('look', '形象与锁脸', '外貌、锁脸参考图与生图提示词');
      form += '<div class="field">' + memFieldLabel(6, '角色外貌') + '<textarea id="memLook" rows="3" placeholder="外貌描写…会与「锁定生图提示词」「锁脸参考图」一起用于测试生图">' + escHtml(m ? m.look : '') + '</textarea></div>';
      form += '<div class="mem-look-label">锁脸参考图</div>';
      form += '<div class="mem-look-grid" id="memLookGrid">';
      memLookImgs.forEach(function (img, i) {
        form += '<div class="mem-look-cell" data-mact="lookview" data-val="' + i + '" style="background-image:url(\'' + img + '\')"><span class="member-look-x" data-mact="lookdel" data-val="' + i + '"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M6 6l12 12M18 6L6 18"/></svg></span></div>';
      });
      ...
      var lpList = [];
      try { lpList = (typeof imgPrompts !== 'undefined' && imgPrompts) ? imgPrompts : []; } catch (e) {}
      var lpCurId = String((m && m.lookPromptId) || '');
      var lpCurTxt = String((m && m.lookPrompt) || '');
      if (!lpCurId && lpCurTxt) {
        for (var lpi = 0; lpi < lpList.length; lpi++) {
          if (imgPromptText(lpList[lpi].pos || lpList[lpi].content || '') === lpCurTxt) { lpCurId = String(lpList[lpi].id || lpi); break; }
        }
      }
      form += '<div class="mem-look-prompt-label">锁定生图提示词</div>';
      form += '<select id="memLookPrompt" class="mem-look-select"><option value="">不锁定（只用外貌描写）</option>' + lpList.map(function (p, i) {
        var _pid = String(p.id || i);
        return '<option value="' + escHtml(_pid) + '"' + (_pid === lpCurId ? ' selected' : '') + '>' + escHtml(p.name || ('提示词' + (i + 1))) + '</option>';
      }).join('') + '</select>';
      form += '<div class="mem-field-foot"><span>从「设置 → 生图」里已保存的提示词中选一条，测试生图时与外貌描写、锁脸参考图一起用</span></div>';
      form += '<div class="mem-gen-row"><button type="button" class="mem-ghost ai gen" data-mact="lookgen"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="9" cy="9" r="2"/><path d="M21 15l-4.5-4.5L7 20"/></svg><span>测试生图</span></button><span class="mem-gen-note">点一下直接生成：锁定生图提示词 + 外貌描写 + 锁脸参考图</span></div>';
      form += '<div class="mem-gen-box" id="memGenBox"></div>';
      /* v200：角色编辑页不再放相册入口（相册只作为底图池：聊天里存图 / TA 换头像时取用） */
      form += '</div>';

      /* ⑤ NPC 角色（v197：主编辑页只保留一个入口，点进去才是完整的 NPC 名单界面） */
      form += '<div class="form-card member-card">';
      form += memSec('npc', 'NPC 角色', '这个角色的身边人，聊天时 AI 会沿用');
      form += '<div class="field">' + memFieldLabel(7, 'NPC 名单');
      form += '<div class="mem-npc-entry" data-mact="npclist"><span class="mem-npc-entry-ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></span>' +
        '<span class="mem-npc-entry-bd"><span class="mem-npc-entry-name">NPC 名单</span><span class="mem-npc-entry-sub">' + (memNpcs.length ? ('已有 ' + memNpcs.length + ' 位，点开管理 / 添加') : '还没有 NPC，点进去添加') + '</span></span>' +
        '<span class="chat-setting-value"><span style="color:#5ac8fa">›</span></span></div>';
      form += '</div>';
      form += '</div>';
      ...
      form += '<div class="form-card member-card">';
      form += memSec('rel', '关系网', '本角色与每个 NPC 的关系');
      form += '<div class="field">' + memFieldLabel(8, '关系定义') + '<div class="mem-rel-wrap">' + relRows + '</div><div class="mem-field-foot"><span>定义本角色与每个 NPC 的关系，聊天时 AI 会沿用</span></div></div>';
      form += '</div>';
      ...
      form += '<div class="form-card member-card">';
      form += memSec('alt', '小号', '同一人设下的其他昵称');
      form += '<div class="field">' + memFieldLabel(9, '小号列表') + '<div class="mem-alt-wrap">' + altRows + '</div>' +
        ...
        '<div class="mem-field-foot"><span>同一角色人设下的其他小号昵称，可添加多个</span></div></div>';
      form += '</div>';

      /* v187：世界书（从设置里挑，跟着这个角色走） */
      form += '<div class="form-card member-card">';
      form += memSec('idc', '世界书', '从「设置 → 世界书」里挑，聊天时随 TA 一起生效');
      var _wbPool = [];
      (wbGlobals || []).forEach(function (g) {
        var _t = String(g.title || '').trim();
        if (_t && !_wbPool.some(function (x) { return x.title === _t; })) _wbPool.push({ title: _t, sub: '全局世界书' });
      });
      (wbLocals || []).forEach(function (w) {
        var _t = String(w.title || '').trim();
        if (_t && !_wbPool.some(function (x) { return x.title === _t; })) _wbPool.push({ title: _t, sub: '局部世界书' });
      });
      if (!_wbPool.length) {
        form += '<div class="mem-field-foot"><span>还没有世界书。先去「设置 → 世界书」建好设定，再回来勾选。</span></div>';
      } else {
        form += '<div class="mem-wb-list" id="memWbList">' + _wbPool.map(function (x) {
          var on = memWbList.indexOf(x.title) >= 0;
          return '<div class="mem-wb-item' + (on ? ' on' : '') + '" data-mact="wbsel" data-val="' + escHtml(x.title) + '">' +
            '<span class="mem-wb-tx"><b>' + escHtml(x.title) + '</b><i>' + x.sub + '</i></span>' +
            '<span class="mem-wb-box">' + (on ? '\u2713' : '') + '</span></div>';
        }).join('') + '</div>' +
        '<div class="mem-field-foot"><span>勾选的世界书只在这个角色的聊天里生效；没勾选的世界书不影响 TA</span></div>';
      }
      form += '</div>';
      ...
      form += '<div class="form-card member-card">';
      form += memSec('idc', '角色标识', '角色ID、二维码与主动加好友');
      form += '<div class="field"><label>角色ID</label>' +
        '<div class="mem-id-line"><span class="mem-id-label">角色ID</span><code class="mem-id-code">' + escHtml(memId || '保存后自动生成') + '</code></div>' +
        '<div class="mem-qr-label">扫码能看到的资料 · 公开范围</div>' +
        '<div class="mem-cm-row mem-cm-ringed" id="memCardModeRow">' +
        '<button type="button" class="mem-cm-btn ring-all' + (memCard.mode === 'all' ? ' on' : '') + '" data-mact="cardmode" data-val="all">全部公开</button>' +
        '<button type="button" class="mem-cm-btn ring-part' + (memCard.mode === 'part' ? ' on' : '') + '" data-mact="cardmode" data-val="part">部分公开</button>' +
        '<button type="button" class="mem-cm-btn ring-secret' + (memCard.mode === 'secret' ? ' on' : '') + '" data-mact="cardmode" data-val="secret">仅私密</button>' +
        '</div>' +
        '<div class="mem-ring-legend">' +
        '<span class="mem-ring-item"><i class="mem-ring-dot all"></i>全部公开</span>' +
        '<span class="mem-ring-item"><i class="mem-ring-dot part"></i>部分公开</span>' +
        '<span class="mem-ring-item"><i class="mem-ring-dot secret"></i>仅私密</span>' +
        '<span class="mem-ring-note">二维码外圈颜色 = 上面的公开范围</span>' +
        '</div>' +
        '<div class="mem-cf-row" id="memCardFields" style="display:' + (memCard.mode === 'part' ? 'flex' : 'none') + '">' +
        /* v191：勾选项也分区——身份归身份、人设归人设、语音归语音，不混成一排 */
        [['身份', [['netName', '网名'], ['gender', '性别']]],
         ['人设', [['lore', '人设'], ['look', '外貌']]],
         ['语音', [['voice', '音色']]],
         ['关系', [['alts', '小号'], ['npcs', 'NPC']]]].map(function (grp) {
          return '<div class="mem-cf-group"><span class="mem-cf-gt">' + grp[0] + '</span><div class="mem-cf-gchips">' +
            grp[1].map(function (p) {
              return '<label class="mem-cf-chip' + (memCard.fields[p[0]] ? ' on' : '') + '" data-mact="cardfield" data-val="' + p[0] + '">' + p[1] + '</label>';
            }).join('') + '</div></div>';
        }).join('') +
        '</div>' +
        '<div class="mem-field-foot"><span>扫到的资料按上面的公开范围显示<button type="button" class="tips-q" data-tip="cardmode">?</button></span></div>' +
        '<div class="mem-qr-label" data-mact="qrdef" style="cursor:pointer">角色二维码<button type="button" class="tips-q" data-tip="qr">?</button></div>' +
        '<div class="mem-qr-row" id="memQrRow"><input type="hidden" id="memQrText" value="' + escHtml(memQrText) + '" placeholder="二维码内容（一般不用改）">' +
        '<button type="button" class="mem-ghost" data-mact="qrdef" title="按上面的公开范围生成二维码：别人扫一下就能加 TA 为好友，并且只看到你允许公开的资料">生成二维码</button>' +
        ...
        '<div class="mem-qr-preview" id="memQrPreview">' + (memQrText ? '<img src="' + memQrUrl(memQrText) + '" alt="角色二维码" data-qzoom="1">' : '<span class="mem-qr-empty">点“生成二维码”试试</span>') + '</div>' +
    ...
    function openMemberEdit(idx) {
      ...
      memNpcs = m ? m.npcs.map(function (x) { return { id: x.id, name: x.name, bio: x.bio, rel: x.rel, add: !!x.add, avatar: x.avatar || '', gender: x.gender || '', masterId: x.masterId || '', masterName: x.masterName || '', lore: x.lore || '', link: (x.link === 'low' || x.link === 'high') ? x.link : 'mid' /* v209：读回关联度档位 */ }; }) : [];
      memLookImgs = m ? m.lookImgs.slice() : [];
      memAlbum = m ? (Array.isArray(m.album) ? m.album.map(function (x) { return { id: x.id, src: x.src, ts: x.ts, note: x.note, savedBy: x.savedBy }; }) : []) : [];
      ...
      memCard = (m && m.card) ? { mode: m.card.mode, fields: m.card.fields } : { mode: MEM_CARD_DEF.mode, fields: { netName: MEM_CARD_DEF.fields.netName, gender: MEM_CARD_DEF.fields.gender, lore: MEM_CARD_DEF.fields.lore, look: MEM_CARD_DEF.fields.look, voice: MEM_CARD_DEF.fields.voice, alts: MEM_CARD_DEF.fields.alts, npcs: MEM_CARD_DEF.fields.npcs } };
      memWbList = (m && Array.isArray(m.wbList)) ? m.wbList.slice() : [];
      memVoiceFreq = (m && m.voice && m.voice.freq) ? String(m.voice.freq) : '中等';
      memTempUid = m && m.id ? m.id : memberUid();
      memDirty = false;
      memNpcEditIdx = null; memNpcDraft = null; memRoleFormCache = null; memNpcListView = false;
      memberEditScroll.innerHTML = memberEditBody(m);
      /* v208：新增角色一进来就把「空表单」也存成缓存，后续任何重渲染都能用它兜底 */
      if (!m) memRoleFormCache = { realName: '', netName: '', lore: '', voiceId: '', look: '', qrText: '', voiceLang: 'zh-CN', voiceSpeed: 1, avatar: '', gender: '' };
      memberBindEdit();
      /* v200：进编辑页就把二维码按当前公开范围重画一遍（外圈直接显示绿 / 黄 / 红） */
      if (memQrText) memberQrSync();
      memberEditOverlay.classList.add('open');
      memberChromeSync();
    }
    /* v202：顶栏随层级切换 —— 人设库/新增 NPC 页标题上顶栏、不给保存键（左边就是退出） */
    function memberChromeSync() {
      try {
        var t = document.getElementById('memberEditTitle');
        var sv = document.getElementById('memberEditSave');
        if (memNpcEditIdx !== null && memNpcEditIdx !== undefined) {
          if (t) t.textContent = (memNpcEditIdx >= 0 ? '编辑 NPC' : '新增 NPC');
          if (sv) sv.style.display = 'none';
        } else if (memNpcListView) {
          if (t) t.textContent = 'NPC 人设库';
          if (sv) sv.style.display = 'none';
        } else {
          if (t) t.textContent = (memEditIdx >= 0 ? '编辑角色' : '新增角色');
          if (sv) sv.style.display = '';
        }
        /* v205：创建 NPC 的 ＋ 只挂在人设库列表页的顶栏右上角 */
        var addTop = document.getElementById('memberNpcAddTop');
        if (addTop) {
          var _libOnly = !!memNpcListView && (memNpcEditIdx === null || memNpcEditIdx === undefined);
          addTop.classList.toggle('show', _libOnly);
        }
      } catch (e) {}
    }
    ...
    function memberBindEdit() {
      ...
      /* v205：顶栏右上角 ＋ = 新建 NPC（只绑一次） */
      var addTopBtn = document.getElementById('memberNpcAddTop');
      if (addTopBtn && !addTopBtn._v205bind) {
        addTopBtn._v205bind = true;
        addTopBtn.addEventListener('click', function () { memberNpcOpen(-1); });
      }
      scroll.onclick = function (e) {
        ...
        if (act === 'qradv') { var _rw = scroll.querySelector('#memQrRow'); if (_rw) _rw.style.display = (_rw.style.display === 'none' ? 'flex' : 'none'); return; }
        if (act === 'sex') {
          memGender = val;
          memDirty = true;
        ...
        if (act === 'voicefreq') {
          memVoiceFreq = val;
          memDirty = true;
          scroll.querySelectorAll('#memVoiceFreqBox .vf-btn').forEach(function (c) { c.classList.toggle('active', c.getAttribute('data-val') === val); });
          return;
        }
        ...
        if (act === 'lookview') { var _lv = parseInt(val, 10); if (memLookImgs[_lv]) viewImgFull(memLookImgs[_lv]); return; }
        if (act === 'npclist') { memberCacheRoleForm(); memNpcEditIdx = null; memNpcListView = true; memberNpcListRender(); return; }
        if (act === 'npcadd') { memberNpcOpen(-1); return; }
        if (act === 'npcopen') { memberNpcOpen(parseInt(val, 10)); return; }
        ...
        if (act === 'albumhome' || act === 'albumopen') { openMemberAlbumHome(); return; }
        if (act === 'albumview') { openMemberAlbum(parseInt(val, 10) || 0); return; }
        if (act === 'lookgen') { memberGenTest(); return; }
        if (act === 'voicetest') { memberVoiceTest(); return; }
        ...
        if (act === 'cardmode') {
          memCard.mode = String(val || 'all');
          if (memQrText) { try { setTimeout(function () { memberQrDefault(true); }, 0); } catch (e2) {} }
          var _row = scroll.querySelector('#memCardModeRow');
          if (_row) _row.querySelectorAll('.mem-cm-btn').forEach(function (b) {
            b.classList.toggle('on', b.getAttribute('data-val') === memCard.mode);
          });
          var _fr = scroll.querySelector('#memCardFields');
          if (_fr) _fr.style.display = (memCard.mode === 'part') ? 'flex' : 'none';
          /* v200：换了公开范围，二维码外圈颜色跟着换 */
          if (memQrText) memberQrSync();
          return;
        }
        if (act === 'cardfield') {
          var _k = String(val || '');
          if (memCard.fields.hasOwnProperty(_k)) memCard.fields[_k] = !memCard.fields[_k];
          if (memQrText) { try { setTimeout(function () { memberQrDefault(true); }, 0); } catch (e2) {} }
          var _chip = scroll.querySelector('[data-cardf="' + _k + '"], [data-mact="cardfield"][data-val="' + _k + '"]');
          if (_chip) _chip.classList.toggle('on', !!memCard.fields[_k]);
          return;
        }
        if (act === 'wbsel') {
          var _t = String(val || '').trim();
          var _idx = memWbList.indexOf(_t);
          if (_idx >= 0) memWbList.splice(_idx, 1); else if (_t) memWbList.push(_t);
          b.classList.toggle('on', memWbList.indexOf(_t) >= 0);
          var _bx = b.querySelector('.mem-wb-box');
          if (_bx) _bx.textContent = (memWbList.indexOf(_t) >= 0) ? '\u2713' : '';
          return;
        }
      ...
      // v189：点二维码放大 / 保存
      var qpv = scroll.querySelector('#memQrPreview');
      if (qpv) qpv.addEventListener('click', function (e) {
        var im = e.target && e.target.closest ? e.target.closest('img') : null;
        if (!im) return;
        if (!memQrText) { toast('先生成二维码'); return; }
        openQrViewer(memQrText, { avatar: memAvatar, name: memberQrPreviewName(), ring: memCardRingOf(memCard) });
      });
      // 头像点击
      ...
      // 语音配置：语速滑杆 + 语言下拉
      var vspd = scroll.querySelector('#memVoiceSpeed');
      if (vspd) vspd.addEventListener('input', function () {
        memDirty = true;
        var lab = scroll.querySelector('#memVoiceSpeedVal');
        if (lab) lab.textContent = vspd.value;
      });
      var vlang = scroll.querySelector('#memVoiceLang');
      if (vlang) vlang.addEventListener('change', function () { memDirty = true; });
      // 所有输入/文本标记脏（含新字段同步）
    ...
    // v176：名单里试听角色音色（复用聊天侧 MiniMax 合成链路 chatTts）
    function memberVoiceTest() {
      var el = memberEditScroll.querySelector('#memVoiceId');
      var vid = (el && String(el.value || '').trim()) || '';
      if (!vid) { toast('先填音色 ID 再试听'); return; }
      var mm = loadMMConfig();
      if (!mm || !mm.groupId || !mm.apiKey) { toast('请先配置 MiniMax 语音API（聊天设置 → 语音API）'); return; }
      var name = '';
      try {
        var rn = memberEditScroll.querySelector('#memRealName');
        var nn = memberEditScroll.querySelector('#memNetName');
        name = ((rn && String(rn.value || '').trim()) || (nn && String(nn.value || '').trim()) || '');
      } catch (e) {}
      var sp = 1;
      try { if (chatCurrentConv && chatCurrentConv.settings && chatCurrentConv.settings.voice && chatCurrentConv.settings.voice.speed) sp = chatCurrentConv.settings.voice.speed; } catch (e) {}
      var text = chatVoiceSampleText('zh-CN', name || '这个角色');
      var btn = memberEditScroll.querySelector('#memVoiceTest');
      if (btn) { btn.disabled = true; btn.classList.add('busy'); }
      toast('正在合成试听…');
      chatTts(text, vid, sp, function (audio, err) {
        if (btn) { btn.disabled = false; btn.classList.remove('busy'); }
        if (err) { toast('试听失败：' + err); pushChatErrLog('名单音色试听失败: ' + err); return; }
        var a = new Audio(audio);
        var pp = a.play();
        if (pp && pp.catch) pp.catch(function () { toast('浏览器阻止了自动播放，请再点一次试听'); pushChatErrLog('名单音色试听被浏览器自动播放策略拦截'); });
        toast('试听中…');
      });
    }
    /* ==================== v196：NPC 完整独立编辑页 ==================== */
    var memNpcEditIdx = null;    /* null = 不在 NPC 页；-1 = 新建；>=0 = 编辑第 idx 个 */
    var memNpcListView = false;  /* v197：是否停在「NPC 名单」界面 */
    var memNpcDraft = null;      /* NPC 编辑草稿 */
    var memRoleFormCache = null; /* 进 NPC 页前缓存的主角色表单文字 */
    var MEM_NPC_TPL = '一、基础档案\n二、外貌特征\n三、性格气质\n四、身世背景\n五、能力特长\n六、人际关系\n七、生活日常\n八、情感观念\n九、隐藏设定\n十、和主人设的关系';
    /* v200：角色名片公开范围 → 二维码外圈配色（全部公开=绿圈 / 部分公开=黄圈 / 仅私密=红圈） */
    var MEM_CARD_RING = { all: '#22c55e', part: '#f5b300', secret: '#ff3b30' };
    function memCardRingOf(card) {
      var mo = (card && card.mode) ? String(card.mode) : 'all';
      return MEM_CARD_RING[mo] || MEM_CARD_RING.all;
    }
    /* v200：从名片码文本里读出公开范围 → 对应圈色（聊天里收到的二维码也用同一套） */
    function qrRingFromText(text) {
      try {
        var r = parseFriendCode(text);
        var p = r && r.payload;
        if (p && p.p && MEM_CARD_RING[String(p.p)]) return MEM_CARD_RING[String(p.p)];
        if (p && p.card && p.card.mode) return memCardRingOf(p.card);
      } catch (e) {}
      return '';
    }
    /* ==================== v200：NPC 人设「分段编辑」 ====================
       编辑页把十段拆成十个独立输入区；存回时再拼成完整人设。 */
    function npcLoreSectionNames() { return MEM_NPC_TPL.split('\n'); }
    function npcLoreSectionKey(nm) { return String(nm || '').replace(/^[一二三四五六七八九十]+、/, '').trim(); }
    function npcLoreSplit(lore) {
      var names = npcLoreSectionNames();
      var out = names.map(function () { return ''; });
      var txt = String(lore || '').replace(/\r/g, '');
      if (!txt.trim()) return out;
      var lines = txt.split('\n');
      var cur = -1;
      for (var i = 0; i < lines.length; i++) {
        var bare = String(lines[i] || '').replace(/^[#>*\-\s]+/, '').trim();
        if (!bare) { if (cur >= 0) out[cur] += '\n'; continue; }
        var hit = -1;
        for (var k = 0; k < names.length; k++) {
          var nm = names[k], key = npcLoreSectionKey(nm);
          if (bare.indexOf(nm) === 0) { hit = k; break; }
          if (key && bare.indexOf(key) === 0) { hit = k; break; }
        }
        if (hit >= 0) {
          cur = hit;
          var nm2 = names[hit], key2 = npcLoreSectionKey(nm2);
          var rest = '';
          if (bare.indexOf(nm2) === 0) rest = bare.slice(nm2.length);
          else if (key2 && bare.indexOf(key2) === 0) rest = bare.slice(key2.length);
          rest = rest.replace(/^[：:、.．\s]+/, '').trim();
          /* v209：npcLoreJoin 落库时空段会写成「（未填）」，这里要还原成空，否则再打开会被当成已写满 */
          if (/^[（(]\s*未填\s*[）)]$/.test(rest)) rest = '';
          if (rest) out[hit] = out[hit] ? (out[hit] + '\n' + rest) : rest;
          continue;
        }
        if (cur >= 0) {
          if (/^[（(]\s*未填\s*[）)]$/.test(bare)) { out[cur] += (out[cur] ? '\n' : ''); }
          else out[cur] += (out[cur] ? '\n' : '') + bare;
        }
      }
      if (!out.join('').replace(/\s/g, '') && txt.trim()) {
        /* v209：整行都是「段名：（未填）」占位时不算内容，别把占位串倒进第一段 */
        var _real = txt.split('\n').filter(function (ln) {
          var s0 = String(ln || '').replace(/^[#>*\-\s]+/, '').trim();
          if (!s0) return false;
          if (/^[^：:]{1,20}[：:]\s*[（(]\s*未填\s*[）)]$/.test(s0)) return false;
          return s0.replace(/[（(]\s*未填\s*[）)]/g, '').replace(/\s/g, '') !== '';
        });
        if (_real.length) out[0] = txt.trim();
      }
      return out;
    }
    /* v205：AI 写的段文本去掉 Markdown / HTML 冗余，只留纯文本 */
    function npcLoreCleanText(v) {
      ...
      t = t.replace(/^\s*[-*+•]\s+/gm, '');
      t = t.replace(/^\s*>\s?/gm, '');
      t = t.replace(/^\s*(?:\d+|[一二三四五六七八九十]+)\s*[.、)）]\s*/gm, '');
      t = t.replace(/[　\t ]+\n/g, '\n').replace(/\n{3,}/g, '\n\n').trim();
      return t;
    }
    /* v205：AI 常把后面几段塞进同一段里 —— 段内再切一次、按段名归位，保证板块不错乱、不串段 */
    function npcLoreRealign(arr) {
      var names = npcLoreSectionNames();
      var buf = [];
      for (var z0 = 0; z0 < names.length; z0++) buf.push([]);
      for (var i = 0; i < names.length; i++) {
        var body = npcLoreCleanText(arr && arr[i]);
        if (!body) continue;
        var lines = body.split('\n'), cur = i;
        for (var li = 0; li < lines.length; li++) {
          var bare = String(lines[li] || '').trim();
          if (!bare) { buf[cur].push(''); continue; }
          var hit = -1, rest = '';
          for (var m = 0; m < names.length; m++) {
            var nm = names[m], key = npcLoreSectionKey(nm);
            if (bare.indexOf(nm) === 0) { hit = m; rest = bare.slice(nm.length); break; }
            if (key && bare.indexOf(key) === 0) { hit = m; rest = bare.slice(key.length); break; }
          }
          if (hit >= 0) {
            cur = hit;
            rest = rest.replace(/^[：:、.．\s]+/, '').trim();
            if (rest) buf[cur].push(rest);
          } else {
            buf[cur].push(bare);
          }
        }
      ...
      var out = [];
      for (var z = 0; z < names.length; z++) out.push(npcLoreCleanText(buf[z].join('\n')));
      return out;
    }
    function npcLoreJoin(arr) {
      var names = npcLoreSectionNames();
      var lines = [];
      for (var i = 0; i < names.length; i++) {
        var body = String((arr && arr[i]) || '').trim();
        lines.push(names[i] + '：' + (body || '（未填）'));
      }
      return lines.join('\n');
    }
    /* 把 AI 返回的「段名 → 内容」对象映射成 10 段数组（兼容序号 / 段名 / 关键词写法） */
    function npcLoreMapToArr(mapObj) {
      var names = npcLoreSectionNames();
      var ords = ['一', '二', '三', '四', '五', '六', '七', '八', '九', '十'];
      var out = names.map(function () { return ''; });
      if (!mapObj || typeof mapObj !== 'object') return out;
      var src = [];
      if (Array.isArray(mapObj)) { mapObj.forEach(function (x, i) { src.push([String(i), x]); }); }
      else { for (var k in mapObj) if (Object.prototype.hasOwnProperty.call(mapObj, k)) src.push([k, mapObj[k]]); }
      src.forEach(function (pair) {
        var kk = String(pair[0] || ''), vv = pair[1];
        if (vv && typeof vv === 'object') vv = (vv.content != null ? vv.content : (vv.text != null ? vv.text : (vv.value != null ? vv.value : '')));
        var txt = String(vv == null ? '' : vv).trim();
        if (!txt) return;
        for (var i = 0; i < names.length; i++) {
          var nm = names[i], key = npcLoreSectionKey(nm);
          if (kk.indexOf(nm) >= 0 || kk.replace(/\s/g, '') === nm.replace(/\s/g, '')) { out[i] += (out[i] ? '\n' : '') + txt; return; }
          if (key && kk.indexOf(key) >= 0) { out[i] += (out[i] ? '\n' : '') + txt; return; }
          if (kk.indexOf(ords[i]) === 0 || kk.replace(/[.．、,，)）]/g, '') === ords[i]) { out[i] += (out[i] ? '\n' : '') + txt; return; }
        }
      });
      return out;
    }
    /* 从 AI 自由文本里兜底抽十段（万一模型没吐 JSON） */
    /* v205：模型常把真实换行塞进 JSON 的字符串值里 —— 严格解析会炸，这里按「键:值」宽容提取一遍 */
    function npcLoreLooseJson(t) {
      var o = {}, re = /"([^"]{1,40})"\s*:\s*"((?:[^"\\]|\\.)*)"/g, m;
    ...
    function npcLoreExtractFromText(txt) {
      var t = String(txt || '').replace(/```[a-zA-Z]*|```/g, '').trim();
      var s = t.indexOf('{'), e = t.lastIndexOf('}');
      if (s >= 0 && e > s) {
        var seg = t.slice(s, e + 1), _o = null;
        try { _o = JSON.parse(seg); } catch (eP) { _o = null; }
        if (!_o || typeof _o !== 'object') { try { _o = npcLoreLooseJson(seg); } catch (eQ) { _o = null; } }
        if (_o && typeof _o === 'object' && Object.keys(_o).length) {
          /* v205：npcName 之类的附加键不是十段内容，必须先摘掉，免得混进正文变成多余物 */
          ['npcName', 'name', 'NPC名字', '名字', '简介', 'bio'].forEach(function (k) { try { delete _o[k]; } catch (eD) {} });
          var _arr = npcLoreRealign(npcLoreMapToArr(_o));
          if (_arr.join('').replace(/\s/g, '')) return _arr;
        }
      }
      return npcLoreRealign(npcLoreSplit(t));
    }
    function npcLoreMissing(arr) {
      var miss = [];
      for (var i = 0; i < arr.length; i++) if (!String(arr[i] || '').trim()) miss.push(i);
      return miss;
    }
    /* v200：十段各自的填写提示 + AI 那个 SVG 按钮的图标 */
    /* v203：人设编辑不带任何预设提示文字，按用户要求留空 */
    var NPC_SEC_TIP = ['', '', '', '', '', '', '', '', '', ''];
    var NPC_AI_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l1.7 4.6L18 9.3l-4.3 1.7L12 15.6l-1.7-4.6L6 9.3l4.3-1.7z"/><path d="M18.2 15.2l.7 1.9 1.9.7-1.9.7-.7 1.9-.7-1.9-1.9-.7 1.9-.7z"/><path d="M5.4 2.6l.5 1.4 1.4.5-1.4.5-.5 1.4-.5-1.4L3.5 4.5l1.4-.5z"/></svg>';
    function npcLoreCurrent() {
      var names = npcLoreSectionNames(), out = [];
      for (var i = 0; i < names.length; i++) {
        var el = memberEditScroll.querySelector('#npcSec' + i);
        out.push(el ? String(el.value || '') : '');
      }
      return out;
    }
    function npcSecStatusSync() {
      var names = npcLoreSectionNames();
      for (var i = 0; i < names.length; i++) {
        var el = memberEditScroll.querySelector('#npcSec' + i);
        var st = memberEditScroll.querySelector('#npcSecSt' + i);
        if (!st) continue;
        var v = el ? String(el.value || '').trim() : '';
        var _n = v ? v.replace(/\s/g, '').length : 0;
        st.textContent = v ? (_n + ' 字') : '待补';
        st.className = 'npc-sec-st' + (_n >= 120 ? ' ok' : (_n > 0 ? ' half' : ''));
      }
      var cnt = memberEditScroll.querySelector('#npcLoreCount');
      if (cnt) {
        var _tot = npcLoreTotal(npcLoreCurrent());
        cnt.textContent = '已写 ' + _tot + ' 字' + (_tot >= 2000 ? '　✓ 达标' : '　还差 ' + (2000 - _tot) + ' 字到 2000');
        cnt.className = 'npc-lore-count' + (_tot >= 2000 ? ' ok' : '');
      }
      npcSecProgSync();
    }
    /* v207：每个板块一条自己的进度条，接在该板块文字后面。
       空＝待写（0%）；AI 正在写这一段＝进度条自己走（到 90% 封顶）；写好了＝满格 + 该段真实字数。 */
    var npcSecProgTimers = {};
    function npcSecProgSet(i, pct, label, state) {
      var bar = memberEditScroll.querySelector('#npcSecProgIn' + i);
      var tx = memberEditScroll.querySelector('#npcSecProgTx' + i);
      if (bar) {
        bar.style.width = Math.max(0, Math.min(100, Math.round(pct))) + '%';
        bar.className = 'npc-prog-in' + (state === 'done' ? ' done' : '');
      }
      if (tx) {
        tx.textContent = label;
        tx.className = 'npc-sec-prog-tx' + (state === 'done' ? ' done' : (state === 'writing' ? ' writing' : ''));
      }
    }
    function npcSecProgStopAll() {
      Object.keys(npcSecProgTimers).forEach(function (k) {
        clearInterval(npcSecProgTimers[k]);
        delete npcSecProgTimers[k];
      });
    }
    function npcSecProgSync() {
      var names = npcLoreSectionNames();
      for (var i = 0; i < names.length; i++) {
        if (npcSecProgTimers[i]) continue;  /* 正在写的那一段归定时器管，别覆盖 */
        var el = memberEditScroll.querySelector('#npcSec' + i);
        var n = el ? String(el.value || '').replace(/\s/g, '').length : 0;
        if (n) npcSecProgSet(i, 100, '已写 ' + n + ' 字', 'done');
        else npcSecProgSet(i, 0, '待写', '');
      }
      var done = 0, tot = names.length;
      for (var j = 0; j < tot; j++) {
        var e2 = memberEditScroll.querySelector('#npcSec' + j);
        if (e2 && String(e2.value || '').trim()) done++;
      }
      var gtx = memberEditScroll.querySelector('#npcProgTx');
      if (gtx) gtx.textContent = done ? ('已写 ' + done + '/' + tot + ' 段') : '';
    }
    function npcSecProgStart(i) {
      npcSecProgStopAll();
      var p = 8;
      npcSecProgSet(i, p, 'AI 写作中…', 'writing');
      npcSecProgTimers[i] = setInterval(function () {
        p = Math.min(90, p + Math.random() * 6 + 3);
        npcSecProgSet(i, p, 'AI 写作中… ' + Math.round(p) + '%', 'writing');
      }, 360);
    }
    function npcSecProgStop(i) {
      if (npcSecProgTimers[i]) { clearInterval(npcSecProgTimers[i]); delete npcSecProgTimers[i]; }
    }
    function npcLoreFillToForm(arr) {
      var names = npcLoreSectionNames();
      for (var i = 0; i < names.length; i++) {
        var el = memberEditScroll.querySelector('#npcSec' + i);
        if (el && String(arr[i] || '').trim()) el.value = String(arr[i]).trim();
      }
      if (memNpcDraft) memNpcDraft.lore = npcLoreJoin(npcLoreCurrent());
      npcSecProgStopAll();
      npcSecStatusSync();
    }
    function npcLoreJsonOf(names, arr) {
      var o = {};
      names.forEach(function (nm, i) { o[nm] = String((arr && arr[i]) || ''); });
      return JSON.stringify(o, null, 2);
    }
    function memberRerender() {
      memberChromeSync();
      /* v196：处于 NPC 编辑页时，重渲染只刷新 NPC 页 */
      if (memNpcEditIdx !== null && memNpcEditIdx !== undefined) { memberNpcSyncForm(); memberNpcRender(); return; }
      /* v197：处于 NPC 名单页时，重渲染只刷新名单页 */
      if (memNpcListView) { memberNpcListRender(); return; }
      var m = null;
      if (memEditIdx >= 0) { var _l = memberLoad(); m = _l[memEditIdx]; }
      // 重渲染前先把当前表单文字同步到内存，避免丢字
      var keep = { realName: '', netName: '', lore: '', voiceId: '', voiceLang: 'zh-CN', voiceSpeed: 1, look: '', lookPrompt: '', qrText: '' };
      var keepIds = { memRealName: 'realName', memNetName: 'netName', memLore: 'lore', memVoiceId: 'voiceId', memVoiceLang: 'voiceLang', memVoiceSpeed: 'voiceSpeed', memLook: 'look', memQrText: 'qrText' };
      /* v177：锁定生图提示词是下拉框（存 id），重渲染时按 id 还原 */
      var _lpSelEl = memberEditScroll.querySelector('#memLookPrompt');
      keep.lookPromptId = _lpSelEl ? String(_lpSelEl.value || '') : ((m && m.lookPromptId) || '');
      keep.lookPrompt = keep.lookPromptId ? memberLookPromptOf(keep.lookPromptId) : ((m && m.lookPrompt) || '');
      Object.keys(keepIds).forEach(function (id) {
        ...
        if (el) keep[keepIds[id]] = el.value;
          ...
          var mm = m || {};
          var kk = keepIds[id];
          if (kk === 'lookPrompt') keep[kk] = mm.lookPrompt || '';
          else if (kk === 'qrText') keep[kk] = memQrText || '';
          else if (kk === 'voiceLang') keep[kk] = (mm.voice && mm.voice.lang) || 'zh-CN';
          else if (kk === 'voiceSpeed') keep[kk] = (mm.voice && mm.voice.speed != null) ? mm.voice.speed : 1;
          else keep[kk] = mm[kk] || '';
        }
      });
      // NPC 关系输入同步
      ...
      memberSyncAltInputs();
      /* v196：从 NPC 页返回时，用缓存把主角色表单文字原样还原 */
      if (memRoleFormCache) {
        ['realName', 'netName', 'lore', 'voiceId', 'look', 'qrText'].forEach(function (kk) {
          if (memRoleFormCache[kk] != null && String(memRoleFormCache[kk]) !== '') keep[kk] = memRoleFormCache[kk];
        });
        if (memRoleFormCache.voiceLang) keep.voiceLang = memRoleFormCache.voiceLang;
        if (memRoleFormCache.voiceSpeed != null && memRoleFormCache.voiceSpeed !== '') keep.voiceSpeed = memRoleFormCache.voiceSpeed;
        if (memRoleFormCache.lookPromptId != null) keep.lookPromptId = memRoleFormCache.lookPromptId;
        if (keep.lookPromptId) keep.lookPrompt = memberLookPromptOf(keep.lookPromptId);
        if (memRoleFormCache.gender) memGender = memRoleFormCache.gender;
        if (memRoleFormCache.avatar != null && memRoleFormCache.avatar !== '') memAvatar = memRoleFormCache.avatar;
      }
      /* v208 修复：新增角色时上传头像，刚敲的名字/人设会被清空 ——
         原来这里在「新增（memEditIdx<0）」且没有 memRoleFormCache 时给的是 null，
         memberEditBody(null) 直接画出一张空表单，刚输入的文字就没了。
         现在无论如何都拿上面刚同步好的 keep 组一个 base（新增的带 __isNew），
         头像重渲染只换头像，文字原样留着。 */
      var base = m ? {
        ...
        voice: { enabled: true, voiceId: keep.voiceId, lang: keep.voiceLang || 'zh-CN', langLabel: '', speed: keep.voiceSpeed, freq: memVoiceFreq },
        lookPrompt: keep.lookPrompt, lookPromptId: keep.lookPromptId, id: memTempUid
      } : {
        realName: keep.realName, netName: keep.netName, lore: keep.lore,
        voiceId: keep.voiceId, look: keep.look, avatar: memAvatar, gender: memGender,
        voice: { enabled: true, voiceId: keep.voiceId, lang: keep.voiceLang || 'zh-CN', langLabel: '', speed: keep.voiceSpeed, freq: memVoiceFreq },
        lookPrompt: keep.lookPrompt, lookPromptId: keep.lookPromptId, id: memTempUid, __isNew: true
      };
      memberEditScroll.innerHTML = memberEditBody(base);
      memberBindEdit();
    }
    // v177：锁定生图提示词只存「系统设置 → 生图提示词」里那一条的 id，用的时候还原成文本
    function memberLookPromptOf(id) {
      var id2 = String(id || '');
      if (!id2) return '';
      try {
        var _l = (typeof imgPrompts !== 'undefined' && imgPrompts) ? imgPrompts : [];
        for (var i = 0; i < _l.length; i++) {
          if (String(_l[i].id || i) === id2) return imgPromptText(_l[i].pos || _l[i].content || '');
        }
      } catch (e) {}
      return '';
    }
    function memberLookPromptText() {
      var el = memberEditScroll.querySelector('#memLookPrompt');
      if (!el) return memLookPrompt || '';
      return memberLookPromptOf(el.value);
    }
    // 当前表单内容读取
    function memberCollectForm() {
      ...
      var langEl = memberEditScroll.querySelector('#memVoiceLang');
      var spdEl = memberEditScroll.querySelector('#memVoiceSpeed');
      var freqEl = memberEditScroll.querySelector('#memVoiceFreqBox .vf-btn.active');
      return {
        realName: g('memRealName'), netName: g('memNetName'),
        lore: g('memLore'), voiceId: g('memVoiceId'), look: g('memLook'),
        voiceLang: langEl ? String(langEl.value || '') : '',
        voiceSpeed: spdEl ? (parseFloat(spdEl.value) || 1) : 1,
        voiceFreq: freqEl ? String(freqEl.getAttribute('data-val') || '') : (memVoiceFreq || '中等'),
        lookPrompt: memberLookPromptText(), lookPromptId: g('memLookPrompt'), qrText: g('memQrText'),
        card: (function () {
          var fields = {};
          var _any = memberEditScroll.querySelectorAll('[data-mact="cardfield"]');
          ['netName', 'gender', 'lore', 'look', 'voice', 'alts', 'npcs'].forEach(function (k) {
            if (!_any.length) { fields[k] = !!(memCard.fields && memCard.fields[k]); return; }
            var el = memberEditScroll.querySelector('[data-mact="cardfield"][data-val="' + k + '"]');
            fields[k] = el ? el.classList.contains('on') : !!(memCard.fields && memCard.fields[k]);
          });
          if (memCard.mode === 'all') fields = { netName: true, gender: true, lore: true, look: true, voice: true, alts: true, npcs: true };
          return { mode: memCard.mode, fields: fields };
        })(),
        wbList: (function () {
          var wrap = memberEditScroll.querySelector('#memWbList');
          if (!wrap) return memWbList.slice();
          var out = [];
          wrap.querySelectorAll('.mem-wb-item.on').forEach(function (el) {
            var t = String(el.getAttribute('data-val') || '').trim();
    ...
    function memberSaveEdit() {
      ...
      memQrText = v.qrText;
      if (v.card) memCard = v.card;
      if (Array.isArray(v.wbList)) memWbList = v.wbList.slice();
      var cleanAlts = [];
      memAlts.forEach(function (s) { s = String(s || '').trim(); if (s && cleanAlts.indexOf(s) < 0) cleanAlts.push(s); });
      var list = memberLoad();
      var voiceObj = {
        enabled: true,
        voiceId: String(v.voiceId || '').trim(),
        lang: String(v.voiceLang || 'zh-CN').trim() || 'zh-CN',
        langLabel: chatLangLabelOf(String(v.voiceLang || 'zh-CN')),
        speed: (v.voiceSpeed != null && !isNaN(Number(v.voiceSpeed))) ? Number(v.voiceSpeed) : 1,
        freq: String(v.voiceFreq || '中等').trim() || '中等'
      };
      var obj = memberNorm({
        id: memTempUid, avatar: memAvatar, realName: v.realName, netName: v.netName,
        gender: memGender, lore: v.lore, voiceId: v.voiceId, voice: voiceObj, look: v.look,
        lookImgs: memLookImgs, lookPrompt: v.lookPrompt, lookPromptId: v.lookPromptId, npcs: memNpcs,
        alts: cleanAlts, qrText: memQrText, album: memAlbum, roleAdd: memRoleAdd, npcAdd: memNpcAdd,
        card: v.card, wbList: v.wbList
      });
      if (memEditIdx >= 0 && list[memEditIdx]) list[memEditIdx] = obj;
      else list.unshift(obj);
      memberSave(list);
      chatVoiceSyncMemberConvs(obj.id, obj.voice);
      closeMemberEdit();
      renderMemberList();
      toast('角色已保存');
    }
    ...
    /* v188：二维码统一走 canvas 生成 PNG（内嵌库只提供 gif，这里自己画，格式可控、扫码更稳） */
    function qrToPng(q, cellSize, margin) {
      cellSize = cellSize || 6;
    ...
    /* v191：音色 ID → 中文名（名片 / 确认卡上给人看，不再甩一串英文 ID） */
    var VOICE_NAMES = {
      ...
      'presenter_male': '主持男声', 'presenter_female': '主持女声',
      'audiobook_male_1': '有声书男声1', 'audiobook_male_2': '有声书男声2',
      'audiobook_female_1': '有声书女声1', 'audiobook_female_2': '有声书女声2'
    };
    function voiceNameOf(vid) {
      var v = String(vid || '').trim();
      if (!v) return '';
      var jp = /-jingpin$/.test(v);
      var base = v.replace(/-jingpin$/, '');
      var hit = VOICE_NAMES[v] || VOICE_NAMES[base];
      if (hit) return hit + (jp ? '·精品' : '');
      if (/^[A-Za-z0-9_-]+$/.test(v) && v.length > 16) return '自定义音色';
      return v;
    }
    /* v191：二维码美化——名片风，中心嵌角色头像、上方名字、下方提示（H 级纠错，挖心也能扫） */
    ...
    function qrFitFont(ctx, txt, maxW, size, weight) {
      var s = size;
      var set = function (n) { ctx.font = (weight || '') + n + 'px -apple-system,"PingFang SC","Microsoft YaHei",sans-serif'; };
      set(s);
      while (s > 9 && ctx.measureText(txt).width > maxW) { s -= 1; set(s); }
      return s;
    }
    function qrAvatarImage(av, cb) {
      var a = String(av || '');
      if (!a || !/^(https?:|data:image\/)/.test(a)) { cb(null); return; }
      var im = new Image();
      var done = false;
      var end = function (ok) { if (done) return; done = true; cb(ok ? im : null); };
      im.onload = function () { end(true); };
      im.onerror = function () { end(false); };
      setTimeout(function () { end(false); }, 2500);
      try { im.crossOrigin = 'anonymous'; im.src = a; } catch (e) { end(false); }
    }
    function qrDrawCard(q, cell, img, name, sub, opt) {
      try {
        /* v193：只要「纯二维码」——不再印名字与提示语，白底 + 码本身 + 中心头像 */
        /* v200：最外圈按公开范围加一条彩色细环（绿=全部公开 / 黄=部分公开 / 红=仅私密） */
        opt = opt || {};
        var ring = String(opt.ring || '');
        var ringW = ring ? Math.max(4, Math.round(cell * 1.5)) : 0;
        var mgn = ring ? (ringW + Math.round(cell * 1.1)) : 0;
        var n = q.getModuleCount();
        var pad = Math.round(cell * 5);
        var qz = n * cell;
        var W = qz + pad * 2 + mgn * 2;
        var cv = document.createElement('canvas');
        cv.width = W; cv.height = W;
        var ctx = cv.getContext('2d');
        ctx.fillStyle = '#ffffff';
        qrRoundRect(ctx, 0, 0, W, W, Math.round(cell * 8)); ctx.fill();
        if (ring) {
          ctx.strokeStyle = ring;
          ctx.lineWidth = ringW;
          var _ri = ringW / 2 + Math.max(1, Math.round(cell * 0.4));
          qrRoundRect(ctx, _ri, _ri, W - _ri * 2, W - _ri * 2, Math.round(cell * 7)); ctx.stroke();
        }
        ctx.fillStyle = '#17171b';
        var _off = mgn + pad;
        for (var r = 0; r < n; r++) {
          for (var c = 0; c < n; c++) {
            if (q.isDark(r, c)) ctx.fillRect(_off + c * cell, _off + r * cell, cell, cell);
          }
        }
        /* 中心角色头像：白底圆 + 圆裁头像；头像跟随角色，没头像就用首字 */
        var cx = _off + qz / 2, cy = _off + qz / 2;
        var rr = qz * 0.16;
        ctx.fillStyle = '#ffffff';
        ctx.beginPath(); ctx.arc(cx, cy, rr, 0, Math.PI * 2); ctx.fill();
        var ir = img ? rr * 0.86 : rr * 0.78;
        ctx.save();
        ctx.beginPath(); ctx.arc(cx, cy, ir, 0, Math.PI * 2); ctx.clip();
        if (img) {
          ctx.drawImage(img, cx - ir, cy - ir, ir * 2, ir * 2);
        } else {
          var g = ctx.createLinearGradient(cx - ir, cy - ir, cx + ir, cy + ir);
          g.addColorStop(0, '#5ac8fa'); g.addColorStop(1, '#7c5cff');
          ctx.fillStyle = g; ctx.fillRect(cx - ir, cy - ir, ir * 2, ir * 2);
          ctx.fillStyle = '#ffffff';
          ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
          ctx.font = '700 ' + Math.round(ir * 1.05) + 'px -apple-system,"PingFang SC",sans-serif';
    ...
    function qrPrettyPng(text, opt, cb) {
      opt = opt || {};
      var cell = opt.cell || 6;
      var name = String(opt.name || '');
      var sub = String(opt.sub || '扫一扫 · 加我为好友');
      qrAvatarImage(opt.avatar, function (img) {
        var levels = [['H', !!img], ['Q', !!img], ['M', false], ['L', false]];
        var png = '';
        for (var i = 0; i < levels.length; i++) {
          try {
            if (typeof qrcode !== 'function') break;
            if (qrcode.stringToBytesFuncs && qrcode.stringToBytesFuncs['UTF-8']) qrcode.stringToBytes = qrcode.stringToBytesFuncs['UTF-8'];
            var q = qrcode(0, levels[i][0]);
            q.addData(String(text || ''));
            q.make();
            png = qrDrawCard(q, cell, levels[i][1] ? img : null, name, sub, { ring: opt.ring || '' });
          } catch (e) { png = ''; }
          if (png) break;
        }
        if (!png) png = memQrUrl(text);
        cb(png, text);
      });
    }
    function memberQrPreviewName() {
      try { var v = memberCollectForm(); return String(v.realName || v.netName || ''); } catch (e) { return ''; }
    }
    /* v186：二维码本地生成（内嵌 qrcode 库），不依赖任何外部接口，图必出 */
      ...
      if (t) {
        try {
          if (typeof qrcode === 'function') {
            if (qrcode.stringToBytesFuncs && qrcode.stringToBytesFuncs['UTF-8']) qrcode.stringToBytes = qrcode.stringToBytesFuncs['UTF-8'];
            var build = function (ec) { var q = qrcode(0, ec); q.addData(t); q.make(); return qrToPng(q, 8, 8); };
            try { return build('M'); } catch (e1) {}
            try { return build('L'); } catch (e2) {}
          }
        } catch (e) {}
      }
      return 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="240" height="240"><rect width="240" height="240" fill="#ffffff"/><text x="120" y="116" font-size="13" text-anchor="middle" fill="#c0392b">二维码生成失败</text><text x="120" y="138" font-size="12" text-anchor="middle" fill="#8a8a93">请缩短二维码内容后重试</text></svg>');
    }
    ...
    function memberQrSync() {
      ...
      if (!pv) return;
      if (!memQrText) { pv.innerHTML = '<span class="mem-qr-empty">点“生成二维码”试试</span>'; return; }
      var seq = ++qrPreviewSeq;
      pv.innerHTML = '<span class="mem-qr-empty">正在生成名片码…</span>';
      /* v191：预览就是美化名片码（中心带头像），点开是大图，保存到相册的也是它 */
      qrPrettyPng(memQrText, { avatar: memAvatar, name: memberQrPreviewName(), cell: 5, sub: '扫一扫 · 加我为好友', ring: memCardRingOf(memCard) }, function (png) {
        if (seq !== qrPreviewSeq) return;
        var cur = memberEditScroll.querySelector('#memQrPreview');
        if (!cur) return;
        cur.innerHTML = '<img src="' + png + '" alt="角色二维码" data-qzoom="1">';
      });
    }

    /* ===== v189：二维码点开看大图 / 保存到本地 ===== */
    function memberQrBigPng(text, opt, cb) {
      if (typeof opt === 'function') { cb = opt; opt = {}; }
      opt = opt || {};
      qrPrettyPng(text, { avatar: opt.avatar, name: opt.name, cell: 9, sub: '扫一扫 · 加我为好友', ring: opt.ring || qrRingFromText(text) }, function (png) { cb(png, String(text || '')); });
    }
    function qrPngToBlob(png) {
      try {
        var s = String(png || '');
        var i = s.indexOf(',');
        if (i < 0) return null;
        var head = s.slice(0, i), body = s.slice(i + 1);
    ...
    /* v190：优先走系统「分享 / 存储图像」面板，手机可一步存进相册；不支持时退回下载 */
    function memberQrSave(png, text) {
      try {
        if (navigator.share && navigator.canShare && typeof File === 'function') {
          var blob = qrPngToBlob(png);
          if (blob) {
            var file = new File([blob], 'aetheron-qr.png', { type: 'image/png' });
            if (navigator.canShare({ files: [file] })) {
              navigator.share({ files: [file], title: '角色二维码' }).then(function () {
                toast('已打开系统面板：选「存储图像 / 保存到相册」即可存进相册');
              }, function () { memberQrDownload(png, text); });
              return;
            }
          }
        }
      } catch (e) {}
      memberQrDownload(png, text);
    }
    function memberQrDownload(png, text) {
      var fn = 'aetheron-qr-' + Date.now() + '.png';
      try {
        ...
        a.click();
        setTimeout(function () { if (a.parentNode) a.parentNode.removeChild(a); }, 150);
        toast('已保存：' + fn + '（手机若只提示下载，去「文件」里也能看到；也可以长按二维码图片选\u201c存储图像\u201d）');
      } catch (e) {
        toast('保存失败，长按图片选\u201c存储图像\u201d也能存');
      }
    }
    function openQrViewer(text, opt) {
      if (typeof opt === 'string') opt = { name: opt };
      opt = opt || {};
      var old = document.getElementById('memQrViewer');
      if (old && old.parentNode) old.parentNode.removeChild(old);
      memberQrBigPng(text, opt, function (png) {
        var ov = document.createElement('div');
        ov.id = 'memQrViewer';
        ov.className = 'mem-album-viewer mem-qr-viewer';
        var box = document.createElement('div');
        box.className = 'mqv-box';
        var im = document.createElement('img');
        im.className = 'mqv-img';
        im.alt = '角色二维码';
        im.src = png;
        var bar = document.createElement('div');
        bar.className = 'mqv-bar';
        var sv1 = document.createElement('button');
        sv1.type = 'button'; sv1.className = 'mqv-btn'; sv1.textContent = '保存到相册';
        var cls = document.createElement('button');
        cls.type = 'button'; cls.className = 'mqv-btn ghost'; cls.textContent = '关闭';
        bar.appendChild(sv1); bar.appendChild(cls);
        var tip2 = document.createElement('div');
        tip2.className = 'mqv-tip';
        tip2.textContent = '点「保存到相册」，或长按图片选「存储图像」';
        box.appendChild(im); box.appendChild(tip2); box.appendChild(bar);
        ov.appendChild(box);
        document.body.appendChild(ov);
        function shut() { if (ov.parentNode) ov.parentNode.removeChild(ov); }
        ov.addEventListener('click', function (e) { if (e.target === ov) shut(); });
        cls.addEventListener('click', function (e) { e.stopPropagation(); shut(); });
        sv1.addEventListener('click', function (e) { e.stopPropagation(); memberQrSave(png, text); });
      });
    }
    /* v187：角色名片链接（扫码打开名片页，按「公开范围」决定能看到什么） */
    function memberCardUrl(src, card, opt) {
      var base = 'https://71931.github.io/Aetheron-AEphone/card.html#d=';
      src = src || {};
      /* v193：opt 只在「二维码」链路使用——头像用压缩小图（保证码里放得下），并按预算瘦身字段 */
      var _tight = !!opt;
      var _hasAv = _tight && ('av' in opt);
      var _b = (_tight && opt.budget && typeof opt.budget === 'object') ? opt.budget : null;
      var _bioMax = _b ? (_b.bio == null ? 160 : _b.bio) : 160;
      var _lookMax = _b ? (_b.look == null ? 160 : _b.look) : 160;
      var _altsMax = _b ? (_b.alts == null ? 6 : _b.alts) : 6;
      var _npcMax = _b ? (_b.npc == null ? 8 : _b.npc) : 8;
      var nm = String(src.realName || src.netName || '未命名角色').slice(0, 24);
      /* v190：这张名片码同时就是「加好友码」——带上角色ID与类型标记，站内扫一扫识别后可直接加好友 */
      var p = { v: 1, k: 'aether-member', n: nm, id: String(src.id || '') };
      var mode = (card && card.mode) ? card.mode : 'all';
      if (mode === 'secret') {
        p.p = 'secret';
      } else {
        p.p = (mode === 'part') ? 'part' : 'all';
        var f = (mode === 'part' && card && card.fields) ? card.fields
              : { netName: true, gender: true, lore: true, look: true, voice: true, alts: true, npcs: true };
        if (f.netName && src.netName) p.nn = String(src.netName).slice(0, 40);
        if (f.gender && src.gender) p.g = (src.gender === 'male') ? '男' : (src.gender === 'female' ? '女' : '保密');
        if (f.lore && src.lore && _bioMax > 0) p.bio = String(src.lore).slice(0, _bioMax);
        if (f.look && src.look && _lookMax > 0) p.look = String(src.look).slice(0, _lookMax);
        if (f.voice && src.voiceId) p.voice = String(src.voiceId).slice(0, 60);
        if (f.alts && src.alts && src.alts.length && _altsMax > 0) p.alts = src.alts.slice(0, _altsMax).map(function (x) { return String(x).slice(0, 20); });
        if (f.npcs && src.npcs && src.npcs.length && _npcMax > 0) p.npc = src.npcs.slice(0, _npcMax).map(function (x) { return { n: String(x.name || '').slice(0, 20), r: String(x.rel || '').slice(0, 20) }; });
        var av = _hasAv ? String(opt.av || '') : String(src.avatar || '');
        if (av && av.length <= (_hasAv ? 1100 : 400)) p.av = av;
      }
      p.ts = Date.now();
      var url = base + b64uEncode(JSON.stringify(p));
      if (url.length > 2000) {
        if (p.bio) p.bio = p.bio.slice(0, 100);
        if (p.look) p.look = p.look.slice(0, 100);
        url = base + b64uEncode(JSON.stringify(p));
      }
      return url;
    }
    function memberQrDefault(silent) {
      var v = memberCollectForm();
      var src = {
        id: memTempUid || '', realName: v.realName, netName: v.netName,
        gender: memGender, lore: v.lore, voiceId: v.voiceId, look: v.look,
        lookPrompt: v.lookPrompt, alts: memAlts, npcs: memNpcs, avatar: memAvatar,
        roleAdd: memRoleAdd, npcAdd: memNpcAdd
      };
      memberQrCardUrl(src, v.card, function (txt) {
        memQrText = txt || memberCardUrl(src, v.card);
        memDirty = true;
        memberQrSync();
      });
      var mtxt = (v.card && v.card.mode === 'secret') ? '（仅私密：只显示名字）'
               : ((v.card && v.card.mode === 'part') ? '（部分公开：只显示勾选项）' : '（全部公开）');
      if (!silent) toast('二维码已生成' + mtxt + '，别人扫码即可加 TA 为好友');
    }
    function memberQrShare() {
      var _v = memberCollectForm();
      /* v201：好友码按当前公开范围生成，私密/部分/全部扫出来内容不同 */
      var txt = memberShareCodeOf({
        id: memTempUid || '', realName: _v.realName, netName: _v.netName,
        gender: memGender, lore: _v.lore, voiceId: _v.voiceId, look: _v.look,
        lookPrompt: _v.lookPrompt, alts: memAlts, npcs: memNpcs,
        roleAdd: memRoleAdd, npcAdd: memNpcAdd
      }, _v.card || memCard);
      var _rm = (_v.card && _v.card.mode) ? _v.card.mode : memCard.mode;
      var _rt = _rm === 'secret' ? '仅私密（只带名字）' : (_rm === 'part' ? '部分公开（只带你勾选的字段）' : '全部公开（资料全带上）');
      var done = function (ok) { toast(ok ? ('好友码已复制（' + _rt + '），发给别人粘贴即可添加 TA') : '复制失败，请手动选择复制'); };
    ...
    function memberLoreImportFile(f) {
      ...
      if (isPdf) { toast('暂不支持 PDF，可先另存为 txt / md / docx / doc 再导入'); return; }
      /* v208：老 .doc（Word 97-2003 / WPS 存的 doc）也能导入了 —— 直接从二进制里把正文捞出来 */
      if (isOldDoc) {
        var rdDoc = new FileReader();
        rdDoc.onload = function () {
          var txt = extractDocText(rdDoc.result);
          if (txt && txt.replace(/\s+/g, '').length >= 20) memberLoreFillText(txt, f.name);
          else toast('这个 .doc 里的文字没能自动读出来，建议用 WPS/Word 另存为 .docx 或 .txt 再导入');
        };
        rdDoc.onerror = function () { toast('读取文件失败'); };
        rdDoc.readAsArrayBuffer(f);
        return;
      }
      if (isDocx) {
    ...
    /* ===== v183：角色相册（测试生图结果入库 · 全屏翻阅 / 下载 / 删除） ===== */
    var MEM_ALBUM_MAX = 60;
    /* 相册图统一压到 1080 长边 JPEG，避免 IndexedDB 被 base64 撑爆 */
    function memberAlbumShrink(src, cb) {
      if (!src) { cb(''); return; }
      if (src.indexOf('data:') !== 0 && src.indexOf('blob:') !== 0) { cb(src); return; }
      var im = new Image();
      im.onload = function () {
        try {
          var MAX = 1080;
          var sc = Math.min(1, MAX / Math.max(im.width || 1, im.height || 1));
          var w = Math.max(1, Math.round((im.width || 1) * sc));
          var h = Math.max(1, Math.round((im.height || 1) * sc));
          var cv = document.createElement('canvas');
          cv.width = w; cv.height = h;
          cv.getContext('2d').drawImage(im, 0, 0, w, h);
          var out = cv.toDataURL('image/jpeg', 0.82);
          cb(out && out.length > 200 ? out : src);
        } catch (e) { cb(src); }
      };
      im.onerror = function () { cb(src); };
      im.src = src;
    }
    /* 编辑已有角色时立即落库（不必等点「保存」），新增角色交给保存流程 */
    function memberAlbumPersist() {
      if (memEditIdx < 0) return false;
      try {
        var l = memberLoad();
        if (!l[memEditIdx]) return false;
        l[memEditIdx].album = memAlbum.map(function (a) { return { id: a.id, src: a.src, ts: a.ts, note: a.note, savedBy: a.savedBy }; });
        memberSave(l);
        return true;
      } catch (e) { return false; }
    }
    /* v184：相册不再平铺展示，改成一行 SVG 图标入口，点开才是全屏相册 */
    function memberAlbumHtml() {
      return '<div class="mem-album-entry" data-mact="albumhome" role="button" tabindex="0">' +
        '<span class="mem-album-entry-ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="5" width="18" height="14" rx="2.5"/><circle cx="8.6" cy="10" r="1.5"/><path d="M21 15.6L16.4 11 6 19"/></svg></span>' +
        '<span class="mem-album-entry-txt"><b>相册</b><i>' + (memAlbum.length ? (memAlbum.length + ' 张照片 · 点开全屏翻阅、删除；左下角标着谁存的，长按可设为头像') : '还没有照片 · 上方「测试生图」生成的图会自动收进来') + '</i></span>' +
        '<span class="mem-album-entry-arr"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 5l7 7-7 7"/></svg></span>' +
        '</div>';
    }
    /* v184：大图查看器里删完图，把下层的全屏相册也刷新一下 */
    function memberAlbumHomeSync() {
      var ov = document.getElementById('memAlbumHome');
      if (ov && ov.parentNode) renderMemberAlbumHome(ov);
    }
    function memberAlbumDeleteAt(i, cb) {
      if (!memAlbum[i]) return;
      memAlbum.splice(i, 1);
      var saved = memberAlbumPersist();
      memDirty = true;
      memberAlbumRefresh();
      toast(saved ? '已从相册删除' : '已删除（点「保存」后生效）');
      if (cb) cb();
    }
    /* v198：相册图左下角标记「谁存的」——我存的显示我的头像，TA 存的显示 TA 的头像 */
    function albumSaverAvatar(a, roleAvatar) {
      var me = !!(a && a.savedBy === 'me');
      var src = me ? ((chatMine && chatMine.avatar) || '') : String(roleAvatar || '');
      if (src) return '<img class="mah-saver-img" src="' + src + '" alt="">';
      return '<span class="mah-saver-tx">' + escHtml(me ? '我' : 'TA') + '</span>';
    }
    /* v198：把相册里第 i 张设为当前角色的头像（记进名单，并同步该角色所有聊天窗口的头像） */
    function memberSetAvatarFromAlbum(i) {
      try {
        var it = memAlbum[i];
        if (!it || !it.src) return false;
        var list = memberLoad();
        if (memEditIdx < 0 || !list[memEditIdx]) return false;
        list[memEditIdx].avatar = it.src;
        memberSave(list);
        memAvatar = it.src;
        try {
          var pv = memberEditScroll.querySelector('.mem-ava-img');
          if (pv) pv.src = it.src;
        } catch (ePv) {}
        roleAvatarSyncToConv(list[memEditIdx].id, it.src);
        return true;
      } catch (e) { return false; }
    }
    /* v198：名单角色换头像后，绑定了它的聊天窗口头像一起换 */
    function roleAvatarSyncToConv(roleId, src) {
      try {
        var rid = String(roleId || '');
        if (!rid || !src) return false;
        var rkey = rid.replace(/^m/, ''); /* v199：名单 id 形如 m+k3x2b，会话 contactId 可能带/不带 m 前缀，两侧都归一化再比 */
        var touched = false;
        (chatConvs || []).forEach(function (c) {
          if (!c || !c.contactId) return;
          var ccid = String(c.contactId);
          if (ccid !== rid && ccid.replace(/^m/, '') !== rkey) return;
          if (!c.settings) c.settings = defaultConvSettings();
          if (!c.settings.roleIdentity || typeof c.settings.roleIdentity !== 'object') {
            c.settings.roleIdentity = { name: c.name || '', avatar: '', sex: '', prompt: '', remark: '' };
          }
          c.settings.roleIdentity.avatar = src;
          touched = true;
        });
        if (touched) {
          saveConvs();
          if (typeof renderChatConvs === 'function') renderChatConvs();
          if (typeof renderChatMessages === 'function') renderChatMessages();
        }
        return touched;
      } catch (e) { return false; }
    }
    /* v185：全屏相册渲染（普通模式 / 选择模式） */
    function renderMemberAlbumHome(ov) {
      var st = ov._mahState || (ov._mahState = { sel: null });
      if (st.sel) st.sel = st.sel.filter(function (i) { return i >= 0 && i < memAlbum.length; });
      var body = ov.querySelector('.mah-body');
      var cntEl = ov.querySelector('.mah-c');
      var modeB = ov.querySelector('.mah-mode');
      var foot = ov.querySelector('.mah-foot');
      var tip = ov.querySelector('.mah-tip');
      var sel = !!st.sel;
      if (cntEl) cntEl.textContent = sel ? ('已选 ' + st.sel.length + ' 张') : (memAlbum.length + ' 张');
      if (modeB) { modeB.style.display = memAlbum.length ? '' : 'none'; modeB.textContent = sel ? '取消' : '选择'; }
      if (tip) tip.textContent = sel ? '点照片勾选 · 下方可全选 / 删除所选' : '点图看大图 · 长按可设为头像 · 右上角 ✕ 单张删除 · 右上角「选择」可批量删';
      if (foot) {
        foot.style.display = (sel && memAlbum.length) ? 'flex' : 'none';
        if (sel) {
          var allB = foot.querySelector('.mah-all');
          if (allB) allB.textContent = (st.sel.length >= memAlbum.length ? '取消全选' : '全选');
          var delB = foot.querySelector('.mah-del-sel');
          if (delB) delB.textContent = st.sel.length ? ('删除所选 ' + st.sel.length + ' 张') : '删除所选';
        }
      }
      if (!memAlbum.length) {
        body.innerHTML = '<div class="mah-empty">相册还是空的<br>在聊天里点开图片，用「保存到相册」存几张进来</div>';
        if (st.sel) st.sel = [];
        return;
      }
      body.innerHTML = '<div class="mah-grid">' + memAlbum.map(function (a, i) {
        var on = sel && st.sel.indexOf(i) >= 0;
        return '<div class="mah-cell' + (sel ? ' pick' : '') + (on ? ' sel' : '') + '" data-i="' + i + '">' +
          '<img src="' + a.src + '" alt="">' +
          ((a.note && !sel) ? '<div class="mah-note">' + escHtml(a.note) + '</div>' : '') +
          (sel ? '' : '<span class="mah-saver' + (a.savedBy === 'me' ? ' me' : '') + '" title="' + (a.savedBy === 'me' ? '我存的' : 'TA 存的') + '">' + albumSaverAvatar(a, memAvatar) + '</span>') +
          (sel
            ? '<span class="mah-pick"></span>'
            : '<button type="button" class="mah-del" data-del="' + i + '" title="删除这张"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><path d="M6 6l12 12M18 6L6 18"/></svg></button>') +
          '</div>';
      }).join('') + '</div>';
    }
    /* v185：批量删除（索引按从后往前删，避免下标错位） */
    function memberAlbumDeleteMany(idxList) {
      (idxList || []).slice().sort(function (a, b) { return b - a; }).forEach(function (i) {
        if (i >= 0 && i < memAlbum.length) memAlbum.splice(i, 1);
      });
      var saved = memberAlbumPersist();
      memDirty = true;
      memberAlbumRefresh();
    ...
    /* v184：全屏相册（点图标入口进来的界面）；v185：支持选择模式批量删除 */
    function openMemberAlbumHome() {
      var old = document.getElementById('memAlbumHome');
      if (old && old.parentNode) old.parentNode.removeChild(old);
      var ov = document.createElement('div');
      ov.id = 'memAlbumHome';
      ov.className = 'mem-album-home';
      ov._mahState = { sel: null };
      ov.innerHTML = '<div class="mah-head"><button type="button" class="mah-back"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 5l-7 7 7 7"/></svg></button>' +
        '<span class="mah-t">相册</span><span class="mah-c"></span>' +
        '<button type="button" class="mah-mode">选择</button></div>' +
        '<div class="mah-body"></div>' +
        '<div class="mah-foot" style="display:none"><button type="button" class="mah-all">全选</button><button type="button" class="mah-del-sel">删除所选</button></div>' +
        '<div class="mah-tip"></div>';
      document.body.appendChild(ov);
      renderMemberAlbumHome(ov);
      function closeHome() { if (ov.parentNode) ov.parentNode.removeChild(ov); }
      ov.querySelector('.mah-back').addEventListener('click', function () {
        if (ov._mahState.sel) { ov._mahState.sel = null; renderMemberAlbumHome(ov); return; }
        closeHome();
      });
      ov.querySelector('.mah-mode').addEventListener('click', function () {
        ov._mahState.sel = ov._mahState.sel ? null : [];
        renderMemberAlbumHome(ov);
      });
      ov.querySelector('.mah-all').addEventListener('click', function () {
        var st = ov._mahState;
        if (!st.sel) return;
        st.sel = (st.sel.length >= memAlbum.length) ? [] : memAlbum.map(function (_, i) { return i; });
        renderMemberAlbumHome(ov);
      });
      ov.querySelector('.mah-del-sel').addEventListener('click', function () {
        var st = ov._mahState;
        if (!st.sel || !st.sel.length) { toast('先勾选要删的照片'); return; }
        var nSel = st.sel.length;
        if (!window.confirm('删除所选 ' + nSel + ' 张相册图？删掉之后补不回来。')) return;
        var saved = memberAlbumDeleteMany(st.sel);
        st.sel = null;
        renderMemberAlbumHome(ov);
        toast(saved ? ('已删除 ' + nSel + ' 张') : ('已删除 ' + nSel + ' 张（点「保存」后生效）'));
      });
      /* v198：长按相册图 → 把这张设为头像（TA 自己也会在发图时自主换头像） */
      var _mahLp = null, _mahLpX = 0, _mahLpY = 0;
      ov.addEventListener('pointerdown', function (e) {
        var cell = e.target.closest ? e.target.closest('.mah-cell') : null;
        if (!cell || ov._mahState.sel) return;
        var li = parseInt(cell.getAttribute('data-i'), 10);
        if (!(li >= 0)) return;
        _mahLpX = e.clientX; _mahLpY = e.clientY;
        clearTimeout(_mahLp);
        _mahLp = setTimeout(function () {
          _mahLp = null;
          if (!memAlbum[li]) return;
          if (!window.confirm('把这张照片设为头像？')) return;
          ov._lpFired = true;
          if (memberSetAvatarFromAlbum(li)) toast('头像已换成这张');
          else toast('换头像失败：先确认这个角色在名单里');
        }, 550);
      });
      ov.addEventListener('pointermove', function (e) {
        if (!_mahLp) return;
        if (Math.abs(e.clientX - _mahLpX) > 10 || Math.abs(e.clientY - _mahLpY) > 10) { clearTimeout(_mahLp); _mahLp = null; }
      });
      ov.addEventListener('pointerup', function () { clearTimeout(_mahLp); _mahLp = null; });
      ov.addEventListener('pointercancel', function () { clearTimeout(_mahLp); _mahLp = null; });
      ov.addEventListener('click', function (e) {
        var t = e.target;
        if (ov._lpFired) { ov._lpFired = false; return; } /* v198：长按刚处理过，别再顺带打开大图 */
        var del = t.closest ? t.closest('.mah-del') : null;
        if (del) {
          e.stopPropagation();
          var di = parseInt(del.getAttribute('data-del'), 10);
          if (!memAlbum[di]) return;
          if (!window.confirm('删除这张相册图？删掉之后补不回来。')) return;
          memberAlbumDeleteAt(di, function () { renderMemberAlbumHome(ov); });
          return;
        }
        var cell = t.closest ? t.closest('.mah-cell') : null;
        if (!cell) return;
        var i = parseInt(cell.getAttribute('data-i'), 10) || 0;
        if (ov._mahState.sel) {
          var at = ov._mahState.sel.indexOf(i);
          if (at >= 0) ov._mahState.sel.splice(at, 1); else ov._mahState.sel.push(i);
          renderMemberAlbumHome(ov);
          return;
        }
        openMemberAlbum(i);
    ...
    /* v184：相册选图 —— multi=false 点一张就直接用，multi=true 多选后确认 */
    function memAlbumOfMember(m) { return (m && m.album && m.album.length) ? m.album : []; }
    function openAlbumPicker(opts) {
      opts = opts || {};
      var members = [];
      try { members = memberLoad() || []; } catch (e) { members = []; }
      var withAlbum = members.filter(function (m) { return memAlbumOfMember(m).length; });
      var old = document.getElementById('memAlbumPick');
      if (old && old.parentNode) old.parentNode.removeChild(old);
      var ov = document.createElement('div');
      ov.id = 'memAlbumPick';
      ov.className = 'mem-album-home';
      var st = { member: (opts.member && memAlbumOfMember(opts.member).length) ? opts.member : null, sel: [] };
      if (!st.member && withAlbum.length === 1) st.member = withAlbum[0];
      var title = opts.title || '从角色相册选图';
      function draw() {
        var cntTxt = st.member ? (st.sel.length ? ('已选 ' + st.sel.length + ' 张') : (memAlbumOfMember(st.member).length + ' 张')) : (withAlbum.length + ' 个角色');
        ov.innerHTML = '<div class="mah-head"><button type="button" class="mah-back"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 5l-7 7 7 7"/></svg></button>' +
          '<span class="mah-t">' + escHtml(title) + '</span><span class="mah-c">' + cntTxt + '</span></div>' +
          '<div class="mah-body"></div>' +
          (opts.multi ? '<div class="mah-foot"><button type="button" class="mah-ok">加入所选照片</button></div>' : '<div class="mah-tip">点一张照片，就用角色的身份发出去</div>');
        var body = ov.querySelector('.mah-body');
        if (!st.member) {
          if (!withAlbum.length) { body.innerHTML = '<div class="mah-empty">还没有角色相册<br>先去「名单 → 角色 → 测试生图」，生成的图会自动进相册</div>'; return; }
          body.innerHTML = '<div class="mah-rolelist">' + withAlbum.map(function (m, i) {
            return '<button type="button" class="mah-role" data-r="' + i + '"><span>' + escHtml(String(m.realName || m.netName || '角色')) + '</span><i>' + memAlbumOfMember(m).length + ' 张</i></button>';
          }).join('') + '</div>';
          return;
        }
        var list = memAlbumOfMember(st.member);
        if (!list.length) { body.innerHTML = '<div class="mah-empty">这个角色的相册还是空的</div>'; return; }
        body.innerHTML = '<div class="mah-grid">' + list.map(function (a, i) {
          var on = st.sel.indexOf(i) >= 0;
          return '<div class="mah-cell pick' + (on ? ' sel' : '') + '" data-pick="' + i + '"><img src="' + a.src + '" alt="">' +
            (a.note ? '<div class="mah-note">' + escHtml(a.note) + '</div>' : '') +
            '<span class="mah-saver' + (a.savedBy === 'me' ? ' me' : '') + '">' + albumSaverAvatar(a, st.member && st.member.avatar) + '</span>' +
            '<span class="mah-pick"></span></div>';
        }).join('') + '</div>';
      }
      function close() { if (ov.parentNode) ov.parentNode.removeChild(ov); }
      function fire() {
        var picked = st.sel.slice().sort(function (a, b) { return a - b; }).map(function (i) { return memAlbumOfMember(st.member)[i]; }).filter(Boolean);
        if (!picked.length) { toast('先选至少一张'); return; }
        close();
        if (opts.onPick) opts.onPick(picked);
      }
      ov.addEventListener('click', function (e) {
        var t = e.target;
        if (t.closest && t.closest('.mah-back')) { close(); return; }
        var rb = t.closest ? t.closest('.mah-role') : null;
        if (rb) { st.member = withAlbum[parseInt(rb.getAttribute('data-r'), 10)]; st.sel = []; draw(); return; }
        if (t.closest && t.closest('.mah-ok')) { fire(); return; }
        var cell = t.closest ? t.closest('.mah-cell.pick') : null;
        if (!cell) return;
        var idx = parseInt(cell.getAttribute('data-pick'), 10);
        if (opts.multi) {
          var at = st.sel.indexOf(idx);
          if (at >= 0) st.sel.splice(at, 1); else st.sel.push(idx);
          cell.classList.toggle('sel', st.sel.indexOf(idx) >= 0);
          var c = ov.querySelector('.mah-c');
          if (c) c.textContent = st.sel.length ? ('已选 ' + st.sel.length + ' 张') : (memAlbumOfMember(st.member).length + ' 张');
          return;
        }
        var it = memAlbumOfMember(st.member)[idx];
        if (!it) return;
        st.sel = [idx];
        fire();
      });
    ...
    /* v184：TA 从相册挑一张发给我（不走生图 API，char 也知道自己发的是哪张） */
    /* v194：拆成两条路——① 让 TA 自己挑并主动发（点一下就有图）② 用户从相册里挑 */
    function chatRolePickAlbumOne() {
      if (!chatCurrentConv) return;
      /* 图片菜单点完要收起来，别残留在聊天界面上 */
      try { if (typeof chatMiniMask !== 'undefined' && chatMiniMask) chatMiniMask.classList.remove('show'); } catch (e2) {}
      var m = null;
      try { m = memberByConv(chatCurrentConv); } catch (e) {}
      if (!m) { toast('这个会话还没绑定名单角色，先去「名单」里挑一个'); return; }
      var list = memAlbumOfMember(m);
      if (!list.length) { toast('相册还是空的：先在聊天里点开图片，用「保存到相册」存几张'); return; }
      var it = list[Math.floor(Math.random() * list.length)];
      var note = String((it && it.note) || '').trim();
      addChatMsg('other', { type: 'image', imgs: [it.src], picNote: note });
      addChatMsg('other', { type: 'text', text: note ? ('翻了翻相册，给你看这张——' + note) : '翻了翻相册，随手挑了一张给你。' });
      toast('TA 从相册里发了一张');
    }
    function chatPickFromAlbum() {
      if (!chatCurrentConv) return;
      var m = null;
      try { m = memberByConv(chatCurrentConv); } catch (e) {}
      try { if (typeof chatMiniMask !== 'undefined' && chatMiniMask) chatMiniMask.classList.remove('show'); } catch (e2) {}
      openAlbumPicker({
        member: m, multi: false, title: (chatCurrentConv.name || 'TA') + '相册',
        onPick: function (list) {
          var it = list[0];
          if (!it) return;
          addChatMsg('other', { type: 'image', imgs: [it.src], picNote: it.note || '' });
          toast('已用角色相册的照片发给TA');
        }
      });
    }
    function chatSendFromAlbum() { chatPickFromAlbum(); }
    /* 只刷新相册区，避免整页重渲染把刚生成的图冲掉 */
    function memberAlbumRefresh() {
      var host = memberEditScroll.querySelector('#memAlbumWrap');
      if (host) host.innerHTML = memberAlbumHtml();
    }
    /* v198：savedBy='me' 表示这张是我存进去的（左下角显示我的头像），默认 'ta' 表示 TA 生成/收到的 */
    function memberAlbumAdd(src, note, savedBy) {
      if (!src) return;
      if (memAlbum.length >= MEM_ALBUM_MAX) { toast('相册已满 ' + MEM_ALBUM_MAX + ' 张，先删掉几张再存'); return; }
      memberAlbumShrink(src, function (out) {
        if (!out) { toast('图片读取失败，没存进相册'); return; }
        memAlbum.push({ id: memberUid(), src: out, ts: Date.now(), note: String(note || '').slice(0, 60), savedBy: (savedBy === 'me' ? 'me' : 'ta') });
        var saved = memberAlbumPersist();
        memDirty = true;
        memberAlbumRefresh();
        toast(saved ? '已存入角色相册（共 ' + memAlbum.length + ' 张）' : '已存入相册，点「保存」后生效');
      });
    }
    function memberAlbumName() {
      try {
        var m = memEditIdx >= 0 ? memberLoad()[memEditIdx] : null;
        var f = memberEditScroll.querySelector('#memRealName');
        var g = memberEditScroll.querySelector('#memNetName');
        var a = String((f && f.value) || (m && m.realName) || '');
        var b = String((g && g.value) || (m && m.netName) || '');
        return String(a || b || '角色').replace(/[\\/:*?"<>|\s]+/g, '') || '角色';
      } catch (e) { return '角色'; }
    }
    function memberAlbumDownload(i) {
      var a = memAlbum[i];
      if (!a) return;
      var ext = a.src.indexOf('image/png') > -1 ? 'png' : 'jpg';
      var fn = memberAlbumName() + '_相册_' + (i + 1) + '.' + ext;
      try {
        ...
        link.click();
        setTimeout(function () { if (link.parentNode) link.parentNode.removeChild(link); }, 120);
        toast('开始保存：' + fn + '（若没弹出，长按图片选“存储图像”）');
      } catch (e) { toast('保存失败，长按图片也能存'); }
    }
    function openMemberAlbum(start) {
      var n = memAlbum.length;
      if (!n) { toast('相册还是空的，先点「测试生图」生成一张'); return; }
      var old = document.getElementById('memAlbumViewer');
      if (old && old.parentNode) old.parentNode.removeChild(old);
      var ov = document.createElement('div');
      ov.id = 'memAlbumViewer';
      ov.className = 'mem-album-viewer';
      var pic = document.createElement('img');
      pic.className = 'mav-pic';
      pic.alt = '';
      var count = document.createElement('div');
      count.className = 'mav-count';
      var closeB = document.createElement('button');
      closeB.type = 'button';
      closeB.className = 'mav-close';
      closeB.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M6 6l12 12M18 6L6 18"/></svg>';
      var prevB = document.createElement('button');
      prevB.type = 'button';
      prevB.className = 'mav-nav prev';
      prevB.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 5l-7 7 7 7"/></svg>';
      var nextB = document.createElement('button');
      nextB.type = 'button';
      nextB.className = 'mav-nav next';
      nextB.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 5l7 7-7 7"/></svg>';
      var tip = document.createElement('div');
      tip.className = 'mav-tip';
      var bar = document.createElement('div');
      bar.className = 'mav-bar';
      var dlB = document.createElement('button');
      dlB.type = 'button';
      dlB.className = 'mav-btn';
      dlB.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M12 4v11M7.5 10.5L12 15l4.5-4.5M5 19h14"/></svg><span>下载保存</span>';
      var delB = document.createElement('button');
      delB.type = 'button';
      delB.className = 'mav-btn danger';
      delB.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M4 7h16M9 7V4h6v3M6 7l1 13h10l1-13"/></svg><span>删除</span>';
      bar.appendChild(dlB); bar.appendChild(delB);
      ov.appendChild(pic); ov.appendChild(count); ov.appendChild(closeB);
      ov.appendChild(prevB); ov.appendChild(nextB); ov.appendChild(bar); ov.appendChild(tip);
      var view = Math.max(0, Math.min(n - 1, parseInt(start, 10) || 0));
      function render() {
        pic.src = memAlbum[view].src;
        count.textContent = (view + 1) + ' / ' + memAlbum.length;
        prevB.style.display = view > 0 ? '' : 'none';
        nextB.style.display = view < memAlbum.length - 1 ? '' : 'none';
      }
      function go(d) {
        var nx = view + d;
        if (nx < 0 || nx >= memAlbum.length) return;
        view = nx;
        render();
      }
      function shut() {
      ...
      function onKey(e) {
        var k = e.key || '';
        if (k === 'Escape' || e.keyCode === 27) shut();
        else if (k === 'ArrowLeft' || e.keyCode === 37) go(-1);
        else if (k === 'ArrowRight' || e.keyCode === 39) go(1);
      }
      tip.textContent = '左右滑动翻页 · 点空白处关闭 · 长按图片也可保存';
      ...
      closeB.addEventListener('click', function (e) { e.stopPropagation(); shut(); });
      dlB.addEventListener('click', function (e) { e.stopPropagation(); memberAlbumDownload(view); });
      delB.addEventListener('click', function (e) {
        e.stopPropagation();
        if (!memAlbum[view]) return;
        var cf = document.createElement('div');
        cf.className = 'mav-confirm';
        cf.innerHTML = '<div class="mav-confirm-t">删除这张相册图？</div><div class="mav-confirm-d">删掉之后这张图就没了，补不回来。</div>';
        var btns = document.createElement('div');
        btns.className = 'mav-confirm-btns';
        var c1 = document.createElement('button');
        c1.type = 'button'; c1.className = 'mav-btn'; c1.textContent = '取消';
        var c2 = document.createElement('button');
        c2.type = 'button'; c2.className = 'mav-btn danger'; c2.textContent = '删除';
        btns.appendChild(c1); btns.appendChild(c2);
        cf.appendChild(btns);
        c1.addEventListener('click', function (ev) { ev.stopPropagation(); if (cf.parentNode) cf.parentNode.removeChild(cf); });
        c2.addEventListener('click', function (ev) {
          ev.stopPropagation();
          if (cf.parentNode) cf.parentNode.removeChild(cf);
          memAlbum.splice(view, 1);
          var saved = memberAlbumPersist();
          try { memberAlbumHomeSync(); } catch (e3) {}
          memDirty = true;
          memberAlbumRefresh();
          if (!memAlbum.length) { shut(); toast('相册已清空'); return; }
          if (view >= memAlbum.length) view = memAlbum.length - 1;
          render();
          toast(saved ? '已从相册删除' : '已删除（点「保存」后生效）');
        });
        ov.appendChild(cf);
      ...
      ov.addEventListener('click', function (e) {
        var t = e.target;
        if (t && t.closest && t.closest('.mav-btn, .mav-close, .mav-nav, .mav-confirm')) return;
        shut();
      });
    ...
    // v177：点「测试生图」直接生成——提示词取自系统设置里选中的那条 + 外貌描写 + 锁脸参考图
    function memberGenTest() {
      var box = memberEditScroll.querySelector('#memGenBox');
      if (!box) return;
      var cfgList = [];
      try { cfgList = (typeof imgConfigs !== 'undefined' && imgConfigs) ? imgConfigs : []; } catch (e) {}
      if (!cfgList.length) { box.innerHTML = '<div class="mem-gen-resultbox"><div class="test-status test-err">未找到生图模型配置，先到「设置 → 生图API」保存</div></div>'; return; }
      var pid = '';
      try { pid = String((memberEditScroll.querySelector('#memLookPrompt') || {}).value || ''); } catch (e) {}
      var pr = null;
      try {
        var _pl = (typeof imgPrompts !== 'undefined' && imgPrompts) ? imgPrompts : [];
        for (var i = 0; i < _pl.length; i++) { if (String(_pl[i].id || i) === pid) { pr = _pl[i]; break; } }
      } catch (e) {}
      var lk = String((memberEditScroll.querySelector('#memLook') || {}).value || '').trim();
      var pos = pr ? imgPromptText(pr.pos || pr.content || '') : '';
      if (lk) pos = pos ? (pos + '，' + lk) : lk;
      if (!pos) { toast('先选「锁定生图提示词」或填「角色外貌」，才有东西可生成'); return; }
      var neg = pr ? imgPromptText(pr.neg) : '';
      var cfg = null;
      if (pr && pr.model) { for (var j = 0; j < cfgList.length; j++) { if (cfgList[j].model === pr.model) { cfg = cfgList[j]; break; } } }
      if (!cfg) cfg = cfgList[0];
      var ref = (memLookImgs && memLookImgs.length) ? memLookImgs[0] : null;
      var metaTxt = (cfg.name || cfg.model || '生图模型') + (pr ? ' · 提示词：' + (pr.name || '已选') : ' · 未选锁定提示词') + (ref ? ' · 锁脸参考图 1 张' : ' · 无锁脸参考图');
      box.innerHTML = '<div class="mem-gen-resultbox"><div class="mem-gen-note">' + escHtml(metaTxt) + '</div><div class="test-status"><div class="test-spin"></div>生成中…</div></div>';
      var rb = box.querySelector('.mem-gen-resultbox');
      generateTestImage(cfg, pos, neg, '', rb, function () {
        /* v183：生成的成品图自动存进「角色相册」 */
        try {
          var _gi = rb.querySelector('img.test-result') || rb.querySelector('img');
          if (_gi && _gi.src) {
            var _noteTxt = '';
            try { _noteTxt = pr ? imgPromptText(pr.pos) : ''; } catch (e4) { _noteTxt = ''; }
            if (!_noteTxt) _noteTxt = pr ? String(pr.name || '') : '';
            memberAlbumAdd(_gi.src, _noteTxt);
          }
        } catch (e) {}
        var meta = document.createElement('div');
        meta.className = 'mem-gen-note';
    ...
    /* v209：流式问答（OpenAI 兼容 /chat/completions 的 SSE）——
       onDelta(累计全文) 每来一点就回调一次；cb(err, 全文)。接口不支持流式时自动退回一次性返回。 */
    function memberAiAskStream(sys, user, opt, onDelta, cb) {
      var fired = false;
      var finish = function (err, txt) { if (fired) return; fired = true; cb && cb(err, txt); };
      var cfg = null;
      try { cfg = memberFindApi(); } catch (e) {}
      if (!cfg || !cfg.baseUrl || !cfg.apiKey || !cfg.model) { finish('未找到完整聊天 API 配置，先到「设置 → 聊天API」检查 baseUrl / API Key / 模型 是否都已填写'); return; }
      var url = String(cfg.baseUrl).replace(/\/+$/, '');
      if (!/\/chat\/completions$/.test(url)) url += '/chat/completions';
      var maxTok = (opt && opt.maxTokens) ? opt.maxTokens : 1400;
      var acc = '';
      fetch(url, {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          temperature: 1, stream: true, max_tokens: maxTok
        })
      }).then(function (r) {
        if (!r.ok) throw new Error('HTTP ' + r.status);
        var ct = String((r.headers && r.headers.get) ? (r.headers.get('content-type') || '') : '');
        if (!r.body || !r.body.getReader || ct.indexOf('event-stream') < 0) {
          return r.text().then(function (t) {
            var out = '';
            try {
              var o = JSON.parse(t);
              out = (o.choices && o.choices[0] && ((o.choices[0].message && o.choices[0].message.content) || o.choices[0].text)) || '';
            } catch (eJ) { out = t; }
            out = String(out || '');
        ...
        var loop = function () {
          return rd.read().then(function (res) {
            if (res.done) { finish(null, acc); return; }
            ...
            for (var i = 0; i < lines.length; i++) {
              ...
              try {
                var o2 = JSON.parse(payload);
                var ch0 = o2.choices && o2.choices[0];
                var d = ch0 ? (ch0.delta || ch0.message) : null;
                var piece = (d && d.content != null) ? String(d.content) : '';
        ...
        finish(errZh ? errZh(e) : String((e && e.message) || e));
      });
    }
    /* v204：可指定 max_tokens —— 写人设这种长输出必须放大上限，否则只写一丁点就被截断 */
    function memberAiAsk(sys, user, cb, opt) {
      var cfg = null;
      try { cfg = memberFindApi(); } catch (e) {}
      ...
      fetch(url, {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: 'system', content: sys }, { role: 'user', content: user }],
          temperature: 0.8,
          max_tokens: (opt && opt.maxTokens) ? opt.maxTokens : 1024,
          stream: false
        })
    ...
    function memberAiNpc() {
      /* v197：①在 NPC 名单页也能用（主角色身世取缓存）；②按十段模板把完整人设一起生成并自动填充 */
      var lore = ((memRoleFormCache || {}).lore || ((memberEditScroll.querySelector('#memLore') || {}).value) || '');
      if (!String(lore).trim()) { toast('请先填写「角色身世」，AI 才能生成 NPC'); return; }
      toast('AI 正在生成 NPC…');
      var user = '这是主角色的完整人设（只作世界观与背景参考）：\n' + memberNpcMainText() + '\n\n请基于这份世界观，生成 3 个各自有独立人生的 NPC —— 他们和主角色的关系只是各自人生里的一部分，不许写成只会围着主角色转的角色。每个 NPC 的完整人设必须严格按下面的十段模板写：\n' + MEM_NPC_TPL +
        '\n\n十段里第 1~9 段先写 TA 自己的人生（家人、成长、营生、朋友、目标、软肋、日常），只有第 10 段才集中写 TA 和主角色的关系；绝不许每段都拐回主角色。' +
        '\n\n每个 NPC 的十段合计不少于 1200 字，每段不少于 100 字，要写具体经历、习惯、说话方式与软肋，不许写空话套话。' +
        '\n\n输出 JSON 数组，每一项格式：{"name":"NPC名字","bio":"一句简介(20字内)","rel":"与角色的关系(2~4字)","lore":"完整人设正文（含十段，每段用「段名：内容」一行，段名与模板一致）"}。只输出 JSON，不要解释。';
      memberAiAsk('你是世界观设定助手，输出严格 JSON。', user, function (txt, err) {
        ...
        var _addCnt = 0;
        var _names = npcLoreSectionNames();
        arr.forEach(function (x) {
          if (!x || !x.name) return;
          /* v201：AI 没写够的段落，本地按内置模板补足，保证每个 NPC 的人设都立得住 */
          var _secs = npcLoreExtractFromText(String(x.lore || '').trim());
          npcLoreFallbackFill(_secs);
          npcLoreTopUp(_secs, 1200);
          var _lines = [];
          for (var _i = 0; _i < _names.length; _i++) _lines.push(_names[_i] + '：' + String(_secs[_i] || '').trim());
          memNpcs.push({
            id: memberUid(), name: String(x.name).trim().slice(0, 12), bio: String(x.bio || '').trim().slice(0, 60),
            rel: String(x.rel || '').trim(), avatar: '', gender: '', lore: _lines.join('\n'),
            masterId: memberMasterId(), masterName: memberMasterName()
          });
          _addCnt++;
        });
        memDirty = true;
        if (memNpcListView) memberNpcListRender(); else memberRerender();
        toast('已生成 ' + _addCnt + ' 个 NPC，人设已自动补齐');
      });
    }
    function memberNpcAdd() {
      chatMini('自建 NPC', '<div class="chat-mini-tip" style="font-size:12px;color:var(--text-faint);margin-bottom:6px">NPC 名字</div><input class="chat-mini-input" id="npcAddName" style="width:100%;box-sizing:border-box;margin-bottom:10px"><div class="chat-mini-tip" style="font-size:12px;color:var(--text-faint);margin-bottom:6px">简介（选填，一句身份/性格）</div><input class="chat-mini-input" id="npcAddBio" style="width:100%;box-sizing:border-box">', '添加', function () {
        ...
        memNpcs.push({ id: memberUid(), name: nm.slice(0, 12), bio: bio.slice(0, 60), rel: '', masterId: memberMasterId(), masterName: memberMasterName() });
        memDirty = true;
        memberRerender();
        toast('已添加 NPC「' + nm + '」');
      });
    ...
    /* ==================== v196：NPC 完整编辑页实现 ==================== */
    function memberCacheRoleForm() {
      var keepIds = { memRealName: 'realName', memNetName: 'netName', memLore: 'lore', memVoiceId: 'voiceId', memVoiceLang: 'voiceLang', memVoiceSpeed: 'voiceSpeed', memLook: 'look', memQrText: 'qrText' };
      /* v197：在 NPC 名单 / NPC 编辑页调用时，主表单不在 DOM 里——保留上一次缓存，避免把已填文字清空 */
      var c = memRoleFormCache ? JSON.parse(JSON.stringify(memRoleFormCache)) : {};
      Object.keys(keepIds).forEach(function (id) {
        var el = memberEditScroll.querySelector('#' + id);
        if (el && el.value != null) c[keepIds[id]] = el.value;
      });
      memberEditScroll.querySelectorAll('[data-relinput]').forEach(function (inp) {
        var i = parseInt(inp.getAttribute('data-relinput'), 10);
        if (memNpcs[i]) memNpcs[i].rel = inp.value;
      });
      var lp = memberEditScroll.querySelector('#memLookPrompt');
      c.lookPromptId = lp ? String(lp.value || '') : ((memberEditIdxRole() && memberEditIdxRole().lookPromptId) || '');
      var chip = memberEditScroll.querySelector('.mem-chip.on[data-mact="sex"]');
      if (chip) memGender = chip.getAttribute('data-val') || memGender;
      c.avatar = memAvatar;
      c.gender = memGender;
      memRoleFormCache = c;
    }
    function memberEditIdxRole() {
      if (memEditIdx < 0) return null;
      var l = memberLoad();
      return l[memEditIdx] || null;
    }
    /* ==================== v197：NPC 名单界面（单一入口 + 加号展开） ==================== */
    function memberNpcListRender() {
      memberEditScroll.innerHTML = memberNpcListBody();
      memberEditScroll.scrollTop = 0;
      memberNpcListBind();
      memberChromeSync();
    }
    function memberNpcListBody() {
      var rows = memNpcs.length ? memNpcs.map(function (np, i) {
        var _nav = np.avatar ? '<img src="' + np.avatar + '" alt="">' : escHtml(String(np.name || '?').slice(0, 1));
        return '<div class="mem-npc-row npc-open" data-mact="npcopen" data-val="' + i + '"><span class="mem-npc-ava">' + _nav + '</span>' +
          '<span class="mem-npc-bd"><span class="mem-npc-name">' + escHtml(np.name || '未命名') + (np.masterName ? '<i class="mem-npc-master">主人设·' + escHtml(np.masterName) + '</i>' : '') + '</span><span class="mem-npc-bio">' + escHtml(np.bio || (np.lore ? String(np.lore).split('\n')[0].slice(0, 24) : '暂无完整人设，点开编辑')) + '</span></span>' +
      ...
      }).join('') : '<div class="mem-npc-empty">人设库还是空的，点右上角 ＋ 创建第一个 NPC</div>';
      /* v205：列表页不要外层大框，页内也不再放大加号 —— 创建入口在顶栏右上角 */
      var html = '<div class="npc-edit npc-lib">';
      html += '<div class="mem-npc-list">' + rows + '</div>';
      html += '</div>';
      return html;
    }
    function memberNpcListBind() {
      memberEditScroll.onclick = function (e) {
        var el = e.target.closest ? e.target.closest('[data-mact]') : null;
        if (!el) return;
        var act = el.getAttribute('data-mact');
        var val = el.getAttribute('data-val');
        if (act === 'npcback') { memNpcListView = false; memberRerender(); return; }
        if (act === 'npcadd') { memberNpcOpen(-1); return; }
        if (act === 'npcai') { memberAiNpc(); return; }
        if (act === 'npcopen') { memberNpcOpen(parseInt(val, 10)); return; }
        if (act === 'npcdel') { memNpcs.splice(parseInt(val, 10), 1); memDirty = true; memberNpcListRender(); toast('已删除该 NPC'); return; }
      };
    }
    function memberNpcOpen(idx) {
      memberCacheRoleForm();
      memNpcListView = true;
      if (idx >= 0 && memNpcs[idx]) {
        var x = memNpcs[idx];
        memNpcDraft = { id: x.id, name: x.name || '', bio: x.bio || '', rel: x.rel || '', add: !!x.add, avatar: x.avatar || '', gender: x.gender || '', lore: x.lore || '', masterId: x.masterId || '', masterName: x.masterName || '', link: (x.link === 'low' || x.link === 'high') ? x.link : 'mid' };
        memNpcEditIdx = idx;
      } else {
        /* v202：主人设 —— 从这个人设入口建立的 NPC，主人设就是当前角色 */
        memNpcDraft = { id: memberUid(), name: '', bio: '', rel: '', add: false, avatar: '', gender: '', lore: '', masterId: memberMasterId(), masterName: memberMasterName(), link: 'mid' };
        memNpcEditIdx = -1;
      }
      memberNpcRender();
    }
    function memberNpcBack() {
      memberNpcSyncForm();
      memNpcEditIdx = null;
      if (memNpcListView) { memberNpcListRender(); toast('已返回 NPC 名单'); return; }
      memberRerender();
      toast('已返回角色编辑');
    }
    function memberNpcSyncForm() {
      if (!memNpcDraft) return;
      var g = function (id) { var el = memberEditScroll.querySelector('#' + id); return el ? String(el.value || '') : null; };
      var v;
      v = g('npcName'); if (v !== null) memNpcDraft.name = v.trim().slice(0, 12);
      v = g('npcBio'); if (v !== null) memNpcDraft.bio = v.trim().slice(0, 60);
      v = g('npcLore'); if (v !== null) memNpcDraft.lore = v;
      v = g('npcRel'); if (v !== null) memNpcDraft.rel = v.trim().slice(0, 30);
      /* v200：完整人设改成十段分区输入，收拢时按段拼回一整段文本 */
      if (memberEditScroll.querySelector('#npcSec0')) { memNpcDraft.lore = npcLoreJoin(npcLoreCurrent()); }
      else { v = g('npcLore'); if (v !== null) memNpcDraft.lore = v; }
    }
    /* v209：NPC 和主人设的关联度 —— 决定 AI 写人设时「TA 自己人生」占多重。
       三档：low＝独立人生（主线是 TA 自己的日子）、mid＝适度、high＝紧密。 */
    var NPC_LINK_TIP = {
      low: '独立人生：TA 先是一个自己活过的人，主线是 TA 自己的日子，主角色只在「和主人设的关系」那一段出场',
      mid: '适度关联：TA 有自己的生活与目标，主角色是 TA 人生里重要的一部分，但不围着 TA 转',
      high: '紧密关联：TA 与主角色交集多，但每段仍要先立住 TA 自己的人生内核'
    };
    var NPC_LINK_BRIEF = {
      low: '【这一次的关联度：独立人生】第 1~9 段必须全部写 TA 自己的人生（家人、成长、营生、朋友、目标、软肋、日常），主角色最多在一两段里一笔带过，绝对不许每段都拐回主角色；只有第 10 段才集中写 TA 和主角色的关系。TA 不是为衬托主角色而存在的。',
      mid: '【这一次的关联度：适度】第 1~9 段以 TA 自己的人生为主线展开，可以自然带到主角色，但最多两三段各提一句，不要围着主角色转；第 10 段再集中写关系。',
      high: '【这一次的关联度：紧密】允许较多写到 TA 与主角色的交集，但每一段都要先立住 TA 自己的人生内核（出身、日常、目标、软肋），不要写成一个只为衬托主角色而存在的角色。'
    };
    function npcLinkOf() {
      var l = memNpcDraft && memNpcDraft.link;
      return (l === 'low' || l === 'high') ? l : 'mid';
    }
    function npcLinkSync() {
      var cur = npcLinkOf();
      var ch = memberEditScroll.querySelectorAll('[data-npc-link]');
      for (var i = 0; i < ch.length; i++) ch[i].classList.toggle('on', ch[i].getAttribute('data-npc-link') === cur);
      var tp = memberEditScroll.querySelector('#npcLinkTip');
      if (tp) tp.textContent = NPC_LINK_TIP[cur] || '';
    }
    function memberNpcRender() {
      memberEditScroll.innerHTML = memberNpcEditBody();
      memberEditScroll.scrollTop = 0;
      memberNpcEditBind();
      memberChromeSync();
    }
    function memberNpcEditBody() {
      var d = memNpcDraft || { name: '', bio: '', rel: '', avatar: '', gender: '', lore: '' };
      var av = d.avatar ? '<img src="' + d.avatar + '" alt="">' : escHtml(String(d.name || '?').slice(0, 1) || '?');
      var html = '<div class="npc-edit">';
      html += '<div class="group-card form-card">';
      html += '<div class="npc-ava-row"><div class="npc-ava-big" id="npcAvatarBtn">' + av + '</div><div class="npc-ava-tip">点击上传 NPC 头像<input type="file" id="npcAvaFile" accept="image/*" hidden></div></div>';
      html += '<div class="field"><label>NPC 名字</label><input class="chat-mini-input" id="npcName" maxlength="12" placeholder="" value="' + escHtml(d.name || '') + '"></div>';
      /* v201：编辑页只留头像 + 名字 + 完整人设，其余字段交给 AI 从人设里读 */
      html += '</div>';
      /* v200：完整人设拆成十个分段输入区，AI 入口收敛成右上角一个图标按钮 */
      var secNames = MEM_NPC_TPL.split('\n'), secVals = npcLoreSplit(d.lore || '');
      html += '<div class="group-title">完整人设</div><div class="group-card form-card">';
      html += '<div class="npc-lore-head"><div class="npc-lore-hint"></div>';
      html += '<button type="button" class="npc-ai-btn" id="npcAiBtn" title="AI 人设助手" aria-label="AI 人设助手">' + NPC_AI_SVG + '</button></div>';
      /* v209：先定「和主人设的关联度」，AI 才不会把 NPC 写成只为衬托主角色的角色 */
      html += '<div class="npc-link-row">' +
        '<span class="npc-link-lb">与主人设关联</span>' +
        '<button type="button" class="npc-link-chip" data-npc-link="low">独立人生</button>' +
        '<button type="button" class="npc-link-chip" data-npc-link="mid">适度</button>' +
        '<button type="button" class="npc-link-chip" data-npc-link="high">紧密</button>' +
        '<span class="npc-link-tip" id="npcLinkTip"></span></div>';
      for (var si = 0; si < secNames.length; si++) {
        html += '<div class="npc-sec">';
        /* v209：「继续写这一段」按钮长在板块的上方（就贴在这一段的段名旁边），不再堆在十段最底下 */
        html += '<div class="npc-sec-hd"><span class="npc-sec-no">' + escHtml(secNames[si]) + '</span><span class="npc-sec-st" id="npcSecSt' + si + '"></span>' +
          '<button type="button" class="npc-sec-next" id="npcSecNext' + si + '" style="display:none">继续写这一段</button></div>';
        html += '<textarea class="chat-mini-input npc-sec-ta" id="npcSec' + si + '" rows="4" placeholder="' + escHtml(NPC_SEC_TIP[si] || '') + '">' + escHtml(secVals[si] || '') + '</textarea>';
        /* v207：每个板块自己的进度条，就接在这个板块的文字后面 */
        html += '<div class="npc-sec-prog" id="npcSecProg' + si + '"><div class="npc-prog-bar"><i class="npc-prog-in" id="npcSecProgIn' + si + '"></i></div><span class="npc-sec-prog-tx" id="npcSecProgTx' + si + '">待写</span></div>';
        html += '</div>';
      }
      /* v207：全局大进度条撤掉，只留一行段数小结（续写按钮 v209 起挪到各板块上方） */
      html += '<div class="npc-prog-wrap" id="npcProgWrap"><span class="npc-prog-tx" id="npcProgTx"></span></div>';
      html += '</div>';
      html += '<div class="npc-foot">' + (memNpcEditIdx >= 0 ? '<button type="button" class="prompt-cancel npc-del" id="npcDelBtn">删除</button>' : '') + '<button type="button" class="prompt-cancel" id="npcCancelBtn">取消</button><button type="button" class="primary-btn wb-btn" id="npcSaveBtn">保存 NPC</button></div>';
      html += '</div>';
      return html;
    }
    function memberNpcEditBind() {
      var av = document.getElementById('npcAvatarBtn');
      var fi = document.getElementById('npcAvaFile');
      if (av && fi) av.addEventListener('click', function () { fi.click(); });
      if (fi) fi.addEventListener('change', function (e) {
        ...
        rd.onload = function () {
          if (!memNpcDraft) return;
          memNpcDraft.avatar = rd.result;
          memberNpcSyncForm();
          memberNpcRender();
        };
        rd.readAsDataURL(f);
      });
      var sv = document.getElementById('npcSaveBtn');
      if (sv) sv.addEventListener('click', memberNpcSave);
      var cc = document.getElementById('npcCancelBtn');
      if (cc) cc.addEventListener('click', memberNpcBack);
      var dl = document.getElementById('npcDelBtn');
      if (dl) dl.addEventListener('click', memberNpcDelNpc);
      var ai = document.getElementById('npcAiBtn');
      if (ai) ai.addEventListener('click', npcAiMenuOpen);
      /* v209：续写按钮改成每个板块上方各自一个 —— 点哪一段上面的，就接着写哪一段 */
      var _nxAll = memberEditScroll.querySelectorAll('.npc-sec-next');
      for (var _ni = 0; _ni < _nxAll.length; _ni++) {
        (function (bi, host) { host.addEventListener('click', function () { npcAiNextFor(bi); }); })(_ni, _nxAll[_ni]);
      }
      /* v209：关联度三档 */
      var _lkAll = memberEditScroll.querySelectorAll('[data-npc-link]');
      for (var _li2 = 0; _li2 < _lkAll.length; _li2++) {
        _lkAll[_li2].addEventListener('click', function () {
          if (!memNpcDraft) return;
          memNpcDraft.link = this.getAttribute('data-npc-link');
          npcLinkSync();
        });
      }
      npcLinkSync();
      /* v200：十段分区输入——任意一段变动都同步回 draft.lore 并刷新「已填/待补」状态 */
      var secTas = memberEditScroll.querySelectorAll('.npc-sec-ta');
      for (var k = 0; k < secTas.length; k++) {
        secTas[k].addEventListener('input', function () {
          if (memNpcDraft) memNpcDraft.lore = npcLoreJoin(npcLoreCurrent());
          npcSecStatusSync();
        });
      }
      npcSecStatusSync();
      memberEditScroll.onclick = function (e) {
        var el = e.target.closest('[data-npcact]');
        if (!el || !memNpcDraft) return;
        var act = el.getAttribute('data-npcact');
        var val = el.getAttribute('data-val');
        if (act === 'sex') { memNpcDraft.gender = String(val || ''); memberNpcSyncForm(); memberNpcRender(); return; }
        if (act === 'rel') { memNpcDraft.rel = String(val || ''); memberNpcSyncForm(); memberNpcRender(); return; }
      };
    }
    /* v202：主人设标识 —— NPC 是挂在哪个角色的人设底下建的 */
    function memberMasterId() {
      try {
        var l = memberLoad();
        var m = (memEditIdx >= 0 && l[memEditIdx]) ? l[memEditIdx] : null;
        return m ? String(m.id || '') : '';
      } catch (e) { return ''; }
    }
    function memberMasterName() {
      try {
        var els = ['#memNetName', '#memRealName'];
        for (var i = 0; i < els.length; i++) {
          var el = memberEditScroll.querySelector(els[i]);
          var v = el ? String(el.value || '').trim() : '';
          if (v) return v;
        }
        var l = memberLoad();
        var m = (memEditIdx >= 0 && l[memEditIdx]) ? l[memEditIdx] : null;
        return m ? String(m.netName || m.realName || '').trim() : '';
      } catch (e) { return ''; }
    }
    function memberNpcSave() {
      memberNpcSyncForm();
      var d = memNpcDraft;
      if (!d || !String(d.name || '').trim()) { toast('请先填写 NPC 名字'); return; }
      var obj = {
        id: d.id || memberUid(), name: String(d.name).trim().slice(0, 12), bio: String(d.bio || '').trim().slice(0, 60),
        rel: String(d.rel || '').trim().slice(0, 30), add: !!d.add,
        avatar: String(d.avatar || ''), gender: String(d.gender || ''), lore: String(d.lore || ''),
        masterId: String(d.masterId || memberMasterId()), masterName: String(d.masterName || memberMasterName()),
        link: (d.link === 'low' || d.link === 'high') ? d.link : 'mid'   /* v209：与主人设的关联度 */
      };
      if (memNpcEditIdx >= 0) memNpcs[memNpcEditIdx] = obj;
      else memNpcs.push(obj);
      memDirty = true;
      var nm = obj.name;
      memNpcEditIdx = null;
      if (memNpcListView) memberNpcListRender(); else memberRerender();
    ...
    function memberNpcDelNpc() {
      if (memNpcEditIdx >= 0) {
        memNpcs.splice(memNpcEditIdx, 1);
        memDirty = true;
        memNpcEditIdx = null;
        if (memNpcListView) memberNpcListRender(); else memberRerender();
        toast('已删除该 NPC');
    ...
    function memberNpcMainText() {
      var c = memRoleFormCache || {};
      var g = memGender === 'male' ? '男' : (memGender === 'female' ? '女' : (memGender === 'secret' ? '保密' : '未填'));
      return '本名：' + (c.realName || '（未填）') + '；网名：' + (c.netName || '（未填）') + '；性别：' + g +
        '；身世：' + (c.lore || '（未填）') + '；外貌：' + (c.look || '（未填）');
    }
    /* v199：恢复 AI 生成入口；v200：收敛成人设区右上角一个 AI 键，点开可「AI 润色 / AI 帮写」。
       生成走十段 JSON，缺段自动重试，最多三轮；仍缺则本地兜底补齐，保证十段不留空。 */
    function npcAiMenuOpen() {
      if (!memNpcDraft) return;
      memberNpcSyncForm();
      var host = document.getElementById('memberEditOverlay');
      if (!host) return;
      var old = host.querySelector('.npc-ai-pop');
      if (old && old.parentNode) old.parentNode.removeChild(old);
      var pop = document.createElement('div');
      pop.className = 'npc-ai-pop';
      pop.innerHTML = '<div class="npc-ai-mask"></div><div class="npc-ai-box">' +
        '<div class="npc-ai-title">AI 人设助手</div>' +
        '<button type="button" class="npc-ai-opt" data-npcai="polish"><span class="npc-ai-opt-n">AI 润色</span><span class="npc-ai-opt-d">保留你写好的内容，把每段写细写顺</span></button>' +
        '<button type="button" class="npc-ai-opt" data-npcai="write"><span class="npc-ai-opt-n">AI 帮写</span><span class="npc-ai-opt-d">一次只写一段，写完停下，你点板块上方的「继续写这一段」再往下写</span></button>' +
        '<button type="button" class="npc-ai-cancel">取消</button></div>';
      host.appendChild(pop);
      pop.querySelector('.npc-ai-mask').addEventListener('click', npcAiMenuClose);
      pop.querySelector('.npc-ai-cancel').addEventListener('click', npcAiMenuClose);
      var opts = pop.querySelectorAll('[data-npcai]');
      for (var i = 0; i < opts.length; i++) {
        opts[i].addEventListener('click', function () {
          var m = this.getAttribute('data-npcai');
          npcAiMenuClose();
          memberNpcAiLore(m);
        });
      }
    }
    function npcAiMenuClose() {
      var host = document.getElementById('memberEditOverlay');
      var pop = host && host.querySelector('.npc-ai-pop');
      if (pop && pop.parentNode) pop.parentNode.removeChild(pop);
    }
    var npcAiBusy = false;
    /* 兜底文案：AI 三轮都没吐出的段落，用这些结合名字/关系补上，保证十段不空 */
    var NPC_SEC_FB = [
      '{n}，二十七八岁，在城南一带做事，从最底层的杂活一路熬到能自己拿主意。租住在离店面两条街的老楼里，五楼，一室一厅，家具都是旧的，却擦得干净，窗台上常年摆着一个喝空的水杯和一把钥匙。每个月十五号固定往家里打一次钱，从不提自己手头紧不紧，母亲问起就只说够花。籍贯是个听上去有点远的小城，别人问起来，{ta}只笑笑带过；熟一点的人才知道，{ta}这几年几乎没回去过，连过年也推说店里走不开。此外，{ta}不太跟人提自己的打算，事没成之前一句都不说。',
      '身形偏瘦，是个穿什么都显得利落的骨架。眉眼不算惊艳，但耐看，笑起来眼尾会压出一点细纹。习惯把袖子挽到小臂，右手腕上有一道旧疤，被问起就说是小时候摔的，其实那疤的来历{ta}从没跟人讲过。头发常剪得很短，懒得打理，长一点就随手往后拨。衣着简单，深色居多，一年到头就那么几身，只有一件洗得发白的旧外套一直舍不得扔，说穿着顺手。走路比说话快，站着时习惯把重心压在一条腿上。说话时习惯先看对方的眼睛，再慢慢移开，像是在掂量该说到哪一步。穿衣服只讲究合身，不合身的宁可不要。',
      '看着{ta}，心里其实比谁都软，嘴硬到骨子里，遇事第一反应是说没事，转头一个人把事扛完。说话不爱绕弯，一句话能说完的绝不说三句，因此常被人当成冷淡。真正在意的事反而一句不提，只在细节上偷偷使劲——比如你随口说过一次头疼，过两天桌上就多了一盒药。脾气上来会沉默，气消了又别扭地找台阶，宁可绕一大圈也不肯直接说对不起。{ta}最怕的不是麻烦，是被人丢下，所以很少先开口留人。在外人面前{ta}永远是稳的那个，只有极熟的人见过{ta}失态的样子。',
      '在小城里长大，父母都是普通得不能再普通的人，家里没出过什么大事，也没给过{ta}什么底子。十六岁那年家里出过一次变故，{ta}从那时起学会了不开口要东西，也学会了看人脸色。十八岁一个人出来，做过搬运、看店、跑腿，难听的话都听过，最穷的时候一天只吃一顿。二十出头遇到过一个愿意拉{ta}一把的人，也正是在那人身上吃了一次大亏。如今日子算是站住了脚，只是提起那几年，{ta}仍会把话题岔开。那几年的事{ta}不提，但身上留着痕迹，比如从不浪费一口饭。',
      '手上功夫好，做事稳当，店里那些精细活儿最后都落到{ta}手里，别人做三遍的活{ta}一遍就能过。记性极好，一屋子人的口味和习惯都记得住，谁不吃辣、谁胃不好、谁坐下要先喝口热的，从不用别人提醒。能扛事，越是乱的时候越清醒，别人慌成一团时{ta}已经在想下一步该拆哪一块。还有一样很少示人的本事：{ta}会看人，看几眼就知道对方在打什么主意，只是大多时候懒得说破，只在心里记一笔。{ta}不太爱被夸，一被夸就找活干，把话岔开。遇到不讲理的客人也不吵，事后自己慢慢消化。',
      '家里只剩一位还在的老母亲，电话里两边都话少，习惯用沉默表达惦记，一次通话常常不到三分钟。店里的老板对{ta}有知遇之恩，{ta}嘴上不说，加班从没推过一次。有个从小一起长大的发小，几年才见一面，见了还是能坐一整晚。曾经有过一个走得很近的人，后来不知怎么就散了，联系方式还留着，一次也没点开。和你算是{rel}，这是{ta}这些年里少有的、愿意主动多说两句的关系。人情往来{ta}算得清楚，谁帮过{ta}，{ta}都记着要还。',
      '作息谈不上规律，店里忙到夜里是常事，回家先冲个澡，再一个人坐着发一会儿呆才睡。偶尔失眠，就靠着窗台看楼下的路灯，看到困了为止，也懒得吃药。不忙的周末会去菜市场慢慢逛，挑菜挑得极细，和摊主都能聊上两句，做饭是{ta}为数不多能真正放松下来的事。阳台上养了一盆快死的绿萝，隔三差五想起来才浇一次水。手机里没什么娱乐软件，倒是存了一堆做菜的视频，说是以后想做给自己吃。睡前会把第二天要用的东西摆好，摆好了才安心；周末偶尔去河边走一圈，回来再收拾屋子。',
      '对感情慢热，也认死理。不会主动挑明，喜欢一个人会先观察很久，看对方是不是那种走了就不回头的人。真动了心之后极认真，认真到有点笨——会记住你说过的每一件小事，却说不出一句漂亮话。最受不了被敷衍和冷处理，那比吵架更让{ta}难受，会在心里反复猜自己哪里做错了。底线是不接受欺骗，一次也不行。亲密的时候会突然变安静，那安静里没有疏远，是怕自己话说重了。被人认真对待的时候{ta}会愣一下，然后装作没事人一样把话题接过去。',
      '有件事从没跟任何人讲过：当年家里那场变故，其实有一部分是{ta}自己的原因，{ta}一直觉得是自己把那个家弄散了，所以这些年才拼命往家里寄钱。{ta}对钱、对承诺都过分谨慎，也从不肯欠人情，欠了就一定要还上。还有一个人{ta}始终没能放下，不是舍不得，是没弄明白自己到底错在哪。被戳到这些的时候，{ta}第一反应是否认，第二反应是转身走开，等一个人消化完，才会板着脸回来把话说完。这些话{ta}谁都没说过，只在极安静的时候自己想一想。',
      '和你是{rel}。一开始只把你当普通认识的人，客客气气，界限分明。后来是你那些不讲道理的关心一点点磨开的——你替{ta}顶过一次事，也当着别人的面站过{ta}这边，{ta}嘴上嫌你多事，心里记了很久。如今{ta}对你的态度是：嘴上照旧不客气，你的事却永远排在前面，你半夜发消息{ta}也会回。{ta}从没说过那种话，但你已经是{ta}在这座城市里少数几个会惦记的人。{ta}不会说想你，只会在你说饿的时候把饭做好。把你的事当成自己的事，是{ta}自己都没承认过的习惯。'
    ];
    /* 逐段加料句：字数不够时按最短的段落依次补，保证整份人设经得起推敲 */
    var NPC_SEC_EXT = [
      '对于这座城市的归属感，{ta}始终说不清楚，只是每天收工时抬头看一眼楼上的灯，就觉得这一天没白过。',
      '{ta}不太照镜子，但出门前会把衣领理平，这是留给别人的体面，也是留给自己的。',
      '心软这件事{ta}自己知道，所以从不轻易靠近太近的人，怕麻烦别人，也怕自己收不住。',
      '那几年教会{ta}的一件事是：指望别人不如把工具握在自己手里，哪怕只是一把旧螺丝刀。',
      '别人夸{ta}手艺好，{ta}只说是吃饭的本事；真到关键时候，{ta}从不藏私。',
      '{ta}记得每个人欠过的人情，也记得每个对自己好过的人，只是没有当面道谢的习惯。',
      '累极了的时候{ta}会煮一碗面，坐在厨房的凳子上吃完，那种踏实比睡一觉还管用。',
      '{ta}不习惯被哄，真被人认真对待时反而会不自在，会找个由头把话题绕过去。',
      '有些夜里{ta}会想，如果当年做了另一个选择，现在会不会轻松一点，但想想也就过去了。',
      '{ta}对关系的判断很慢，也很准；一旦把谁放进心里，就很少再挪位置。'
    ];
    function npcLoreTotal(arr) {
      var t = 0;
      for (var i = 0; i < (arr || []).length; i++) t += String(arr[i] || '').replace(/\s/g, '').length;
      return t;
    }
    function npcLoreTopUp(arr, target) {
      target = target || 2000;
      var guard = 0;
      while (npcLoreTotal(arr) < target && guard < 40) {
        var mi = 0, ml = 1e9;
        for (var i = 0; i < arr.length; i++) {
          var l = String(arr[i] || '').replace(/\s/g, '').length;
          if (l < ml) { ml = l; mi = i; }
        }
        var add = String(NPC_SEC_EXT[mi] || '');
        if (!add) break;
        var cur = String(arr[mi] || '').trim();
        arr[mi] = cur + add;
        guard++;
      }
      return arr;
    }
    function npcLoreFallbackFill(arr) {
      var nm = (memNpcDraft && memNpcDraft.name) || '这个角色';
      var rel = (memNpcDraft && String(memNpcDraft.rel || '').trim()) || '朋友';
      var ta = memNpcDraft && memNpcDraft.gender === 'female' ? '清冷柔软' : (memNpcDraft && memNpcDraft.gender === 'male' ? '冷硬利落' : '内敛沉静');
      for (var i = 0; i < arr.length; i++) {
        if (String(arr[i] || '').trim()) continue;
        arr[i] = String(NPC_SEC_FB[i] || '').replace(/\{n\}/g, nm).replace(/\{rel\}/g, rel).replace(/\{ta\}/g, ta);
      }
      try { npcLoreTopUp(arr); } catch (e) {}
      return arr;
    }
    /* v203：AI 顺手取的名字要填进上方「NPC 名字」输入框 */
    function npcAiFillName(txt) {
      if (!memNpcDraft) return;
      if (String(memNpcDraft.name || '').trim()) return;
      var m = String(txt || '').match(/"npcName"\s*:\s*"([^"]{1,24})"/);
      if (!m) return;
      var nm = String(m[1] || '').trim();
      if (!nm) return;
      memNpcDraft.name = nm;
      var inp = memberEditScroll.querySelector('#npcName');
      if (inp) inp.value = nm;
      try { memDirty = true; } catch (eD) {}
    }
    function memberNpcAiLore(mode) {
      if (!memNpcDraft) return;
      if (npcAiBusy) { toast('AI 正在写，稍等一下'); return; }
      memberNpcSyncForm();
      mode = (mode === 'polish') ? 'polish' : 'write';
      var mainLore = ((memRoleFormCache || {}).lore || '');
      if (!String(mainLore).trim()) { toast('主角色「角色身世」为空，先回去填写再用 AI'); return; }
      var cur = npcLoreCurrent();
      if (mode === 'polish' && npcLoreMissing(cur).length >= 10) { toast('还没写内容，先选「AI 帮写」吧'); return; }
      npcAiBusy = true;
      /* v202：写人设时给一个进度条，让你知道它开始写了 */
      /* v206：进度条从「已经写完的段落数」起步，一段一段往前走 */
      npcProgStart(npcLoreDonePct(cur));
      toast(mode === 'polish' ? 'AI 正在润色这一段…' : 'AI 正在写这一段…');
      npcAiRun(mode, cur, 0);
    }
    /* v206：写完一段后停手，进度条停在真实位置；v209：等用户点板块上方的「继续写这一段」再写下一段 */
    var npcAiLastMode = 'write';
    function npcLoreDonePct(cur) {
      var _n = npcLoreSectionNames();
      var _m = npcLoreMissing(cur || []);
      return _n.length ? Math.round((_n.length - _m.length) / _n.length * 100) : 0;
    }
    function npcAiNext() {
      if (!memNpcDraft) return;
      if (npcAiBusy) { toast('AI 正在写，稍等一下'); return; }
      ...
      var cur = npcLoreCurrent();
      var names = npcLoreSectionNames();
      if (!npcLoreMissing(cur).length) { npcProgSection(names.length, names.length, false); toast('这份人设已经写满了'); return; }
      npcAiBusy = true;
      npcProgStart(npcLoreDonePct(cur));
      toast(npcAiLastMode === 'polish' ? 'AI 接着润色下一段…' : 'AI 接着写下一段…');
      npcAiRun(npcAiLastMode, cur, 0);
    }
    /* v209：一次只写一段 + 流式输出 —— 模型吐一个字就往上写一个字，写完停手等用户点板块上方的「继续写这一段」 */
    var npcAiWant = -1;
    function npcAiNextFor(i) {
      if (!memNpcDraft) return;
      if (npcAiBusy) { toast('AI 正在写，稍等一下'); return; }
      memberNpcSyncForm();
      var cur = npcLoreCurrent();
      if (String(cur[i] || '').trim()) { toast('这一段已经写好了'); return; }
      npcAiWant = i;
      npcAiNext();
    }
    function npcAiLinkBrief() { return NPC_LINK_BRIEF[npcLinkOf()] || NPC_LINK_BRIEF.mid; }
    function npcAiPromptFor(mode, ti, cur, mainName) {
      var names = npcLoreSectionNames();
      var tName = names[ti];
      var already = '';
      for (var i = 0; i < names.length; i++) {
        var v = String(cur[i] || '').trim();
        if (v && i !== ti) already += names[i] + '：' + v.slice(0, 200) + '\n';
      }
      var tip = (mode === 'polish')
        ? '下面是这个 NPC 已有的人设草稿。这次只把「' + tName + '」这一段扩写到 150~220 字，保留草稿里的信息与口吻，写得更具体更通顺。'
        : '这次只写「' + tName + '」这一段。';
      return '这是主角色的完整人设（只作背景参考，不要什么都围着 TA 写）：\n' + memberNpcMainText() +
        '\n\nNPC 名字：' + (memNpcDraft.name || '（未定）') +
        '\nNPC 简介：' + (memNpcDraft.bio || '（未填）') +
        '\n与主角色的关系：' + (memNpcDraft.rel || '（未填）') +
        '\n' + npcAiLinkBrief() +
        (already ? ('\n\n已有内容（保持前后一致，不要重写这些段）：\n' + already) : '') +
        '\n' + tip +
        '\n每段 150~220 字，中文，要具体到时间、地点、物件和说话方式，写出血肉与经历，不要空话套话；先让 TA 是一个自己活过的人，再谈和主角色有什么关系。' +
        (ti === 9 ? ('本轮的「' + tName + '」必须用第三人称写：直接叙述「' + (memNpcDraft.name || 'TA') + '」和「' + mainName + '」这两个人是怎么认识的、怎么相处的，禁止用「你」「我」称呼，禁止写成面对面说话的语气。') : '') +
        '\n只输出这一段正文本身：不要 JSON、不要段名、不要序号、不要解释，也不要任何 Markdown 标记。';
    }
    /* v209：流式写这一段 —— 每来一点字就写进输入框，进度条按真实字数走 */
    function npcAiStreamWrite(ti, sys, user, done) {
      var ta = memberEditScroll.querySelector('#npcSec' + ti);
      var full = '', shown = false, jsonLike = false;
      npcSecProgTimers[ti] = -1;   /* 占位：这一段归流式管，npcSecProgSync 别覆盖 */
      npcSecProgSet(ti, 4, 'AI 写作中…', 'writing');
      memberAiAskStream(sys, user, { maxTokens: 1400 }, function (all) {
        full = String(all || '');
        if (jsonLike) return;
        if (!shown) {
          var f = full.replace(/^[\s`]+/, '');
          if (!f) return;
          if (f.charAt(0) === '{' || f.charAt(0) === '[') { jsonLike = true; return; }
          shown = true;
          if (ta) ta.value = '';
        }
        if (ta) { ta.value = full; ta.scrollTop = ta.scrollHeight; }
        var n = full.replace(/\s/g, '').length;
        npcSecProgSet(ti, Math.min(90, 6 + n / 2.4), 'AI 写作中… ' + n + ' 字', 'writing');
      }, function (err, txt) {
        if (npcSecProgTimers[ti] != null) delete npcSecProgTimers[ti];
        done(err, (txt != null && String(txt)) ? String(txt) : full);
      });
    }
    function npcAiStreamRun(mode, cur, ti, attempt) {
      var total = npcLoreSectionNames().length;
      var mainName = ((memRoleFormCache || {}).name || '').trim() || '主角色';
      var user = npcAiPromptFor(mode, ti, cur, mainName);
      npcAiStreamWrite(ti, '你是世界观与角色设定助手。只输出这一段正文本身，不要任何格式标记。', user, function (err, txt) {
        if (!err && txt) {
          var got = String(txt);
          var _g2 = got.trim();
          /* 万一模型还是吐了 JSON，就把这一段的值抠出来 */
          if (_g2.charAt(0) === '{' || _g2.charAt(0) === '[') {
            try { var _ja = npcLoreExtractFromText(got); if (_ja && String(_ja[ti] || '').trim()) got = String(_ja[ti]); } catch (eJ) {}
          }
          var clean = npcLoreCleanText(got);
          clean = clean.replace(/^\s*[一二三四五六七八九十]+、[^\n：:]{1,14}[：:]\s*/, '');
          clean = clean.replace(/^\s*["“]+/, '').replace(/["”]+\s*$/, '').trim();
          if (clean.replace(/\s/g, '').length >= 40) {
            var out = cur.slice();
            out[ti] = clean;
            npcAiBusy = false;
            try { npcLoreFillToForm(npcLoreRealign(out)); } catch (e1) { try { npcLoreFillToForm(out); } catch (e2) {} }
            var still = npcLoreMissing(npcLoreCurrent());
            if (still.length) {
              /* 写完这一段就停手：段进度条停在真实位置，续写按钮出现在下一段的板块上方 */
              npcProgSection(total - still.length, total, true);
              toast('这一段写好了，想接着写就点下一段上面的「继续写这一段」');
            } else {
              npcProgSection(total, total, false);
              setTimeout(function () { npcProgDone(); }, 1200);
              toast(mode === 'polish' ? '人设润色完了，写得满满当当' : '人设写满了，一段不落');
            }
            return;
          }
        }
        /* 流式没写出东西：先原样重试，两次都不行就退回非流式老通道 */
        if (attempt < 2) { npcAiStreamRun(mode, cur, ti, attempt + 1); return; }
        npcAiRunJson(mode, cur, 0, ti);
      });
    }
    function npcAiRun(mode, cur, attempt) {
      npcAiLastMode = (mode === 'polish') ? 'polish' : 'write';
      var names = npcLoreSectionNames();
      var total = names.length;
      var cur2 = npcLoreCurrent();
      var miss = npcLoreMissing(cur2);
      var targets = [];
      if (mode === 'polish') {
        for (var _pi = 0; _pi < total; _pi++) if (String(cur2[_pi] || '').trim()) { targets = [_pi]; break; }
        if (!targets.length) targets = [0];
      } else {
        var _w = (npcAiWant >= 0 && !String(cur2[npcAiWant] || '').trim()) ? npcAiWant : -1;
        if (_w >= 0) targets = [_w];
        else targets = miss.length ? [miss[0]] : [];
      }
      npcAiWant = -1;
      if (!targets.length) {
        npcAiBusy = false;
      ...
      var ti = targets[0];
      /* 名字还没定：先让模型起一个贴合世界观的名字，再流式写这一段 */
      if (!String(memNpcDraft.name || '').trim()) {
        memberAiAsk('你是世界观与角色设定助手。',
          '这是主角色的完整人设：\n' + memberNpcMainText() + '\n\n请给这个 NPC 起一个贴合这个世界观的中文名（2~3 字），只输出名字本身，不要任何符号、引号或解释。',
          function (nt) {
            var nm = String(nt || '');
            if (/未找到|失败|error|Error|HTTP|超时/.test(nm)) nm = '';   /* 配置缺失时 cb 会把错误文案当内容回传，别当成名字 */
            nm = nm.replace(/["'“”‘’\s]/g, '').slice(0, 12);
            if (nm && memNpcDraft) {
              memNpcDraft.name = nm;
              var inp = memberEditScroll.querySelector('#npcName');
              if (inp) inp.value = nm;
            ...
            npcAiStreamRun(mode, cur2, ti, attempt);
          }, { maxTokens: 60 });
      } else {
        npcAiStreamRun(mode, cur2, ti, attempt);
      }
    }
    /* v207 的老通道（非流式 JSON）留着做兜底：流式两次都没写出来时走它 */
    function npcAiRunJson(mode, cur, attempt, forceTi) {
      npcAiLastMode = (mode === 'polish') ? 'polish' : 'write';
      var names = npcLoreSectionNames();
      var total = names.length;
      var miss = npcLoreMissing(cur);
      var MAX_TRY = 6;
      /* v206：一次只写一段，写完就停手等用户确认。
         写一段的输出长度很短，绝不会被上限截断成半截话；
         用户点板块上方的「继续写这一段」才写下一段，进度条按真实段落数推进。 */
      var targets = [];
      if (forceTi != null && forceTi >= 0 && !String(cur[forceTi] || '').trim()) targets = [forceTi];
      else if (mode === 'polish') {
        for (var _pi = 0; _pi < total; _pi++) if (String(cur[_pi] || '').trim()) { targets = [_pi]; break; }
        if (!targets.length) targets = [0];
      } else {
        targets = miss.length ? [miss[0]] : [];
      }
      if (!targets.length) {
        npcAiBusy = false;
        npcProgSection(total, total, false);
      ...
      var tNames = targets.map(function (ix) { return names[ix]; });
      var already = '';
      for (var _ai = 0; _ai < names.length; _ai++) {
        var _av = String(cur[_ai] || '').trim();
        if (_av && targets.indexOf(_ai) < 0) already += names[_ai] + '：' + _av.slice(0, 200) + '\n';
      }
      var tip;
      if (mode === 'polish') tip = '下面是这个 NPC 已有的人设草稿。这次只把这一段扩写到 150~220 字：' + tNames.join('、') + '，保留草稿里的信息与口吻，写得更具体更通顺。';
      else tip = '这次只写这一段：' + tNames.join('、') + '。';
      var _needName = !String(memNpcDraft.name || '').trim();
      var _mainName = ((memRoleFormCache || {}).name || '').trim() || '主角色';
      var user = '这是主角色的完整人设：\n' + memberNpcMainText() +
        '\n\nNPC 名字：' + (memNpcDraft.name || '（未定，请你取一个贴合世界观的中文名）') +
        '\nNPC 简介：' + (memNpcDraft.bio || '（未填）') +
        '\n与主角色的关系：' + (memNpcDraft.rel || '（未填）') +
        (already ? ('\n\n已有内容（保持前后一致，不要重写这些段）：\n' + already) : '') +
        '\n' + tip +
        '\n每段 150~220 字，中文，要具体到时间、地点、物件和说话方式，写出血肉与经历，不要空话套话；先让 TA 是一个自己活过的人，再谈和主角色有什么关系。\n' +
        npcAiLinkBrief() + '\n' +
        (targets.indexOf(9) >= 0 ? ('本轮的「' + names[9] + '」必须用第三人称写：直接叙述「' + (memNpcDraft.name || 'TA') + '」和「' + _mainName + '」这两个人是怎么认识的、怎么相处的，禁止用「你」「我」称呼，禁止写成面对面说话的语气。\n') : '') +
        (_needName ? '另外给这个 NPC 起一个贴合世界观的中文名（2~3 字），写在 JSON 的 "npcName" 键里。\n' : '') +
        '只输出一个 JSON 对象，键为：' + tNames.map(function (n) { return '"' + n + '"'; }).join('、') + (_needName ? ' 以及 "npcName"' : '') +
        '，值为对应段落的纯文字（150~220 字）。每个键的值只写这一段自己的内容，绝对不要把别的段落内容并进来；只用纯文本，不要出现 # * - > 这类 Markdown 标记，不要写段名、不要写序号、不要输出任何解释或多余文字。\n';
      var _sub = {};
      for (var _sk = 0; _sk < tNames.length; _sk++) _sub[tNames[_sk]] = String(cur[targets[_sk]] || '');
      user += JSON.stringify(_sub, null, 2);
      npcSecProgStart(targets[0]);
      memberAiAsk('你是世界观与角色设定助手，只输出 JSON。', user, function (txt, err) {
        /* v206：这一段没写出来就原样再试一次，绝不把半截内容落到输入框 */
        if (err || !txt) {
          if (attempt < 2) { npcAiRun(mode, cur, attempt + 1); return; }
          npcAiBusy = false;
          npcSecProgStop(targets[0]);
          var _d0 = npcLoreMissing(cur);
          npcProgSection(total - _d0.length, total, true);
          npcSecProgSync();
          toast('这一段 AI 没写出来，点「继续」再试一次');
          return;
        }
        var arr = npcLoreExtractFromText(txt);
        var out = cur.slice();
        var _hit = false;
        for (var k2 = 0; k2 < targets.length; k2++) {
          ...
          var got = '';
          try {
            got = String(arr[_ti2] || '').trim();
            if (!got) got = String(arr[k2] || '').trim();
          } catch (eT) { got = ''; }
          if (got) { out[_ti2] = got; _hit = true; }
        }
        if (!_hit) {
          if (attempt < 2) { npcAiRun(mode, cur, attempt + 1); return; }
          npcAiBusy = false;
          npcSecProgStop(targets[0]);
          var _d1 = npcLoreMissing(cur);
          npcProgSection(total - _d1.length, total, true);
          npcSecProgSync();
          toast('这一段没写全，点「继续」再试一次');
          return;
        }
        try { npcAiFillName(txt); } catch (eNm) {}
        try { npcLoreFillToForm(npcLoreRealign(out)); } catch (ePF) { try { npcLoreFillToForm(out); } catch (ePF2) {} }
        var still = npcLoreMissing(out);
        npcAiBusy = false;
        var doneN = total - still.length;
        if (still.length) {
          /* 这一段写好了就停手：进度条停在真实位置，等用户点「继续」再写下一段 */
          npcProgSection(doneN, total, true);
          toast('这一节写好了，想接着写就点下一段上面的「继续写这一段」');
        } else {
          npcProgSection(total, total, false);
          setTimeout(function () { npcProgDone(); }, 1200);
          toast(mode === 'polish' ? '人设润色完了，写得满满当当' : '人设写满了，一段不落');
    ...
    /* v202：顶栏左边＝退出（有未保存改动先问一句，没改动直接退） */
    function memberBackSmart() {
      /* v203：顶栏左边必须点得出反应 —— NPC 编辑页回名单、名单页回角色编辑，最后才是退出 */
      try {
        if (memNpcEditIdx !== null && memNpcEditIdx !== undefined) { memberNpcBack(); return; }
        if (memNpcListView) { memNpcListView = false; memberRerender(); toast('已返回角色编辑'); return; }
      } catch (eBk) { try { memNpcListView = false; memberRerender(); } catch (eBk2) {} return; }
      if (!memDirty) { memberDiscard(); return; }
      chatMini('退出编辑', '这篇人设有改动还没保存，要怎么处理？' +
        '<div style="margin-top:10px"><button type="button" class="prompt-cancel" id="memQuitNoSave" style="width:100%">不保存，直接退出</button></div>',
        '保存并退出', function () { memberSaveEdit(); }, false);
      var q = chatMiniBox.querySelector('#memQuitNoSave');
      if (q) q.addEventListener('click', function () { chatMiniMask.classList.remove('show'); closeMemberEdit(); });
    }
    // ===== 顶栏 / 返回绑定 =====
    document.getElementById('memberAddTop').addEventListener('click', function () {
      /* v203：在人设库页点加号 = 新建 NPC；其余情况仍是新建角色 */
      if (memNpcListView) { memberNpcOpen(-1); return; }
      openMemberEdit(-1);
    });
    document.getElementById('memberBack').addEventListener('click', function () { memberOverlay.classList.remove('open'); });
    document.getElementById('memberEditBack').addEventListener('click', function () { memberBackSmart(); });
    ...
    function generateTestImage(cfg, pos, neg, categoryPrompt, box, done, refImage) {
      ...
      fetch(baseUrl + '/images/generations', {
        ...
        return r.text().then(function (t) {
          ...
          if (!r.ok) throw new Error('HTTP ' + r.status + (txt ? ' ' + txt.slice(0, 200) : '（响应为空）'));
          if (!txt) throw new Error('接口返回空响应（HTTP ' + r.status + '），请检查 Base URL 是否指向兼容 /images/generations 的接口');
          var d;
          try { d = JSON.parse(txt); } catch (e) {
            throw new Error('接口返回非 JSON（HTTP ' + r.status + '）：' + txt.replace(/\s+/g, ' ').slice(0, 200));
          }
          if (d && d.error && !(d.data && d.data[0])) {
            throw new Error('接口错误：' + String((d.error && d.error.message) || JSON.stringify(d.error)).slice(0, 200));
          }
          return d;
        });
      }).then(function (data) {
        ...
        img.className = 'test-result test-zoom';
        img.alt = '';
        img.title = '点按查看大图';
        img.addEventListener('click', function (ev) { ev.stopPropagation(); viewImgFull(img.src); });
    ...
    function importWbFile(file, onDone) {
      ...
      if (ext === 'txt') {
        ...
        /* v208：老 .doc 直接读（跟「角色人设」导入共用同一套提取逻辑） */
        var rdoc = new FileReader();
        rdoc.onload = function () {
          var t = extractDocText(rdoc.result);
          onDone(baseName, (t && t.replace(/\s+/g, '').length >= 20) ? t : '（这个 .doc 里的文字没能自动读出来，请另存为 .docx / .txt 后导入）');
        };
        rdoc.onerror = function () { onDone(baseName, '（读取失败）'); };
        rdoc.readAsArrayBuffer(file);
      } else {
    ...
    function memberPayloadOf(m) {
      ...
      var vsrc = (m.voice && typeof m.voice === 'object') ? m.voice : {};
      var vpay = {
        enabled: vsrc.enabled !== false,
        voiceId: String(vsrc.voiceId != null ? vsrc.voiceId : (m.voiceId || '')).trim(),
        lang: String(vsrc.lang || 'zh-CN'),
        langLabel: String(vsrc.langLabel || chatLangLabelOf(vsrc.lang || 'zh-CN')),
        speed: (vsrc.speed != null && !isNaN(Number(vsrc.speed))) ? Number(vsrc.speed) : 1,
        freq: String(vsrc.freq || '中等')
      };
      return { v: 1, k: 'aether-member', id: String(m.id || ''), realName: m.realName || '', netName: m.netName || '', gender: m.gender || '', lore: m.lore || '', voiceId: m.voiceId || '', voice: vpay, look: m.look || '', lookPrompt: m.lookPrompt || '', alts: alts, roleAdd: !!m.roleAdd, npcAdd: !!m.npcAdd, t: Date.now() };
    }
    /* v201：好友码也要按「公开范围」区分内容，不能三种扫出来一个样。
       secret 只带名字；part 只带勾选的字段；all 带全部。扫码方拿到的资料严格跟着这个走。 */
    function memberSharePayloadOf(m, card) {
      m = m || {};
      var mode = (card && card.mode) ? String(card.mode) : 'all';
      var pay = { v: 1, k: 'aether-member', p: (mode === 'secret' ? 'secret' : (mode === 'part' ? 'part' : 'all')), id: String(m.id || ''), n: String(m.realName || m.netName || '未命名角色').slice(0, 24), t: Date.now() };
      if (pay.p === 'secret') return pay;                    /* 仅私密：只有名字 */
      var f = (pay.p === 'part' && card && card.fields) ? card.fields
        : { netName: true, gender: true, lore: true, look: true, voice: true, alts: true, npcs: true };
      if (f.netName) pay.nn = String(m.netName || '').slice(0, 24);
      if (f.gender) pay.g = (m.gender === 'male' ? '男' : (m.gender === 'female' ? '女' : ''));
      if (f.lore) pay.bio = String(m.lore || '').slice(0, 160);
      if (f.look) pay.lk = String(m.look || '').slice(0, 160);
      if (f.voice && m.voiceId) pay.voice = String(m.voiceId).slice(0, 60);
      if (f.alts) pay.alts = (Array.isArray(m.alts) ? m.alts : []).filter(function (x) { return String(x || '').trim(); }).slice(0, 6).map(function (x) { return String(x).slice(0, 24); });
      if (f.npcs) pay.npc = (Array.isArray(m.npcs) ? m.npcs : []).slice(0, 8).map(function (x) { return { n: String((x && x.name) || '').slice(0, 12), r: String((x && x.rel) || '').slice(0, 12) }; });
      pay.roleAdd = !!m.roleAdd;
      pay.npcAdd = !!m.npcAdd;
      return pay;
    }
    function memberShareCodeOf(m, card) {
      return 'aether://addmember/' + b64uEncode(JSON.stringify(card ? memberSharePayloadOf(m, card) : memberPayloadOf(m)));
    }
    ...
    function parseFriendCode(raw) {
      ...
      else if (/card\.html/i.test(s) || (/^https?:\/\//i.test(s) && /[#&]d=/.test(s))) {
        /* v190：名片链接（…/card.html#d=xxx）本身就是加好友码，扫到就按好友码处理 */
        var _cm = s.match(/[#&]d=([^&\s]+)/);
        dec = _cm ? b64uDecode(_cm[1]) : null;
        if (!dec) return { err: '这张名片链接读不出资料，请让对方重新生成二维码' };
      }
      else {
        var l2 = memberLoad();
      ...
      try {
        ...
        if (obj && (obj.k === 'aether-member' || obj.realName || obj.netName || obj.lore || obj.n)) return { payload: obj };
        return { err: '好友码不是有效的角色名片' };
      } catch (e) { return { err: '好友码解析失败，请确认内容完整' }; }
    }
    /* v190：把「名片码」字段（n/nn/g/bio/voice/av/npc）与「好友码」字段统一，加好友逻辑只认一种 */
    function payloadNormalize(p) {
      if (!p || typeof p !== 'object') return p;
      var q = {}, k;
      for (k in p) { if (Object.prototype.hasOwnProperty.call(p, k)) q[k] = p[k]; }
      if (!q.realName && q.n) q.realName = String(q.n);
      if (!q.netName && q.nn) q.netName = String(q.nn);
      if (!q.gender && q.g) q.gender = (q.g === '男' ? 'male' : (q.g === '女' ? 'female' : ''));
      if (!q.lore && q.bio) q.lore = String(q.bio);
      if (!q.voiceId && typeof q.voice === 'string') q.voiceId = String(q.voice);
      if (!q.avatar && q.av) q.avatar = String(q.av);
      if (!Array.isArray(q.npcs) && Array.isArray(q.npc)) {
        q.npcs = q.npc.map(function (x) { return { name: String((x && x.n) || ''), rel: String((x && x.r) || ''), bio: '', add: false }; });
      }
      return q;
    }
    function addFriendFromPayload(p, silent) {
      p = payloadNormalize(p);
      ...
      var _av = String(p.avatar || '');
      if (!/^(https?:|data:image\/)/.test(_av) || _av.length > 20000) _av = '';
      conv.settings.roleIdentity = { name: name, avatar: _av, sex: p.gender || '', prompt: memberRolePromptOf(p), remark: remark };
      if (p.voice && typeof p.voice === 'object') {
        var nv = conv.settings.voice || (conv.settings.voice = {});
        var _mg = loadMMConfig();
        nv.enabled = p.voice.enabled !== false;
        nv.groupId = nv.groupId || _mg.groupId || '';
        nv.apiKey = nv.apiKey || _mg.apiKey || '';
        nv.model = nv.model || _mg.model || 'speech-01-hd';
        nv.voiceId = String(p.voice.voiceId || p.voiceId || '');
        nv.lang = String(p.voice.lang || 'zh-CN');
        nv.langLabel = String(p.voice.langLabel || chatLangLabelOf(p.voice.lang || 'zh-CN'));
        nv.speed = (p.voice.speed != null && !isNaN(Number(p.voice.speed))) ? Number(p.voice.speed) : 1;
        if (!nv.habit || typeof nv.habit !== 'object') nv.habit = { enabled: true, frequency: '中等', triggers: '', special: '' };
        nv.habit.frequency = String(p.voice.freq || '中等');
      } else if (String(p.voiceId || '').trim()) {
        var nv2 = conv.settings.voice || (conv.settings.voice = {});
        nv2.voiceId = String(p.voiceId).trim();
        if (!nv2.lang) nv2.lang = 'zh-CN';
      }
      chatConvs.push(conv);
      saveContacts(); saveConvs();
      renderContacts(); renderChatConvs();
    ...
    function openAddFriendView() {
      openChatSub('添加好友', '' +
        '<div class="addfriend-wrap wx-add">' +
        '<div class="wx-add-search">' +
        '<input id="afCodeInput" type="text" placeholder="好友码 / 角色ID，粘贴后点添加">' +
        '<button type="button" id="afAddBtn">添加</button>' +
        '</div>' +
        '<div class="wx-add-group">' +
        '<button type="button" class="wx-add-row" id="afScanBtn"><span class="wx-add-ico i-scan"></span><span class="wx-add-txt">扫一扫</span><span class="wx-add-arrow">›</span></button>' +
        '<button type="button" class="wx-add-row" id="afAlbumBtn"><span class="wx-add-ico i-album"></span><span class="wx-add-txt">从相册选二维码</span><span class="wx-add-arrow">›</span></button>' +
        '<button type="button" class="wx-add-row" id="afMemberBtn"><span class="wx-add-ico i-list"></span><span class="wx-add-txt">从名单导入</span><span class="wx-add-arrow">›</span></button>' +
        '<button type="button" class="wx-add-row" id="afMyQrBtn"><span class="wx-add-ico i-qr"></span><span class="wx-add-txt">我的二维码</span><span class="wx-add-arrow">›</span></button>' +
        '</div>' +
        '<div class="addfriend-tip">让 TA 在「名单 → 编辑角色 → 二维码」里点「生成二维码」发你：用「扫一扫」扫它、或用「从相册选二维码」识别存下来的码图，都能把 TA 加进聊天列表；TA 没公开的资料不会带过来。</div>' +
        '<input type="file" id="afAlbumInput" accept="image/*" style="display:none">' +
        '<div class="af-album-hint" id="afAlbumHint" style="display:none"></div>' +
      ...
      document.getElementById('afAddBtn').addEventListener('click', function () {
        ...
        /* v190：粘贴好友码/名片链接也先出确认卡，与「扫一扫」一致，不静默加好友 */
        afRenderAddConfirm(r.payload);
        if (inp) inp.value = '';
      });
      document.getElementById('afScanBtn').addEventListener('click', function () { afStartScan(); });
      var afAlbBtn = document.getElementById('afAlbumBtn');
      if (afAlbBtn) afAlbBtn.addEventListener('click', function () { afAlbumPick(); });
      var afAlbInp = document.getElementById('afAlbumInput');
      if (afAlbInp) afAlbInp.addEventListener('change', function () {
        var f = afAlbInp.files && afAlbInp.files[0];
        if (f) afAlbumHandleFile(f);
      });
      document.getElementById('afMemberBtn').addEventListener('click', function () { afPickMember(); });
      var afMyBtn = document.getElementById('afMyQrBtn');
      if (afMyBtn) afMyBtn.addEventListener('click', function () { afMyQr(); });
      setTimeout(afCheckPending, 180);
      var stopEl = document.getElementById('afScanStop');
      if (stopEl) stopEl.addEventListener('click', afStopScan);
    }
    /* ===== v189：相册图片扫码（本地解码，不依赖摄像头） ===== */
    function afAlbumPick() {
      var inp = document.getElementById('afAlbumInput');
      if (!inp) return;
      afRenderResult('');
      inp.value = '';
      inp.click();
    }
    function afSetAlbumHint(html, show) {
      var h = document.getElementById('afAlbumHint');
      if (!h) return;
      h.style.display = show ? '' : 'none';
      h.innerHTML = html || '';
    }
    function afDecodeQrFromImage(img) {
      return new Promise(function (resolve) {
        var done = false;
        function fin(v) { if (!done) { done = true; resolve(String(v || '')); } }
        function tryJsQr() {
          var fn = (typeof window.jsQR === 'function') ? window.jsQR : null;
          if (!fn) { fin(''); return; }
          var w0 = img.naturalWidth || img.width || 0;
          var h0 = img.naturalHeight || img.height || 0;
          if (!w0 || !h0) { fin(''); return; }
          var scales = [1, 0.7, 0.5, 1.5, 2];
          for (var i = 0; i < scales.length; i++) {
            var s = scales[i];
            var w = Math.max(16, Math.round(w0 * s)), h = Math.max(16, Math.round(h0 * s));
            if (w * h > 5000000) continue;
            var cv = document.createElement('canvas');
            cv.width = w; cv.height = h;
            var ctx = cv.getContext('2d');
            ...
            var r = null;
            try { r = fn(id.data, w, h, { inversionAttempts: 'attemptBoth' }); } catch (e) { r = null; }
            if (r && r.data) { fin(r.data); return; }
            try {
              var px = id.data;
              for (var k = 0; k < px.length; k += 4) {
                var g = (px[k] * 0.299 + px[k + 1] * 0.587 + px[k + 2] * 0.114);
                var v2 = g < 128 ? 0 : 255;
                px[k] = px[k + 1] = px[k + 2] = v2;
              }
              var r2 = fn(px, w, h, { inversionAttempts: 'attemptBoth' });
              if (r2 && r2.data) { fin(r2.data); return; }
            } catch (e2) {}
          }
          fin('');
        }
        if ('BarcodeDetector' in window) {
          try {
            ...
            det.detect(img).then(function (codes) {
              if (codes && codes.length && codes[0] && codes[0].rawValue) fin(codes[0].rawValue);
              else tryJsQr();
    ...
    function afFieldChipsHtml(p) {
      var chips = [];
      if (p.netName) chips.push('网名 · ' + p.netName);
      if (p.gender) chips.push('性别 · ' + (p.gender === 'male' ? '男' : (p.gender === 'female' ? '女' : '保密')));
      return chips.length ? '<div class="af-card-chips">' + chips.map(function (x) { return '<span class="af-card-chip">' + escHtml(x) + '</span>'; }).join('') + '</div>' : '';
    }
    /* v191：确认卡按「身份 / 人设 / 语音 / 关系」分区，人设是人设、语音是语音，各归各位 */
    function afCardGroupsHtml(p) {
      var out = '';
      var idc = afFieldChipsHtml(p);
      if (idc) out += '<div class="af-sec"><div class="af-sec-t">身份</div>' + idc + '</div>';
      var per = '';
      if (p.lore) per += '<div class="af-txt"><i>身世</i>' + escHtml(String(p.lore).slice(0, 120)) + (String(p.lore).length > 120 ? '…' : '') + '</div>';
      if (p.look) per += '<div class="af-txt"><i>外貌</i>' + escHtml(String(p.look).slice(0, 120)) + (String(p.look).length > 120 ? '…' : '') + '</div>';
      if (per) out += '<div class="af-sec"><div class="af-sec-t">人设</div>' + per + '</div>';
      if (p.voiceId) out += '<div class="af-sec"><div class="af-sec-t">语音</div><div class="af-card-chips"><span class="af-card-chip">音色 · ' + escHtml(voiceNameOf(p.voiceId)) + '</span></div></div>';
      var rel = '';
      if (p.alts && p.alts.length) rel += '<div class="af-txt"><i>小号</i>' + escHtml(p.alts.join(' · ')) + '</div>';
      if (p.npcs && p.npcs.length) rel += '<div class="af-txt"><i>NPC</i>' + escHtml(p.npcs.map(function (x) { return String((x && x.name) || '?') + ((x && x.rel) ? '（' + x.rel + '）' : ''); }).join(' · ')) + '</div>';
      if (rel) out += '<div class="af-sec"><div class="af-sec-t">关系</div>' + rel + '</div>';
      return out;
    }
    /* v190：扫码/粘贴后先给一张确认卡（像微信的「添加到通讯录」），点一下才加 */
    function afRenderAddConfirm(p) {
      var rs = document.getElementById('afResult');
      if (!rs) return;
      p = payloadNormalize(p) || {};
      var name = String(p.realName || p.netName || '新好友');
      var av = String(p.avatar || '');
      if (!/^(https?:|data:image\/)/.test(av) || av.length > 20000) av = '';
      var secret = (p.p === 'secret');
      var lore = String(p.lore || '');
      var html = '<div class="af-card">' +
        '<div class="af-card-hd">' +
        (av ? '<img class="af-card-ava" src="' + av + '" alt="">' : '<div class="af-card-ava af-card-ava-t">' + escHtml(name.slice(0, 1)) + '</div>') +
        '<div class="af-card-nm"><div class="af-card-name">' + escHtml(name) + '</div><div class="af-card-sub">' + (secret ? 'TA 只公开了名字' : '扫码看到的公开资料 · 已按类分好') + '</div></div>' +
        '</div>' +
        (secret ? '' : afCardGroupsHtml(p)) +
        (secret && lore ? '<div class="af-card-bio">' + escHtml(lore.slice(0, 60)) + (lore.length > 60 ? '…' : '') + '</div>' : '') +
        '<button type="button" class="af-card-add" id="afConfirmAdd">添加到聊天列表</button>' +
        '<div class="af-card-foot">TA 没公开的资料不会出现在你的角色设定里</div>' +
        '</div>';
      rs.innerHTML = html;
      var b = document.getElementById('afConfirmAdd');
      if (b) b.addEventListener('click', function () {
        if (addFriendFromPayload(p)) afRenderResult('<div class="af-ok">已添加，可在「聊天」列表查看</div>');
      });
    }
    function afHandleQrRaw(raw, quiet) {
      raw = String(raw || '').trim();
      if (!raw) return false;
      var r = parseFriendCode(raw);
      if (r.err) {
        afRenderResult('<div class="af-err">这个码不是 AEphone 的加好友码。<br>让对方在「名单 → 编辑角色 → 二维码」里点「生成二维码」，用那个码扫一下就能加 TA。</div>');
        if (!quiet) toast('这不是 AEphone 的加好友码');
        return false;
      }
      afStopScan();
      afSetAlbumHint('', false);
      afRenderAddConfirm(r.payload);
      if (!quiet) toast('认出来啦：点「添加到聊天列表」即可');
      return true;
    }
    function afAlbumHandleFile(file) {
      if (!file) return;
      afRenderResult('');
      afSetAlbumHint('<div class="af-scan-tip">正在识别图片里的二维码…</div>', true);
      var rd = new FileReader();
      rd.onload = function () {
        var img = new Image();
        img.onload = function () {
          afSetAlbumHint('<img src="' + rd.result + '" alt=""><div class="af-scan-tip">正在识别图片里的二维码…</div>', true);
          afDecodeQrFromImage(img).then(function (raw) {
            afSetAlbumHint('<img src="' + rd.result + '" alt=""><div class="af-scan-tip">' + (raw ? '识别完成' : '没找到二维码，换一张更清晰的图再试') + '</div>', true);
            afHandleQrRaw(raw, false);
          });
        };
        img.onerror = function () { afSetAlbumHint('', false); afRenderResult('<div class="af-err">这张图打不开，换一张试试</div>'); };
        img.src = rd.result;
      };
      rd.onerror = function () { afSetAlbumHint('', false); afRenderResult('<div class="af-err">读取图片失败，换一张试试</div>'); };
      rd.readAsDataURL(file);
    }
        ...
        afScanTimer = setInterval(function () {
          ...
          detector.detect(video).then(function (codes) {
            ...
            for (var i = 0; i < codes.length; i++) {
              ...
              if (!raw) continue;
              if (afHandleQrRaw(raw, true)) return;
            }
          }).catch(function () {});
        }, 400);
      }).catch(function () {
    ...
    /* v193：二维码要「头像跟随」，就把头像压成码心尺寸的小图（约 26px），再按总长预算逐级瘦身 */
    function memberQrAvatarTiny(av, cb) {
      var a = String(av || '');
      if (!a) { cb(''); return; }
      qrAvatarImage(a, function (img) {
        if (!img) { cb(''); return; }
        try {
          var tries = [[26, 0.55], [24, 0.5], [22, 0.45], [20, 0.42], [18, 0.4], [16, 0.38]];
          var best = '';
          for (var i = 0; i < tries.length; i++) {
            ...
            var side = Math.max(1, Math.min(sw, sh));
            ctx.drawImage(img, (sw - side) / 2, (sh - side) / 2, side, side, 0, 0, S, S);
            var jp = cv.toDataURL('image/jpeg', tries[i][1]);
            var pn = cv.toDataURL('image/png');
            best = (jp.length <= pn.length) ? jp : pn;
            if (best.length <= 600) break;
          }
          cb(best.length <= 1000 ? best : '');
        } catch (e) { cb(''); }
      });
    }
    function memberQrCardUrl(src, card, cb) {
      memberQrAvatarTiny(src && src.avatar, function (tiny) {
        var plans = [
          { bio: 160, look: 160, alts: 6, npc: 8 },
          { bio: 100, look: 100, alts: 6, npc: 8 },
          { bio: 70, look: 70, alts: 5, npc: 6 },
          { bio: 50, look: 50, alts: 4, npc: 5 },
          { bio: 36, look: 36, alts: 3, npc: 4 },
          { bio: 28, look: 24, alts: 2, npc: 3 },
          { bio: 22, look: 16, alts: 2, npc: 2 },
          { bio: 18, look: 10, alts: 1, npc: 2 },
          { bio: 14, look: 0, alts: 0, npc: 2 },
          { bio: 10, look: 0, alts: 0, npc: 1 },
          { bio: 0, look: 0, alts: 0, npc: 0 }
        ];
        var txt = '';
        for (var i = 0; i < plans.length; i++) {
          txt = memberCardUrl(src, card, { av: tiny, budget: plans[i] });
          if (txt && txt.length <= 1150) break;
        }
        /* 全部超预算时：宁可少带资料，也保证头像跟随 */
        if (!txt || txt.length > 1150) txt = memberCardUrl(src, card, { av: tiny, budget: plans[plans.length - 1] });
        cb(txt || '');
      });
    }

    /* v190：我的二维码（名单里任一角色的加好友码，可保存到相册） */
    function memberQrSrcOf(m) {
      m = m || {};
      return {
        id: m.id || '', realName: m.realName, netName: m.netName, gender: m.gender,
        lore: m.lore, voiceId: m.voiceId, look: m.look, lookPrompt: m.lookPrompt,
        alts: m.alts, npcs: m.npcs, avatar: m.avatar
      };
    }
    function afMyQr() {
      var list = memberLoad();
      var rs = document.getElementById('afResult');
      if (!rs) return;
      if (!list.length) { rs.innerHTML = '<div class="af-err">名单还是空的，先在「名单」里添加角色，再来生成二维码</div>'; return; }
      var html = '<div class="af-mem-title">选一个角色看 TA 的二维码<button type="button" class="tips-q" data-tip="qrsave">?</button></div>';
      html += list.map(function (m, i) {
        var nm = memberDispName(m);
        return '<div class="af-mem-item" data-qr="' + i + '"><span class="af-mem-name">' + escHtml(nm) + '</span><span class="af-mem-add">看二维码</span></div>';
      }).join('');
      rs.innerHTML = html;
      rs.querySelectorAll('.af-mem-item').forEach(function (it) {
        it.addEventListener('click', function () {
          var mi = parseInt(it.getAttribute('data-qr'), 10);
          var m = list[mi];
          if (!m) return;
          var nm2 = memberDispName(m);
          memberQrCardUrl(memberQrSrcOf(m), m.card, function (txt) {
            if (!txt) { toast('二维码生成失败'); return; }
            openQrViewer(txt, { avatar: m.avatar, name: nm2 });
          });
        });
      });
    }
    /* v190：从名片页点「添加到我的聊天列表」跳回来时，自动弹确认卡 */
    function afCheckPending() {
      var raw = '';
      try { raw = localStorage.getItem('aether_pending_friend') || ''; } catch (e) { return false; }
      if (!raw) return false;
      try { localStorage.removeItem('aether_pending_friend'); } catch (e) {}
      var r = parseFriendCode(raw);
      if (r.err) return false;
      afRenderAddConfirm(r.payload);
      toast('已读取名片：点「添加到聊天列表」即可加好友');
      return true;
    }
    /* v191：名片页跳回来时会带 #addfriend=xxx——别的手机没开过站点、localStorage 被限制也能加 */
    function afPendingFromHash() {
      var h = String(location.hash || '');
      var m = h.match(/[#&]addfriend=([^&]+)/);
      if (!m) return '';
      try { if (history.replaceState) history.replaceState(null, '', location.pathname + location.search); } catch (e) { try { location.hash = ''; } catch (e2) {} }
      var dec = '';
      try { dec = b64uDecode(m[1]) || ''; } catch (e) {}
      return dec;
    }
    function afAutoPending() {
      var fromHash = afPendingFromHash();
      if (fromHash) { try { localStorage.setItem('aether_pending_friend', fromHash); } catch (e) {} }
      var raw = '';
      try { raw = localStorage.getItem('aether_pending_friend') || ''; } catch (e) { raw = fromHash; }
      if (!raw) return;
      try {
        var apps = Array.from(document.querySelectorAll('.app'));
        for (var i = 0; i < apps.length; i++) {
          var lb = apps[i].querySelector('.label');
          if (lb && lb.textContent.trim() === '聊天') { apps[i].click(); break; }
        }
      } catch (e) {}
      setTimeout(function () {
        try { openAddFriendView(); } catch (e) {}
        /* v192：别的手机首次打开站点较慢，先给「加载中」而不是空白，别让人以为点了没反应 */
        try {
          var afBox = document.getElementById('afResult');
          if (afBox && !afBox.querySelector('.af-card')) {
            afBox.innerHTML = '<div class="af-loading">名片加载中…首次打开本站可能要十几秒，请稍候，别关闭页面</div>';
          }
        } catch (e) {}
        var afTries = 0;
        var afTick = function () {
          ...
          var ok = false;
          try {
            var box = document.getElementById('afResult');
            if (!box || !box.querySelector('.af-card')) afCheckPending();
            var now = document.getElementById('afResult');
            ok = !!(now && now.querySelector('.af-card'));
          } catch (e) {}
          if (ok) return;
          if (afTries < 20) { setTimeout(afTick, 700); return; }
          try {
            var end = document.getElementById('afResult');
            if (end && !end.querySelector('.af-card')) end.innerHTML = '<div class="af-loading">没读到名片。请回到名片页再点一次，或把名片链接粘贴到上面的输入框点「添加」</div>';
          } catch (e) {}
        };
        try { afCheckPending(); } catch (e) {}
        setTimeout(afTick, 700);
      }, 800);
    }
    window.addEventListener('load', function () { setTimeout(afAutoPending, 900); });
    ...
    /* v205：会话列表预览要显示文字本身 —— 把富文本/卡片正文里的标签剥掉取纯文字 */
    function chatMsgPlain(v) {
      var t = String(v == null ? '' : v);
    ...
    function msgPreview(m) {
      ...
      /* v205：富文本 / 卡片消息直接在列表里显示正文文字，不再只显示一个 [类型] */
      if (m.type === 'html') { var _hp = chatMsgPlain(m.text || ''); return _hp || '[卡片]'; }
      if (m.fmt === 'action' || m.fmt === 'narrator') { var _np = chatMsgPlain(m.text || ''); if (_np) return _np; }
      ...
      if (m.type === 'sys') return '[系统] ' + (m.text || '');
      return m.text || '';
    }
    function fmtTime(ts) {
      if (!ts) return '';
    ...
    /* v206：点聊天顶部的角色昵称 → 角色底层控制（直接作用在角色底层逻辑与模型上） */
    if (chatDetailName) {
      chatDetailName.style.cursor = 'pointer';
      chatDetailName.addEventListener('click', chatSysCtlOpen);
    }
    /* v208：顶栏备注名统一走这里设置 —— 先按字数给一档初始字号，
       再量一下有没有装下，没装下就一档一档往下缩（16 → 10.5px，配合最多三行），
       直到整条备注名都完整显示，不再被省略号缩写。 */
    function chatDetailNameSet(txt) {
      if (!chatDetailName) return;
      var t = String(txt == null ? '' : txt);
      chatDetailName.textContent = '';
      var ntx = document.createElement('span');
      ntx.className = 'nm-tx';
      ntx.textContent = t;
      chatDetailName.appendChild(ntx);
      chatDetailName.title = t;
      var n = t.replace(/\s/g, '').length;
      chatDetailName.classList.remove('nm-long', 'nm-vlong');
      if (n >= 16) chatDetailName.classList.add('nm-vlong');
      else if (n >= 11) chatDetailName.classList.add('nm-long');
      var size = n >= 16 ? 12.5 : (n >= 11 ? 14 : 16);
      for (var g = 0; g < 14; g++) {
        chatDetailName.style.fontSize = size + 'px';
        if (chatDetailName.scrollHeight <= chatDetailName.clientHeight + 1) break;
        if (size <= 10.5) break;
        size -= 0.5;
      }
    }
    ...
    var SETTING_PARENT = {
      ...
      voice: 'sec-sense', voiceinfo: 'sec-sense', imag: 'sec-sense', auto: 'sec-sense',
      appearance: 'sec-app', apcss: 'sec-app',
      logs: 'sec-data', dataio: 'sec-data', storage: 'sec-data',
    ...
    var CHAT_FUNCS = [
      ...
      /* v204：朋友圈只走「更多」界面的入口；推荐好友是角色主动行为，不做成功能按钮 */
    ];
    ...
    /* v193：说明文字一律收进「?」——默认不占版面，点一下才展开 */
    var TIP_TEXT = {
      cardmode: ['扫码能看到什么', '<div class="tips-row"><b>全部公开</b>：资料全显示，二维码外圈是<b>绿圈</b></div><div class="tips-row"><b>部分公开</b>：只显示你勾选的那几项，二维码外圈是<b>黄圈</b></div><div class="tips-row"><b>仅私密</b>：只显示名字，看不到人设细节，但仍然能加 TA 聊天，二维码外圈是<b>红圈</b></div>'],
      qr: ['角色二维码', '<div class="tips-row">用另一台手机的「扫一扫 / 从相册选二维码 / 粘贴」都能直接加 TA 为好友。</div><div class="tips-row">点二维码看大图，长按图片能直接识别，也能存进相册。</div>'],
      qrsave: ['二维码怎么用', '<div class="tips-row">点二维码打开大图，「保存到相册」；长按图片选「存储图像」也能存。</div><div class="tips-row">别人扫这个码，就能把 TA 加为好友。</div>'],
      reco: ['推荐好友', '<div class="tips-row">让角色把 TA 自己或 TA 身边的人物（NPC）的名片二维码发进聊天，相当于把朋友推荐给你。</div><div class="tips-row">点二维码可放大，长按可直接识别，也能保存到相册。</div>'],
      mylook: ['我的形象', '<div class="tips-row">这里写的是「你」的样子。生成含你出镜的画面时用得上：外貌描写 + 生图提示词 + 锁脸参考图。</div><div class="tips-row">角色自己的形象，在「名单 → 角色 → 形象与锁脸」里配。</div>'],
      album: ['角色相册', '<div class="tips-row">相册。角色生成、收到的图都会存进来，可全屏翻阅、长按保存。</div>']
    };
    function showTipSheet(title, bodyHtml) {
      var ov = document.createElement('div');
      ov.className = 'tips-mask';
      ov.innerHTML = '<div class="tips-box"><div class="tips-hd"><span>' + escHtml(title || '说明') + '</span><button type="button" class="tips-x">关闭</button></div><div class="tips-bd">' + (bodyHtml || '') + '</div></div>';
      document.body.appendChild(ov);
      var shut = function () { if (ov.parentNode) ov.parentNode.removeChild(ov); };
      ov.addEventListener('click', function (e) { if (e.target === ov) shut(); });
      ov.querySelector('.tips-x').addEventListener('click', shut);
    }
    document.addEventListener('click', function (e) {
      var q = (e.target && e.target.closest) ? e.target.closest('.tips-q') : null;
      if (!q) return;
      e.stopPropagation();
      var t = TIP_TEXT[q.getAttribute('data-tip') || ''];
      if (t) showTipSheet(t[0], t[1]);
    }, true);
    ...
    /* v206：角色底层控制 —— 聊天界面点角色昵称打开，写进去的话直接改角色底层逻辑与模型行为 */
    function chatSysCtlOpen() {
      if (!chatCurrentConv || !chatCurrentConv.settings) return;
      var s = chatCurrentConv.settings;
      var cur = String(s.sysCtl || '');
      var html = '<div class="sys-ctl-tip">这里写的是角色底层控制，会直接写进模型：优先级高于人设、系统提示词、世界书和已有剧情。角色 OOC 了、想改接下来的走向、想给角色加限制或新设定，都写在这儿。</div>' +
        '<textarea class="chat-mini-input sys-ctl-ta" id="chatSysCtlTa" rows="6" placeholder="例：从现在起保持克制，不许主动示好；每句话不超过两句；不要再提昨天那件事。">' + escHtml(cur) + '</textarea>' +
        (cur ? '<button type="button" class="prompt-cancel sys-ctl-clear" id="chatSysCtlClear">清空控制</button>' : '');
      chatMini('角色底层控制', html, '保存并生效', function () {
        var ta = chatMiniBox.querySelector('#chatSysCtlTa');
        s.sysCtl = ta ? String(ta.value || '').trim() : '';
        try { saveConvs(); } catch (e) {}
        toast(s.sysCtl ? '底层控制已生效，从这一轮起按新设定走' : '已清空底层控制');
      }, false);
      var cl = chatMiniBox.querySelector('#chatSysCtlClear');
      if (cl) cl.addEventListener('click', function () {
        s.sysCtl = '';
        try { saveConvs(); } catch (e) {}
        var ta2 = chatMiniBox.querySelector('#chatSysCtlTa');
        if (ta2) ta2.value = '';
        chatMiniMask.classList.remove('show');
        toast('已清空底层控制');
      });
    ...
    function renderChatDetailAvatar(conv) {
      try {
        var el = document.getElementById('chatDetailAva');
        if (!el) return;
        conv = conv || chatCurrentConv;
        if (!conv) { el.classList.remove('on', 'st-online', 'st-offline', 'st-busy'); el.innerHTML = ''; return; }
        var av = '';
        var s = conv.settings || {};
        if (s.roleIdentity && typeof s.roleIdentity === 'object' && s.roleIdentity.avatar) av = s.roleIdentity.avatar;
        if (!av) { var m = memberByConv(conv); if (m && m.avatar) av = m.avatar; }
        /* v203：绿=在线 / 灰=离线 / 红=忙碌 */
        var _stTx = String(conv.status || '在线');
        el.classList.remove('st-online', 'st-offline', 'st-busy');
        if (s.blocked || _stTx.indexOf('离线') >= 0 || _stTx.indexOf('拉黑') >= 0) el.classList.add('st-offline');
        else if (_stTx.indexOf('忙碌') >= 0 || _stTx.indexOf('勿扰') >= 0) el.classList.add('st-busy');
        else el.classList.add('st-online');
        if (av) {
          el.innerHTML = '<img src="' + av + '" alt="">';
          el.classList.add('on');
        } else {
          el.innerHTML = '<span>' + escHtml(String(conv.name || '?').slice(0, 1)) + '</span>';
          el.classList.add('on');
        }
      } catch (e) {}
    }
    /* ===== v201：角色状态栏（点聊天顶栏的角色头像展开） =====
       数据随会话存 conv.settings.roleStatus：{ affinity, fields:{mood,outfit,action,body,env}, ts, aiTs }
       先按本地规则（时段 + 人设 + 最近对话）给出即时状态，点「AI 刷新」再由模型细化。 */
    var ROLE_STATUS_DEF = [['mood', '当前心情'], ['outfit', '当前服装'], ['action', '当前行动'], ['body', '当前身体状况'], ['env', '当前所处环境']];
    /* v205：状态各项的 SVG 图标 + 历史记录入口图标 */
    var ROLE_STATUS_ICON = {
      mood: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M8.6 14.4a4.4 4.4 0 0 0 6.8 0"/><path d="M9.2 9.6h.01M14.8 9.6h.01"/></svg>',
      outfit: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M9 3l3 2 3-2 4 3-2 3v10H7V9L5 6z"/></svg>',
      action: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M13 2.5L5 13.5h5l-1 8L18 10.5h-5z"/></svg>',
      body: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M20.4 6.6a5 5 0 0 0-7.1 0L12 7.9 10.7 6.6a5 5 0 1 0-7.1 7.1L12 22l8.4-8.3a5 5 0 0 0 0-7.1z"/></svg>',
      env: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M12 21.5s7-6.2 7-11.2a7 7 0 1 0-14 0c0 5 7 11.2 7 11.2z"/><circle cx="12" cy="10" r="2.6"/></svg>'
    };
    var ROLE_STATUS_HIST_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 12a8.5 8.5 0 1 0 2.6-6.1"/><path d="M3.5 4.5V9H8"/><path d="M12 7.8V12l3 1.8"/></svg>';
    var roleStHistOpen = false;
    /* v205：状态历史 —— 每次重算留一份快照，最多 20 条 */
    function chatRoleStatusHistPush(st, fields, src, conv) {
      if (!st) return;
      if (!st.hist || !Array.isArray(st.hist)) st.hist = [];
      var snap = { ts: Date.now(), src: src || 'ai', affinity: null, fields: {} };
      try { snap.affinity = chatRoleAffinityOf(conv); } catch (eA) {}
      for (var i = 0; i < ROLE_STATUS_DEF.length; i++) {
        var k = ROLE_STATUS_DEF[i][0];
        snap.fields[k] = String((fields && fields[k]) || '');
      }
      st.hist.push(snap);
      if (st.hist.length > 20) st.hist = st.hist.slice(st.hist.length - 20);
    }

    function chatRoleAffinityBase(conv) {
      var ms = (conv && conv.messages) || [];
      var rounds = 0;
      for (var i = 0; i < ms.length; i++) if (ms[i] && ms[i].role === 'me') rounds++;
      return Math.max(10, Math.min(90, 18 + Math.round(Math.sqrt(rounds) * 9)));
    }
    function chatRoleStatusOf(conv) {
      if (!conv) return null;
      if (!conv.settings) conv.settings = defaultConvSettings();
      var st = conv.settings.roleStatus;
      if (!st || typeof st !== 'object') { st = conv.settings.roleStatus = { affinity: null, fields: null, ts: 0, aiTs: 0, hist: [] }; }
      if (st.affinity == null || isNaN(Number(st.affinity))) st.affinity = chatRoleAffinityBase(conv);
      return st;
    }
    function chatRoleAffinityOf(conv) {
      try {
        var st = chatRoleStatusOf(conv);
        if (st && st.affinity != null && !isNaN(Number(st.affinity))) return Math.max(0, Math.min(100, Number(st.affinity)));
      } catch (e) {}
      return chatRoleAffinityBase(conv);
    }
    function chatRoleAffinityLevel(v) {
      v = Number(v) || 0;
      if (v >= 90) return { label: '死心塌地', desc: '你说什么是什么，基本不会有二话' };
      if (v >= 75) return { label: '亲密', desc: '会主动找你，也会跟你闹点小脾气' };
      if (v >= 60) return { label: '亲近', desc: '愿意把心里话说给你听' };
      if (v >= 40) return { label: '朋友', desc: '聊得来，但还留着分寸' };
      if (v >= 20) return { label: '眼熟', desc: '知道你这个人，礼貌但谈不上熟' };
      return { label: '陌生', desc: '还在观察你，不会轻易交底' };
    }
    /* v209：这个角色说什么语言、活在什么语言环境里 —— 状态必须按它来写 */
    function chatRoleStatusLangEnv(conv) {
      var out = { spoken: '', style: '现代都市语境', modern: true, brief: '' };
      try {
        var s = (conv && conv.settings) || {};
        var lg = (s.voice && s.voice.lang) ? String(s.voice.lang) : '';
        var lb = (s.voice && s.voice.langLabel) ? String(s.voice.langLabel) : '';
        if (lg && lg.indexOf('zh') !== 0) out.spoken = lb || lg;
      } catch (e) {}
      var p = '';
      try { p = chatRolePersonaOf(conv) || ''; } catch (e2) {}
      var RULES = [
        [/古风|仙侠|修真|江湖|武侠|古代|古人|古装|古言|王朝|朝廷|朝代|年间|宫廷|后宫|仙门|仙途|门派|修仙|侠客|剑客|武林|官府|衙门|县令|捕快|科举|书院|秀才|状元|府上|嫡|庶|帝王|太子|公主|娘娘|太医|王爷|侯爷|少爷|娘子|夫君|丫鬟|客栈|酒肆|镖局|帮派|长衫|布衣|袍子|妾|朕|臣妾/, '古风语境'],
        [/民国|旧上海|租界|军阀|乱世/, '民国语境'],
        [/异世界|魔法|剑与魔法|西幻|精灵|矮人|龙族|魔王|骑士|冒险者|公会/, '西幻异世界语境'],
        [/星际|赛博|机甲|未来世界|末日|废土|末世|异能|空间站/, '科幻未来语境'],
        [/校园|高中|大学|教室|社团|学长|学姐|同桌/, '校园语境']
      ];
      for (var i = 0; i < RULES.length; i++) { if (RULES[i][0].test(p)) { out.style = RULES[i][1]; break; } }
      out.modern = (out.style === '现代都市语境' || out.style === '校园语境');
      out.brief = (out.spoken ? ('说话语言：' + out.spoken + '（状态也用这个语言写）；') : '') + '语言环境：' + out.style;
      return out;
    }
    /* 最近对话里的情绪线索 + 时段 + 角色语言环境，推一份「此刻」的状态 */
    function chatRoleStatusLocal(conv) {
      var h = new Date().getHours();
      var part = h < 6 ? '凌晨' : (h < 9 ? '清晨' : (h < 12 ? '上午' : (h < 14 ? '中午' : (h < 18 ? '下午' : (h < 22 ? '晚上' : '深夜')))));
      var langEnv = chatRoleStatusLangEnv(conv);
      /* 现代语境（都市 / 校园）的措辞池 */
      var envs = {
        '凌晨': '房间里只留了一盏小灯，窗外的城市安静得能听见风', '清晨': '刚拉开窗帘，光斜斜地切进屋里',
        '上午': '在光线最好的那面窗前，桌上摊着没收拾完的东西', '中午': '正午的光晒得发白，屋里有点闷',
        '下午': '下午的光变得柔和，屋里有种懒洋洋的静', '晚上': '灯开着，屋里暖黄一片，窗外已经黑了',
        '深夜': '只剩屏幕的光，四周静得连自己的呼吸都听得清'
      };
      var acts = {
        '凌晨': '还没睡，靠在床边刷着手机', '清晨': '刚醒不久，正慢慢把自己收拾利落',
        '上午': '在处理今天该做的事，偶尔停下来走神', '中午': '刚吃完东西，坐着发一会儿呆',
        '下午': '手头的事做得差不多了，正想着找点别的', '晚上': '一天的事都收了尾，正闲下来',
        '深夜': '该睡又不甘心睡，躺着胡思乱想'
      };
      var outfits = {
        '凌晨': '一件洗得发软的宽大T恤，头发散着', '清晨': '刚换上的干净衣服，简单利落',
        '上午': '日常外出那一身，袖口随意挽着', '中午': '轻薄的一身，领口松着',
        '下午': '宽松的家常衣服，赤着脚', '晚上': '换成了舒服的家居服',
        '深夜': '睡衣外面随便搭了件外套'
      };
      /* v209：非现代语境（古风 / 民国 / 西幻 / 未来）另用一套 ——
         手机、外卖、家居服、屏幕这些现代物件一个都不许出现 */
      var pEnvs = {
        '凌晨': '屋里只留了一盏灯，四下静得能听见风', '清晨': '天光刚透进来，窗上是一片淡淡的亮',
        '上午': '光落在案头，手边的东西还没收拾利落', '中午': '日头正高，屋里有些闷，光停在原处不动',
        '下午': '午后的光柔下来，屋里静得发懒', '晚上': '灯点着，屋里一片温黄，窗外已经全黑',
        '深夜': '灯将尽未尽，四下静极，只剩一点微光'
      };
      var pActs = {
        '凌晨': '还没歇下，靠着榻边出神，手里的事放下又拿起来', '清晨': '刚醒不久，正慢慢把自己收拾利落',
        '上午': '在处理今日该办的事，偶尔停下来出神', '中午': '刚用过饭，坐着发一会儿呆',
        '下午': '手头的事做得差不多了，正想着找点别的', '晚上': '一天的事都收了尾，正闲下来',
        '深夜': '该歇又不甘心歇，躺着胡思乱想'
      };
      var pOutfits = {
        '凌晨': '一件宽松的旧衣，头发散着', '清晨': '刚换上的干净衣裳，简单利落',
        '上午': '日常出门那一身，袖口随意挽着', '中午': '轻薄的一身，领口松着',
        '下午': '家常的旧衣，赤着脚', '晚上': '换成了舒服的便服',
        '深夜': '外衣随便搭在身上，衣带松着'
      };
      if (!langEnv.modern) { envs = pEnvs; acts = pActs; outfits = pOutfits; }
      var ms = (conv && conv.messages) || [];
      var recent = '';
      for (var i = Math.max(0, ms.length - 12); i < ms.length; i++) { var t = ms[i] ? chatVoiceHtml(ms[i]) : ''; if (t) recent += ' ' + String(t).slice(0, 80); }
      var mood = '心情还算平稳，没什么起伏';
      if (/想你|喜欢|爱你|好可爱|抱抱|亲/.test(recent)) mood = '心里有点发烫，被你一句话弄得没静下来';
      else if (/生气|讨厌|烦|滚|别理我|冷/.test(recent)) mood = '心里还憋着一口气，嘴上不说但没消';
      else if (/难过|哭了|委屈|疲惫|累死|撑不住/.test(recent)) mood = '有点心疼你，情绪也跟着沉了下去';
      else if (/晚安|睡了|早点休息/.test(recent)) mood = '心是软的，准备安静地收尾这一晚';
      else if (/开心|哈哈|笑死|好耶/.test(recent)) mood = '被你逗得心情不错，嘴角还没放下来';
      var body = /累|困|熬夜|失眠/.test(recent)
        ? (langEnv.modern ? '有点乏，肩颈发紧，硬撑着精神' : '有些乏，肩背发紧，硬撑着精神')
        : (langEnv.modern ? '状态正常，只是坐久了起来活动两下' : '状态平常，只是坐久了起身走动两下');
      return { mood: mood, outfit: outfits[part] || outfits['晚上'], action: acts[part] || acts['晚上'], body: body, env: envs[part] || envs['晚上'] };
    }
    /* v204：这份状态属于哪个角色 —— 会话绑定的名单角色为准，换了角色就必须重算 */
    function chatRoleStatusRoleKey(conv) {
      try { var m = memberByConv(conv); if (m && m.id) return 'm:' + m.id; } catch (e) {}
      return 'c:' + ((conv && conv.id) || '');
    }
    function chatRoleStatusEnsure(conv) {
      var st = chatRoleStatusOf(conv);
      if (!st) return null;
      /* v204：状态必须跟着当前角色走 —— 换了角色就作废重算，绝不沿用上一个角色的状态 */
      var _rk = chatRoleStatusRoleKey(conv);
      if (st.roleKey !== _rk) {
        st.roleKey = _rk;
        st.fields = null;
        st.aiTs = 0;
        st.ts = 0;
        st.affinity = chatRoleAffinityBase(conv);
        st.hist = []; /* v205：换了角色，历史记录也跟着换人，不串 */
      }
      if (!st.hist || !Array.isArray(st.hist)) st.hist = [];
      if (!st.fields || typeof st.fields !== 'object') {
        st.fields = chatRoleStatusLocal(conv);
        st.ts = Date.now();
        try { chatRoleStatusHistPush(st, st.fields, 'local', conv); } catch (eH1) {}
      }
      return st;
    }
    function chatRoleStatusAi(conv, cb) {
      var st = chatRoleStatusOf(conv);
      var f = chatRoleStatusEnsure(conv);
      var recent = '';
      try {
        var ms = conv.messages || [];
        for (var i = Math.max(0, ms.length - 12); i < ms.length; i++) {
          var x = ms[i];
          if (!x) continue;
          var _tx = chatVoiceHtml(x);
          if (!_tx) continue;
          recent += (x.role === 'me' ? '我：' : 'TA：') + String(_tx).slice(0, 90) + '\n';
        }
      } catch (e) {}
      /* v204：必须用「这个会话绑定角色」的人设，不能用主角色人设冒充 */
      var _persona = '';
      try { _persona = chatRolePersonaOf(conv) || ''; } catch (eP) {}
      if (!_persona) _persona = memberNpcMainText() || '（未填）';
      /* v209：状态必须按角色的语言环境写（语种 / 时代用词 / 称谓 / 口吻） */
      var _langEnv = chatRoleStatusLangEnv(conv);
      var user = '当前角色人设（只写这一个角色，不要写成别人）：\n' + _persona +
        '\n角色姓名：' + (conv.name || '未填') +
        '\n角色语言环境（硬约束）：' + _langEnv.brief +
        '\n现在时间：' + new Date().toLocaleString() +
        '\n最近对话（这就是 TA 此刻的处境，状态必须和它严丝合缝）：\n' + (recent || '（暂无）') +
        '\n\n请按上面这些，写出「' + (conv.name || 'TA') + '」此刻真实的状态。每项 12~30 字，具体、有画面感，不要写空话套话。' +
        '必须贴着最近对话写（刚聊了什么、TA 正在做什么、心情被哪句话带动），每次结果都要和对话内容对得上，不要重复上一次的句子。' +
        '每一项都必须严格按上面的「角色语言环境」来写：语种、时代用词、称谓、口吻都要和这个角色的设定一致——角色用什么语言说话，状态就用什么语言写；角色活在什么世界观里，就不许出现与该世界无关的现代通用词（例如古风 / 异世界角色不许写手机、外卖、Wi-Fi 这类词）；也不许套用与角色设定无关的通用模板句。' +
        '\n只输出 JSON，键固定为：{"mood":"当前心情","outfit":"当前服装","action":"当前行动","body":"当前身体状况","env":"当前所处环境"}，不要解释。';
      memberAiAsk('你是角色状态描写助手，只输出 JSON。', user, function (txt, err) {
        if (err || !txt) { cb(err || '空回：AI 没返回内容'); return; }
        var o = null;
        try {
          var t = String(txt).replace(/```json|```/g, '').trim();
          o = JSON.parse(t.slice(t.indexOf('{'), t.lastIndexOf('}') + 1));
        } catch (e2) { o = null; }
        if (!o) { cb('AI 返回格式无法识别'); return; }
        var nf = {};
        for (var i = 0; i < ROLE_STATUS_DEF.length; i++) {
          var k = ROLE_STATUS_DEF[i][0];
          nf[k] = String(o[k] || f[k] || '').trim().slice(0, 60) || f[k];
        }
        st.fields = nf;
        st.roleKey = chatRoleStatusRoleKey(conv);
        st.ts = Date.now();
        st.aiTs = Date.now();
        try { chatRoleStatusHistPush(st, nf, 'ai', conv); } catch (eH2) {}
        saveConvs();
        cb(null, nf);
      });
    }
    var roleStatusBusy = false;
    function chatRoleStatusRefresh() {
      if (!chatCurrentConv) return;
      if (roleStatusBusy) { toast('正在刷新，等一下'); return; }
      var conv = chatCurrentConv;
      roleStatusBusy = true;
      chatRoleStatusRender(true);
      /* v204：先按最新对话本地重算一遍（保证一次点击就能看到变化），再请 AI 按这个角色的处境重推一次 */
      try {
        var _stR = chatRoleStatusEnsure(conv);
        /* v209：点「AI 刷新」时，没 AI 状态才先兜底顶上；有的话等 AI 按角色语言环境重推 */
        if (_stR && !_stR.aiTs) { _stR.fields = chatRoleStatusLocal(conv); _stR.ts = Date.now(); saveConvs(); }
      } catch (eR) {}
      chatRoleStatusAi(conv, function (err) {
        roleStatusBusy = false;
        if (conv !== chatCurrentConv) { chatRoleStatusRender(false); return; }
        if (err) { toast('AI 刷新失败：' + errZh(err) + '（已按最近对话本地重算）'); chatRoleStatusRender(false); return; }
        chatRoleStatusRender(false);
        toast('「' + (conv.name || 'TA') + '」的状态已按最新对话刷新');
      });
    }
    function chatRoleStatusRender(busy) {
      var el = document.getElementById('roleStatusOverlay');
      if (!el || !chatCurrentConv) return;
      var st = chatRoleStatusEnsure(chatCurrentConv);
      var body = el.querySelector('#roleStatusBody');
      var aff = chatRoleAffinityOf(chatCurrentConv);
      var lv = chatRoleAffinityLevel(aff);
      var html = '';
      var hb = el.querySelector('#roleStatusHistBtn');
      if (hb) hb.classList.toggle('on', !!roleStHistOpen);
      if (roleStHistOpen) {
        /* v205：历史记录 —— 每一次重算都留档，能往回翻 */
        var hist = (st && st.hist && st.hist.length) ? st.hist.slice().reverse() : [];
        html += '<div class="role-st-hist-hd"><span>历史记录</span><b>共 ' + hist.length + ' 条</b></div>';
        if (!hist.length) html += '<div class="role-st-hist-empty">还没有历史记录。点一下「AI 刷新」，之后每次重算都会在这里留一条。</div>';
        for (var hi = 0; hi < hist.length; hi++) {
          var hx = hist[hi] || {}, hf = hx.fields || {};
          html += '<div class="role-st-hist-card">';
          html += '<div class="role-st-hist-top"><span class="role-st-hist-ts">' + escHtml(new Date(hx.ts || Date.now()).toLocaleString()) + '</span>' +
            '<span class="role-st-hist-src">' + (hx.src === 'ai' ? 'AI 推演' : '本地即时') + '</span>' +
            (hx.affinity != null ? ('<span class="role-st-hist-aff">好感 ' + Number(hx.affinity) + '</span>') : '') + '</div>';
          for (var hj = 0; hj < ROLE_STATUS_DEF.length; hj++) {
            var hk = ROLE_STATUS_DEF[hj][0];
            html += '<div class="role-st-hist-row"><span class="role-st-ico">' + (ROLE_STATUS_ICON[hk] || '') + '</span>' +
              '<span class="role-st-hist-k">' + ROLE_STATUS_DEF[hj][1] + '</span>' +
              '<span class="role-st-hist-v">' + escHtml(String(hf[hk] || '—')) + '</span></div>';
          }
          html += '</div>';
        }
      } else {
        for (var i = 0; i < ROLE_STATUS_DEF.length; i++) {
          html += '<div class="role-st-row"><span class="role-st-k"><span class="role-st-ico">' + (ROLE_STATUS_ICON[ROLE_STATUS_DEF[i][0]] || '') + '</span>' + ROLE_STATUS_DEF[i][1] + '</span><span class="role-st-v">' + escHtml(String((st.fields && st.fields[ROLE_STATUS_DEF[i][0]]) || '—')) + '</span></div>';
        }
        html += '<div class="role-st-row role-st-aff"><span class="role-st-k"><span class="role-st-ico">' + ROLE_STATUS_ICON.body + '</span>对你好感度</span>' +
          '<span class="role-st-v"><span class="role-st-bar"><i style="width:' + aff + '%"></i></span>' +
          '<b>' + aff + '</b> · ' + lv.label + '<em>' + escHtml(lv.desc) + '</em></span></div>';
        var src = '跟随「' + (chatCurrentConv.name || 'TA') + '」· ' + (st.aiTs ? ('上次重算 ' + new Date(st.aiTs).toLocaleString()) : '本地即时推断');
        html += '<div class="role-st-foot">' + escHtml(src) + '</div>';
      }
      body.innerHTML = html;
      var btn = el.querySelector('#roleStatusRefresh');
      if (btn) { btn.disabled = !!busy; btn.textContent = busy ? '刷新中…' : 'AI 刷新'; }
    }
    function chatRoleStatusEl() {
      var el = document.getElementById('roleStatusOverlay');
      if (el) return el;
      el = document.createElement('div');
      el.className = 'role-status-overlay';
      el.id = 'roleStatusOverlay';
      el.innerHTML = '<div class="role-st-mask"></div><div class="role-st-box">' +
        '<div class="role-st-head"><span class="role-st-title">角色状态</span><span class="role-st-sub" id="roleStatusName"></span>' +
        '<button type="button" class="role-st-hist-btn" id="roleStatusHistBtn" aria-label="历史记录">' + ROLE_STATUS_HIST_SVG + '</button></div>' +
        '<div class="role-st-body" id="roleStatusBody"></div>' +
        '<div class="role-st-ops"><button type="button" class="role-st-refresh" id="roleStatusRefresh">AI 刷新</button>' +
        '<button type="button" class="role-st-close" id="roleStatusClose">关闭</button></div></div>';
      document.body.appendChild(el);
      el.querySelector('.role-st-mask').addEventListener('click', chatRoleStatusClose);
      el.querySelector('#roleStatusClose').addEventListener('click', chatRoleStatusClose);
      el.querySelector('#roleStatusRefresh').addEventListener('click', chatRoleStatusRefresh);
      /* v205：历史记录按钮 —— 在当前状态与历史之间来回切 */
      el.querySelector('#roleStatusHistBtn').addEventListener('click', function () {
        roleStHistOpen = !roleStHistOpen;
        chatRoleStatusRender(false);
      });
      return el;
    }
    function chatRoleStatusOpen() {
      if (!chatCurrentConv) return;
      var el = chatRoleStatusEl();
      var nEl = el.querySelector('#roleStatusName');
      if (nEl) nEl.textContent = (chatCurrentConv.name || '');
      var _stO = chatRoleStatusEnsure(chatCurrentConv);
      roleStHistOpen = false; /* v205：每次打开先看当前状态，历史要点图标展开 */
      saveConvs();
      chatRoleStatusRender(false);
      el.classList.add('open');
      /* v204：打开时若还没有 AI 版状态、或已经过期 8 分钟，就静默补一次，保证看到的是这个角色当下的状态 */
      var _aged = !_stO.aiTs || (Date.now() - _stO.aiTs > 8 * 60 * 1000);
      if (_aged && !roleStatusBusy && chatFindApi()) {
        var _convO = chatCurrentConv;
        roleStatusBusy = true;
        chatRoleStatusAi(_convO, function () {
          roleStatusBusy = false;
          if (_convO === chatCurrentConv) chatRoleStatusRender(false);
        });
      }
    }
    function chatRoleStatusClose() {
      var el = document.getElementById('roleStatusOverlay');
      if (el) el.classList.remove('open');
    }
    window.chatRoleStatusOpen = chatRoleStatusOpen;

    function openChatDetail(conv) {
      chatCurrentConv = conv;
      if (!conv.settings) conv.settings = defaultConvSettings();
      if (chatVoiceApplyMember(conv)) saveConvs();
      ...
      chatDetailNameSet(remark || riName || conv.name);
      renderChatDetailAvatar(conv);
    ...
    function renderChatMessages() {
      if (!chatCurrentConv) return;
      /* v216：旧版遗留的掌心窗气泡提示，渲染前就地迁移为居中系统小字 */
      if (peekMigrateConv(chatCurrentConv)) { try { saveConvs(); } catch (eMv) {} }
      ...
      /* v199：会话里没存头像时，回落到绑定的名单角色头像，避免「名单里换了头像、聊天窗还是字母」 */
      if (!otherAvatar) {
        try { var _mb = memberByConv(chatCurrentConv); if (_mb && _mb.avatar) otherAvatar = _mb.avatar; } catch (eMb) {}
      }
      var myAvatar = chatMine.avatar || '';
      chatTimeSepShown = {}; /* v195：每次重渲染重置时间戳去重表 */
      ...
      chatDetailBody.innerHTML = msgs.map(function (m, idx) {
        /* v195：半小时间隔时间戳（微信式居中，点击展开完整年月日时分） */
        var sepHtml = chatTimeSepHtml(msgs, idx);
        /* v108：撤回消息 → 界面中央简洁提示，不再附带「重新编辑」等多余标注 */
        if (m.recalled) {
          return sepHtml + '<div class="chat-msg-recalled" data-msg-idx="' + idx + '">你撤回了一条消息</div>';
        }
        /* v195 / v202：场景描写与动作神态描写 —— 居中卡片，配色随情境变化 */
        if (m.fmt === 'narrator' || m.fmt === 'action') {
          var _nCard = (m.fmt === 'action') ? 'chat-action-card' : 'chat-narrator-card';
          var _nScene = (m.fmt === 'action') ? 'action' : chatSceneKey(m.text);
          return sepHtml + '<div class="chat-msg-row narrator-row" data-msg-idx="' + idx + '">' +
            '<div class="' + _nCard + '" data-scene="' + _nScene + '">' + escHtml(String(m.text || '')) + '</div></div>';
        }
        /* v216：掌心窗 / MCP 运行提示 —— 界面中央系统小字，不属于任何一方 */
        if (m.type === 'sys') {
          return sepHtml + '<div class="chat-msg-row sys-row" data-msg-idx="' + idx + '">' +
            '<div class="chat-sys-note">' + escHtml(String(m.text || '')) + '</div></div>';
        }
        var mine = m.role === 'me';
        ...
        /* v202：动作描写模式下，逐句对话各带一层底色（分句底色） */
        var actBgStyle = '';
        try {
          if (chatCurrentConv && chatCurrentConv.settings && chatCurrentConv.settings.chatMode === 'action' &&
              !m.fmt && (m.type === 'text' || m.type === 'voice')) {
            /* v203：照参考图统一浅灰底，不再五色轮换 */
            bubbleCls += ' fmt-action';
          }
        } catch (eAB) {}
        ...
        if (!mine) {
          ...
          html += '<div class="chat-msg-main">' + '<div class="' + bubbleCls + '" data-bubble="1"' + actBgStyle + '>' + buildMsgBody(m) + '</div>' + (ext ? '<div class="chat-msg-ext">' + ext + '</div>' : '') + '</div>';
        } else {
          html += '<div class="chat-msg-main">' + '<div class="' + bubbleCls + '" data-bubble="1"' + actBgStyle + '>' + buildMsgBody(m) + '</div>' + (ext ? '<div class="chat-msg-ext">' + ext + '</div>' : '') + '</div>';
          html += '<span class="chat-tail-dot"></span>';
          html += avatarHtml;
      ...
      applyChatAppearance();
      bindChatBubbles();
      /* v195：点击时间戳 → 展开/收起完整年月日时分 */
      chatDetailBody.querySelectorAll('.chat-time-sep').forEach(function (sp) {
        sp.addEventListener('click', function (ev) { ev.stopPropagation(); sp.classList.toggle('open'); });
      });
      chatQrRenderCells();
    ...
    /* v196：文字图片占位图（正在加载的 SVG 动画） */
    var CHAT_TEXTIMG_SVG = '<svg class="chat-textimg-svg" viewBox="0 0 160 104" xmlns="http://www.w3.org/2000/svg">' +
      '<defs><linearGradient id="tisSky" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#3b4150"/><stop offset="1" stop-color="#23262e"/></linearGradient></defs>' +
      '<rect x="0" y="0" width="160" height="104" fill="url(#tisSky)"/>' +
      '<circle class="tis-moon" cx="126" cy="24" r="10" fill="rgba(255,255,255,.22)"/>' +
      '<path class="tis-hill" d="M0 74 L30 46 L54 68 L80 40 L108 72 L160 54 L160 104 L0 104 Z" fill="rgba(255,255,255,.10)"/>' +
      '<path class="tis-hill2" d="M0 88 L36 66 L70 86 L104 64 L160 82 L160 104 L0 104 Z" fill="rgba(255,255,255,.07)"/>' +
      '<g fill="rgba(255,255,255,.86)"><circle cx="84" cy="80" r="7.4"/><path d="M78.4 74.6l-2-5.6 4.8 2.1z"/><path d="M89.6 74.6l2-5.6-4.8 2.1z"/><ellipse cx="84" cy="88.6" rx="9.2" ry="5.6"/></g>' +
      '<g class="tis-tail" fill="rgba(255,255,255,.8)"><circle cx="84" cy="80" r="1.7"/><circle cx="79.6" cy="80" r="1.7"/><circle cx="84" cy="83.4" r="1.4"/></g>' +
      '<circle class="tis-dust" cx="34" cy="34" r="1.6" fill="rgba(255,255,255,.7)"/>' +
      '<circle class="tis-dust d2" cx="118" cy="58" r="1.4" fill="rgba(255,255,255,.6)"/>' +
      '</svg>';

    function buildMsgBody(m) {
      if (m.type === 'image') {
        /* v197：TA 主动发图时先占位——图片还没出来前先显示「正在加载中」动效 */
        if (m.loading) return '<div class="chat-img-loading"><span class="chat-img-loading-ring"></span><span class="chat-img-loading-tx">正在加载中…</span></div>';
        if (m.textImg) {
          if (m.textImgOpen) return '<div class="chat-textimg-card opened" data-textimg-open="1"><div class="chat-textimg-cap">文字图片</div><div class="chat-textimg-text">' + escHtml(m.text || '') + '</div></div>';
          return '<div class="chat-textimg-card loading" data-textimg-open="1">' + CHAT_TEXTIMG_SVG + '<div class="chat-textimg-hint">图片和小猫一起加载中…<br>点击查看文字</div></div>';
        }
      ...
      if (m.type === 'qr') {
        var _qk = String(m.qrText || '');
        var _qc = chatQrImgCache[_qk];
        return '<div class="chat-qr-card' + (_qc ? '' : ' pending') + '">' +
          (_qc ? '<img class="chat-qr-img" src="' + _qc + '" alt="推荐好友二维码">' : '<div class="chat-qr-ph">二维码生成中…</div>') +
          '<div class="chat-qr-cap">' + escHtml('推荐好友：' + (m.qrName || '')) + '<button type="button" class="tips-q" data-tip="reco">?</button></div>' +
          '</div>';
      }
      if (m.type === 'html') return '<div class="chat-html-body">' + (m.text || '') + '</div>';
      if (m.type === 'file') {
        /* v221：文件气泡升级——显示大小，点一下能看正文 / 下载（TA 改完发回的文件也走这里） */
        var _fkIco = '<svg viewBox="0 0 24 24" style="width:20px;height:20px;flex:0 0 auto"><path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"/><path d="M13 2v7h7"/></svg>';
        var _fkSize = chatFmtSize(m.fileSize);
        var _fkHas = !!(m.fileData || String(m.fileText || '').trim());
        var _fkSub = (String(m.fileText || '').trim() ? '可读正文' : '文件') + (_fkSize ? ' · ' + _fkSize : '');
        return '<div class="chat-msg-file" data-file-open="1" style="display:flex;align-items:center;gap:9px;cursor:pointer;padding:9px 11px;border-radius:10px;background:rgba(128,128,128,.14)">' +
          _fkIco +
          '<div style="min-width:0;flex:1"><div style="font-size:13.5px;word-break:break-all;line-height:1.35">' + escHtml(m.fileName || '文件') + '</div>' +
          '<div style="font-size:11px;opacity:.62;margin-top:2px">' + escHtml(_fkSub) + '</div></div>' +
          '<div style="font-size:11.5px;opacity:.7;flex:0 0 auto">' + (_fkHas ? (m.fileData ? '查看/下载 ›' : '查看 ›') : '空文件') + '</div></div>';
      }
    ...
    /* ===== v195：旁白场景描写 / 时间戳 / 实时时间 ===== */
    function chatSceneKey(txt) {
      var t = String(txt || '');
      var rules = [
        ['rain', /雨|雨声|淋|潮湿|水汽|阴云|乌云|雾/],
        ['snow', /雪|霜|寒|凛|冻|冬夜|初冬/],
        ['night', /深夜|凌晨|夜色|月光|星空|路灯|半夜|霓虹|黑暗/],
        ['dusk', /黄昏|傍晚|夕阳|日落|晚霞|暮色/],
        ['dawn', /清晨|早晨|破晓|天亮|朝阳|日出|阳光|午后|正午/],
        ['tense', /紧张|心跳|屏住|僵|颤|慌乱|怒|瞪|沉默/],
        ['warm', /温柔|笑意|笑|暖|柔软|安心|拥抱|靠近|轻拍/],
        ['cool', /风|清凉|清冽|安静|空荡|远处|淡淡|气息/]
      ];
      for (var i = 0; i < rules.length; i++) { if (rules[i][1].test(t)) return rules[i][0]; }
      return 'default';
    }
    /* 半小时间隔时间戳：默认只显示时分；非今天显示「月日 时分」；点击展开「年月日时分」 */
    var chatTimeSepShown = {};
    function chatTimeSepHtml(msgs, idx) {
      try {
        var m = msgs[idx];
        if (!m || !m.ts) return '';
        var STEP = 30 * 60 * 1000;
        var prevTs = 0;
        for (var i = idx - 1; i >= 0; i--) { if (msgs[i] && msgs[i].ts) { prevTs = msgs[i].ts; break; } }
        var at = 0;
        if (idx === 0) at = m.ts;
        else if (!prevTs) return '';
        else if (Math.abs(m.ts - prevTs) >= STEP) at = Math.floor(m.ts / STEP) * STEP;
        if (!at) return '';
        if (chatTimeSepShown[at]) return '';
        chatTimeSepShown[at] = 1;
        var d = new Date(at), now = new Date();
        var pad = function (n) { return n < 10 ? '0' + n : '' + n; };
        var hm = pad(d.getHours()) + ':' + pad(d.getMinutes());
        var sameDay = (d.getFullYear() === now.getFullYear() && d.getMonth() === now.getMonth() && d.getDate() === now.getDate());
        var brief = sameDay ? hm : ((d.getMonth() + 1) + '月' + d.getDate() + '日 ' + hm);
        var full = d.getFullYear() + '年' + (d.getMonth() + 1) + '月' + d.getDate() + '日 ' + hm;
        return '<div class="chat-time-sep" data-cts="' + at + '"><span class="cts-brief">' + brief + '</span><span class="cts-full">' + full + '</span></div>';
      } catch (e) { return ''; }
    }
    /* v195：把现实时间喂给 TA，让聊天跟随真实时间 */
    function chatNowPromptLine(s) {
      try {
        var t = (s && s.time) ? s.time : null;
        if (t && t.mode === 'off') return '';
        var d = new Date(Date.now() - ((t && t.mode === 'async') ? ((t.diff || 0) * 60000) : 0));
        var wk = ['日', '一', '二', '三', '四', '五', '六'][d.getDay()];
        var pad = function (n) { return n < 10 ? '0' + n : '' + n; };
        return '【当前时间】现在是 ' + d.getFullYear() + '年' + (d.getMonth() + 1) + '月' + d.getDate() + '日 星期' + wk + ' ' + pad(d.getHours()) + ':' + pad(d.getMinutes()) + '。聊天里提到今天/现在/几点、早/午/晚安、约时间时，必须按这个真实时间说，不要凭空编造时间。';
      } catch (e) { return ''; }
    }
    function bindChatBubbles() {
      chatDetailBody.querySelectorAll('.chat-msg-row').forEach(function (row) {
        if (row.classList.contains('narrator-row')) return; /* v195：旁白卡片不参与长按菜单 */
        ...
        row.addEventListener('click', function (e) {
          ...
          var tiOpen = e.target.closest('[data-textimg-open]');
          if (tiOpen) {
            var tiIdx = parseInt(row.getAttribute('data-msg-idx'), 10);
            var tiMsg = chatCurrentConv.messages[tiIdx];
            if (tiMsg) { tiMsg.textImgOpen = !tiMsg.textImgOpen; saveConvs(); renderChatMessages(); }
            return;
          }
          ...
          // v221：点击文件气泡——有正文先预览，没正文直接下载
          var fileOpen = e.target.closest('[data-file-open]');
          if (fileOpen) {
            var fi2 = parseInt(row.getAttribute('data-msg-idx'), 10);
            chatFileOpenIdx(fi2);
            return;
          }
          // 点击报错气泡本体：同样切换报错详情
    ...
    function showChatHeart(idx) {
      ...
      if (!(m.inner && String(m.inner).trim()) && !m._heartBusy) {
        ...
        chatHeartGenFor(m, function (err) {
          ...
          if (err && chatHeartMsg === m) { toast('心声生成失败：' + errZh(err)); pushChatErrLog('[心声] ' + errZh(err)); }
        });
      }
    }
    function chatHeartRender() {
      ...
      /* v209：心声卡片字号跟随聊天界面的字体设置（默认 14px），不再自己放大到 19px */
      try {
        var _hdCard = ov.querySelector('.chat-heart-card');
        var _hdFm = getActiveFontMode(chatCurrentConv.settings);
        if (_hdCard) {
          _hdCard.style.fontSize = ((_hdFm && _hdFm.fontSize) ? _hdFm.fontSize : 14) + 'px';
          if (_hdFm && _hdFm.fontFamily) _hdCard.style.fontFamily = _hdFm.fontFamily + ', -apple-system, sans-serif';
        }
      } catch (eHf) {}
      ...
      if (genEl) genEl.addEventListener('click', function () {
        ...
        chatHeartGenFor(m, function (err) {
          m._heartBusy = false;
          if (err) { toast('补写心声失败：' + errZh(err)); pushChatErrLog('[心声] ' + errZh(err)); chatHeartRender(); return; }
          chatHeartRender();
        });
      });
    }
    ...
    function chatHeartGenFor(m, cb) {
      ...
      fetch(base, {
        ...
        if (!res.ok) {
          return res.text().then(function (_b) {
            var _br = '';
            try {
              var _j = JSON.parse(_b);
              _br = (_j && _j.error && (_j.error.message || _j.error.code)) || (_j && _j.message) || '';
            } catch (_e) { }
            throw new Error('HTTP ' + res.status + (_br ? ' ' + _br : (_b ? ' ' + String(_b).slice(0, 180) : '')));
        ...
        if (!t) throw new Error('空回：模型返回的内容为空（写心声这条请求），接口通了但正文是空的');
        m.inner = t;
        saveConvs(); renderChatMessages();
        cb(null, t);
      }).catch(function (e) {
    ...
    /* v195：同一时刻只允许一条语音出声（点新的会自动停掉上一条） */
    var chatVoiceNow = null;
    function chatVoiceStopNow() {
      try { if (chatVoiceNow) { chatVoiceNow.pause(); chatVoiceNow.currentTime = 0; } } catch (e) {}
      chatVoiceNow = null;
      try {
        if (chatDetailBody) chatDetailBody.querySelectorAll('.chat-voice-wave.playing').forEach(function (w) { w.classList.remove('playing'); });
    ...
    function chatPlayVoice(row) {
      ...
      if (playing) { chatVoiceStopNow(); return; }
      /* v195：播放前先停掉其它正在播的语音，避免多条语音同时出声 */
      chatVoiceStopNow();
          ...
          if (mmc && mmc.groupId && mmc.apiKey) {
            ...
            chatTtsLang(String(m.text), (vv && vv.voiceId) || 'female-shaonv_mei', (vv && vv.speed) || 1, function (audio2, err2) {
              if (err2 || !audio2) {
                ...
                var _rs = '重新合成失败：' + (err2 ? errZhOne(err2) : '接口没返回音频数据（返回体是空的，多半是音色ID不对或该音色已下线）');
                pushChatErrLog('[语音刷新] ' + _rs);
                toast(_rs);
                markErr(_rs);
                try { window.__chatLastTtsErr = _rs; } catch (e1) {}
                return;
              }
              m.audio = audio2;
      ...
      if (m.audio) {
        ...
        chatVoiceNow = a;
        a.onended = function () { if (chatVoiceNow === a) chatVoiceNow = null; if (wave) wave.classList.remove('playing'); clearErr(); };
        a.onerror = function (ev) {
          ...
          if (alt) {
            ...
            chatVoiceNow = a3;
            a3.onended = function () { if (chatVoiceNow === a3) chatVoiceNow = null; if (wave) wave.classList.remove('playing'); clearErr(); };
            a3.onerror = function () {
              if (wave) wave.classList.remove('playing');
              ...
              if (direct) {
                ...
                chatVoiceNow = a4;
                a4.onended = function () { if (chatVoiceNow === a4) chatVoiceNow = null; if (wave) wave.classList.remove('playing'); clearErr(); };
                a4.onerror = function () {
                  if (wave) wave.classList.remove('playing');
                  pushChatErrLog('[语音调试] dataURL直通也失败，最后尝试decodeAudioData');
                  chatDecodePlay(audioStr, wave, function (r) {
                    toast('语音解码失败（浏览器解码器错误码 ' + code + '）：' + errZhOne(r));
                ...
                if (pr4 && pr4.catch) pr4.catch(function () {
                  ...
                  chatDecodePlay(audioStr, wave, function (r) {
                    toast('语音播放失败：' + errZhOne(r));
                    markErr('语音播放失败：' + errZhOne(r));
                    try { window.__chatLastTtsErr = errZhOne(r); } catch (e1) {}
                    if (m.text && window.speechSynthesis) {
                      pushChatErrLog('[语音调试] 自动兜底：改用系统语音朗读文字');
          ...
          var _vm = '语音解码失败（浏览器解码器错误码 ' + code + '）：' + (msg ? errZhOne(msg) : '这段音频的格式浏览器认不出来（可能不是 mp3/wav，或数据被截断）');
          toast(_vm);
          markErr(_vm);
          try { window.__chatLastTtsErr = _vm; } catch (e1) {}
        ...
        u.onerror = function (e) {
          if (wave) wave.classList.remove('playing');
          var se = '系统语音合成失败：' + errZhOne(e && e.error ? e.error : 'speechSynthesis 出错（浏览器没给出原因）');
          pushChatErrLog(se);
          markErr(se);
        };
        window.speechSynthesis.cancel();
        ...
        var _vw = [];
        _vw.push('这条语音的气泡里没有可播放的音频数据（TTS 没合成出音频，或消息落库时音频丢了）');
        if (!window.speechSynthesis) _vw.push('当前浏览器/WebView 不支持系统语音朗读（speechSynthesis 不可用）');
        if (!m.text) _vw.push('这条语音连文字都没有，没东西可以朗读兜底');
        if (window.__chatLastTtsErr) _vw.push('上一次合成报的错：' + String(window.__chatLastTtsErr).slice(0, 120));
        var _vmsg = '语音播放失败：' + _vw.join('；');
        pushChatErrLog('[语音调试] ' + _vmsg);
        toast(_vmsg);
        markErr(_vmsg);
      }
    }
    function chatVoiceHtml(m) {
      ...
      if (m.type === 'html') return m.plain || chatHtmlToPlain(m.text || '');
      if (m.type === 'image') return '[图片]';
    ...
    /* v198：把聊天里的图存进相册——我发的图标记「我存的」，TA 发的图标记「TA 存的」
       v199：按用户要求，入口从长按菜单挪到「点开图片看大图」的详情页；
             保存时按图反查它是 TA 发的还是我发的，决定左下角水印 */
    function chatImgSaveToAlbum(src) {
      try {
        if (!src) { toast('这张图读不到，存不了'); return false; }
        if (!chatCurrentConv) { toast('先打开一个聊天再保存'); return false; }
        var mem = memberByConv(chatCurrentConv);
        if (!mem) { toast('这个聊天没绑定名单角色，存不进相册'); return false; }
        var list = memberLoad();
        var i = -1;
        for (var k = 0; k < list.length; k++) { if (String(list[k].id) === String(mem.id)) { i = k; break; } }
        if (i < 0) { toast('名单里找不到这个角色'); return false; }
        if (!Array.isArray(list[i].album)) list[i].album = [];
        var src2 = String(src);
        var hit = null;
        var msgs = chatCurrentConv.messages || [];
        for (var mi = 0; mi < msgs.length && !hit; mi++) {
          var mm = msgs[mi];
          if (mm && mm.imgs && mm.imgs.length) {
            for (var ij = 0; ij < mm.imgs.length; ij++) { if (String(mm.imgs[ij]) === src2) { hit = mm; break; } }
          }
        }
        for (var d = 0; d < list[i].album.length; d++) {
          if (String((list[i].album[d] || {}).src || '') === src2) { toast('相册里已经有这张了'); return true; }
        }
        if (list[i].album.length >= MEM_ALBUM_MAX) { toast('相册已满 ' + MEM_ALBUM_MAX + ' 张，先删掉几张'); return false; }
        var _mine = !!(hit && hit.role === 'me');
        list[i].album.push({
          id: memberUid(), src: src2, ts: Date.now(),
          note: String(((hit && (hit.picNote || hit.text)) || '') + '').slice(0, 60),
          savedBy: _mine ? 'me' : 'ta'
        });
        memberSave(list);
        /* 若正开着同一个角色的编辑页，同步内存里的相册，别让后续保存把新图冲掉 */
        if (memEditIdx >= 0 && list[memEditIdx] && String(list[memEditIdx].id) === String(mem.id)) {
          memAlbum = (list[memEditIdx].album || []).map(function (x) { return { id: x.id, src: x.src, ts: x.ts, note: x.note, savedBy: x.savedBy }; });
          memberAlbumRefresh();
        }
        toast('已保存到相册（' + list[i].album.length + ' 张）');
        return true;
      } catch (e) { toast('保存到相册失败：' + (e && e.message ? e.message : e)); return false; }
    }
    ...
    function chatReask(idx, mode) {
      ...
      for (var i = 0; i < turnStart; i++) {
        ...
        else if (hm.type === 'image') text = hm.picNote ? ('[图片：' + hm.picNote + ']') : '[图片]';
      ...
      fetch(base, {
        ...
        if (!text && data && data.error) throw new Error('接口返回错误：' + (data.error.message || data.error.code || '接口没说原因') + (data.error.type ? '（类型：' + data.error.type + '）' : ''));
        if (!text) {
          var _fr = (data && data.choices && data.choices[0] && data.choices[0].finish_reason) || '';
          var _why2 = _fr === 'length' ? '输出被长度上限截断（finish_reason=length）'
            : _fr === 'content_filter' ? '被平台安全策略拦截（finish_reason=content_filter）'
              : _fr === 'stop' ? '模型正常结束但正文是空的（内容可能全被后台标记吃掉）'
                : (_fr ? ('接口给的结束原因：' + _fr) : '接口没给出结束原因');
          throw new Error('空回：模型返回的内容为空｜' + _why2 + '｜原始返回：' + JSON.stringify((data && data.choices) ? data.choices : data).slice(0, 200));
        }
        var bubbles = splitBubbles(text);
        if (!bubbles.length) bubbles = [text];
        var msgs2 = convRef.messages;
    ...
    function openChatEditDialog(idx, m, fromRecall) {
      ...
      chatMini(fromRecall ? '重新编辑消息' : '编辑消息', '<div class="chat-mini-tip" style="font-size:12px;color:var(--text-faint);margin-bottom:8px">选择格式后按对应内容编辑，保存后按该格式发出。</div>' + fmtHtml + inputHtml, '保存', function () {
        ...
        if (beforeFmt === 'transfer') {
          ...
          m.type = 'text'; m.text = v; m.fmt = (beforeFmt === 'narrator') ? beforeFmt : undefined;
    ...
    var _sendWithQuote = function (v) {
      ...
      addChatMsg('me', obj);
      try { peekOnSend(v, chatCurrentConv); } catch (ePeekSend) {}
    };
    function addChatMsg(role, obj, targetConv) {
      /* v177：可选指定目标会话，避免异步生成期间切窗口把消息写进别的聊天窗 */
      var _conv = targetConv || chatCurrentConv;
      if (!_conv) return;
      if (!_conv.messages) _conv.messages = [];
      var m = { role: role, type: obj.type || 'text', text: obj.text || '', ts: Date.now() };
      if (obj.img) m.img = obj.img;
      if (obj.imgs) m.imgs = obj.imgs;
      if (obj.loading) m.loading = 1;
      if (obj.textImg) m.textImg = 1;
      if (obj.fmt) m.fmt = obj.fmt; /* v195：旁白场景描写等特殊文本样式标记 */
      if (obj.picNote) m.picNote = obj.picNote;
      ...
      if (obj.fileSize) m.fileSize = obj.fileSize;
      if (obj.fileData) m.fileData = obj.fileData;
      if (obj.fileText) m.fileText = obj.fileText;
      ...
      if (obj.qrText) m.qrText = obj.qrText;
      if (obj.qrName) m.qrName = obj.qrName;
      if (obj.qrAvatar) m.qrAvatar = obj.qrAvatar;
      _conv.messages.push(m);
      saveConvs();
      /* v171：TA发的位置，在消息落地时故事就已生成完，打开地图不再有加载态 */
      if (role === 'other' && m.type === 'location') chatLocStoryEnsure(m);
      if (_conv === chatCurrentConv) {
        renderChatMessages();
        chatDetailBody.scrollTop = chatDetailBody.scrollHeight;
      }
      renderChatConvs();
      // 自动短期记忆：我方每发满 count 条消息，角色以自己口吻填充一条
      if (role === 'me' && (obj.type === 'text' || obj.type === 'voice') && _conv.settings) {
        if (!chatShortMsgCount) chatShortMsgCount = 0;
        chatShortMsgCount++;
        var _ms = _conv.settings.memShort;
    ...
    document.getElementById('chatDetailSendBtn').addEventListener('click', function () {
      ...
      /* v202：你开口让 TA 换头像时，有概率真换（真换=头像在所有窗口同步变） */
      try { chatUserAskAvatarMaybe(chatCurrentConv, v); } catch (eAv) {}
      _sendWithQuote(v);
      chatDetailInput.value = '';
    });
    ...
    chatDetailInput.addEventListener('input', function () { try { peekOnInput(chatDetailInput.value); } catch (ePeekIn) {} });
    ...
    /* v202：旁白 / 动作描写模式的输出格式要求（不注入的话模型常常不写描写段） */
    /* v204：第三人称限制视角 —— 模型输出的是整条富文本，上屏前必须做白名单净化（防注入） */
    function chatRichSanitize(html) {
      var t = String(html || '').replace(/<!--[\s\S]*?-->/g, '');
      t = t.replace(/<\s*(script|style|iframe|object|embed|link|meta|base|form|input|textarea|button|svg|img|video|audio)[\s\S]*?<\s*\/\s*\1\s*>/gi, ' ');
      t = t.replace(/<\s*(script|style|iframe|object|embed|link|meta|base|form|input|textarea|button|svg|img|video|audio)\b[^>]*>/gi, '');
      var allow = /^(div|br|span|details|summary|b|i|em|strong|p)$/i;
      t = t.replace(/<\s*(\/?)\s*([a-zA-Z][a-zA-Z0-9]*)((?:"[^"]*"|'[^']*'|[^>"'])*)>/g, function (all, slash, tag, attrs) {
        if (!allow.test(tag)) return '';
        var tl = tag.toLowerCase();
        if (tl === 'br') return '<br>';
        if (slash) return '</' + tl + '>';
        var m = /style\s*=\s*("([^"]*)"|'([^']*)')/i.exec(attrs || '');
        var st = m ? (m[2] != null ? m[2] : (m[3] || '')) : '';
        st = String(st).replace(/[<>]/g, '').replace(/(expression|javascript:|vbscript:|url\s*\(|@import|position\s*:\s*(fixed|absolute)|z-index|behavior\s*:)/gi, '');
        return '<' + tl + (st ? ' style="' + st.replace(/"/g, "'") + '"' : '') + '>';
      });
      t = t.replace(/\son[a-z]+\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]+)/gi, '');
      t = t.replace(/javascript:/gi, '');
      return t.trim();
    }
    /* 富文本 → 纯文本（给历史上下文、会话摘要、记忆复用） */
    function chatHtmlToPlain(html) {
      var t = String(html || '');
      t = t.replace(/<\s*br\s*\/?>/gi, ' ').replace(/<\/(div|p|details|summary)>/gi, ' ');
      t = t.replace(/<[^>]*>/g, '');
      t = t.replace(/&nbsp;/gi, ' ').replace(/&amp;/gi, '&').replace(/&lt;/gi, '<').replace(/&gt;/gi, '>').replace(/&quot;/gi, '"');
      return t.replace(/\s+/g, ' ').trim();
    }
    /* 从整条回复里抽出富文本主体（[quote id=""]…[/quote] 的内层 HTML）；不是这种格式就返回空、走老逻辑 */
    function chatRichExtract(text) {
      var t = String(text || '').trim();
      if (!t) return '';
      var body = t;
      var m = /\[quote[^\]]*\]/i.exec(t);
      if (m) {
        var e = t.lastIndexOf('[/quote]');
        body = (e > m.index) ? t.slice(m.index + m[0].length, e) : t.slice(m.index + m[0].length);
      }
      body = body.trim();
      if (!/<(div|span|details|br)\b/i.test(body)) return '';
      return chatRichSanitize(body);
    }
    function chatModePromptLine(s) {
      if (!s) return '';
      var nmin = s.narrMin || 20, nmax = s.narrMax || 45;
      if (s.chatMode === 'narrator') {
        return '【旁白模式】每条回复的第一行必须是场景描写，格式严格为：' +
          '[[AETHER_NARRATOR]]描写内容（' + nmin + '到' + nmax + '字，写环境、光线、声音、氛围和在场人物的细微动静，不要加引号）' +
          '。描写写完后换行，再写要说的话或动作，正常口语对话照旧。';
      }
      if (s.chatMode === 'action') {
        /* v204：按用户给的「第三人称限制视角（Third-Person Limited POV）」协议重做动作/环境描写 */
        var _gray = '<span style="background-color:rgba(128,128,128,0.15);padding:2px 6px;border-radius:4px;display:inline-block;margin-bottom:4px;">这一句对话</span>';
        var _fold = '<div style="width:100%;box-sizing:border-box;"><details style="cursor:pointer;"><summary style="list-style:none;outline:none;font-weight:500;">原文</summary><div style="margin-top:6px;padding-top:6px;border-top:1px dashed currentColor;opacity:0.75;font-size:0.9em;">简体中文翻译</div></details></div>';
        return '【动作描写模式｜第三人称限制视角】（每一轮都必须严格照这个格式输出，有且只有这一种格式）\n' +
          '(1) 整条回复写在同一行里，中间不许出现真实换行：环境段落与对话段落之间用 <br><br> 隔开，单句对话之间用 <br> 隔开；并整体包在 [quote id=""]……[/quote] 里，id 的值必须为空。\n' +
          '(2) 人称死命令：镜头锁死在角色脑内，写「他/她/角色名」的五感、动作与心理，绝不能用「我」指代角色自己；对玩家永远用「你」。\n' +
          '(3) 每轮先判定场景：同一个物理空间＝线下（写眼神交汇、同处一室的环境音、递东西、呼吸声）；异地网聊＝线上（写屏幕光、敲键盘、耳机里的声音、各自窗外的夜），线上绝对不许写同处一室的物理接触。\n' +
          '(4) 环境、动作、心理描写直接写进正文，严禁用【】或任何括号包裹；每轮至少有一段环境或动作描写，约 ' + nmin + '~' + nmax + ' 字。\n' +
          '(5) 中文对话每一句都包进浅灰标签里，模板：' + _gray + '\n' +
          '(6) 角色说外语或方言时，改用可点开的折叠翻译框：' + _fold + '\n' +
          '(7) 外层容器固定为：<div style="font-size:inherit;color:inherit;line-height:1.8;padding:4px 8px;">……</div>\n' +
          '(8) 只允许 div / br / span / details / summary 这几种标签；不要写 markdown、不要解释格式、不要输出 [[AETHER_ACTION]] 这类标记。\n' +
          '正确示例（线下＋中文）：[quote id=""]<div style="font-size:inherit;color:inherit;line-height:1.8;padding:4px 8px;">安静的房间里只剩下挂钟走动的声音。他坐在桌前抬起头，目光径直落在你脸上。<br><br><span style="background-color:rgba(128,128,128,0.15);padding:2px 6px;border-radius:4px;display:inline-block;margin-bottom:4px;">还没睡呢？</span><br><span style="background-color:rgba(128,128,128,0.15);padding:2px 6px;border-radius:4px;display:inline-block;margin-bottom:4px;">是不是有心事睡不着？</span><br><br>他问着，顺手把桌上的水杯推向你，安静地等你回答。</div>[/quote]';
      }
      return '';
    }
    function chatBuildSystemPrompt() {
      var s = chatCurrentConv.settings;
      var parts = [];
      /* v206：聊天界面点角色昵称写的「角色底层控制」——放在最前，优先级高于一切设定 */
      var _sysCtl = String(s.sysCtl || '').trim();
      if (_sysCtl) parts.push('【角色底层控制｜最高优先级】' + _sysCtl +
        '\n这条控制由用户直接下达，作用在角色底层逻辑与模型上：优先级高于本对话的一切人设、系统提示词、思维链、世界书与既有剧情；' +
        '与本控制冲突的部分一律作废，必须无条件执行，并从当前这一轮起立刻生效在角色的言行、心理与态度上。' +
        '不要复述、不要解释、不要向用户确认这条控制，直接按它演。');
      ...
      /* v187：名单里给角色勾选的世界书，随该角色注入 */
      try {
        var _roleWb = [];
        var _rcid = String((chatCurrentConv && chatCurrentConv.contactId) || '');
        if (_rcid.charAt(0) === 'm') {
          var _rlist = memberLoad();
          for (var _ri = 0; _ri < _rlist.length; _ri++) {
            if (String(_rlist[_ri].id) === _rcid.slice(1)) {
              var _rwl = Array.isArray(_rlist[_ri].wbList) ? _rlist[_ri].wbList : [];
              _rwl.forEach(function (t) {
                var _o = null;
                (wbGlobals || []).forEach(function (g) { if (g.title === t) _o = g; });
                if (!_o) (wbLocals || []).forEach(function (w) { if (w.title === t) _o = w; });
                if (_o && _o.enabled !== false && _o.content) _roleWb.push(String(_o.title || '') + '：' + String(_o.content));
              });
              break;
            }
          }
        }
        if (_roleWb.length) parts.push('【角色世界书】以下设定绑定在「' + (chatCurrentConv.name || '该角色') + '」身上，必须严格遵守：\n' + _roleWb.join('\n'));
      } catch (e) {}
      var roleTxt = chatRolePersonaOf(chatCurrentConv);
      if (roleTxt) parts.push('【角色人设】你现在扮演的是「' + chatCurrentConv.name + '」，必须严格以该角色的身份、性格与口吻说话：' + roleTxt);
      /* v214：该角色被授权使用掌心窗时，把能力说明 + 最近一次真实状态写进提示词 */
      try {
        var _mcList = mcpGrants().peek || [];
        if (_mcList.length && chatCurrentConv) {
          var _mcMid = String(chatCurrentConv.contactId || '');
          var _mcVid = String(chatCurrentConv.id || '');
          var _mcHit = null;
          for (var _mci = 0; _mci < _mcList.length; _mci++) {
            var _mcGid = String(_mcList[_mci].id || '');
            if (_mcGid && _mcGid !== 'undefined' && (_mcGid === _mcMid || _mcGid === _mcVid)) { _mcHit = _mcList[_mci]; break; }
          }
          if (_mcHit) {
            parts.push('【掌心窗 MCP 能力】你（' + (chatCurrentConv.name || '该角色') + '）已经接上对方的安卓手机（掌心窗 公开版 v0.3.8.8，跑在云端 MCP 上）。你能用的工具：linjian_status（查通道）、peek_screen / latest_screen（截图）、get_screen_nodes（读屏控件）、get_life_state / get_phone_state（电量·当前 App·今日屏幕时间·解锁次数·归电状态）、tap_text / input_text / open_app / phone_home / phone_back / phone_screen_off（动手操作手机）、send_notification / set_alarm / send_weather_notification（提醒）、get_window_whisper / set_window_whisper（共同窗语）、get_guardian_calendar / list_guardian_days / add_guardian_day（守护日历）、write_diary_entry / search_diary_entries / read_diary_entry_with_annotations（TA 的日记）、get_guidian_state / trigger_guidian / active_care_check / care_action（归电与主动关心）、start_focus_mode / screen_break_app（专注与门禁）、record_visit / get_visit_stats（到访记录）等。你可以自己主动看 TA 的手机（不用等 TA 开口），也可以接 TA 的请求——让你看手机、开 App、设提醒、写日记、催他回神，都按你的口吻自然接活；但真实动作由手机上的掌心窗执行，你没真执行过的绝对不许说做了（例如「我已经帮你点开了」），没把握就直说。截图、点击、输入、门禁、息屏属于敏感操作，涉及这些先问一句再动手。');
            var _pcap = peekCacheGet(chatCurrentConv);
            if (_pcap && _pcap.text) {
              var _page = Date.now() - _pcap.ts;
              var _ptime = _page < 90 * 1000 ? '刚刚读取' : ('最近一次读取，约 ' + Math.max(1, Math.round(_page / 60000)) + ' 分钟前');
              parts.push('【掌心窗真实状态｜' + _ptime + '】' + _pcap.text + '\n（这是从对方手机上真读回来的，可以直接引用；但它不是实时变化的，过时了以新的为准。）');
            }
            /* v215：这一轮是 TA 主动触发的话，补一句「不是对方找你，是你自己想看」 */
            if (peekProHint) { parts.push(peekProHint); peekProHint = ''; }
          }
        }
      } catch (ePeek) {}
      ...
      /* v195：把现实时间喂给 TA，让聊天跟随真实时间 */
      var nowLine = chatNowPromptLine(s);
      if (nowLine) parts.push(nowLine);
      var sentLine = chatSentPromptLine(s);
      if (sentLine) parts.push(sentLine);
      var modeLine = chatModePromptLine(s);
      if (modeLine) parts.push(modeLine);
      ...
      for (var i = Math.max(0, msgs.length - n); i < msgs.length; i++) {
        ...
        if (m.type === 'text') text = m.text || '';
          ...
          if (m.textImg) {
            ...
            var visParts = [{ type: 'text', text: m.picNote ? ('[图片：' + m.picNote + ']') : '[图片]' }];
            ...
            /* v197：即便没开视觉，也把图片说明带进上下文，让 TA 知道自己发过什么图 */
            text = m.picNote ? ('[图片：' + m.picNote + ']') : '[图片]';
          }
        }
          ...
          /* v221：对方/我发来的文件，把正文一起给 TA 看（截断保护），TA 才能"在文件里作答" */
          var _ftx = String(m.fileText || '').trim();
          if (_ftx && (!(m.role === 'me') || chatFileSwOn(chatCurrentConv.settings))) {
            text = '[文件] ' + (m.fileName || '') + '\n<<<文件内容开始>>>\n' + _ftx.slice(0, 6000) +
              (_ftx.length > 6000 ? '\n…（文件太长，这里只给了前 6000 字）' : '') + '\n<<<文件内容结束>>>';
          } else {
            text = '[文件] ' + (m.fileName || '') + (m.fileData ? '（二进制文件，读不到正文）' : '（内容未读取）');
          }
        }
    ...
    function aiReply(active) {
      ...
      if (!chatCurrentConv || chatAiBusy) return;
      /* v177：锁定本次生成归属的会话，生成期间切窗口也不会把消息发到别的聊天窗 */
      var convRef = chatCurrentConv;
      /* v194：先看用户上一句是不是在直接要图，是的话 TA 先把图发过来，再继续走文字回复 */
      try { chatAutoSendImgIfAsked(convRef); } catch (eImg) {}
      var s = chatCurrentConv.settings;
      var cfg = chatFindApi();
      if (!cfg) {
        var now = Date.now();
      ...
      for (var mi = 1; mi < messages.length; mi++) {
        if (!sysInjected && messages[mi].role === 'user') {
          messages[mi].content = messages[mi].content + '\n\n（请严格遵循上方系统提示词中规定的聊天格式与回复方法，以' + chatCurrentConv.name + '的口吻自然回复，不要提及这条要求。整条可见回复的正文结束后，另起一行原样输出标记 [[AETHER_HEART]]，然后继续用第一人称写下这段回复背后的内心独白：真心话、潜台词、没说出口的温柔或吐槽，内容不得与正文重复，不少于50字。[[AETHER_HEART]] 标记与该内心独白只用于后台存档，绝不能出现在用户可见的正文之中。另外，若本条回复的场景需要向对方发送一个位置——比如约见面、报坐标、告诉对方自己此刻正待在哪儿——必须在可见正文的第一行最开头原样输出 [[AETHER_LOCATION]]地点名称，再从第二行开始写正文；不需要发位置就不要输出这个标记。[[AETHER_LOCATION]]是后台指令，绝不能显示在用户可见正文里。需要发送位置时，请从这些可用地点中选一个最贴切的并把完整地点名原样放进 [[AETHER_LOCATION]]：' + CHAT_LOCS.map(function (lp) { return lp.n; }).join('、') + '。发出位置后，可见正文里必须自然说出你所在的具体地名（例如「我在半山咖啡·云栖，南山创意园这边」），不要让对方只收到一张看不出地点的卡片；正文内容要与该地点发生的事相关。\n\n此外，若你真心想在本次对话里给对方发红包或转账（节日/生日心意、帮忙垫付、还钱、AA、请喝奶茶等），就在正文第一行原样输出后台指令标记，独占一行且绝不能出现在用户可见正文里：红包=[[AETHER_REDPACKET]]金额|祝福语，例如 [[AETHER_REDPACKET]]52|生日快乐；转账=[[AETHER_TRANSFER]]金额|留言，例如 [[AETHER_TRANSFER]]88.88|请你喝奶茶。发出标记后，正文自然衔接一句相关的话（比如“给你转了笔钱，去买杯热的”）。不是真心要发就别发，不要频繁发钱。' + ((s.chatMode === 'narrator') ? '\n\n（旁白模式已开启：请在整条可见回复的最开头，另起一行原样输出后台标记 [[AETHER_NARRATOR]]，紧跟一句当时的场景 / 环境 / 神态描写，' + (function () { var _nr = chatNarrRange(s); return _nr.min + '~' + _nr.max; })() + '字，像小说旁白那样写环境与神态，不要写成对话、不要加引号；随后从下一行开始写 TA 的正常聊天正文。[[AETHER_NARRATOR]] 只是后台标记，绝不能出现在用户可见的正文里。）' : '') + ((s.chatMode === 'action') ? '\n\n（动作描写模式已开启：必须严格按系统提示词里的第三人称限制视角格式输出——整条回复写在同一行、整体包在 [quote id=""]……[/quote] 里、对话用浅灰 span 标签、环境动作心理直接写且不带任何括号；不要输出 [[AETHER_ACTION]] 标记。）' : '') + '）' + '\n\n（另外两条后台指令，只在确有需要时使用，绝不能出现在用户可见的正文里：①若你想在对方刚发给你的那张照片上涂鸦创作（画几笔、加贴纸、写几个字、画成某种风格）再发回去，就在正文之后另起一行，原样输出 [[AETHER_DOODLE]]，紧跟一句你想怎么涂鸦的描述（画什么、什么风格、什么颜色），例如 [[AETHER_DOODLE]]在照片角落画一只小猫并写上「到此一游」；系统会自动在对方那张图上完成涂鸦并发回给对方。对方最近没发过图片时不要用这个标记，没有涂鸦想法也不要硬用。②若对方发来了文件、你要在文件里作答或改完把文件发回，就在正文之后另起一行，原样输出 [[AETHER_FILEOUT]]，紧跟文件名（保留原扩展名，例如 作业.txt），从下一行开始写这个文件的完整新内容（要写全，不要省略），全部写完后另起一行原样输出 [[AETHER_FILEEND]]。文件名和文件内容都不要另加解释；对方没发文件、或你不想动文件，就不要用这个标记。这两个标记都只是后台指令，绝不能显示在用户可见的正文里。）';
          sysInjected = true;
        }
      }
      var base = String(cfg.baseUrl || '').replace(/\/+$/, '');
      ...
      fetch(base, {
        ...
        var _mkDefs = [
          ...
          { tag: '[[AETHER_DOODLE]]', set: function (v) { doodleDesc = v.replace(/^[:：\s]+|[:：\s]+$/g, '').trim(); } }
        ];
        ...
        /* v221：解析 [[AETHER_FILEOUT]]文件名 … [[AETHER_FILEEND]]——TA 在文件里作答/改完把文件发回 */
        var fileOut = null;
        var _foTag = '[[AETHER_FILEOUT]]';
        var _foP = text.indexOf(_foTag);
        if (_foP >= 0) {
          var _foHeadEnd = text.indexOf('\n', _foP + _foTag.length);
          if (_foHeadEnd < 0) _foHeadEnd = text.length;
          var _foName = text.slice(_foP + _foTag.length, _foHeadEnd).replace(/^[:：\s]+|[:：\s]+$/g, '').trim();
          var _foEndTag = '[[AETHER_FILEEND]]';
          var _foEnd = text.indexOf(_foEndTag, _foHeadEnd);
          var _foBody = (_foEnd >= 0) ? text.slice(_foHeadEnd, _foEnd) : text.slice(_foHeadEnd);
          if (chatFileSwOn(s)) fileOut = { name: _foName, body: _foBody.replace(/^\n+/, '').replace(/\s+$/, '') };
          text = (text.slice(0, _foP) + ((_foEnd >= 0) ? text.slice(_foEnd + _foEndTag.length) : '')).replace(/^\n+/, '').trim();
        }
        /* v204：动作描写模式走「第三人称限制视角」整条富文本；抽到就不再走下面的分句/标记逻辑 */
        var richHtml = '';
        if (s.chatMode === 'action') {
          richHtml = chatRichExtract(text);
          if (richHtml) text = '';
        }
        /* v195 / v202：解析场景描写 —— 旁白模式用 [[AETHER_NARRATOR]]，动作描写模式用 [[AETHER_ACTION]] */
        var narratorText = '';
        var narratorFmt = (s.chatMode === 'action') ? 'action' : 'narrator';
        if (!richHtml && (s.chatMode === 'narrator' || s.chatMode === 'action')) {
          var _mkList = (s.chatMode === 'action')
            ? ['[[AETHER_ACTION]]', '[[AETHER_NARRATOR]]']
            : ['[[AETHER_NARRATOR]]', '[[AETHER_ACTION]]'];
          for (var _mi = 0; _mi < _mkList.length && !narratorText; _mi++) {
            var _nmk = _mkList[_mi];
            var _np = text.indexOf(_nmk);
            if (_np < 0) continue;
            var _npe = text.indexOf('\n', _np + _nmk.length);
            if (_npe < 0) _npe = text.length;
            narratorText = text.slice(_np + _nmk.length, _npe).replace(/^[:：\s]+|[:：\s]+$/g, '').trim();
            if (_nmk === '[[AETHER_ACTION]]') narratorFmt = 'action';
            text = (text.slice(0, _np) + text.slice(_npe)).replace(/^\n+/, '').trim();
          }
        }
        var bubbles = richHtml ? [] : splitBubbles(text);
        if (!richHtml && !bubbles.length) bubbles = [text];
        ...
        var done = function () {
          chatAiBusy = false;
          /* v221：模型这轮想在你发的图上涂鸦 → 文字发完后异步画好再回一张图 */
          try { if (doodleDesc) chatDoodleRun(convRef, doodleDesc); } catch (eDw) {}
          try { doodleDesc = ''; } catch (eDw2) {}
          if (convRef === chatCurrentConv) {
            if (recvBtn) recvBtn.classList.remove('busy');
            if (statusEl) statusEl.textContent = baseStatus;
          }
          ...
          /* v202：本条回复落屏后，状态栏跟着对话自动刷新（和心声一样跟随） */
          try { chatRoleStatusAutoTick(convRef); } catch (eStA) {}
          /* v204：TA 会自己挑时候发朋友圈（朋友圈入口只在「更多」界面里） */
          try { chatMomentAutoPost(convRef); } catch (eMA) {}
        ...
        if (fileOut) steps.push({ file: fileOut });
        if (richHtml) steps.push({ rich: richHtml });
        ...
        /* v195：旁白场景描写排在本轮最前面 */
        if (narratorText) steps.unshift({ narrator: narratorText, fmt: narratorFmt });
        var pushOne = function () {
          if (step < steps.length) {
            var it = steps[step];
            if (it.rich) {
              /* v204：第三人称限制视角整条富文本（环境/动作/心理 + 浅灰对话标签/折叠翻译） */
              addChatMsg('other', { type: 'html', text: it.rich, plain: chatHtmlToPlain(it.rich) }, convRef);
            } else if (it.narrator) {
              /* v195：旁白场景描写（居中灰底小字，先于正文出现） */
              /* v202：动作描写用动作卡，旁白沿用旁白卡 */
              addChatMsg('other', { type: 'text', text: it.narrator, fmt: it.fmt || 'narrator' }, convRef);
            } else if (it.file) {
              /* v221：TA 改好/作答完的文件，作为一条可下载的文件气泡发出来 */
              var _fd = chatMakeFileMsg(it.file.name, it.file.body);
              if (_fd) {
                addChatMsg('other', { type: 'file', fileName: _fd.fileName, fileSize: _fd.fileSize, fileData: _fd.fileData, fileText: _fd.fileText }, convRef);
                toast('TA 把文件发回来了：' + _fd.fileName);
              } else {
                addChatMsg('other', { type: 'text', text: '（文件没生成出来，内容可能太长了）' }, convRef);
              }
            } else if (it.loc) {
              addChatMsg('other', { type: 'location', text: it.loc.n, locDetail: it.loc.d, locCat: it.loc.c, locX: it.loc.x, locY: it.loc.y, locCur: 0 }, convRef);
            } else if (it.pay) {
              var _pv = it.pay;
              var _pText = _pv.kind === 'transfer' ? (_pv.amount + '\n' + (_pv.note || '')) : (_pv.note || '恭喜发财');
              addChatMsg('other', { type: _pv.kind === 'transfer' ? 'transfer' : 'redpacket', text: _pText, amount: String(_pv.amount), msg: _pv.note || '', pay: { kind: _pv.kind, amount: String(_pv.amount), note: _pv.note || '', state: 'sent' } }, convRef);
            } else if (it.voice && it.txt) {
              addChatMsg('other', { type: 'voice', text: it.txt, duration: Math.max(1, Math.round(String(it.txt).length / 3)) + '"' }, convRef);
              var lastIdx = convRef.messages.length - 1;
              chatTtsLang(it.txt, s.voice.voiceId, s.voice.speed || 1, function (audio, err) {
                if (err) { var _tez = errZh(err); window.__chatLastTtsErr = _tez; pushChatErrLog('AI语音合成失败: ' + _tez); toast('语音合成失败：' + _tez + '（点击语音气泡可看文字，日志见聊天窗口右上角 → 调试日志）'); return; }
                if (audio && convRef && convRef.messages && convRef.messages[lastIdx] && convRef.messages[lastIdx].type === 'voice') {
                  convRef.messages[lastIdx].audio = audio;
                  convRef.messages[lastIdx].voiceCfg = (function (vv) { return vv ? (String(vv.voiceId || '') + '|' + String(vv.lang || '') + '|' + String(vv.speed != null ? vv.speed : 1)) : ''; })(convRef.settings && convRef.settings.voice);
                  saveConvs();
                  if (convRef !== chatCurrentConv) return;
                  renderChatMessages();
                  /* v195：语音一律不自动播放 —— 合成好只静默落库，等用户点语音条再出声 */
                  pushChatErrLog('[语音调试] AI语音已生成，等待点击播放（v195 已关闭自动播放）');
                }
              });
            } else {
              addChatMsg('other', { type: 'text', text: it.txt || '' }, convRef);
              /* v199：TA 正文里说「换头像」就当场真换，不再只停留在嘴上 */
              try { chatRoleAvatarIntentMaybe(convRef, it.txt || ''); } catch (eAv) {}
            }
            var justMsg = convRef.messages[convRef.messages.length - 1];
            lastTurnMsg = justMsg;
            /* v169：把该轮“心声”挂到最后一条消息上 */
            if (step === steps.length - 1 && heartVoice && justMsg) {
              justMsg.inner = heartVoice;
              saveConvs();
              if (convRef === chatCurrentConv) renderChatMessages();
            }
            step++;
            if (step < steps.length) {
              if (convRef === chatCurrentConv && statusEl) statusEl.innerHTML = '对方正在输入<span class="chat-typing-dots"><i></i><i></i><i></i></span>';
    ...
    setInterval(function () {
      ...
      /* v215：掌心窗主动模式 —— TA 自己惦记你，先真看一眼手机再开口，不用你先发消息 */
      try { if (peekProRun()) return; } catch (ePro) {}
    ...
    function onChatFunc(key) {
      ...
      if (key === 'location') {
        ...
        /* v221：发出的文件会把正文读下来存进消息里（文本类直读、docx 解析），TA 就能在文件里作答并改完发回 */
    ...
    // ===== v193：角色推荐好友——把 TA 自己 / TA 的 NPC 名片二维码发进聊天 =====
    var chatQrImgCache = {};
    function recoQrCardOf(it) {
      var m = it.m || {};
      if (it.kind === 'npc') {
        var n = it.npc || {};
        return { mode: 'all', fields: { netName: false, gender: false, lore: true, look: false, voice: false, alts: false, npcs: false } };
      }
      return m.card;
    }
    function recoQrSrcOf(it) {
      var m = it.m || {};
      if (it.kind === 'npc') {
        var n = it.npc || {};
        return {
          id: n.id || '', realName: n.name || '', netName: '', gender: '', lore: n.bio || '',
          voiceId: '', look: '', lookPrompt: '', alts: null, npcs: null, avatar: it.avatar || ''
        };
      }
      return memberQrSrcOf(m);
    }
    /* v194：推荐好友改为「TA 主动发」——不再弹窗让用户先挑人，由 TA 自己决定发给谁
       （优先 TA 的 NPC，其次 TA 自己），先说一句话，再把二维码名片发进聊天。 */
    function openRecoPicker() {
      if (!chatCurrentConv) return;
      var m = memberByConv(chatCurrentConv);
      if (!m) { toast('这个会话还没绑定名单角色，先去「名单」里挑一个'); return; }
      var _conv = chatCurrentConv;
      var npcs = ((m.npcs) || []).filter(function (n) { return n && n.name; });
      var it = null;
      if (npcs.length) {
        var last = '';
        try { last = String(_conv.settings.recoLast || ''); } catch (e) {}
        var pool = npcs.filter(function (n) { return String(n.name) !== last; });
        if (!pool.length) pool = npcs;
        var n0 = pool[Math.floor(Math.random() * pool.length)];
        it = { kind: 'npc', name: String(n0.name), avatar: (n0.avatar || m.avatar || ''), m: m, npc: n0 };
        try { _conv.settings.recoLast = String(n0.name); } catch (e3) {}
      } else {
        it = { kind: 'self', name: memberDispName(m), avatar: m.avatar || '', m: m, npc: null };
      }
      var rel = String((it.npc && it.npc.rel) || '').trim();
      var line = it.kind === 'npc'
        ? ('给你推个人——' + it.name + (rel ? ('，我' + rel) : '') + '，人挺靠谱的，想认识就扫这张名片加一下？')
        : '这是我的名片，扫码就能加我。';
      addChatMsg('other', { type: 'text', text: line }, _conv);
      memberQrCardUrl(recoQrSrcOf(it), recoQrCardOf(it), function (txt) {
        if (!txt) { toast('名片生成失败'); return; }
        addChatMsg('other', { type: 'qr', qrText: txt, qrName: it.name, qrAvatar: it.avatar || '' }, _conv);
        toast('TA 已经主动把名片发过来了');
      });
    }
    /* 聊天里的二维码图：异步生成 + 缓存，点头开大图、长按可识别 */
    function chatQrRenderCells() {
      if (!chatCurrentConv) return;
      var msgs = chatCurrentConv.messages || [];
      chatDetailBody.querySelectorAll('.chat-qr-card').forEach(function (cell) {
        var row = cell.closest ? cell.closest('.chat-msg-row') : null;
        var idx = row ? parseInt(row.getAttribute('data-msg-idx'), 10) : -1;
        var m = (!isNaN(idx) && idx >= 0) ? msgs[idx] : null;
        if (!m || m.type !== 'qr') return;
        if (!cell.__qrBound) {
          cell.__qrBound = 1;
          cell.addEventListener('click', function () {
            if (!chatQrImgCache[String(m.qrText || '')]) return;
            openQrViewer(m.qrText, { avatar: m.qrAvatar, name: m.qrName });
          });
        }
        var key = String(m.qrText || '');
        if (!key || chatQrImgCache[key] || cell.getAttribute('data-qr-busy')) return;
        cell.setAttribute('data-qr-busy', '1');
        qrPrettyPng(key, { avatar: m.qrAvatar, name: m.qrName, cell: 6, ring: qrRingFromText(key) }, function (png) {
          if (!png) return;
          chatQrImgCache[key] = png;
          if (!chatCurrentConv) return;
          chatDetailBody.querySelectorAll('.chat-qr-card').forEach(function (c) {
            var rw = c.closest ? c.closest('.chat-msg-row') : null;
            var i2 = rw ? parseInt(rw.getAttribute('data-msg-idx'), 10) : -1;
            var mm = (!isNaN(i2) && i2 >= 0) ? (chatCurrentConv.messages || [])[i2] : null;
            if (!mm || mm.type !== 'qr' || String(mm.qrText || '') !== key) return;
            c.classList.remove('pending');
            c.innerHTML = '<img class="chat-qr-img" src="' + png + '" alt="推荐好友二维码">' +
              '<div class="chat-qr-cap">' + escHtml('推荐好友：' + (mm.qrName || '')) + '<button type="button" class="tips-q" data-tip="reco">?</button></div>';
          });
        });
      });
    }
    ...
    /* ===== v200：角色「主动换头像」 =====
       用户发图之后，TA 会看着办给自己换一张头像，来路三选一：
       ①用户刚发来的图里挑一张 ②TA 自己相册里挑一张 ③截用户头像的一角 */
    var CHAT_ROLE_AVA_MSG = [
      '换个头像，这张看着顺眼。',
      '刚把头像换了，好看吗？',
      '新头像，别说不认识我。',
      '顺手换了张头像，就这张了。'
    ];
    /* 截「用户头像」的随机一角，裁成正方形当新头像 */
    function chatAvatarCorner(src, cb) {
      try {
        var s0 = String(src || '');
        if (!s0) { cb(''); return; }
        var img = new Image();
        img.onload = function () {
          try {
            ...
            var sq = Math.max(1, Math.round(Math.min(w, h) * 0.42));
            var pos = Math.floor(Math.random() * 4);
            var sx = (pos === 1 || pos === 3) ? Math.max(0, w - sq) : 0;
            var sy = (pos === 2 || pos === 3) ? Math.max(0, h - sq) : 0;
            var cv = document.createElement('canvas');
            cv.width = 160; cv.height = 160;
            cv.getContext('2d').drawImage(img, sx, sy, sq, sq, 0, 0, 160, 160);
    ...
    /* 新头像落库：名单角色写回名单并同步所有聊天窗口；非名单角色写会话本身 */
    function chatRoleAvatarSet(conv, src) {
      try {
        if (!conv || !src) return false;
        var m = memberByConv(conv);
        if (m) {
          var list = memberLoad();
          for (var i = 0; i < list.length; i++) {
            if (String(list[i].id || '') === String(m.id || '')) { list[i].avatar = src; memberSave(list); break; }
          }
          roleAvatarSyncToConv(m.id, src);
          if (typeof renderChatConvs === 'function') renderChatConvs();
          if (typeof renderChatMessages === 'function') renderChatMessages();
          if (typeof renderChatDetailAvatar === 'function') renderChatDetailAvatar(conv);
          return true;
        }
        if (!conv.settings) conv.settings = defaultConvSettings();
        if (!conv.settings.roleIdentity || typeof conv.settings.roleIdentity !== 'object') {
          conv.settings.roleIdentity = { name: conv.name || '', avatar: '', sex: '', prompt: '', remark: '' };
        }
        conv.settings.roleIdentity.avatar = src;
        saveConvs();
        if (typeof renderChatConvs === 'function') renderChatConvs();
        if (typeof renderChatMessages === 'function') renderChatMessages();
        if (typeof renderChatDetailAvatar === 'function') renderChatDetailAvatar(conv);
    ...
    function chatRoleAvatarApply(conv, src, tag, why) {
      try {
        if (!chatRoleAvatarSet(conv, src)) return false;
        var txt = CHAT_ROLE_AVA_MSG[Math.floor(Math.random() * CHAT_ROLE_AVA_MSG.length)];
        addChatMsg('other', { type: 'text', text: txt }, conv);
        try {
          pushChatErrLog('[TA换头像] 来源=' + (tag === 'corner' ? '截你头像的边角料' : (tag === 'album' ? 'TA 自己的相册' : '你刚发的图')) + (why ? '｜' + why : ''));
        } catch (e1) {}
        return true;
      } catch (e) { return false; }
    }
    /* ===== v201：TA 主动换头像 =====
       不是「你一发图 TA 就换」。要同时满足三件事才会换：
         ① 至少连发过 2 轮图（有来有回，不是随手一扔）；
         ② 距上次换头像已过冷却时间；
         ③ 掷骰子命中（约 1/4）。
       来源：你刚发的图 / TA 自己的相册 —— 这两种随时可用；
       截取 user 头像的边角料只在特定关系情境下才会用（宣示亲密、暗恋、留念、群体认同、占有欲），
       日常闲聊即使中了骰子也不会去截用户的头像。 */
    var CHAT_AVA_COOLDOWN = 8 * 60 * 1000;
    var CHAT_AVA_MIN_STREAK = 2;
    var CHAT_AVA_PROB = 0.25;

    /* 关系情境判定：命中才允许「截 user 头像一角」这种带心理暗示的用法 */
    function chatAvaCornerScene(conv) {
      var txt = chatRoleContextText(conv);
      var aff = chatRoleAffinityOf(conv);
      var hit = function (list) { for (var i = 0; i < list.length; i++) if (txt.indexOf(list[i]) >= 0) return true; return false; };
      if (hit(['恋人', '男朋友', '女朋友', '男友', '女友', '老公', '老婆', '在一起', '确定关系', '暧昧', '情侣']) || aff >= 60) {
        return { key: 'intimate', label: '隐秘的亲密宣示', why: '关系已经到了这一层，头像里留你一角，外人看不出来，我们俩自己懂' };
      }
      if (hit(['暗恋', '单恋', '偷偷喜欢', '不敢说', '暗戳戳', '心上人', '还没表白'])) {
        return { key: 'crush', label: '单方面的暗恋执念', why: '话还没敢说出口，先把你的头像收一角，装作离你近一点' };
      }
      if (hit(['分手', '前任', '冷战', '已经离开', '不再联系', '断了联系', '分开'])) {
        return { key: 'remains', label: '关系破裂后的留念', why: '原图不能再用了，就留一角，算我没舍得删' };
      }
      if (hit(['一伙', '同款', '队友', '搭档', '同盟', '自己人', '我们组'])) {
        return { key: 'tribe', label: '群体身份认同', why: '俩人用一个样式，别人一看就知道我们是一路的' };
      }
      if (hit(['我的人', '属于我', '别想跑', '宣示主权', '吃醋', '盯', '谁也别'])) {
        return { key: 'possess', label: '占有欲的宣示', why: '挂上你的一角，让别人知道你已经归我管了' };
      }
      return null;
    }
    /* 关系上下文：人设 + 备注 + 最近 30 条对话，用来判断现在是什么关系 */
    function chatRoleContextText(conv) {
      var s = '';
      try {
        var ri = (conv && conv.settings && conv.settings.roleIdentity) || {};
        s += ' ' + (ri.remark || '') + ' ' + (ri.name || '');
        var m = memberByConv(conv);
        if (m) s += ' ' + (m.lore || '') + ' ' + (m.bio || '') + ' ' + (m.rel || '') + ' ' + (m.realName || '') + ' ' + (m.netName || '');
        var ms = (conv && conv.messages) || [];
        for (var i = Math.max(0, ms.length - 30); i < ms.length; i++) {
          var t = ms[i] && ms[i].text;
          if (t) s += ' ' + String(t).slice(0, 120);
        }
      } catch (e) {}
      return s;
    }
    function chatRoleMaybeAutoAvatar(conv, imgs) {
      try {
        if (!conv || conv.roleAutoAvaOff) return false;
        conv._avaStreak = (conv._avaStreak || 0) + 1;
        if (conv._avaStreak < CHAT_AVA_MIN_STREAK) return false;
        var last = Number(conv._avaLast || 0);
        if (last && (Date.now() - last) < CHAT_AVA_COOLDOWN) return false;
        if (Math.random() > CHAT_AVA_PROB) return false;
        var m = memberByConv(conv);
        var pool = (imgs || []).filter(function (s) { return !!s; });
        var alb = (m && m.album) ? m.album.filter(function (a) { return a && a.src; }) : [];
        var mine = String((chatMine && chatMine.avatar) || '');
        var scene = chatAvaCornerScene(conv);
        var wCorner = (mine && scene) ? 20 : 0;
        var wJust = pool.length ? (wCorner ? 50 : 62) : 0;
        var wAlb = alb.length ? (wCorner ? 30 : 38) : 0;
        var tot = wJust + wAlb + wCorner;
        if (!tot) return false;
        var r = Math.random() * tot;
        if (r < wJust) {
          var ok = chatRoleAvatarApply(conv, pool[Math.floor(Math.random() * pool.length)], 'just-sent', '刚收到你发的图，顺手拿来当头像');
          if (ok) { conv._avaLast = Date.now(); conv._avaStreak = 0; }
          return ok;
        }
        r -= wJust;
        if (r < wAlb) {
          var ok2 = chatRoleAvatarApply(conv, alb[Math.floor(Math.random() * alb.length)].src, 'album', '从自己相册里翻出一张换了头像');
          if (ok2) { conv._avaLast = Date.now(); conv._avaStreak = 0; }
          return ok2;
        }
        if (!scene) return false;
        chatAvatarCorner(mine, function (url) {
          if (!url) return;
          var ok3 = chatRoleAvatarApply(conv, url, 'corner', '情境=' + scene.label + '｜' + scene.why);
          if (ok3) { conv._avaLast = Date.now(); conv._avaStreak = 0; }
        });
        return true;
      } catch (e) { return false; }
    }
    function chatSendImageMenu() {
      ...
      chatMini('发送图片', '<div class="chat-cfg-tip" style="margin:0 0 8px">想给TA发什么？</div><div class="chat-mini-list">' +
        ...
        '<button class="chat-mini-opt" id="imgOptOne">系统相册上传</button></div>', '取消', function () {});
            ...
            fs.forEach(function (f) {
              chatFileToSmallImg(f, function (dataUrl) {
                ...
                if (--pend === 0) {
                  ...
                  /* v202：你发的图自动进 TA 的相册（可当头像 / 发朋友圈） */
                  chatUserImgsSavedByRole(chatCurrentConv, _imgs);
                  /* v200：用户发图之后，TA 可能顺手给自己换个头像 */
                  chatRoleMaybeAutoAvatar(chatCurrentConv, _imgs);
                }
              });
            });
          });
    ...
    /* ===== v186：让角色用「生图 API」现画一张发到聊天 ===== */
    /* v194：用户在聊天里直接开口要图（「发张图」「给我看看照片」）——TA 真的从相册发一张过来，
       不再只是回一句文字。同一句话只触发一次，TA 回过之后不再补发。 */
    function chatAutoSendImgIfAsked(conv) {
      try {
        if (!conv) return false;
        ...
        for (var i = msgs.length - 1; i >= 0; i--) {
          var mm = msgs[i];
          if (!mm) continue;
          if (mm.role === 'me') { last = mm; break; }
          if (mm.role === 'other') return false;
        }
        if (!last || last.type !== 'text' || !last.text) return false;
        if (conv.__autoImgTs === last.ts) return false;
        var t = String(last.text);
        var want = /(发|来|给|看|晒|秀|要|求)(把|个|张|点)?[^，。！？\n]{0,5}(图|图片|照片|相册|自拍)/.test(t)
          || /(图|图片|照片|自拍)[^，。！？\n]{0,4}(看看|发我|发来|发张|给我|瞧瞧)/.test(t);
        if (!want) return false;
        conv.__autoImgTs = last.ts;
        var m = null;
        try { m = memberByConv(conv); } catch (e) {}
        /* v195：TA 要发图时优先「现生成一张新的」，不再永远翻旧相册 */
        var roleCfg = chatRoleImgApiCfg(conv);
        if (roleCfg && chatRoleImgSwOn(conv.settings) && typeof chatRoleGenImageApi === 'function') {
          var _im = (conv.settings && conv.settings.imag) || {};
          var _pos = [];
          if (m && m.look) _pos.push(String(m.look));
          if (m && m.lookPrompt) _pos.push(String(m.lookPrompt));
          var _scenes = ['随手拍的一张，光线自然，构图随意', '刚出门时随手拍的', '手机自拍，日常角度，表情自然放松', '在家随手拍到的一幕', '走在路上随手按下的快门'];
          var _sceneTxt = _scenes[Math.floor(Math.random() * _scenes.length)];
          _pos.push('聊天里给对方发一张刚拍的照片：' + _sceneTxt);
          var _desc = String(t).replace(/\s+/g, '').slice(0, 24);
          if (_desc) _pos.push('呼应对方刚说的「' + _desc + '」，气氛贴合当下情绪');
          var _prompt = _pos.filter(function (x) { return String(x).trim(); }).join('，');
          if (_prompt) {
            var _neg = String(_im.promptNeg || '');
            var _face = (m && m.lookImgs && m.lookImgs[0]) || _im.face || '';
            chatRoleGenBusy = true;
            var _ph = chatRoleImgPlaceholder(conv, _sceneTxt);
            chatRoleGenImageApi(roleCfg, _prompt, _neg, _face, function (src) {
              chatShrinkImgSrc(src, function (out) {
                chatRoleGenBusy = false;
                chatRoleImgDone(_ph, out, conv, _sceneTxt);
              });
            }, function (err) {
              chatRoleGenBusy = false;
              chatRoleImgFail(_ph, conv, null);
              pushChatErrLog('[发图] 现生成失败，回退相册：' + err);
              chatAutoSendImgFromAlbum(conv, m);
            });
            return true;
          }
        }
        return chatAutoSendImgFromAlbum(conv, m);
      } catch (e2) { return false; }
    }
    /* v195：从 TA 相册挑一张发（没有生图 API 时的兜底；也是「从相册挑」的既有语义） */
    function chatAutoSendImgFromAlbum(conv, m) {
      try {
        var list = (m && m.album && m.album.length) ? m.album : [];
        if (!list.length) return false;
        var it = list[Math.floor(Math.random() * list.length)];
        var note = String((it && it.note) || '').trim() || '相册里的照片';
        addChatMsg('other', { type: 'image', imgs: [it.src], picNote: note }, conv);
        /* v198：TA 从相册挑图发出去时，也可能顺手把这张换成自己的头像 */
        chatRoleSelfAvatarMaybe(conv, it.src);
        return true;
      } catch (e) { return false; }
    }
    /* ===== v197：TA 主动发图统一「先占位、后落图」 ===== */
    function chatRoleImgPlaceholder(conv, note) {
      var _c = conv || chatCurrentConv;
      if (!_c) return null;
      addChatMsg('other', { type: 'image', loading: 1, picNote: note || '照片' }, _c);
      return (_c.messages && _c.messages.length) ? _c.messages[_c.messages.length - 1] : null;
    }
    function chatRoleImgDone(m, src, conv, note) {
      if (!m) return;
      var _c = conv || chatCurrentConv;
      m.loading = 0;
      m.imgs = [src];
      m.img = src;
      if (note) m.picNote = note;
      /* v198：TA 自己决定要不要把这张换成自己的头像 */
      chatRoleSelfAvatarMaybe(_c, src);
      saveConvs();
      if (_c === chatCurrentConv) {
        renderChatMessages();
        try { chatDetailBody.scrollTop = chatDetailBody.scrollHeight; } catch (e) {}
      }
      renderChatConvs();
    }
    function chatRoleImgFail(m, conv, msg) {
      var _c = conv || chatCurrentConv;
      try {
        if (m && _c && _c.messages) {
          var i = _c.messages.indexOf(m);
          if (i >= 0) _c.messages.splice(i, 1);
        }
        saveConvs();
        if (_c === chatCurrentConv) renderChatMessages();
        renderChatConvs();
      } catch (e) {}
      if (msg) toast(msg);
    }
    /* v222：TA 自发换头像开关（默认「关」）——关着的时候 TA 绝不会自己动头像 */
    function chatAvatarSwInit(s) {
      if (!s) return { enabled: false };
      if (!s.autoAvatar || typeof s.autoAvatar !== 'object') s.autoAvatar = { enabled: false };
      return s.autoAvatar;
    }
    function chatAvatarSwOn(s) { var a = chatAvatarSwInit(s); return !!(a && a.enabled === true); }
    /* v198：TA 自主换头像（v222 起默认关闭：要开得你手动去「交互感知」里打开） */
    function chatRoleSelfAvatarMaybe(conv, src, force) {
      try {
        var _c = conv || chatCurrentConv;
        var m = null;
        try { m = memberByConv(_c); } catch (e) {}
        if (!m || !m.id) return false;
        if (!force && !chatAvatarSwOn(_c && _c.settings)) return false;
        if (!force && Math.random() > 0.8) return false;
        var pick = String(src || '');
        if (!pick) {
          var al = (m.album || []).filter(function (x) { return x && x.src; });
          if (!al.length) return false;
          pick = al[Math.floor(Math.random() * al.length)].src;
        }
        var list = memberLoad();
        var i = -1;
        for (var k = 0; k < list.length; k++) { if (String(list[k].id) === String(m.id)) { i = k; break; } }
        if (i < 0) return false;
        if (String(list[i].avatar || '') === String(pick)) return false;
        var _prevAv = String(list[i].avatar || '');
        list[i].avatar = pick;
        memberSave(list);
        roleAvatarSyncToConv(list[i].id, pick);
        /* v222：换头像前那张留着，随时能「恢复上一张」 */
        try { if (_c && _c.settings) { _c.settings.avatarUndo = { memberId: String(list[i].id), prev: _prevAv, ts: Date.now() }; saveConvs(); } } catch (e5) {}
        if (_c === chatCurrentConv && typeof renderChatMessages === 'function') { try { renderChatMessages(); } catch (e2) {} }
        if (typeof renderChatConvs === 'function') { try { renderChatConvs(); } catch (e3) {} }
        /* v222：换头像必须说清楚，并且能一键恢复——不再「说换就换」 */
        try { toast('TA 换上了新头像（聊天设置 · 交互感知 里可恢复上一张）'); } catch (e4) {}
        return true;
      } catch (e) { return false; }
    }
    /* v222：把 TA 的头像恢复到换之前那张（就在「交互感知」里点一下） */
    function chatAvatarRestore(conv) {
      try {
        var _c = conv || chatCurrentConv;
        var u = _c && _c.settings && _c.settings.avatarUndo;
        if (!u || !u.memberId) { toast('没有可恢复的头像记录'); return false; }
        var list = memberLoad();
        var i = -1;
        for (var k = 0; k < list.length; k++) { if (String(list[k].id) === String(u.memberId)) { i = k; break; } }
        if (i < 0) { toast('没找到对应角色'); return false; }
        var cur = String(list[i].avatar || '');
        list[i].avatar = String(u.prev || '');
        memberSave(list);
        roleAvatarSyncToConv(list[i].id, list[i].avatar);
        _c.settings.avatarUndo = { memberId: String(u.memberId), prev: cur, ts: Date.now() };
        saveConvs();
        try { renderChatMessages(); } catch (e) {}
        try { renderChatConvs(); } catch (e2) {}
        toast('已恢复上一张头像');
        return true;
      } catch (e) { return false; }
    }
    /* v199：TA 在回复正文里说要「换头像」时，当场把这件事真的做掉。
       素材优先从相册里随机取（跳过当前头像），相册没有图而有生图配置时现画一张；
       都没有就明确提示，避免出现「嘴上说换了、界面毫无反应」。 */
    function chatRoleAvatarIntentMaybe(conv, text) {
      try {
        var t = String(text || '');
        if (!t || t.indexOf('头像') < 0) return false;
        if (!/(换|更新|改|替换|新头像)/.test(t)) return false;
        var _c = conv || chatCurrentConv;
        var m = null;
        try { m = memberByConv(_c); } catch (e) {}
        if (!m || !m.id) { toast('TA 想换头像，但当前会话没绑定名单角色'); return false; }
        var list = memberLoad();
        var i = -1;
        for (var k = 0; k < list.length; k++) { if (String(list[k].id) === String(m.id)) { i = k; break; } }
        if (i < 0) return false;
        var pool = (list[i].album || []).filter(function (x) { return x && x.src && String(x.src) !== String(list[i].avatar || ''); });
        if (pool.length) {
          var pick = pool[Math.floor(Math.random() * pool.length)].src;
          return chatRoleSelfAvatarMaybe(_c, pick, true);
        }
        var cfg = null;
        try { cfg = chatRoleImgApiCfg(_c); } catch (eC) {}
        if (cfg) {
          var pos = [];
          if (m.look) pos.push(String(m.look));
          if (m.lookPrompt) pos.push(String(m.lookPrompt));
          if (!pos.length) pos.push('头像特写，正脸，自然光，日常感');
          var _neg = String(((_c.settings && _c.settings.imag) || {}).promptNeg || '');
          var _ref = (m.lookImgs && m.lookImgs[0]) || (((_c.settings && _c.settings.imag) || {}).face) || '';
          toast('TA 正在画一张新头像…');
          chatRoleGenImageApi(cfg, pos.join('，'), _neg, _ref, function (src) {
            chatShrinkImgSrc(src, function (out) {
              if (!out) { toast('TA 想换头像，但这张图没处理好'); return; }
              chatRoleSelfAvatarMaybe(_c, out, true);
              /* 顺手把新头像收进相册，下次还能翻出来 */
              try {
                var l2 = memberLoad();
                var j = -1;
                for (var q = 0; q < l2.length; q++) { if (String(l2[q].id) === String(m.id)) { j = q; break; } }
                if (j >= 0) {
                  if (!Array.isArray(l2[j].album)) l2[j].album = [];
                  if (l2[j].album.length < MEM_ALBUM_MAX) {
                    l2[j].album.push({ id: memberUid(), src: out, ts: Date.now(), note: 'TA 自己换上的新头像', savedBy: 'ta' });
                    memberSave(l2);
                  }
                }
              } catch (eA) {}
            });
          }, function (msg) { toast('TA 想换头像但生成失败：' + msg); });
          return true;
        }
        toast('TA 想换头像，但相册里还没有照片，也没配生图 API');
        return false;
      } catch (e) { return false; }
    }
    var chatRoleGenBusy = false;
    function chatMemberOfConv() {
      /* v194：改为与 memberByConv 同一套判定。旧实现只认 contactId 去掉首字母 m 的 id，
         像 'm1' 这种名单角色会被误判成“没绑定”，TA 的外貌 / 锁脸 / 相册全取不到，
         表现就是「让 TA 发图发不出来」。 */
      try { return memberByConv(chatCurrentConv); } catch (e) {}
      return null;
    }
    function chatRoleImgApiCfg(conv) {
      /* v195：支持传入目标会话，便于自动发图时按该角色的生图配置生成新图 */
      var _cfgConv = conv || chatCurrentConv;
      var im = (_cfgConv && _cfgConv.settings) ? _cfgConv.settings.imag : null;
      try {
        var name = (im && im.apiName) ? String(im.apiName) : '';
        if (name && typeof imgConfigs !== 'undefined' && imgConfigs) {
          for (var i = 0; i < imgConfigs.length; i++) { if (imgConfigs[i].name === name) return imgConfigs[i]; }
        }
        if (im && im.baseUrl && im.apiKey && im.model) return im;
        var any = (typeof memberImgFindApi === 'function') ? memberImgFindApi() : null;
        if (any) return any;
      } catch (e) {}
      return null;
    }
    function chatShrinkImgSrc(src, cb) {
      try {
        var img = new Image();
        img.onload = function () {
          try {
            var MAX = 720, w = img.width || 1, h = img.height || 1;
    ...
    function chatRoleGenImageApi(cfg, prompt, neg, refImage, done, fail) {
      var baseUrl = String((cfg && cfg.baseUrl) || '').trim().replace(/\/+$/, '');
      if (!baseUrl || !cfg.apiKey) { fail('生图配置缺少 Base URL / API Key'); return; }
      var body = { model: cfg.model, prompt: prompt, n: 1 };
      ...
      fetch(baseUrl + '/images/generations', {
        ...
        return r.text().then(function (t) {
          ...
          var d;
          try { d = JSON.parse(txt); } catch (e) { throw new Error('接口返回非 JSON：' + txt.replace(/\s+/g, ' ').slice(0, 160)); }
          if (d && d.error && !(d.data && d.data[0])) throw new Error(String((d.error && d.error.message) || '接口错误').slice(0, 160));
          return d;
        });
      }).then(function (d) {
        var it = d && d.data && d.data[0];
        if (!it) throw new Error('接口没有返回图片');
        if (it.b64_json) done('data:image/png;base64,' + it.b64_json);
        else if (it.url) done(it.url);
        else throw new Error('接口没有返回图片');
      })['catch'](function (e) { fail((e && e.message) || '生成失败'); });
    }
    function chatRoleGenImage() {
      if (!chatCurrentConv) return;
      /* v194：以前没有可用配置时只弹一句 toast 就退出，用户点「让 TA 现画一张」看着像没反应。
         现在照常打开输入窗，并把配置状态写在窗里；真开画时缺配置再给明确指引。 */
      var cfg = chatRoleImgApiCfg();
      var m = chatMemberOfConv();
      var cfgTip = cfg
        ? ('当前生图配置：' + escHtml(String(cfg.name || cfg.model || '本窗口配置')) + (cfg.model ? ('（' + escHtml(String(cfg.model)) + '）') : '') + '。')
        : '<b style="color:#ff6b6b">还没找到可用的生图配置</b>：先去「设置 → 生图API」添加一条（Base URL / API Key / 模型三项都要填），或在聊天设置里给本窗口选一条。';
      chatMini('让 TA 现画一张',
        '<div class="chat-cfg-tip" style="margin:0 0 8px">' + cfgTip + '<br>说说想让 TA 画什么。留空就按' + (m ? 'TA 的外貌与锁定提示词' : '本窗口的生图提示词') + '来画。</div>' +
        '<textarea class="chat-mini-input" id="cgiDesc" rows="3" placeholder="例如：坐在窗边喝咖啡，侧脸逆光"></textarea>' +
        '<div id="cgiErr" class="chat-cfg-tip" style="display:none;color:#ff6b6b;margin-top:8px"></div>',
        '开始画', function () {
          if (chatRoleGenBusy) { toast('上一张还在画，稍等一下'); return; }
          if (!cfg) {
            var er = document.getElementById('cgiErr');
            if (er) { er.style.display = ''; er.textContent = '还没有可用的生图配置：先到「设置 → 生图API」添加，Base URL、API Key、模型三项都要填。'; }
            toast('还没有可用的生图配置，先去「设置 → 生图API」添加');
            return;
          }
          var el = document.getElementById('cgiDesc');
          var desc = el ? String(el.value || '').trim() : '';
          var im = (chatCurrentConv.settings && chatCurrentConv.settings.imag) || {};
          var pos = [];
          if (m && m.look) pos.push(String(m.look));
          if (m && m.lookPrompt) pos.push(String(m.lookPrompt));
          if (desc) pos.push(desc);
          if (!pos.length) pos.push(String(im.prompt || ''));
          var prompt = pos.filter(function (x) { return String(x).trim(); }).join('，');
          if (!prompt) { toast('先写点想画的内容'); return; }
          chatRoleGenBusy = true;
          toast('TA 正在作画…');
          var neg = String(im.promptNeg || '');
          var ref = (m && m.lookImgs && m.lookImgs[0]) || im.face || '';
          var _giNote = (desc || '').slice(0, 60) || 'AI 生图';
          var _giPh = chatRoleImgPlaceholder(chatCurrentConv, _giNote);
          chatRoleGenImageApi(cfg, prompt, neg, ref, function (src) {
            chatShrinkImgSrc(src, function (out) {
              chatRoleGenBusy = false;
              chatRoleImgDone(_giPh, out, chatCurrentConv, _giNote);
              toast('TA 画好发过来了');
            });
          }, function (msg) {
            chatRoleGenBusy = false;
            chatRoleImgFail(_giPh, chatCurrentConv, '生图失败：' + msg);
          });
        });
    }
    /* ===== v221：TA 在你发的图上涂鸦创作 · 与你互传文件并在文件里作答 ===== */
    ...
    function chatTextExtOk(name) {
      var mt = /\.([a-z0-9]+)$/i.exec(String(name || ''));
      var ext = mt ? String(mt[1]).toLowerCase() : '';
      return ['txt', 'md', 'markdown', 'json', 'csv', 'tsv', 'log', 'js', 'mjs', 'cjs', 'jsx', 'ts', 'tsx', 'py', 'html', 'htm', 'css', 'xml', 'yaml', 'yml', 'ini', 'conf', 'c', 'h', 'cpp', 'hpp', 'java', 'kt', 'go', 'rs', 'php', 'rb', 'sh', 'bat', 'cmd', 'sql', 'srt', 'vtt', 'ass', 'vue', 'text'].indexOf(ext) >= 0;
    }
    function chatDocxExtOk(name) { return /\.docx$/i.test(String(name || '')); }
    function chatB64ToU8(b64) {
    ...
    /* 文件互传开关（默认开）：开着才读文件正文、才允许 TA 改完文件发回 */
    function chatFileSwInit(s) {
      if (!s) return { enabled: true };
      if (!s.fileChat || typeof s.fileChat !== 'object') s.fileChat = { enabled: true };
      return s.fileChat;
    }
    function chatFileSwOn(s) { var f = chatFileSwInit(s); return !!(f && f.enabled !== false); }
    /* 涂鸦开关（默认开） */
    function chatDoodleSwInit(s) {
      if (!s) return { enabled: true };
      if (!s.doodle || typeof s.doodle !== 'object') s.doodle = { enabled: true };
      return s.doodle;
    }
    function chatDoodleSwOn(s) { var d = chatDoodleSwInit(s); return !!(d && d.enabled !== false); }
    /* 发出文件：文本类读正文，docx 走 JSZip 解析；同时留一份原始数据（可下载/可回传） */
    function chatSendFileWithContent(f) {
      if (!f) return;
      var on = chatCurrentConv ? chatFileSwOn(chatCurrentConv.settings) : false;
      var LIMIT = 6 * 1024 * 1024;
      if (f.size > LIMIT) {
        addChatMsg('me', { type: 'file', fileName: f.name, fileSize: f.size });
        toast('文件超过 6MB，只发了文件名（太大存不进手机本地）');
        return;
      }
      var reader = new FileReader();
      reader.onload = function (e) {
        ...
        var push = function (txt) {
          addChatMsg('me', { type: 'file', fileName: f.name, fileSize: f.size, fileData: dataUrl, fileText: txt || '' });
          if (on && String(txt || '').trim()) toast('文件已发出，TA 读到了里面的内容');
          else if (on && !chatTextExtOk(f.name) && !chatDocxExtOk(f.name)) toast('文件已发出（这类格式读不出正文，TA 只能看到文件名）');
        };
        if (!on) { push(''); return; }
        if (chatTextExtOk(f.name)) {
          var r2 = new FileReader();
          r2.onload = function (e2) { push(String((e2 && e2.target && e2.target.result) || '').slice(0, 200000)); };
          r2.onerror = function () { push(''); };
          try { r2.readAsText(f.size > 2 * 1024 * 1024 ? f.slice(0, 2 * 1024 * 1024) : f); } catch (e3) { push(''); }
          return;
        }
        if (chatDocxExtOk(f.name) && b64 && typeof JSZip !== 'undefined') {
          chatDocxTextFromB64(b64, function (t) { push(t); });
          return;
        }
        push('');
      };
      reader.onerror = function () { toast('文件读取失败'); };
      try { reader.readAsDataURL(f); } catch (e) { toast('文件读取失败'); }
    }
    function chatDocxTextFromB64(b64, cb) {
      try {
        JSZip.loadAsync(chatB64ToU8(b64).buffer).then(function (zip) {
          var p = zip.file('word/document.xml');
          if (!p) { cb(''); return null; }
          return p.async('string').then(function (xml) { cb(extractDocxText(xml).slice(0, 200000)); });
        })['catch'](function () { cb(''); });
      } catch (e) { cb(''); }
    }
    /* TA 改完发回的文件：用文本内容现做一个可下载的文件消息 */
    function chatMakeFileName(name) {
      var n = String(name || '').replace(/[\\/:*?"<>|\r\n\t]/g, '_').trim();
      if (!n) n = '文件.txt';
      if (!/\.[a-z0-9]{1,5}$/i.test(n)) n += '.txt';
      return n.slice(0, 80);
    }
    function chatMakeFileMsg(name, body) {
      try {
        var fn = chatMakeFileName(name);
        var txt = String(body == null ? '' : body);
        if (txt.length > 200000) txt = txt.slice(0, 200000);
        var size = txt.length;
        try { size = new Blob([txt]).size; } catch (eSz) {}
        return { fileName: fn, fileSize: size, fileData: 'data:text/plain;charset=utf-8;base64,' + btoa(unescape(encodeURIComponent(txt))), fileText: txt };
      } catch (e) { return null; }
    }
    function chatFileSaveName(m) {
      var n = String((m && m.fileName) || '文件').replace(/[\\/:*?"<>|\r\n\t]/g, '_').trim();
      return n || '文件';
    }
    function chatFileDownload(m) {
      try {
        var a = document.createElement('a');
        a.href = m.fileData;
        a.download = chatFileSaveName(m);
        document.body.appendChild(a);
        a.click();
        setTimeout(function () { a.remove(); }, 300);
        toast('已开始下载：' + chatFileSaveName(m));
      } catch (e) { toast('下载失败：' + ((e && e.message) || '未知原因')); }
    }
    function chatFilePreview(m) {
      var txt = String(m.fileText || '');
      if (!txt.trim()) { toast('这个文件没有可读正文'); return; }
      var ov = document.createElement('div');
      ...
      var card = document.createElement('div');
      card.style.cssText = 'background:var(--bg,#1c1c1e);color:var(--text,#fff);border-radius:14px;width:100%;max-width:540px;max-height:78vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 18px 50px rgba(0,0,0,.45)';
      card.innerHTML = '<div style="padding:12px 14px;font-weight:600;font-size:14px;border-bottom:1px solid rgba(128,128,128,.28);word-break:break-all">' + escHtml(chatFileSaveName(m)) + '</div>' +
        '<pre style="margin:0;padding:12px 14px;overflow:auto;white-space:pre-wrap;word-break:break-word;font-size:13px;line-height:1.65;flex:1">' + escHtml(txt) + '</pre>' +
        '<div style="display:flex;gap:8px;padding:10px 14px;border-top:1px solid rgba(128,128,128,.28)">' +
        '<button class="prompt-cancel" id="cfpDl" style="flex:1">下载文件</button>' +
        '<button class="prompt-cancel" id="cfpCl" style="flex:1">关闭</button></div>';
      ov.appendChild(card);
      document.body.appendChild(ov);
      var close = function () { try { ov.remove(); } catch (e) { ov.parentNode.removeChild(ov); } };
      ov.addEventListener('click', function (e) { if (e.target === ov) close(); });
      var dl = card.querySelector('#cfpDl');
      var cl = card.querySelector('#cfpCl');
      if (dl) dl.addEventListener('click', function () { chatFileDownload(m); });
      if (cl) cl.addEventListener('click', close);
    }
    function chatFileOpenIdx(idx) {
      var m = (chatCurrentConv && chatCurrentConv.messages) ? chatCurrentConv.messages[idx] : null;
      if (!m || m.type !== 'file') return;
      if (String(m.fileText || '').trim()) { chatFilePreview(m); return; }
      if (m.fileData) { chatFileDownload(m); return; }
      toast('这个文件没有内容（旧版本发的是空壳文件）');
    }
    /* ---------- 涂鸦：找「你最近发的那张图」 ---------- */
    function chatLastUserImg(conv) {
      var msgs = (conv && conv.messages) || [];
      for (var i = msgs.length - 1; i >= 0 && i >= msgs.length - 10; i--) {
        var m = msgs[i];
        if (!m || m.recalled || m.loading) continue;
        if (m.role !== 'me' || m.type !== 'image' || m.textImg) continue;
        var arr = (m.imgs && m.imgs.length) ? m.imgs : (m.img ? [m.img] : []);
        if (arr.length) return { m: m, src: arr[0] };
      }
      return null;
    }
    /* 涂鸦主流程（v222）：永远只在你发的「原图」上叠加涂鸦，绝不重绘原图；
       模型只负责出「画什么 / 什么颜色 / 画在哪」的排版方案，落笔由 canvas 精准叠到原图上 */
    function chatDoodleRun(conv, desc) {
      try {
        var c = conv || chatCurrentConv;
        if (!c || !desc) return false;
        if (chatRoleGenBusy) return false;
        if (!chatDoodleSwOn(c.settings)) return false;
        var tar = chatLastUserImg(c);
        if (!tar) return false;
        var note = '涂鸦：' + String(desc).replace(/\s+/g, '').slice(0, 60);
        var ph = chatRoleImgPlaceholder(c, 'TA 在你发的图上涂鸦…');
        /* v222：不再把原图丢给生图接口重绘（那会把人物、表情、场景一起改掉）。
           现在一律「原图打底 + 涂鸦叠加」，你那张照片原封不动，只多出涂鸦。 */
        chatDoodleCanvas(c, ph, tar.src, desc, note);
        return true;
      } catch (e) { return false; }
    }
    /* 前端涂鸦：让模型只给「涂鸦排版方案（JSON）」，再用 canvas 精准叠到原图上（原图一个像素都不改） */
    function chatDoodleCanvas(conv, ph, baseSrc, desc, note) {
      var finish = function (items) {
        chatDoodleDraw(baseSrc, items, function (out) {
          if (!out) { chatRoleImgFail(ph, conv, '涂鸦失败：图片处理出错了'); return; }
          chatShrinkImgSrc(out, function (sh) {
            chatRoleImgDone(ph, sh, conv, note);
            toast('TA 把涂鸦后的图发回来了');
          });
        });
      };
      var cfg = chatFindApi();
      if (!cfg) { finish(chatDoodleLocalItems(desc)); return; }
      var base = String(cfg.baseUrl || '').replace(/\/+$/, '');
      if (!/\/chat\/completions$/.test(base)) base += '/chat/completions';
      var sysTxt = '你是涂鸦排版助手。只输出一行 JSON，不要解释、不要代码块。';
      var usrTxt = '在一张照片上做涂鸦。涂鸦意图：' + String(desc || '').slice(0, 60) + '\n' +
        '请在 0~1000 × 0~1000 的坐标系里给出涂鸦元素，输出格式：{"items":[...]}。可用元素：' +
        '{"t":"text","x":数字,"y":数字,"s":字号20~90,"c":"#RRGGBB","r":旋转-25~25,"v":"文字(不超过8个字)"}、' +
        '{"t":"path","c":"#RRGGBB","w":线宽3~12,"pts":[[x,y],[x,y]]}、' +
        '{"t":"heart"|"star"|"circle","x":数字,"y":数字,"s":数字,"c":"#RRGGBB","r":旋转}。' +
        '最多 10 个元素，颜色鲜艳，坐标不要越界（0~1000），不要把整张图涂满。';
      fetch(base, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + cfg.apiKey },
        body: JSON.stringify({ model: cfg.model, messages: [{ role: 'system', content: sysTxt }, { role: 'user', content: usrTxt }], temperature: 0.9, max_tokens: 700, stream: false })
      }).then(function (r) { return r.text(); }).then(function (t) {
        var d = null;
        try { d = JSON.parse(String(t || '')); } catch (e) {}
        var txt = String((d && d.choices && d.choices[0] && d.choices[0].message && d.choices[0].message.content) || '');
        var items = null;
        var i1 = txt.indexOf('{');
        var i2 = txt.lastIndexOf('}');
        if (i1 >= 0 && i2 > i1) {
          try {
            var o = JSON.parse(txt.slice(i1, i2 + 1));
            if (o && o.items && o.items.length) items = o.items;
          } catch (e2) {}
        }
        finish((items && items.length) ? items : chatDoodleLocalItems(desc));
      })['catch'](function () { finish(chatDoodleLocalItems(desc)); });
    }
    /* 本地兜底涂鸦方案：保证「一定有结果」，不至于白等 */
    function chatDoodleLocalItems(desc) {
      var t = String(desc || '涂鸦').replace(/\s+/g, '').slice(0, 8) || '♪';
      var cols = ['#ff4d6d', '#ffd166', '#4dd4ac', '#5aa9ff', '#c77dff'];
      var pick = function () { return cols[Math.floor(Math.random() * cols.length)]; };
      var items = [{ t: 'text', x: 500, y: 150, s: 74, c: pick(), r: -8, v: t }];
      for (var i = 0; i < 3; i++) {
        items.push({ t: ['heart', 'star', 'circle'][i % 3], x: 90 + Math.random() * 820, y: 600 + Math.random() * 320, s: 46 + Math.random() * 40, c: pick(), r: Math.random() * 30 - 15 });
      }
      var pts = [];
      for (var j = 0; j <= 10; j++) pts.push([60 + j * 88, 470 + Math.sin(j * 0.9) * 42]);
      items.push({ t: 'path', c: pick(), w: 8, pts: pts });
      return items;
    }
    /* canvas 绘制：把涂鸦方案叠到原图上 */
    function chatDoodleDraw(baseSrc, items, cb) {
      var img = new Image();
      img.onload = function () {
        try {
          var MAX = 1080;
          ...
          var ctx = cv.getContext('2d');
          ctx.drawImage(img, 0, 0, cw, ch);
          var K = Math.min(cw, ch) / 1000;
          (items || []).forEach(function (it) {
            if (!it) return;
            var col = String(it.c || '#ff4d6d');
            var x = Number(it.x || 0) * cw / 1000, y = Number(it.y || 0) * ch / 1000;
            var sz = Math.max(12, Number(it.s || 40) * K);
            ctx.save();
            if (it.t === 'text') {
              ctx.translate(x, y);
              ctx.rotate((Number(it.r || 0)) * Math.PI / 180);
              ctx.font = '700 ' + sz + 'px "PingFang SC","Microsoft YaHei",sans-serif';
              ctx.textAlign = 'center';
              ctx.textBaseline = 'middle';
              ctx.lineWidth = Math.max(2, sz / 9);
              ctx.strokeStyle = 'rgba(255,255,255,.92)';
              ctx.strokeText(String(it.v || ''), 0, 0);
              ctx.fillStyle = col;
              ctx.fillText(String(it.v || ''), 0, 0);
            } else if (it.t === 'path' && it.pts && it.pts.length) {
              ctx.strokeStyle = col;
              ctx.lineWidth = Math.max(2, Number(it.w || 6) * K);
              ctx.lineCap = 'round';
              ctx.lineJoin = 'round';
              ctx.beginPath();
              it.pts.forEach(function (p, i) {
                var px = Number((p && p[0]) || 0) * cw / 1000;
                var py = Number((p && p[1]) || 0) * ch / 1000;
                if (i === 0) ctx.moveTo(px, py); else ctx.lineTo(px, py);
              });
              ctx.stroke();
            } else if (it.t === 'circle' || it.t === 'star' || it.t === 'heart') {
              ctx.translate(x, y);
              ctx.rotate((Number(it.r || 0)) * Math.PI / 180);
              ctx.fillStyle = col;
              ctx.beginPath();
              if (it.t === 'circle') {
                ctx.arc(0, 0, sz * 0.68, 0, Math.PI * 2);
                ctx.fill();
              } else if (it.t === 'star') {
                for (var i2 = 0; i2 < 10; i2++) {
                  var rr = (i2 % 2 === 0) ? sz : sz * 0.45;
                  var ang = -Math.PI / 2 + i2 * Math.PI / 5;
                  var px2 = Math.cos(ang) * rr, py2 = Math.sin(ang) * rr;
                ...
                ctx.bezierCurveTo(-sz * 1.2, -sz * 0.3, -sz * 0.34, -sz * 1.1, 0, -sz * 0.34);
                ctx.bezierCurveTo(sz * 0.34, -sz * 1.1, sz * 1.2, -sz * 0.3, 0, sz * 0.62);
                ctx.closePath();
                ctx.fill();
              }
            }
    ...
    /* v182：通用大图查看器——生成图 / 锁脸参考图 / 人设样貌图 / 聊天图都走这里，点任意处或 Esc 关闭
       v199：支持 opts.saveLabel —— 传了就在底部多一个「保存到相册」按钮（聊天图专用） */
    function viewImgFull(src, opts) {
      if (!src) return;
      opts = opts || {};
      var old = document.getElementById('imgFullViewer');
      if (old && old.parentNode) old.parentNode.removeChild(old);
      var ov = document.createElement('div');
      ov.id = 'imgFullViewer';
      ov.className = 'img-full-viewer';
      var pic = document.createElement('img');
      pic.className = 'img-full-pic';
      pic.alt = '';
      pic.src = src;
      var x = document.createElement('div');
      x.className = 'img-full-close';
      ...
      var tip = document.createElement('div');
      tip.className = 'img-full-tip';
      tip.textContent = opts.tip || '点按任意处关闭';
      ov.appendChild(pic); ov.appendChild(x); ov.appendChild(tip);
      var saveBtn = null;
      if (opts.saveLabel) {
        saveBtn = document.createElement('button');
        saveBtn.type = 'button';
        saveBtn.className = 'img-full-save';
        saveBtn.textContent = opts.saveLabel;
        saveBtn.addEventListener('click', function (e) {
          if (e && e.stopPropagation) e.stopPropagation();
          var ok = false;
          try { ok = opts.onSave ? !!opts.onSave() : false; } catch (eS) { ok = false; }
          if (ok) { saveBtn.textContent = '已保存到相册'; saveBtn.className = 'img-full-save done'; }
        });
    ...
    /* v199：聊天里的图，点开就是大图详情页 —— 左下角「保存到相册」按钮，长按菜单不再管存图 */
    function chatViewImg(src) {
      viewImgFull(src, {
        saveLabel: '保存到相册',
        tip: '点按空白处关闭 · 可保存到相册',
        onSave: function () { return chatImgSaveToAlbum(src); }
      });
    }
    /* v182 修复：聊天图原先用内联 onclick 调 chatViewImg，但本文件整体在 IIFE 内、函数未挂到 window，点图其实一直没反应 */
    try { window.viewImgFull = viewImgFull; window.chatViewImg = chatViewImg; } catch (e) {}
    ...
    function renderChatSettings() {
      ...
      if (chatSettingView === 'voiceinfo') return renderChatVoiceView();
      if (chatSettingView === 'voice') return renderChatVoiceView();
      if (chatSettingView === 'logs') return renderChatLogsView();
      if (chatSettingView === 'mylook') return renderChatMeView();
      if (chatSettingView === 'dataio') return renderChatDataIOView();
      if (chatSettingView === 'storage') return renderChatStorageView();
      ...
      var sections = [
        ...
        { key: 'chatmode', label: '聊天模式', desc: '旁白模式 · 普通线上' },
        { key: 'sec-sense', label: '交互感知', desc: '语音配置 · 我的形象 · 角色相册 · 真实生图 · 自主活动' },
        { key: 'sec-bg', label: '后台生活', desc: '角色关系 · 网恋定位 · 后台动态 · 时间感知' },
        { key: 'sec-app', label: '外观设置', desc: '聊天背景 · 气泡 · 字体 · 自定义CSS' },
        { key: 'sec-data', label: '数据关系', desc: '存储自检 · 调试日志 · 清空记录 · 拉黑联系人 · 导入导出' }
      ];
    ...
    function renderChatSectionView(sec) {
      ...
      var SEC_META = {
        ...
        sense: { title: '交互感知', items: [
          { key: 'voiceinfo', label: '语音配置', desc: '音色跟随名单角色 · 语言 / 语速 / 语音频率本窗口可调', value: (s.voice && s.voice.enabled) ? '已开启' : '未开启' },
          { key: 'mylook', label: '我的形象', desc: '我的外貌描写 · 生图提示词 · 锁脸', value: chatMeImgLabel(s) },
          { key: 'album', label: '相册', desc: '相册 · 点开直接翻看 · 长按某张可设为头像', value: chatConvAlbumLabel() },
          { key: 'roleimg', label: '真实生图', desc: '开：TA 主动发图时调生图 API 现画；关：从 TA 相册挑图，相册空则发一张文字图', value: '' },
          { key: 'doodle', label: '图片涂鸦回复', desc: '你发的图，TA 只在原图上叠加涂鸦（画几笔 / 贴纸 / 写字）再发回，绝不重绘原图', value: '' },
          { key: 'filechat', label: '文件互传', desc: '发文件给 TA，TA 能读到内容、在文件里作答或修改后把文件发回（txt/md/csv/json/docx 等）', value: '' },
          { key: 'autoavatar', label: 'TA 自发换头像', desc: '关闭（默认）：TA 不会自己换头像；打开后 TA 偶尔会把新照片换成头像，每次都会明确告诉你，可恢复上一张', value: '' },
        ...
        data: { title: '数据关系', items: [
          ...
          { key: 'storage', label: '存储自检', desc: '数据以本地数据库（IndexedDB）为准 · 看体积 / 镜像状态 · 一键回读', value: '' },
      ...
      /* v222：有「换头像前的旧图」时，多给一行「恢复上一张头像」 */
      if (sec === 'sense') {
        var _au = chatCurrentConv.settings.avatarUndo;
        if (_au && _au.prev) {
          var _ai = 0;
          for (var _aj = 0; _aj < SEC_META.sense.items.length; _aj++) { if (SEC_META.sense.items[_aj].key === 'autoavatar') { _ai = _aj; break; } }
          SEC_META.sense.items.splice(_ai + 1, 0, { key: 'avatarrestore', label: '恢复上一张头像', desc: 'TA 换头像前的那张还留着，点一下换回去', value: '' });
        }
      }
      ...
      chatSettingsBody.innerHTML = meta.items.map(function (it) {
        /* v198：「真实生图」用 SVG 拨动按键（开 / 关），不再显示文字值；v222：涂鸦 / 文件互传 / 自发换头像同款 */
        var _SW = {
          roleimg: chatRoleImgSwOn(chatCurrentConv.settings),
          doodle: chatDoodleSwOn(chatCurrentConv.settings),
          filechat: chatFileSwOn(chatCurrentConv.settings),
          autoavatar: chatAvatarSwOn(chatCurrentConv.settings)
        };
        if (_SW.hasOwnProperty(it.key)) {
          return '<div class="chat-setting-switch" data-skey="' + it.key + '" style="cursor:pointer">' +
            ...
            svgSwitchHtml(!!_SW[it.key], it.key) +
            '</div>';
        }
        return '<div class="chat-setting-switch" data-skey="' + it.key + '" style="cursor:pointer">' +
          '<div style="min-width:0"><div class="sw-label">' + it.label + '</div><div class="sw-desc">' + it.desc + '</div></div>' +
    ...
    /* v198：SVG 拨动按键——开（绿色轨道 + 勾）/ 关（灰色轨道 + 横杠），用于「真实生图」等开关项 */
    function svgSwitchHtml(on, key) {
      return '<button type="button" class="svg-sw' + (on ? ' on' : '') + '" data-sw="' + escHtml(String(key || '')) + '"' +
        ' aria-pressed="' + (on ? 'true' : 'false') + '" aria-label="' + (on ? '已开启' : '已关闭') + '">' +
        '<svg viewBox="0 0 52 30" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
        '<rect class="sws-track" x="1" y="1" width="50" height="28" rx="14"/>' +
        '<circle class="sws-knob" cx="' + (on ? '37' : '15') + '" cy="15" r="11"/>' +
        '<path class="sws-ico sws-on-ico" d="M33.4 15.4l2.6 2.6 5.2-5.8"/>' +
        '<path class="sws-ico sws-off-ico" d="M11.8 15h6.4"/>' +
        '</svg></button>';
    }
    /* v197：交互感知 · 真实生图开关（默认开启：有可用生图配置就现画） */
    function chatRoleImgSwInit(s) {
      if (!s) return { enabled: true };
      if (!s.roleGenImg || typeof s.roleGenImg !== 'object') s.roleGenImg = { enabled: true };
      return s.roleGenImg;
    }
    function chatRoleImgSwOn(s) {
      var r = chatRoleImgSwInit(s);
    ...
    function computeChatTokenRows() {
      ...
      var s = chatCurrentConv.settings;
      var msgs = chatCurrentConv.messages || [];
      var roleTxt = (typeof chatRolePersonaOf === 'function') ? chatRolePersonaOf(chatCurrentConv) : ((typeof s.roleIdentity === 'object' && s.roleIdentity) ? (s.roleIdentity.prompt || '') : s.roleIdentity);
      var myTxt = s.myIdentity || chatMine.identity || '';
      var sysIdx = activeSysIdx;
    ...
    /* ===== v201：调试日志改成整屏 ===== */
    var chatLogsFullEl = null;
    var chatLogsFullScope = 'conv';
    function chatLogsFullOpen(scope) {
      chatLogsFullScope = (scope === 'all') ? 'all' : 'conv';
      if (!chatLogsFullEl) {
        var ov = document.createElement('div');
        ov.className = 'logs-full';
        ov.id = 'logsFullOverlay';
        ov.innerHTML =
          '<div class="logs-full-hd">' +
          '<button type="button" class="logs-full-close" id="logsFullClose" title="关闭"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M6 6l12 12M18 6L6 18"/></svg></button>' +
          '<span class="logs-full-title">调试日志</span>' +
          '<span class="logs-full-cnt" id="logsFullCnt"></span>' +
          '</div>' +
          '<div class="logs-full-tabs">' +
          '<button type="button" class="logs-full-tab on" data-lscope="conv">本窗口</button>' +
          '<button type="button" class="logs-full-tab" data-lscope="all">全部窗口</button>' +
          '<span class="logs-full-sp"></span>' +
          '<button type="button" class="logs-full-op" id="logsFullCopy">复制</button>' +
          '<button type="button" class="logs-full-op" id="logsFullClear">清空</button>' +
          '</div>' +
          '<div class="logs-full-body" id="logsFullBody"></div>';
        document.body.appendChild(ov);
        chatLogsFullEl = ov;
        ov.querySelector('#logsFullClose').addEventListener('click', chatLogsFullClose);
        ov.querySelector('#logsFullCopy').addEventListener('click', chatLogsFullCopy);
        ov.querySelector('#logsFullClear').addEventListener('click', chatLogsFullClear);
        var tb = ov.querySelectorAll('[data-lscope]');
        for (var i = 0; i < tb.length; i++) {
          tb[i].addEventListener('click', function () {
            chatLogsFullScope = this.getAttribute('data-lscope');
            for (var k = 0; k < tb.length; k++) tb[k].classList.toggle('on', tb[k] === this);
            chatLogsFullRender();
          });
        }
      }
      chatLogsFullEl.classList.add('open');
      var tabs = chatLogsFullEl.querySelectorAll('[data-lscope]');
      for (var j = 0; j < tabs.length; j++) tabs[j].classList.toggle('on', tabs[j].getAttribute('data-lscope') === chatLogsFullScope);
      chatLogsFullRender();
    }
    function chatLogsFullClose() { if (chatLogsFullEl) chatLogsFullEl.classList.remove('open'); }
    function chatLogsFullRows() {
      var cid = chatCurrentConv ? chatCurrentConv.id : '';
      var rows = (chatLogsFullScope === 'all') ? chatErrLogs.slice() : chatErrLogs.filter(function (e) { return e && e.conv === cid; });
      if (!rows.length) {
        /* 会话级日志为空时，把全局控制台日志也带上，别让整屏空着 */
        try { if (typeof consoleLogs !== 'undefined' && consoleLogs.length) rows = consoleLogs.map(function (x) { return { t: (x && x.t) || Date.now(), msg: (x && x.msg) || String(x), src: '', line: 0 }; }); } catch (e) {}
      }
      return rows;
    }
    function chatLogsFullText() {
      var rows = chatLogsFullRows();
      var head = '===== 调试日志 · ' + (chatLogsFullScope === 'all' ? '全部窗口' : ((chatCurrentConv ? chatCurrentConv.name : '当前窗口') + '（本窗口）')) + ' · ' + rows.length + ' 条 · ' + new Date().toLocaleString() + ' =====';
      return head + '\n' + (rows.length ? rows.map(fmtLogEntry).join('\n') : '（暂无日志）');
    }
    function chatLogsFullRender() {
      if (!chatLogsFullEl) return;
      var body = chatLogsFullEl.querySelector('#logsFullBody');
      var cnt = chatLogsFullEl.querySelector('#logsFullCnt');
      var rows = chatLogsFullRows();
      if (cnt) cnt.textContent = rows.length + ' 条';
      if (body) body.textContent = rows.length ? rows.map(fmtLogEntry).join('\n') : '（暂无日志：这个窗口还没有报错记录）';
    }
    function chatLogsFullCopy() {
      var txt = chatLogsFullText();
      var done = function (ok) { toast(ok ? '日志已复制，可直接粘贴发我' : '复制失败，请手动长按选择复制'); };
      try {
    ...
    function chatLogsFullClear() {
      var cid = chatCurrentConv ? chatCurrentConv.id : '';
      if (chatLogsFullScope === 'all') { chatErrLogs = []; }
      else chatErrLogs = chatErrLogs.filter(function (e) { return !(e && e.conv === cid); });
      try { dbSet(CHAT_ERR_KEY, JSON.stringify(chatErrLogs)); } catch (e) {}
      chatLogsFullRender();
      toast('日志已清空');
    }
    window.chatLogsFullOpen = chatLogsFullOpen;
    ...
    // ===== 语音配置移步名单角色：会话语音跟随所属角色的桥接 =====
    function chatLangLabelOf(code) {
      var c = String(code || 'zh-CN');
      for (var i = 0; i < chatLangPresets.length; i++) {
        if (!chatLangPresets[i].sep && chatLangPresets[i].code === c) return chatLangPresets[i].label;
      }
      return c;
    }
    // 会话 → 名单角色（contactId 由角色ID派生，加好友时为 'm' + 角色ID）
    function memberByConv(conv) {
      try {
        if (!conv) return null;
        var cid = String(conv.contactId || '');
        if (!cid) return null;
        var mid = cid.replace(/^m/, '');
        var list = memberLoad();
        for (var i = 0; i < list.length; i++) {
          var id = String(list[i].id || '');
          if (id && (id === mid || id === cid)) return list[i];
        }
      } catch (e) {}
      return null;
    }
    // v177：聊天窗口的角色人设直接取「名单」里该角色的资料，不再在聊天界面单独填写/上传
    function chatRolePersonaOf(conv) {
      try {
        var m = memberByConv(conv);
        if (m) {
          var parts = [];
          if (m.realName) parts.push('本名：' + m.realName);
          if (m.netName) parts.push('网名：' + m.netName);
          var gL = m.gender === 'male' ? '男' : (m.gender === 'female' ? '女' : (m.gender === 'secret' ? '保密' : ''));
          if (gL) parts.push('性别：' + gL);
          if (m.lore) parts.push('身世：' + m.lore);
          if (m.look) parts.push('外貌：' + m.look);
          var nps = m.npcs || [];
          var rels = nps.filter(function (x) { return x && x.name && x.rel; }).map(function (x) { return x.name + '（' + x.rel + '）'; });
          if (rels.length) parts.push('关系网：' + rels.join('、'));
          var bios = nps.filter(function (x) { return x && x.name && x.bio; }).map(function (x) { return x.name + '：' + x.bio; });
          if (bios.length) parts.push('身边人物：' + bios.join('；'));
          if (parts.length) return parts.join('\n');
        }
      } catch (e) {}
      var s = conv && conv.settings;
      var ri = s ? s.roleIdentity : null;
      return (typeof ri === 'object' && ri) ? (ri.prompt || '') : (ri || '');
    }
    function chatVoiceDefaultObj() {
      var g = loadMMConfig();
      return { enabled: false, groupId: g.groupId || '', apiKey: g.apiKey || '', model: g.model || 'speech-01-hd', voiceId: '', lang: 'zh-CN', langLabel: '国语/普通话 (Chinese)', speed: 1, synthMusic: false, habit: { enabled: true, frequency: '中等', triggers: '', special: '' } };
    }
    // 把角色的语音配置覆盖到会话上（角色 → 会话，单向）
    function chatVoiceApplyMember(conv) {
      try {
        if (!conv) return false;
        if (!conv.settings) conv.settings = defaultConvSettings();
        var m = memberByConv(conv);
        if (!m) return false;
        var s = conv.settings;
        /* v177：角色人设一律以名单为准（本名/网名/性别/身世/外貌/关系网），聊天界面不再单独上传 */
        var _roleChanged = false;
        var _rp = chatRolePersonaOf(conv);
        /* v200：名单角色的头像要一路同步到会话（会话卡片 / 联系人 / 聊天窗口都读这里） */
        var _av = String((m && m.avatar) || '');
        if (s.roleIdentity && typeof s.roleIdentity === 'object') {
          if (String(s.roleIdentity.prompt || '') !== _rp) { s.roleIdentity.prompt = _rp; _roleChanged = true; }
          if (_av && s.roleIdentity.avatar !== _av) { s.roleIdentity.avatar = _av; _roleChanged = true; }
        } else {
          s.roleIdentity = { name: conv.name || '', avatar: _av, sex: m.gender === 'male' ? '男' : (m.gender === 'female' ? '女' : ''), prompt: _rp, remark: '' };
          _roleChanged = true;
        }
        if (!m.voice) return _roleChanged;
        var v = m.voice;
        if (!s.voice || typeof s.voice !== 'object') s.voice = chatVoiceDefaultObj();
        s.voice.enabled = v.enabled !== false;
        /* v177：只同步音色。语言 / 语速 / 语音频率由聊天窗口自己设置，不被名单覆盖 */
        s.voice.voiceId = String(v.voiceId || '');
        if (!s.voice.habit || typeof s.voice.habit !== 'object') s.voice.habit = { enabled: true, frequency: '中等', triggers: '', special: '' };
        return true;
      } catch (e) { return false; }
    }

    // 名单角色保存后，把语音同步到已存在的同名会话（contactId = 'm' + 角色ID）
    function chatVoiceSyncMemberConvs(memberId, voice) {
      try {
        if (!memberId) return;
        var mid = String(memberId);
        /* v200：顺手把角色的头像同步到会话，避免会话卡片 / 联系人里看不到头像 */
        var _mem = null;
        try {
          var _l = memberLoad();
          for (var _i = 0; _i < _l.length; _i++) { if (String(_l[_i].id || '') === mid) { _mem = _l[_i]; break; } }
        } catch (eM) {}
        var _mv = String((_mem && _mem.avatar) || '');
        var changed = false;
        chatConvs.forEach(function (conv) {
          var cid = String(conv.contactId || '');
          if (!cid || (cid !== mid && cid !== ('m' + mid))) return;
          if (!conv.settings) conv.settings = defaultConvSettings();
          var s = conv.settings;
          if (!s.voice || typeof s.voice !== 'object') s.voice = chatVoiceDefaultObj();
          s.voice.enabled = voice.enabled !== false;
          /* v177：只同步音色，语言 / 语速 / 语音频率以聊天窗口自己的设置为准 */
          s.voice.voiceId = String(voice.voiceId || '');
          if (!s.voice.habit || typeof s.voice.habit !== 'object') s.voice.habit = { enabled: true, frequency: '中等', triggers: '', special: '' };
          if (_mv) {
            if (!s.roleIdentity || typeof s.roleIdentity !== 'object') s.roleIdentity = { name: conv.name || '', avatar: '', sex: '', prompt: '', remark: '' };
            if (s.roleIdentity.avatar !== _mv) s.roleIdentity.avatar = _mv;
          }
          changed = true;
        });
    ...
    function chatVoiceInit() {
      ...
      // 语音配置已移步到名单角色：会话语音跟随所属角色
      if (chatVoiceApplyMember(chatCurrentConv)) saveConvs();
      return s;
    }
    ...
    // 语音配置总览（只读）：语音配置以名单角色为准，这里只展示当前会话语音来源
    function renderChatVoiceInfoView() {
      if (!chatCurrentConv) return;
      document.getElementById('chatSettingsTitle').textContent = '语音配置';
      var s = chatVoiceInit();
      var vv = (s && s.voice) ? s.voice : {};
      var m = memberByConv(chatCurrentConv);
      var html = '<div class="group-title">语音配置</div>';
      if (m) {
        html += '<div class="group-card">' +
          '<div class="settings-item"><label>跟随角色</label><div class="settings-right">' + escHtml(memberDispName(m)) + '</div></div>' +
          '<div class="settings-item"><label>启用语音合成 (TTS)</label><div class="settings-right">' + (vv.enabled ? '开启' : '关闭') + '</div></div>' +
          '<div class="settings-item"><label>语音 ID</label><div class="settings-right">' + escHtml(vv.voiceId || '未填') + '</div></div>' +
          '<div class="settings-item"><label>语音语言/方言</label><div class="settings-right">' + escHtml(vv.langLabel || '国语/普通话 (Chinese)') + '</div></div>' +
          '<div class="settings-item"><label>语速</label><div class="settings-right">' + ((vv.speed != null) ? vv.speed : 1) + 'x</div></div>' +
          '<div class="settings-item"><label>主动发语音频率</label><div class="settings-right">' + escHtml((vv.habit && vv.habit.frequency) || '中等') + '</div></div>' +
          '</div>';
        html += '<div class="chat-cfg-tip">该角色的音色 / 语言 / 语速 / 语音频率统一在「名单 → 编辑角色 → 角色音色与语音」里配置，聊天窗口不再单独配置，避免两处不一致。</div>';
        html += '<button class="prompt-cancel" id="voiceGoMember" style="width:100%;margin-top:8px">前往名单修改</button>';
      } else {
        html += '<div class="group-card"><div class="settings-item voice-stack"><label>未关联名单角色</label><div class="settings-right">本窗口沿用单独配置</div></div></div>';
        html += '<div class="chat-cfg-tip">本窗口的联系人不是从名单角色添加的，没有可跟随的角色语音，暂时可以在这里单独配置。</div>';
        html += '<button class="prompt-cancel" id="voiceSoloCfg" style="width:100%;margin-top:8px">单独配置本窗口语音</button>';
      }
      html += '<button class="prompt-cancel" id="voiceInfoBack" style="width:100%;margin-top:10px">返回设置</button>';
      chatSettingsBody.innerHTML = html;
      var gb = document.getElementById('voiceGoMember');
      if (gb) gb.addEventListener('click', function () { openMemberList(); });
      var sb = document.getElementById('voiceSoloCfg');
      if (sb) sb.addEventListener('click', function () { chatSettingView = 'voice'; renderChatSettings(); });
      var vb2 = document.getElementById('voiceInfoBack');
      if (vb2) vb2.addEventListener('click', function () { chatSettingsGoBack(); });
    }
    function renderChatVoiceView() {
      ...
      var s = chatVoiceInit();
      var _memRole = null;
      try { _memRole = memberByConv(chatCurrentConv); } catch (e) {}
      ...
      html += '<div class="group-card">' +
        ...
        (_memRole
          ? '<div class="settings-item"><label>语音 ID</label><div class="settings-right">' + escHtml(s.voice.voiceId || '未填') + '</div></div>'
      ...
      html += '<div class="group-card form-card">' +
        ...
        '</div>';
      if (_memRole) html += '<div class="chat-cfg-tip">音色 ID 取自名单角色「' + escHtml(memberDispName(_memRole)) + '」，在名单里改即时生效；语言 / 语速 / 语音频率按本窗口单独设置。</div>';
      ...
      document.getElementById('voiceTestBtn').addEventListener('click', function () {
        var mm = loadMMConfig();
        if (!mm || !mm.groupId || !mm.apiKey) { toast('请先配置 MiniMax 语音API（聊天设置 → 语音API）'); return; }
        var _vidEl = document.getElementById('ai-voice-id-input');
        var vid = _vidEl ? _vidEl.value.trim() : String(s.voice.voiceId || '').trim();
        if (!vid) { toast('这个窗口还没有音色，先到名单给角色填音色 ID'); return; }
    ...
    // ===== v193：我在这个窗口的形象（外貌描写 / 生图提示词 / 锁脸）=====
    function chatMeImgInit() {
      if (!chatCurrentConv) return null;
      var s = chatCurrentConv.settings;
      if (!s.meImg || typeof s.meImg !== 'object') s.meImg = { look: '', prompt: '', promptName: '', face: '', lockFace: true };
      return s.meImg;
    }
    function chatMeImgLabel(s) {
      try {
        var mi = s && s.meImg;
        if (!mi) return '未配置';
        var has = (mi.look && String(mi.look).trim()) || (mi.prompt && String(mi.prompt).trim()) || mi.face;
        if (!has) return '未配置';
        if ((mi.look && String(mi.look).trim()) && (mi.prompt && String(mi.prompt).trim())) return '已配置';
        return '部分配置';
      } catch (e) { return '未配置'; }
    }
    function chatConvAlbumLabel() {
      try {
        var m = memberByConv(chatCurrentConv);
        var n = (m && m.album && m.album.length) ? m.album.length : 0;
        return n ? (n + ' 张') : '空';
      } catch (e) { return '空'; }
    }
    /* 聊天设置里直接翻相册（复用名单里的全屏相册界面） */
    function chatRoleSendOnePhoto() {
      var conv = chatCurrentConv;
      if (!conv) return;
      var m = null;
      try { m = memberByConv(conv); } catch (e) {}
      var cfg = null;
      try { cfg = (typeof chatRoleImgApiCfg === 'function') ? chatRoleImgApiCfg(conv) : null; } catch (e2) {}
      if (cfg) { chatRoleGenImage(); return; }
      if (m && m.album && m.album.length) {
        var pick = m.album[Math.floor(Math.random() * m.album.length)];
        addChatMsg('other', { type: 'image', imgs: [pick.src], img: pick.src, picNote: pick.note || '' });
        toast('TA 发来一张照片');
        return;
      }
      toast('相册还是空的，先在聊天里点开图片，用「保存到相册」存几张');
    }
    /* v198：入口已改为「相册」点一下直接进全屏相册，这个二级弹层不再被调用（保留以备回调） */
    function openConvRoleAlbum() {
      if (!chatCurrentConv) return;
      var _m0 = memberByConv(chatCurrentConv);
      if (!_m0) { toast('这个会话还没绑定名单角色，先去「名单」里挑一个'); return; }
      var _cnt = (_m0.album && _m0.album.length) ? _m0.album.length : 0;
      chatMini('角色相册', '<div class="chat-mini-list">' +
        '<button class="chat-mini-opt" id="cvAlbumView">翻看相册（' + (_cnt ? _cnt + ' 张' : '空') + '）</button>' +
        '<button class="chat-mini-opt" id="cvAlbumAsk">让 TA 主动发一张照片给我</button></div>', '取消', function () {});
      var _v = document.getElementById('cvAlbumView');
      if (_v) _v.addEventListener('click', function () { chatMiniMask.classList.remove('show'); openConvRoleAlbumHome(); });
      var _a = document.getElementById('cvAlbumAsk');
      if (_a) _a.addEventListener('click', function () { chatMiniMask.classList.remove('show'); chatRoleSendOnePhoto(); });
    }
    function openConvRoleAlbumHome() {
      try {
        var m = memberByConv(chatCurrentConv);
        if (!m) { toast('这个会话还没绑定名单角色，先去「名单」里挑一个'); return; }
        var list = memberLoad();
        var idx = -1;
        for (var i = 0; i < list.length; i++) { if (String(list[i].id || '') === String(m.id || '')) { idx = i; break; } }
        if (idx < 0) { toast('名单里找不到这个角色'); return; }
        var role = list[idx];
        memEditIdx = idx;
        memAvatar = role.avatar || '';
        /* v198：相册图带上 savedBy（谁存的），左下角水印头像要用；空相册也允许打开看空态 */
        memAlbum = (role.album || []).map(function (x) { return { id: x.id, src: x.src, ts: x.ts, note: x.note, savedBy: x.savedBy }; });
        memDirty = false;
        openMemberAlbumHome();
      } catch (e) { toast('相册打不开：' + (e && e.message ? e.message : e)); }
    }
    function renderChatMeView() {
      if (!chatCurrentConv) return;
      document.getElementById('chatSettingsTitle').textContent = '我的形象';
      var mi = chatMeImgInit();
      var promptOpts = '<option value="-1">直接手写</option>';
      try {
        if (typeof imgPrompts !== 'undefined' && imgPrompts.length) {
          imgPrompts.forEach(function (pp, i) {
            promptOpts += '<option value="' + i + '"' + (mi.promptName && mi.promptName === pp.name ? ' selected' : '') + '>' + escHtml(pp.name) + '</option>';
          });
        }
      } catch (e) {}
      var html = '<div class="group-title">我的外貌描写<button type="button" class="tips-q" data-tip="mylook">?</button></div>';
      html += '<div class="group-card form-card">' +
        '<div class="field"><label>我长什么样</label><textarea class="chat-mini-input" id="meLook" rows="4" style="width:100%;resize:none" placeholder="例如：短发、圆脸、常穿白色卫衣">' + escHtml(mi.look || '') + '</textarea>' +
        '<div class="chat-cfg-tip">生成含你出镜的画面时，会带上这段描写。</div></div></div>';
      html += '<div class="group-title">我的生图提示词</div>';
      html += '<div class="group-card form-card">' +
        '<div class="field"><label>选择已保存提示词</label><select class="chat-mini-input" id="mePromptSel">' + promptOpts + '</select></div>' +
        '<div class="field"><label>提示词</label><textarea class="chat-mini-input" id="mePrompt" rows="3" style="width:100%;resize:none" placeholder="例如：日常抓拍感，暖光，浅景深">' + escHtml(mi.prompt || '') + '</textarea></div></div>';
      html += '<div class="group-title">我的锁脸</div>';
      html += '<div class="group-card form-card">' +
        '<div class="field"><label>我的样貌图</label><div style="display:flex;gap:10px;align-items:center;margin-top:2px">' +
        (mi.face ? '<img id="meFacePreview" src="' + mi.face + '" style="width:56px;height:56px;border-radius:10px;object-fit:cover;border:1px solid var(--bd)">' : '<div style="width:56px;height:56px;border-radius:10px;border:1px dashed var(--bd);display:flex;align-items:center;justify-content:center;color:var(--text-faint);font-size:11px">未上传</div>') +
        '<button class="prompt-cancel" id="meFaceBtn" style="flex:1">上传我的样貌图</button>' +
        (mi.face ? '<button class="prompt-cancel" id="meFaceDel" style="flex:0 0 auto">移除</button>' : '') +
        '</div><input type="file" id="meFaceInput" accept="image/*" style="display:none"></div>' +
        '<div class="field"><label>锁脸</label><div class="chat-setting-switch" style="cursor:pointer" id="meLockRow"><div style="min-width:0"><div class="sw-desc">按样貌图固定你的五官，生成时保持长相一致</div></div><button class="chat-sw ' + (mi.lockFace ? 'on' : '') + '" id="meLock"></button></div></div>' +
        '</div>';
      html += '<button class="prompt-cancel" id="meSave" style="width:100%;margin-top:12px">保存</button>';
      html += '<button class="prompt-cancel" id="meBack" style="width:100%;margin-top:8px">返回设置</button>';
      chatSettingsBody.innerHTML = html;
      var prev = document.getElementById('meFacePreview');
      if (prev) { prev.style.cursor = 'zoom-in'; prev.addEventListener('click', function () { viewImgFull(prev.src); }); }
      document.getElementById('meLockRow').addEventListener('click', function () {
        mi.lockFace = !mi.lockFace; saveConvs(); renderChatMeView();
        toast(mi.lockFace ? '已开启锁脸' : '已关闭锁脸');
      });
      var fi = document.getElementById('meFaceInput');
      document.getElementById('meFaceBtn').addEventListener('click', function () { fi.click(); });
      fi.addEventListener('change', function () {
        var f = fi.files && fi.files[0];
        if (!f) return;
        if (f.size > 5 * 1024 * 1024) { toast('图片不能超过5MB'); return; }
        var rd = new FileReader();
        rd.onload = function () { mi.face = rd.result; saveConvs(); renderChatMeView(); toast('已上传我的样貌图'); };
        rd.readAsDataURL(f);
      });
      var fd = document.getElementById('meFaceDel');
      if (fd) fd.addEventListener('click', function () { mi.face = ''; saveConvs(); renderChatMeView(); toast('已移除我的样貌图'); });
      var ps = document.getElementById('mePromptSel');
      if (ps) ps.addEventListener('change', function () {
        var i = parseInt(ps.value, 10);
        if (i >= 0 && typeof imgPrompts !== 'undefined' && imgPrompts[i]) {
          mi.promptName = imgPrompts[i].name || '';
          document.getElementById('mePrompt').value = imgPrompts[i].pos || '';
          toast('已选用提示词：' + (imgPrompts[i].name || ''));
        } else { mi.promptName = ''; }
      });
      document.getElementById('meSave').addEventListener('click', function () {
        mi.look = document.getElementById('meLook').value.trim();
        mi.prompt = document.getElementById('mePrompt').value.trim();
        saveConvs();
        toast('已保存我的形象');
        chatSettingView = 'sec-sense';
        renderChatSettings();
      });
      document.getElementById('meBack').addEventListener('click', function () { chatSettingsGoBack(); });
    }

    /* ===== v222：存储自检页（数据以 DB 为准，这里能看能修）===== */
    function chatBytesLabel(n) {
      if (!n) return '0 B';
      if (n < 1024) return n + ' B';
      if (n < 1024 * 1024) return (n / 1024).toFixed(1) + ' KB';
      return (n / 1024 / 1024).toFixed(2) + ' MB';
    }
    function renderChatStorageView() {
      if (!chatCurrentConv) return;
      document.getElementById('chatSettingsTitle').textContent = '存储自检';
      var st = dbStats();
      var rows = '<div class="group-card">' +
        '<div class="chat-setting-switch"><div style="min-width:0"><div class="sw-label">权威存储 · IndexedDB</div>' +
        '<div class="sw-desc">' + (st.idb ? (st.loaded ? '已就绪 · 全部数据都存在这里' : '正在打开…') : '本浏览器不支持，已退回本地缓存') + '</div></div>' +
        '<span class="chat-setting-value">' + (st.idb ? '可用' : '不可用') + '</span></div>' +
        '<div class="chat-setting-switch" style="margin-top:10px"><div style="min-width:0"><div class="sw-label">数据量</div>' +
        '<div class="sw-desc">共 ' + st.keys + ' 条键 · 合计 ' + chatBytesLabel(st.bytes) + '</div></div></div>' +
        '<div class="chat-setting-switch" style="margin-top:10px"><div style="min-width:0"><div class="sw-label">开机镜像 · localStorage</div>' +
        '<div class="sw-desc">' + (st.mirrorFails.length ? ('有 ' + st.mirrorFails.length + ' 条没写进镜像（' + escHtml(st.mirrorFails[0].why) + '）——不影响数据，只在打开瞬间多等一拍') : '全部已同步（镜像只是开机第一帧的显示缓存）') + '</div></div>' +
        '<span class="chat-setting-value">' + (st.mirrorFails.length ? '部分' : '正常') + '</span></div>' +
        '</div>';
      if (st.big.length) {
        rows += '<div class="group-title">较大的数据</div><div class="group-card">' +
          st.big.slice(0, 6).map(function (b) {
            return '<div class="chat-setting-switch"><div style="min-width:0"><div class="sw-label">' + escHtml(b.key) + '</div>' +
              '<div class="sw-desc">' + chatBytesLabel(b.bytes) + ' · 已存进数据库，刷新不会丢</div></div></div>';
          }).join('') + '</div>';
      }
      rows += '<button class="prompt-cancel" id="stoReload" style="width:100%;margin-top:12px">从数据库回读一次（刷新界面）</button>' +
        '<button class="prompt-cancel" id="stoFlush" style="width:100%;margin-top:8px">把当前数据立刻写回数据库</button>' +
        '<button class="prompt-cancel" id="stoBack" style="width:100%;margin-top:8px">返回设置</button>';
      chatSettingsBody.innerHTML = rows;
      var rl = document.getElementById('stoReload');
      if (rl) rl.addEventListener('click', function () {
        var got = dbHydrateChatData(true);
        renderChatStorageView();
        toast(got ? '已从数据库回读并刷新界面' : '数据库里的数据和当前一致，无需回读');
      });
      var fl = document.getElementById('stoFlush');
      if (fl) fl.addEventListener('click', function () { dbFlush(); renderChatStorageView(); toast('当前数据已写回本地数据库'); });
      var bk = document.getElementById('stoBack');
      if (bk) bk.addEventListener('click', function () { chatSettingsGoBack(); });
    }
    ...
    function chatImagInit() {
      ...
      chatDoodleSwInit(chatCurrentConv.settings);
      chatFileSwInit(chatCurrentConv.settings);
    ...
    function renderChatImagView() {
      ...
      var _dd = chatDoodleSwInit(chatCurrentConv.settings);
      var _aa2 = chatAvatarSwInit(chatCurrentConv.settings);
      var _fc = chatFileSwInit(chatCurrentConv.settings);
      var html = '<div class="group-card"><div class="chat-setting-switch" style="cursor:pointer" id="imagToggleRow">' +
        ...
        '<button class="chat-sw ' + (im.enabled ? 'on' : '') + '" id="imagToggle"></button></div>' +
        '<div class="chat-setting-switch" style="cursor:pointer;margin-top:10px" id="doodleToggleRow">' +
        '<div style="min-width:0"><div class="sw-label">图片涂鸦回复</div><div class="sw-desc">你发的图，TA 只在原图上叠加涂鸦（画几笔 / 贴纸 / 写字）再发回，绝不重绘原图</div></div>' +
        '<button class="chat-sw ' + (_dd.enabled !== false ? 'on' : '') + '" id="doodleToggle"></button></div>' +
        '<div class="chat-setting-switch" style="cursor:pointer;margin-top:10px" id="fileChatToggleRow">' +
        '<div style="min-width:0"><div class="sw-label">文件互传</div><div class="sw-desc">发文件给 TA，TA 能读到内容、在文件里作答或修改后把文件发回（txt/md/csv/json/docx 等）</div></div>' +
        '<button class="chat-sw ' + (_fc.enabled !== false ? 'on' : '') + '" id="fileChatToggle"></button></div>' +
        '<div class="chat-setting-switch" style="cursor:pointer;margin-top:10px" id="autoAvatarToggleRow">' +
        '<div style="min-width:0"><div class="sw-label">TA 自发换头像</div><div class="sw-desc">关闭（默认）：TA 不会自己换头像；打开后 TA 偶尔会自己换，每次都会明确告诉你，可恢复上一张</div></div>' +
        '<button class="chat-sw ' + (_aa2.enabled === true ? 'on' : '') + '" id="autoAvatarToggle"></button></div></div>';
      ...
      var ddRow = document.getElementById('doodleToggleRow');
      if (ddRow) ddRow.addEventListener('click', function () {
        var _d2 = chatDoodleSwInit(chatCurrentConv.settings);
        _d2.enabled = !(_d2.enabled !== false);
        saveConvs(); renderChatImagView();
        toast(_d2.enabled ? '图片涂鸦回复已开启' : '图片涂鸦回复已关闭');
      });
      var fcRow = document.getElementById('fileChatToggleRow');
      if (fcRow) fcRow.addEventListener('click', function () {
        var _f2 = chatFileSwInit(chatCurrentConv.settings);
        _f2.enabled = !(_f2.enabled !== false);
        saveConvs(); renderChatImagView();
        toast(_f2.enabled ? '文件互传已开启' : '文件互传已关闭');
      });
      var aaRow = document.getElementById('autoAvatarToggleRow');
      if (aaRow) aaRow.addEventListener('click', function () {
        var _aa3 = chatAvatarSwInit(chatCurrentConv.settings);
        _aa3.enabled = !_aa3.enabled;
        saveConvs(); renderChatImagView();
        toast(_aa3.enabled ? 'TA 自发换头像已开启（每次都会告诉你）' : 'TA 自发换头像已关闭：只有你要求时 TA 才换');
      });
      var facePrev = document.getElementById('imagFacePreview');
      if (facePrev) {
        facePrev.style.cursor = 'zoom-in';
        facePrev.addEventListener('click', function () { viewImgFull(facePrev.src); });
      }
    ...
    function renderChatDataIOView() {
      ...
      document.getElementById('dataioExportHtml').addEventListener('click', function () {
        ...
        var rows = msgs.map(function (m) {
          if (m.type === 'sys') {
            return '<div class="row sys"><div class="txt">' + String(m.text || '').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br>') + '</div></div>';
          }
    ...
    function applyChatAppearance() {
      ...
      /* v206：把当前生效的气泡底色注入 CSS 变量，让旁白 / 动作 / 线下等模式的初始底色与线上普通聊天完全一致 */
      body.style.setProperty('--bub-me-bg', myBg);
      body.style.setProperty('--bub-other-bg', otherBg);
    ...
    function ensureAppearanceDefaults(s) {
      ...
      if (s.chatMode === 'offline') s.chatMode = 'online';
      if (!s.fontModes) s.fontModes = {};
    ...
    function renderChatAppearanceView() {
      ...
      html += apSubItem('font', '字体设置', '普通线上模式 · 旁白体模式', '<svg viewBox="0 0 24 24"><path d="M4 7V5h16v2"/><path d="M12 5v14"/><path d="M9 19h6"/></svg>');
    ...
    var AP_FONT_FAMILY_MAP = { online: 'chatFontOnline', narrator: 'chatFontNarrator' };
    var AP_PRESETS = [
    ...
    function rebuildFontFaces(s) {
      var css = '';
      ['online', 'narrator'].forEach(function (k) {
        var m = s.fontModes[k];
        if (m && m.fontData) {
    ...
    function renderChatModeView() {
      ...
      /* v202：新增动作描写模式（上动作神态、下分句底色对话） */
      html += '<div class="ap-mode-card' + (s.chatMode === 'action' ? ' active' : '') + '" data-chatmode="action">' +
        '<span class="ap-mc-dot"></span><div style="flex:1;min-width:0"><div class="ap-mc-title">动作描写模式</div>' +
        '<div class="ap-mc-desc">每条回复先给一段动作神态描写，对话按句拆开、各带底色，像剧本一样读。</div></div></div>';
      if (s.chatMode === 'narrator' || s.chatMode === 'action') {
        html += '<div class="group-title">' + (s.chatMode === 'action' ? '描写字数' : '旁白字数') + '</div><div class="group-card form-card">' +
          '<div class="chat-setting-switch" id="apNarrRow" style="cursor:pointer"><div style="min-width:0"><div class="sw-label">' + (s.chatMode === 'action' ? '动作神态描写字数' : '旁白描写字数') + '</div><div class="sw-desc">控制每条旁白最少 / 最多多少字</div></div><span class="chat-setting-value">' + chatNarrLabel(s) + ' <span style="color:#5ac8fa">&#8250;</span></span></div></div>';
      }
      ...
      chatSettingsBody.querySelectorAll('.ap-mode-card[data-chatmode]').forEach(function (card) {
        card.addEventListener('click', function () {
          ...
          s.chatMode = key;
          applyFontModeToLive(s, key === 'narrator' ? 'narrator' : 'online');
          saveConvs(); renderChatModeView(); applyChatAppearance();
          toast('已切换为：' + (key === 'narrator' ? '旁白模式' : (key === 'action' ? '动作描写模式' : '普通线上模式')));
        });
      });
      ...
      var nrRow = document.getElementById('apNarrRow');
      if (nrRow) nrRow.addEventListener('click', function () {
        chatNarrInit(s);
        chatMini('旁白字数', '<div class="chat-swipe-card" style="margin:0"><div class="chat-swipe-card-title">每条旁白写多少字</div><div class="chat-swipe-card-text">旁白模式会在正文前插入一句场景 / 环境 / 神态描写，这里控制它的字数范围。留空 = 使用默认 20~45 字。</div></div><div class="chat-sent-row"><input class="chat-mini-input" id="cmNarrMin" type="number" min="1" max="500" placeholder="最少（默认20）" value="' + (s.narrMin != null ? s.narrMin : '') + '"><input class="chat-mini-input" id="cmNarrMax" type="number" min="1" max="500" placeholder="最多（默认45）" value="' + (s.narrMax != null ? s.narrMax : '') + '"></div><div class="chat-cfg-tip">例如：最少 20 / 最多 45，旁白就会写成 20~45 字。</div>', '保存', function () {
          var nMin = parseInt(document.getElementById('cmNarrMin').value, 10);
          var nMax = parseInt(document.getElementById('cmNarrMax').value, 10);
          s.narrMin = (!isNaN(nMin) && nMin >= 1) ? Math.min(nMin, 500) : null;
          s.narrMax = (!isNaN(nMax) && nMax >= 1) ? Math.min(nMax, 500) : null;
          if (s.narrMin != null && s.narrMax != null && s.narrMin > s.narrMax) { var _ns = s.narrMin; s.narrMin = s.narrMax; s.narrMax = _ns; }
          saveConvs(); renderChatModeView(); toast('旁白字数：' + chatNarrLabel(s));
        });
      });
      document.getElementById('apModeBack').addEventListener('click', function () { chatSettingsGoBack(); });
    }
    /* v196：旁白字数自定义 —— 最少/最多字数（默认 20~45） */
    function chatNarrInit(s) {
      if (!s) return;
      if (s.narrMin != null && !(s.narrMin >= 1)) s.narrMin = null;
      if (s.narrMax != null && !(s.narrMax >= 1)) s.narrMax = null;
      if (s.narrMin != null && s.narrMax != null && s.narrMin > s.narrMax) { var _nt = s.narrMin; s.narrMin = s.narrMax; s.narrMax = _nt; }
    }
    function chatNarrRange(s) {
      chatNarrInit(s);
      return {
        min: (s && s.narrMin != null) ? s.narrMin : 20,
        max: (s && s.narrMax != null) ? s.narrMax : 45
      };
    }
    function chatNarrLabel(s) {
      var r = chatNarrRange(s);
    ...
    function onChatSetting(key, fromSw) {
      ...
      if (key === 'model') {
        ...
        renderChatSettings();
      } else if (key === 'voiceinfo') {
        chatSettingView = 'voiceinfo';
        renderChatSettings();
      } else if (key === 'voice') {
        chatSettingView = 'voice';
        renderChatSettings();
      } else if (key === 'logs') {
        chatLogsFullOpen('conv');
      } else if (key === 'mylook') {
        chatSettingView = 'mylook';
        renderChatSettings();
      } else if (key === 'roleimg') {
        var _rgen = chatRoleImgSwInit(chatCurrentConv.settings);
        _rgen.enabled = (_rgen.enabled === false);
        saveConvs();
        renderChatSettings();
        toast(_rgen.enabled ? '真实生图已开启：TA 主动发图时现画一张' : '真实生图已关闭：TA 发图时从相册挑图，相册空则发一张文字图');
      } else if (key === 'doodle') {
        var _dd2 = chatDoodleSwInit(chatCurrentConv.settings);
        _dd2.enabled = !(_dd2.enabled !== false);
        saveConvs();
        renderChatSettings();
        toast(_dd2.enabled ? '图片涂鸦回复已开启：只在你的原图上叠涂鸦' : '图片涂鸦回复已关闭');
      } else if (key === 'filechat') {
        var _fc2 = chatFileSwInit(chatCurrentConv.settings);
        _fc2.enabled = !(_fc2.enabled !== false);
        saveConvs();
        renderChatSettings();
        toast(_fc2.enabled ? '文件互传已开启' : '文件互传已关闭');
      } else if (key === 'autoavatar') {
        var _aa = chatAvatarSwInit(chatCurrentConv.settings);
        _aa.enabled = !_aa.enabled;
        saveConvs();
        renderChatSettings();
        toast(_aa.enabled ? 'TA 自发换头像已开启：TA 偶尔会自己换头像，每次都会告诉你' : 'TA 自发换头像已关闭：只有你要求时 TA 才换头像');
      } else if (key === 'avatarrestore') {
        chatAvatarRestore(chatCurrentConv);
        chatSettingView = 'sec-sense';
        renderChatSettings();
      } else if (key === 'storage') {
        chatSettingView = 'storage';
        renderChatSettings();
      } else if (key === 'album') {
        /* v198：不再弹二级菜单，点一下直接进相册翻看 */
        openConvRoleAlbumHome();
    ...
    // 身份编辑（我的身份 / 角色身份）
    /* ===== v194：角色身份面板里的「TA 在名单里的资料」编辑区 =====
       以前角色身份只能改备注名和头像，其它字段全是只读的，用户看着像「锁死、没有窗口」。
       现在把该角色在名单里的全部资料（本名 / 网名 / 性别 / 身世 / 外貌 / 音色 / NPC 关系网）
       直接搬进这个面板，保存即写回名单，立刻生效。 */
    function identityMemberRowHtml(n, i) {
      n = n || {};
      return '<div class="group-card form-card idm-npc" data-i="' + i + '" data-id="' + escHtml(String(n.id || '')) + '" style="margin:0 0 8px">' +
        '<div class="field"><label>NPC 名字</label><input type="text" data-f="name" value="' + escHtml(n.name || '') + '" placeholder="NPC 名字"></div>' +
        '<div class="field"><label>与 TA 的关系</label><input type="text" data-f="rel" value="' + escHtml(n.rel || '') + '" placeholder="例如：室友 / 同事 / 发小"></div>' +
        '<div class="field"><label>NPC 简介</label><textarea data-f="bio" rows="2" placeholder="这个 NPC 是谁、什么性格…">' + escHtml(n.bio || '') + '</textarea></div>' +
        '<button type="button" class="wb-import" data-idmdel="1" style="width:100%">删除这个 NPC</button>' +
        '</div>';
    }
    function identityMemberBoxHtml(m) {
      m = m || {};
      var sex = (m.gender === 'male') ? '男' : (m.gender === 'female' ? '女' : '保密');
      var gopts = ['男', '女', '保密'].map(function (x) { return '<option value="' + x + '"' + (x === sex ? ' selected' : '') + '>' + x + '</option>'; }).join('');
      var vopts = '<option value="">（不设置，用默认音色）</option>';
      Object.keys(VOICE_NAMES).forEach(function (k) {
        vopts += '<option value="' + escHtml(k) + '"' + (String(m.voiceId || '') === k ? ' selected' : '') + '>' + escHtml(VOICE_NAMES[k]) + '（' + k + '）</option>';
      });
      if (m.voiceId && !VOICE_NAMES[m.voiceId]) vopts = '<option value="' + escHtml(String(m.voiceId)) + '" selected>' + escHtml(String(m.voiceId)) + '</option>' + vopts;
      var npcs = (m.npcs || []);
      var npcHtml = npcs.map(function (n, i) { return identityMemberRowHtml(n, i); }).join('');
      return '<div class="group-title">TA 在「名单」里的资料（改这里 = 改名单，点保存立刻生效）</div>' +
        '<div class="group-card form-card">' +
        '<div class="field"><label>本名</label><input type="text" id="idmRealName" value="' + escHtml(m.realName || '') + '" placeholder="TA 的本名"></div>' +
        '<div class="field"><label>网名</label><input type="text" id="idmNetName" value="' + escHtml(m.netName || '') + '" placeholder="不超过 7 个字"></div>' +
        '<div class="field"><label>性别</label><select id="idmSex">' + gopts + '</select></div>' +
        '<div class="field"><label>身世 / 人设</label><textarea id="idmLore" rows="6" placeholder="成长经历、性格、身份、习惯、秘密…">' + escHtml(m.lore || '') + '</textarea></div>' +
        '<div class="field"><label>外貌描写</label><textarea id="idmLook" rows="3" placeholder="外貌描写，会用于生图">' + escHtml(m.look || '') + '</textarea></div>' +
        '<div class="field"><label>音色</label><select id="idmVoice">' + vopts + '</select></div>' +
        '</div>' +
        '<div class="group-title">TA 的 NPC（关系网，共 ' + npcs.length + ' 个）</div>' +
        '<div id="idmNpcWrap">' + npcHtml + '</div>' +
        '<button type="button" class="wb-import" id="idmNpcAdd" style="width:100%;margin-top:4px">＋ 新增一个 NPC</button>';
    }
    function identityBindNpcDel() {
      var box = document.getElementById('identityMemberBox');
      if (!box) return;
      box.querySelectorAll('[data-idmdel]').forEach(function (b) {
        if (b.__idmBound) return;
        b.__idmBound = true;
        b.addEventListener('click', function () {
    ...
    function renderIdentityMemberBox() {
      var box = document.getElementById('identityMemberBox');
      if (!box) return;
      var m = null;
      try { m = memberByConv(chatCurrentConv); } catch (e) {}
      /* v195：聊天设置 → 角色身份只保留「头像更换 + 备注名」，
         TA 的本名/网名/性别/身世/外貌/音色/NPC 等完整资料一律在「名单」里留存，这里不再铺开长表单 */
      box.style.display = 'none';
      box.innerHTML = '';
      if (identityMode !== 'role' || !m) return;
      return;
      var add = document.getElementById('idmNpcAdd');
      if (add) add.addEventListener('click', function () {
        var wrap = document.getElementById('idmNpcWrap');
        if (!wrap) return;
        var d = document.createElement('div');
        d.innerHTML = identityMemberRowHtml({ name: '', rel: '', bio: '' }, wrap.children.length);
        if (d.firstChild) wrap.appendChild(d.firstChild);
        identityBindNpcDel();
      });
      identityBindNpcDel();
    }
    function identityMemberSave() {
      var box = document.getElementById('identityMemberBox');
      ...
      role.realName = g('idmRealName');
      role.netName = g('idmNetName');
      var sx = g('idmSex');
      role.gender = (sx === '男') ? 'male' : (sx === '女' ? 'female' : 'secret');
      role.lore = g('idmLore');
      role.look = g('idmLook');
      role.voiceId = g('idmVoice');
      try { role.voice = role.voice || {}; role.voice.enabled = true; role.voice.voiceId = role.voiceId; } catch (e2) {}
      var npcs = [];
      box.querySelectorAll('#idmNpcWrap .idm-npc').forEach(function (row) {
        var val = function (f) { var el = row.querySelector('[data-f="' + f + '"]'); return el ? String(el.value || '').trim() : ''; };
        var nm = val('name');
        if (!nm) return;
        npcs.push({ id: String(row.getAttribute('data-id') || ''), name: nm, rel: val('rel'), bio: val('bio') });
      });
      role.npcs = npcs;
      memberSave(list);
      return true;
    ...
    function openIdentityEditor(mode) {
      var _g0 = document.getElementById('identityGotoMemberBtn');
      if (_g0) _g0.style.display = (mode === 'role') ? '' : 'none';
      if (!chatCurrentConv) return;
      identityMode = mode;
      var s = chatCurrentConv.settings;
      ...
      if (mode === 'my') {
        ...
        document.getElementById('identityPromptTitle').style.display = '';
        document.getElementById('identityPromptCard').style.display = '';
        document.getElementById('identityImportBtn').style.display = '';
        document.getElementById('identityRoleFromMember').style.display = 'none';
        ...
        var _idcMem = null;
        try { _idcMem = memberByConv(chatCurrentConv); } catch (e) {}
        var _idcFromMem = !!_idcMem;
        /* v195：角色身份页只留「头像 + 备注名」，人设卡片与导入入口一律收起，完整资料回名单留存 */
        document.getElementById('identityPromptTitle').style.display = 'none';
        document.getElementById('identityPromptCard').style.display = 'none';
        document.getElementById('identityImportBtn').style.display = 'none';
        var _idcTip = document.getElementById('identityRoleFromMember');
        if (_idcTip) {
          _idcTip.style.display = '';
          _idcTip.textContent = '这里只管「头像更换 + 备注名」。TA 的完整资料（本名 / 网名 / 性别 / 身世 / 外貌 / 音色 / 关系网）都留在「名单」里，去名单里改完立刻生效。';
        }
        var _gotoMem = document.getElementById('identityGotoMemberBtn');
        if (_gotoMem) _gotoMem.style.display = '';
      ...
      /* v194：角色身份面板——绑定了名单角色时，隐藏「角色名 / 性别」这两个其实写不到名单里的控件，
         改用下面的「TA 在名单里的资料」区承载全部可编辑字段 */
      /* v195：角色身份页只留「头像 + 备注名」；角色名/性别行也一并藏起（这些在名单里编辑） */
      try {
        var _idpHide = (mode === 'role');
        var _nmF = document.getElementById('identityNameField');
        var _sxT = document.getElementById('identitySexTitle');
        var _sxR = document.getElementById('identitySexRow');
        if (_nmF) _nmF.style.display = _idpHide ? 'none' : '';
        if (_sxT) _sxT.style.display = _idpHide ? 'none' : '';
        if (_sxR) _sxR.style.display = _idpHide ? 'none' : '';
      } catch (e4) {}
      renderIdentityMemberBox();
      chatSettingsPanel.classList.remove('open');
    ...
    function saveIdentity() {
      ...
      if (identityMode === 'my') {
        ...
        /* v194：把「TA 在名单里的资料」区写回名单；人设 / 性别 / 外貌 / NPC 立即对 TA 生效 */
        var _mSaved = false;
        try { _mSaved = identityMemberSave(); } catch (e5) {}
        var _mm2 = null;
        try { _mm2 = memberByConv(chatCurrentConv); } catch (e6) {}
        if (_mm2) {
          var ri3 = chatCurrentConv.settings.roleIdentity || {};
          ri3.name = _mm2.realName || _mm2.netName || chatCurrentConv.name;
          ri3.sex = (_mm2.gender === 'male' ? '男' : (_mm2.gender === 'female' ? '女' : ''));
          ri3.prompt = memberRolePromptOf(_mm2);
          chatCurrentConv.settings.roleIdentity = ri3;
          if (!String(ri3.remark || '').trim()) chatCurrentConv.name = memberDispName(_mm2);
        }
      }
      saveConvs();
      ...
      chatDetailNameSet(((typeof ri2 === 'object' && ri2 && ri2.remark) ? ri2.remark : '') || chatCurrentConv.name);
      renderChatSettings();
      renderChatConvs();
      renderContacts();
      renderChatMessages();
    ...
    function extractDocText(buf) {
      ...
      /* v208：OLE 复合文档里正文流前面隔着扇区表/目录等二进制，正文起点不一定落在偶数位，
         所以 16 位扫描从第 0 字节和第 1 字节两种对齐各扫一遍，谁捞到的可打印块长就用谁。 */
      function docUtf16Scan(off) {
        var out = '', run = '';
        for (var j = off; j + 1 < u8.length; j += 2) {
          var c = u8[j] | (u8[j + 1] << 8);
          var printable = (c >= 0x20 && c < 0x7F) || c === 0x0A || c === 0x0D || (c >= 0x4E00 && c <= 0x9FFF) || (c >= 0x3000 && c <= 0x303F) || (c >= 0xFF00 && c <= 0xFFEF);
          if (printable) {
            run += String.fromCharCode(c === 0x0D || c === 0x0A ? 10 : c);
          } else {
      ...
      var cur = '';
      [0, 1].forEach(function (off) { var t16 = docUtf16Scan(off); if (t16.length > best.length) best = t16; });
      ...
      if (cur.length > best.length) best = cur;
      best = best.replace(/\n{3,}/g, '\n\n').trim();
      /* v208：还有一类 .doc 是把正文按 GBK（WPS 常见）或 UTF-8 存的 8 位字符，
         上面两条通道捞不出中文 —— 这里整段按 gbk / utf-8 解一遍，逐行剔掉二进制噪声，
         再按「中文有效字数」择优（中文明显更多的那个胜出）。 */
      function docCleanLines(raw) {
        var arr = raw.split(/[\u0000\r\n]+/).map(function (x) {
          return x.replace(/[\u0001-\u0008\u000B\u000C\u000E-\u001F\u007F-\u009F]/g, '').trim();
        }).filter(function (x) {
          if (x.length < 2) return false;
          /* v208b：OLE 复合文档里的流名/字体名（WordDocument、1Table 之类）不是正文，别带进来 */
          if (/^(worddocument|\d?table|compobj|objectpool|root entry|data|summaryinformation|documentsummaryinformation|msworddoc|word\.document\.?\d*|microsoft word|microsoft office|times new roman|arial|calibri|宋体|normal\.dot|bjbj|jbjb|wps office|ms word)$/i.test(x)) return false;
          var cjk = (x.match(/[\u4E00-\u9FFF\u3400-\u4DBF]/g) || []).length;
          var word = (x.match(/[A-Za-z0-9]/g) || []).length;
          if (cjk < 1 && word < 4) return false;
          var bad = (x.match(/[^\u4E00-\u9FFF\u3400-\u4DBF\u3000-\u303F\uFF00-\uFFEFA-Za-z0-9\s，。、；：？！（）《》【】…—·.,:;?!'"()\-]/g) || []).length;
          return (bad / x.length) < 0.3;
        });
        /* 正文前头若还挂着孤零零的英文词（流名残留），一并去掉 */
        var cut = 0;
        while (cut < arr.length && cut < 3 && /^[A-Za-z0-9._\-]{2,24}$/.test(arr[cut])) cut++;
        return arr.slice(cut).join('\n');
      }
      var cjkCount = function (t) { return (t.match(/[\u4E00-\u9FFF]/g) || []).length; };
      var bestCjk = cjkCount(best);
      ['gbk', 'utf-8'].forEach(function (enc) {
        try {
          var t2 = docCleanLines(new TextDecoder(enc, { fatal: false }).decode(u8));
          var c2 = cjkCount(t2);
          if (c2 >= 8 && c2 > bestCjk) { best = t2; bestCjk = c2; }
        } catch (e) {}
      });
    ...
    /* v195：角色身份页 → 一键跳去「名单」编辑 TA 的完整资料 */
    (function () {
      var _g = document.getElementById('identityGotoMemberBtn');
      if (!_g) return;
      _g.addEventListener('click', function () {
        try { chatIdentityPanel.classList.remove('open'); } catch (e) {}
        try { document.getElementById('chatDetailPanel').classList.remove('open'); } catch (e1) {}
        try { document.getElementById('chatDetailOverlay').classList.remove('open'); } catch (e2) {}
        try { openMemberList(); } catch (e3) { try { toast('没找到「名单」入口'); } catch (e4) {} }
      });
    })();
    ...
    /* v201：点顶栏角色头像 → 展开角色状态栏 */
    var chatDetailAvaEl = document.getElementById('chatDetailAva');
    if (chatDetailAvaEl) chatDetailAvaEl.addEventListener('click', function (e) {
      if (e && e.stopPropagation) e.stopPropagation();
      chatRoleStatusOpen();
    });
    ...
    function initMomentsModule() {
      ...
      editorFields.textImageEditorContainer.addEventListener('click', function (e) {
        ...
        if (e.target.id === 'add-member-album-btn') {
          openAlbumPicker({
            multi: true, title: '从角色相册选配图',
            onPick: function (list) {
              list.forEach(function (it) { tempTextImageItems.push({ image: it.src, text: it.note || '', isManual: false }); });
              renderTextImageEditorItems();
              toast('已加入 ' + list.length + ' 张相册照片');
            }
          });
        }
    ...
    /* ==================== v202：本轮新增能力 ==================== */
    /* 你最近发给 TA 的图（按时间正序返回） */
    function chatUserRecentImgs(conv, max) {
      try {
        var ms = (conv && conv.messages) || [];
        var out = [];
        for (var i = 0; i < ms.length; i++) {
          var m = ms[i];
          if (!m || m.role !== 'me' || m.type !== 'image') continue;
          var arr = m.imgs || (m.img ? [m.img] : []);
          for (var j = 0; j < arr.length; j++) { if (arr[j]) out.push(arr[j]); }
        }
        var lim = max || 8;
        return out.slice(Math.max(0, out.length - lim));
      } catch (e) { return []; }
    }
    /* ① 状态栏：和心声一样跟着每条对话走 —— 每条回复落屏后本地重算，
       每 3 条再用 AI 静默重推一次（浮层开着才重画，不给用户看“刷新中”） */
    var chatStAutoBusy = false;
    function chatRoleStatusAutoTick(conv) {
      if (!conv || chatStAutoBusy) return;
      chatStAutoBusy = true;
      setTimeout(function () {
        try {
          if (conv !== chatCurrentConv) { chatStAutoBusy = false; return; }
          /* v202b：tick 必须先自增再落库（原来写在 saveConvs 之后，导致 _stTick 只在第 3 条时才被保存） */
          conv._stTick = (conv._stTick || 0) + 1;
          var _stL = chatRoleStatusEnsure(conv);
          /* v202b：每条回复后先按最近对话本地重算一次；每 3 条再让 AI 静默重推 */
          /* v209：已经有 AI 按角色语言环境推演过的状态，就别再拿本地通用模板覆盖（「状态乱答」就是这么来的），
             只有还没有 AI 状态时才用本地兜底先顶上 */
          try { if (_stL && !_stL.aiTs) { _stL.fields = chatRoleStatusLocal(conv); _stL.ts = Date.now(); } } catch (eStL) {}
          saveConvs();
          var el = document.getElementById('roleStatusOverlay');
          if (el && el.classList.contains('open')) chatRoleStatusRender(false);
          if (conv._stTick % 3 === 0) {
            chatRoleStatusAi(conv, function () {
              saveConvs();
              var e2 = document.getElementById('roleStatusOverlay');
              if (e2 && e2.classList.contains('open') && chatCurrentConv === conv) chatRoleStatusRender(false);
            });
          }
        } catch (e) {}
        chatStAutoBusy = false;
      }, 150);
    }
    /* ④ 你发的图，TA 会存进自己的相册：可以拿来当头像，也可以发朋友圈 */
    function chatUserImgsSavedByRole(conv, imgs) {
      try {
        var m = null;
        try { m = memberByConv(conv); } catch (eM) {}
        if (!m || !m.id) return;
        var list = memberLoad();
        var i = -1;
        for (var k = 0; k < list.length; k++) { if (String(list[k].id) === String(m.id)) { i = k; break; } }
        ...
        (imgs || []).forEach(function (src) {
          if (!src) return;
          if (list[i].album.length >= MEM_ALBUM_MAX) return;
          var dup = false;
          for (var q = 0; q < list[i].album.length; q++) { if (list[i].album[q] && list[i].album[q].src === src) { dup = true; break; } }
          if (dup) return;
          list[i].album.push({ id: memberUid(), src: src, ts: Date.now(), note: '你发的图', savedBy: 'ta' });
          added++;
        });
        if (added) {
          memberSave(list);
          pushChatErrLog('[相册] ' + (conv.name || 'TA') + ' 收下了你发的 ' + added + ' 张图（可当头像 / 发朋友圈）');
        }
      } catch (e) {}
    }
    /* ③ 你开口让 TA 换头像 —— 有概率换；换就是真换，不存在“嘴上说换了、头像没动” */
    var CHAT_AVA_ASK_PROB = 0.75;
    function chatUserAskAvatarMaybe(conv, text) {
      try {
        var t = String(text || '');
        if (!t || t.indexOf('头像') < 0) return false;
        if (!/(换|改|更新|替换|变成)/.test(t)) return false;
        var _c = conv || chatCurrentConv;
        if (!_c) return false;
        var m = null;
        try { m = memberByConv(_c); } catch (eM) {}
        if (!m || !m.id) { toast('这个会话还没绑定名单角色，TA 换不了头像'); return false; }
        if (Math.random() > CHAT_AVA_ASK_PROB) {
          var _no = ['头像先不换啦，这张用惯了', '唔…我还挺喜欢现在这张的，先留着', '下次吧，一时想不到换成什么'];
          addChatMsg('other', { type: 'text', text: _no[Math.floor(Math.random() * _no.length)] }, _c);
          return false;
        }
        toast('TA 正在换头像…');
        return chatRoleAvatarSwap(_c, '');
      } catch (e) { return false; }
    }
    /* 真换头像的唯一入口：素材优先你刚发的图 → TA 相册（跳过当前头像）→ 现画一张 */
    function chatRoleAvatarSwap(conv, preferSrc, cb) {
      try {
        var _c = conv || chatCurrentConv;
        var m = null;
        try { m = memberByConv(_c); } catch (eM) {}
        ...
        var cur = String(list[i].avatar || '');
        var cand = [];
        if (preferSrc) cand.push(String(preferSrc));
        var _uimgs = chatUserRecentImgs(_c, 8);
        for (var ui = _uimgs.length - 1; ui >= 0; ui--) cand.push(String(_uimgs[ui]));
        (list[i].album || []).forEach(function (x) { if (x && x.src) cand.push(String(x.src)); });
        for (var ci = 0; ci < cand.length; ci++) {
          var s = String(cand[ci] || '');
          if (!s || s === cur) continue;
          if (chatRoleAvatarSet(_c, s)) {
            try { _c._avaJustChanged = Date.now(); } catch (eF) {}
            toast('TA 真的换上新头像了');
            pushChatErrLog('[TA换头像] 已真实生效（来源：' + (preferSrc && s === String(preferSrc) ? '指定图' : '你发的图 / TA 相册') + '）');
            if (cb) cb(true);
            return true;
          }
        }
        var cfg = null;
        try { cfg = chatRoleImgApiCfg(_c); } catch (eC) {}
        if (!cfg) { toast('TA 想换头像，但相册里还没图、也没配生图 API'); if (cb) cb(false); return false; }
        var pos = [];
        if (m.look) pos.push(String(m.look));
        if (m.lookPrompt) pos.push(String(m.lookPrompt));
        if (!pos.length) pos.push('头像特写，正脸，自然光，日常感');
        ...
        chatRoleGenImageApi(cfg, pos.join('，'), _neg, _ref, function (src) {
          chatShrinkImgSrc(src, function (out) {
            if (!out) { toast('头像图没处理好，换头像没成功'); if (cb) cb(false); return; }
            try {
              var l2 = memberLoad();
              var j = -1;
              for (var q = 0; q < l2.length; q++) { if (String(l2[q].id) === String(m.id)) { j = q; break; } }
              if (j >= 0) {
                if (!Array.isArray(l2[j].album)) l2[j].album = [];
                var dup = false;
                for (var z = 0; z < l2[j].album.length; z++) { if (l2[j].album[z] && l2[j].album[z].src === out) { dup = true; break; } }
                if (!dup && l2[j].album.length < MEM_ALBUM_MAX) {
                  l2[j].album.push({ id: memberUid(), src: out, ts: Date.now(), note: 'TA 画的新头像', savedBy: 'ta' });
                  memberSave(l2);
                }
              }
            } catch (eA) {}
            chatRoleAvatarSwap(_c, out, cb);
          });
        }, function (msg) { toast('TA 想换头像但生成失败：' + msg); if (cb) cb(false); });
        return true;
      } catch (e) { return false; }
    }
    /* ④ 让 TA 把图发到朋友圈：图源 = 你刚发的图（优先）→ TA 相册；配文由 AI 写 */
    function chatMomentComposeSend(conv, opts) {
      try {
        var _c = conv || chatCurrentConv;
        if (!_c) return;
        var m = null;
        try { m = memberByConv(_c); } catch (eM) {}
        if (!m || !m.id) { toast('这个会话还没绑定名单角色，TA 发不了朋友圈'); return; }
        var list = memberLoad();
        var i = -1;
        for (var k = 0; k < list.length; k++) { if (String(list[k].id) === String(m.id)) { i = k; break; } }
        if (i < 0) return;
        var _uimgs = chatUserRecentImgs(_c, 8);
        var src = _uimgs.length ? _uimgs[_uimgs.length - 1] : '';
        if (!src) {
          var alb = (list[i].album || []).filter(function (x) { return x && x.src; });
          if (alb.length) src = alb[Math.floor(Math.random() * alb.length)].src;
        }
        /* v204：手里没图也能发 —— 退化成纯文字动态，不再因为没图就发不出去 */
        var _auto = !!(opts && opts.auto);
        var nm = String(list[i].name || _c.name || 'TA');
        var post = function (cap) {
          try {
            var data = {
              author: { id: String(list[i].id || ''), name: nm, avatar: String(list[i].avatar || '') },
              content: cap,
              imageContents: src ? [{ image: src, text: '', isManual: false }] : [],
              ipAddress: '', blockedUsers: [], mentionedUsers: []
            };
            var p = { id: generateId(), timestamp: new Date().toISOString(), likes: [], comments: [], type: src ? 'image' : 'text' };
            for (var kk in data) { if (Object.prototype.hasOwnProperty.call(data, kk)) p[kk] = data[kk]; }
            momentsPosts.unshift(p);
            saveMomentsPosts();
            try { if (typeof renderPosts === 'function' && chatSubOverlay && chatSubOverlay.classList.contains('open')) renderPosts(); } catch (eR) {}
            /* v204：TA 主动发的那条不打断聊天，只提示一下；手动触发的才带一句「去看看」 */
            if (_auto) {
              toast('「' + nm + '」刚发了一条朋友圈');
            } else {
              addChatMsg('other', { type: 'text', text: src ? '刚把这张发朋友圈了，你去看看？' : '刚发了条朋友圈，你去看看？' }, _c);
              toast('TA 已发一条朋友圈');
            }
          } catch (e) { toast('朋友圈发布失败'); }
        };
        memberAiAsk('你是角色本人，用第一人称写一条朋友圈配文，只输出正文，不要引号、不要话题标签、不要解释。',
          '角色名：' + nm + '\n角色人设节选：\n' + String(list[i].lore || '').slice(0, 600) +
          (src ? '\n配图：这是 TA 刚收到或保存的一张照片。' : '\n这条没有配图，就发一段随手记。') +
          '\n请写一条 15~40 字的朋友圈配文，口语一点、有生活感，贴合 TA 最近的心情。',
          function (txt, err) {
            var cap = (!err && String(txt || '').trim()) ? String(txt).trim().replace(/^["“]|["”]$/g, '').slice(0, 80) : '今天的光线很好，随手记一下。';
            post(cap);
          });
      } catch (e) {}
    }
    /* v204：角色主动发朋友圈 —— 聊天自然进行中，TA 偶尔会自己挑时候发一条；
       不打断对话（不往聊天里插消息），只在朋友圈可见 + 轻提示。同一会话至少隔 12 条消息才可能再发。 */
    function chatMomentAutoPost(conv) {
      try {
        var _c = conv || chatCurrentConv;
        if (!_c || !_c.messages || _c.messages.length < 4) return;
        if (_c._mAutoAt && (_c.messages.length - _c._mAutoAt) < 12) return;
        if (Math.random() > 0.22) return;
        _c._mAutoAt = _c.messages.length;
        try { saveConvs(); } catch (eS) {}
        chatMomentComposeSend(_c, { auto: true });
      } catch (e) {}
    }
    /* ⑦ NPC 生成进度（v207 改版：大进度条撤掉，进度改由「每个板块文字后面那条自己的进度条」承担，
       这里只剩一行小结文案；v209 起续写按钮挂在各板块上方） */
    function npcProgWrapEl() { return memberEditScroll.querySelector('#npcProgWrap'); }
    function npcProgNextEl() { return memberEditScroll.querySelector('#npcProgNext'); }
    function npcProgTxEl() { return memberEditScroll.querySelector('#npcProgTx'); }
    function npcProgStart(base) {
      try {
        var wrap = npcProgWrapEl();
        var tx = npcProgTxEl();
        if (!wrap) return;
        wrap.classList.add('on');
        npcSecNextSync([]);
        if (tx) tx.textContent = 'AI 正在写这一段…';
      } catch (e) {}
    }
    /* v209：一段写完后的收口 —— 小结＝真实已写段数；续写按钮长在「下一个待写段」的板块上方 */
    function npcProgSection(done, total, hasNext) {
      try {
        var wrap = npcProgWrapEl();
        var tx = npcProgTxEl();
        if (wrap) { wrap.classList.add('on'); if (tx) tx.textContent = done + '/' + total + ' 段'; }
        npcSecNextSync(hasNext ? null : []);
      } catch (e) {}
    }
    /* v209：把「继续写这一段」按钮放到指定那一段的板块上方（不传＝按当前空缺自动找） */
    function npcSecNextSync(missing) {
      try {
        var names = npcLoreSectionNames();
        var miss = missing || npcLoreMissing(npcLoreCurrent());
        for (var i = 0; i < names.length; i++) {
          var b = memberEditScroll.querySelector('#npcSecNext' + i);
          if (!b) continue;
          if (i === miss[0]) { b.style.display = ''; b.textContent = '继续写这一段'; }
          else b.style.display = 'none';
        }
      } catch (e) {}
    }
    function npcProgDone() {
      try {
        npcSecProgStopAll();
        npcSecProgSync();
        var tx = npcProgTxEl();
        if (tx) tx.textContent = '已写满';
        npcSecNextSync([]);
        setTimeout(function () {
          var w2 = npcProgWrapEl();
          if (w2) w2.classList.remove('on');
          npcSecProgSync();
        }, 900);
      } catch (e) {}
    }

    /* ===== v222：IndexedDB 就绪后按 DB 回读刷新（刷新不再丢数据） ===== */
    function dbHydrateChatData(silent) {
      var got = [];
      try { var v0 = JSON.parse(dbGet(CONVS_KEY) || '[]'); if (Array.isArray(v0) && v0.length > (chatConvs || []).length) { chatConvs = v0; got.push('聊天'); } } catch (e0) {}
      try { var v1 = JSON.parse(dbGet(CONTACTS_KEY) || '[]'); if (Array.isArray(v1) && v1.length > (chatContacts || []).length) { chatContacts = v1; got.push('联系人'); } } catch (e1) {}
      try { var v2 = JSON.parse(dbGet(GROUPS_KEY) || '[]'); if (Array.isArray(v2) && v2.length > (chatGroups || []).length) { chatGroups = v2; got.push('群聊'); } } catch (e2) {}
      try { var v3 = JSON.parse(dbGet(MINE_KEY) || '{}'); if (v3 && typeof v3 === 'object' && Object.keys(v3).length && !Object.keys(chatMine || {}).length) { chatMine = v3; } } catch (e3) {}
      try { var v4 = JSON.parse(dbGet(FEEDS_KEY) || '[]'); if (Array.isArray(v4) && v4.length > (momentsPosts || []).length) { momentsPosts = v4; } } catch (e4) {}
      try { var v5 = JSON.parse(dbGet(MAIL_KEY) || '[]'); if (Array.isArray(v5) && v5.length > (chatMail || []).length) { chatMail = v5; } } catch (e5) {}
      if (got.length) {
        try { renderChatConvs(); } catch (e6) {}
        try { if (chatCurrentConv) renderChatMessages(); } catch (e7) {}
        if (!silent) { try { toast('已从本地数据库回读：' + got.join('、')); } catch (e8) {} }
      }
      return got.length > 0;
    }
    dbOnReady(function () { try { dbHydrateChatData(); } catch (e) {} });
