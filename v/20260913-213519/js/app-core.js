  (function () {
    ...
    function chatSendImageMenu() {
      ...
      chatMini('发送图片', '<div class="chat-cfg-tip" style="margin:0 0 8px">想给TA发什么？</div><div class="chat-mini-list">' +
        ...
        '<button class="chat-mini-opt" id="imgOptRoleAlbum">让 TA 从自己的相册挑一张发给我（TA 主动发）</button>' +
        '<button class="chat-mini-opt" id="imgOptRoleAlbumPick">我来从 TA 的相册里挑一张发出去</button>' +
        '<button class="chat-mini-opt" id="imgOptGen">让 TA 现画一张发我（走生图 API）</button></div>', '取消', function () {});
    ...
    function chatRoleImgApiCfg() {
      var im = (chatCurrentConv && chatCurrentConv.settings) ? chatCurrentConv.settings.imag : null;
      try {
    ...
    function renderIdentityMemberBox() {
      ...
      var m = null;
      try { m = memberByConv(chatCurrentConv); } catch (e) {}
      if (identityMode !== 'role' || !m) { box.style.display = 'none'; box.innerHTML = ''; return; }
      box.style.display = '';
      box.innerHTML = identityMemberBoxHtml(m);
    ...
    function openIdentityEditor(mode) {
      ...
      if (mode === 'my') {
        ...
        document.getElementById('identityPromptTitle').style.display = _idcFromMem ? 'none' : '';
        document.getElementById('identityPromptCard').style.display = _idcFromMem ? 'none' : '';
        document.getElementById('identityImportBtn').style.display = _idcFromMem ? 'none' : '';
        var _idcTip = document.getElementById('identityRoleFromMember');
        if (_idcTip) {
          _idcTip.style.display = _idcFromMem ? '' : 'none';
          if (_idcFromMem) _idcTip.textContent = '角色人设直接取自「名单」中「' + memberDispName(_idcMem) + '」的资料（本名 / 网名 / 性别 / 身世 / 外貌 / 关系网），在名单里改完即刻生效，这里不用再上传文本。';
        }
      ...
      var _idcBound = false;
      if (mode === 'role') { try { _idcBound = !!memberByConv(chatCurrentConv); } catch (e3) {} }
      try {
        ...
        if (_nmF) _nmF.style.display = _idcBound ? 'none' : '';
        if (_sxT) _sxT.style.display = _idcBound ? 'none' : '';
        if (_sxR) _sxR.style.display = _idcBound ? 'none' : '';
      } catch (e4) {}
