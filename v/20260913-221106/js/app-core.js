  (function () {
    ...
    function chatSendImageMenu() {
      ...
      chatMini('发送图片', '<div class="chat-cfg-tip" style="margin:0 0 8px">想给TA发什么？</div><div class="chat-mini-list">' +
        ...
        '<button class="chat-mini-opt" id="imgOptMany">从系统相册选择 · 多张合并</button>' +
        '<button class="chat-mini-opt" id="imgOptRoleAlbum">让 TA 从自己的相册挑一张旧图发我（TA 主动发）</button>' +
        '<button class="chat-mini-opt" id="imgOptRoleAlbumPick">我来从 TA 的相册里挑一张发出去</button>' +
        '<button class="chat-mini-opt" id="imgOptGen">让 TA 现拍一张新的发我（AI 现场生成新图）（走生图 API）</button></div>', '取消', function () {});
      ...
      var _raB = document.getElementById('imgOptRoleAlbum');
      if (_raB) _raB.addEventListener('click', function () { chatRolePickAlbumOne(); });
      var _raP = document.getElementById('imgOptRoleAlbumPick');
      if (_raP) _raP.addEventListener('click', function () { chatPickFromAlbum(); });
      var _gB = document.getElementById('imgOptGen');
      if (_gB) _gB.addEventListener('click', function () { chatRoleGenImage(); });
