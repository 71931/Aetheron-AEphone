  (function () {
    ...
    function memberNorm(m) {
      ...
      var n = {
        ...
        npcs: Array.isArray(m.npcs) ? m.npcs.map(function (x) {
          x = x || {};
          return { id: x.id || memberUid(), name: String(x.name || '').trim(), bio: String(x.bio || '').trim(), rel: String(x.rel || '').trim(), add: !!x.add, avatar: String(x.avatar || ''), gender: String(x.gender || ''), masterId: String(x.masterId || ''), masterName: String(x.masterName || ''), lore: String(x.lore || '') };
        }) : [],
    ...
    function openMemberEdit(idx) {
      ...
      memNpcs = m ? m.npcs.map(function (x) { return { id: x.id, name: x.name, bio: x.bio, rel: x.rel, add: !!x.add, avatar: x.avatar || '', gender: x.gender || '', masterId: x.masterId || '', masterName: x.masterName || '', lore: x.lore || '' }; }) : [];
    ...
    function npcLoreSplit(lore) {
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        if (cur >= 0) out[cur] += (out[cur] ? '\n' : '') + bare;
      }
      if (!out.join('').replace(/\s/g, '') && txt.trim()) out[0] = txt.trim();
      return out;
    }
    ...
    function memberAiNpc() {
      ...
      toast('AI 正在生成 NPC…');
      var user = '这是主角色的完整人设：\n' + memberNpcMainText() + '\n\n请基于这份身世，生成 3 个与角色有关联的 NPC。每个 NPC 的完整人设必须严格按下面的十段模板写：\n' + MEM_NPC_TPL +
    ...
    function memberNpcOpen(idx) {
      ...
      if (idx >= 0 && memNpcs[idx]) {
        var x = memNpcs[idx];
        memNpcDraft = { id: x.id, name: x.name || '', bio: x.bio || '', rel: x.rel || '', add: !!x.add, avatar: x.avatar || '', gender: x.gender || '', lore: x.lore || '', masterId: x.masterId || '', masterName: x.masterName || '' };
        memNpcEditIdx = idx;
      } else {
        /* v202：主人设 —— 从这个人设入口建立的 NPC，主人设就是当前角色 */
        memNpcDraft = { id: memberUid(), name: '', bio: '', rel: '', add: false, avatar: '', gender: '', lore: '', masterId: memberMasterId(), masterName: memberMasterName() };
        memNpcEditIdx = -1;
      }
      memberNpcRender();
    }
    ...
    function memberNpcEditBody() {
      ...
      for (var si = 0; si < secNames.length; si++) {
        ...
        html += '<div class="npc-sec-hd"><span class="npc-sec-no">' + escHtml(secNames[si]) + '</span><span class="npc-sec-st" id="npcSecSt' + si + '"></span></div>';
      ...
      /* v207：全局大进度条撤掉，只留一行「继续写第 N 段」按钮 + 段数小结 */
      html += '<div class="npc-prog-wrap" id="npcProgWrap"><span class="npc-prog-tx" id="npcProgTx"></span><button type="button" class="npc-prog-next" id="npcProgNext" style="display:none">继续写下一段</button></div>';
      html += '</div>';
    ...
    function memberNpcEditBind() {
      ...
      /* v206：一段写完后点「继续写第 N 段」接着写下一段 */
      var pnx = document.getElementById('npcProgNext');
      if (pnx) pnx.addEventListener('click', npcAiNext);
    ...
    function npcAiMenuOpen() {
      ...
      pop.innerHTML = '<div class="npc-ai-mask"></div><div class="npc-ai-box">' +
        ...
        '<button type="button" class="npc-ai-opt" data-npcai="write"><span class="npc-ai-opt-n">AI 帮写</span><span class="npc-ai-opt-d">一次只写一段，写完停下，你点「继续写第 N 段」再往下写</span></button>' +
    ...
    /* v206：写完一段后停手，进度条停在真实位置，等用户点「继续写第 N 段」再写下一段 */
    ...
    function npcAiRun(mode, cur, attempt) {
      ...
      /* v206：一次只写一段，写完就停手等用户确认。
         ...
         用户点「继续写第 N 段」才写下一段，进度条按真实段落数推进。 */
      var targets = [];
      if (mode === 'polish') {
      ...
      var user = '这是主角色的完整人设：\n' + memberNpcMainText() +
        ...
        (already ? ('\n\n已有内容（保持前后一致，不要重写这些段）：\n' + already) : '') +
        '\n' + tip +
        '\n每段 150~220 字，中文，要具体到时间、地点、物件和说话方式，写出血肉与经历，不要空话套话，贴合主角色世界观与关系。\n' +
      ...
      memberAiAsk('你是世界观与角色设定助手，只输出 JSON。', user, function (txt, err) {
        ...
        if (still.length) {
          ...
          toast('第 ' + doneN + ' 段写好了，点「继续写第 ' + (doneN + 1) + ' 段」接着写');
        } else {
          npcProgSection(total, total, false);
    ...
    function chatRoleStatusLocal(conv) {
      ...
      var body = /累|困|熬夜|失眠/.test(recent) ? '有点乏，肩颈发紧，硬撑着精神' : '状态正常，只是坐久了起来活动两下';
    ...
    function chatRoleStatusRefresh() {
      ...
      try {
        ...
        _stR.fields = chatRoleStatusLocal(conv);
        _stR.ts = Date.now();
        saveConvs();
      } catch (eR) {}
    ...
    function chatRoleStatusAutoTick(conv) {
      ...
      setTimeout(function () {
        try {
          ...
          /* v202b：每条回复后先按最近对话本地重算一次，状态栏真的跟着对话走；每 3 条再让 AI 静默重推 */
          try { if (_stL) { _stL.fields = chatRoleStatusLocal(conv); _stL.ts = Date.now(); } } catch (eStL) {}
          saveConvs();
          var el = document.getElementById('roleStatusOverlay');
    ...
    /* 一段写完后的收口 —— 小结＝真实已写段数，右侧给出「继续写第 N 段」按钮 */
    function npcProgSection(done, total, hasNext) {
      try {
        ...
        if (!wrap) return;
        wrap.classList.add('on');
        if (tx) tx.textContent = done + '/' + total + ' 段';
        if (nx) {
          if (hasNext) { nx.style.display = ''; nx.textContent = '继续写第 ' + (done + 1) + ' 段'; }
