  (function () {
    ...
    function memberEditBody(m) {
      ...
      form += '<div class="field"><label>角色ID</label>' +
        ...
        [['netName', '网名'], ['gender', '性别'], ['lore', '人设'], ['look', '外貌'], ['voice', '音色'], ['alts', '小号'], ['npcs', 'NPC']].map(function (p) {
    ...
    function memberQrSync() {
      ...
      if (pv) pv.innerHTML = memQrText ? '<img src="' + memQrUrl(memQrText) + '" alt="角色二维码" data-qzoom="1">' + '<span class="mem-qr-tip">点二维码放大 · 长按图片可存相册 · 别人扫这个码能直接加 TA 为好友</span>' : '<span class="mem-qr-empty">填写二维码内容后点“生成”</span>';
    }

    /* ===== v189：二维码点开看大图 / 保存到本地 ===== */
    function memberQrBigPng(text, cb) {
      var t = String(text || '');
      try {
        if (t && typeof qrcode === 'function') {
          if (qrcode.stringToBytesFuncs && qrcode.stringToBytesFuncs['UTF-8']) qrcode.stringToBytes = qrcode.stringToBytesFuncs['UTF-8'];
          var build = function (ec) { var q = qrcode(0, ec); q.addData(t); q.make(); return qrToPng(q, 10, 6); };
          try { cb(build('M'), t); return; } catch (e1) {}
          try { cb(build('L'), t); return; } catch (e2) {}
        }
      } catch (e) {}
      cb(memQrUrl(t), t);
    }
    ...
    function openQrViewer(src, text) {
      var old = document.getElementById('memQrViewer');
      if (old && old.parentNode) old.parentNode.removeChild(old);
      memberQrBigPng(text, function (png) {
        ...
        tip2.textContent = '长按二维码图片，选「存储图像 / 保存图片」也能直接存进相册';
    ...
    function afFieldChipsHtml(p) {
      ...
      if (p.voiceId) chips.push('音色 · ' + p.voiceId);
    ...
    function afRenderAddConfirm(p) {
      ...
      var html = '<div class="af-card">' +
        ...
        '<div class="af-card-nm"><div class="af-card-name">' + escHtml(name) + '</div><div class="af-card-sub">' + (secret ? 'TA 只公开了名字' : '对方二维码里公开的资料') + '</div></div>' +
        '</div>' + afFieldChipsHtml(p) +
        (lore && !secret ? '<div class="af-card-bio">' + escHtml(lore.slice(0, 60)) + (lore.length > 60 ? '…' : '') + '</div>' : '') +
    ...
    function afAutoPending() {
      var raw = '';
      try { raw = localStorage.getItem('aether_pending_friend') || ''; } catch (e) { return; }
      if (!raw) return;
      try {
