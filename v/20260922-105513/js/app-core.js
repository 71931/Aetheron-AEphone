  (function () {
    ...
    /* v198：TA 自主换头像——发完图偶尔就顺手把这张（或相册里另一张）设成自己的头像 */
    function chatRoleSelfAvatarMaybe(conv, src, force) {
      try {
        ...
        /* v199：换完给一句可见反馈，别再出现「嘴上说换了、界面没动静」 */
        try { toast('TA 换上了新头像'); } catch (e4) {}
        return true;
      } catch (e) { return false; }
    }
    ...
    /* 涂鸦主流程：优先真·AI 改图（把原图当参考图丢生图接口），接口不认就前端 canvas 精准叠加，再不行本地兜底 */
    function chatDoodleRun(conv, desc) {
      try {
        ...
        var cfg = null;
        try { cfg = chatRoleImgApiCfg(c); } catch (e) {}
        if (cfg && chatRoleImgSwOn(c.settings)) {
          var pos = [];
          try { var mm = memberByConv(c); if (mm && mm.look) pos.push(String(mm.look)); } catch (e2) {}
          pos.push('在对方发来的这张照片上做涂鸦创作：' + desc);
          pos.push('照片里原本的人物、场景与构图保持一致，只在画面上叠加涂鸦笔迹、贴纸或手写文字，风格俏皮可爱');
          var neg = String(((c.settings && c.settings.imag) || {}).promptNeg || '');
          chatRoleGenBusy = true;
          chatRoleGenImageApi(cfg, pos.join('，'), neg, tar.src, function (src) {
            chatShrinkImgSrc(src, function (out) {
              chatRoleGenBusy = false;
              if (!out) { chatRoleImgFail(ph, c, '涂鸦失败：生成的图读不出来'); return; }
              chatRoleImgDone(ph, out, c, note);
              toast('TA 把涂鸦后的图发回来了');
            });
          }, function () {
            chatRoleGenBusy = false;
    ...
    /* 前端涂鸦：让模型只给「涂鸦排版方案（JSON）」，再用 canvas 精准叠到原图上 */
    ...
    function renderChatSectionView(sec) {
      ...
      chatSettingsBody.innerHTML = meta.items.map(function (it) {
        /* v198：「真实生图」用 SVG 拨动按键（开 / 关），不再显示文字值 */
        if (it.key === 'roleimg') {
          var _rOn = chatRoleImgSwOn(chatCurrentConv.settings);
          return '<div class="chat-setting-switch" data-skey="roleimg" style="cursor:pointer">' +
            '<div style="min-width:0"><div class="sw-label">' + it.label + '</div><div class="sw-desc">' + it.desc + '</div></div>' +
            svgSwitchHtml(_rOn, 'roleimg') +
            '</div>';
        }
    ...
    function renderChatImagView() {
      ...
      var html = '<div class="group-card"><div class="chat-setting-switch" style="cursor:pointer" id="imagToggleRow">' +
        ...
        '<div style="min-width:0"><div class="sw-label">图片涂鸦回复</div><div class="sw-desc">你发的图，TA 可以在上面涂鸦创作后再发回（优先真·AI 改图，接口不支持就本地涂鸦兜底）</div></div>' +
        ...
        '<button class="chat-sw ' + (_fc.enabled !== false ? 'on' : '') + '" id="fileChatToggle"></button></div></div>';
