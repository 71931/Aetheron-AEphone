  (function () {
    ...
    function memberNorm(m) {
      ...
      var n = {
        ...
        npcs: Array.isArray(m.npcs) ? m.npcs.map(function (x) {
          x = x || {};
          return { id: x.id || memberUid(), name: String(x.name || '').trim(), bio: String(x.bio || '').trim(), rel: String(x.rel || '').trim(), add: !!x.add, avatar: String(x.avatar || ''), gender: String(x.gender || ''), lore: String(x.lore || '') };
        }) : [],
    ...
    function memberEditBody(m) {
      ...
      form += '<div class="field"><label>角色ID</label>' +
        ...
        '<div class="mem-qr-label">角色二维码<button type="button" class="tips-q" data-tip="qr">?</button></div>' +
        '<div class="mem-qr-row" id="memQrRow" style="display:none"><input type="hidden" id="memQrText" value="' + escHtml(memQrText) + '" placeholder="二维码内容（一般不用改）">' +
    ...
    function openMemberEdit(idx) {
      ...
      memNpcs = m ? m.npcs.map(function (x) { return { id: x.id, name: x.name, bio: x.bio, rel: x.rel, add: !!x.add, avatar: x.avatar || '', gender: x.gender || '', lore: x.lore || '' }; }) : [];
    ...
    function memberNpcListBody() {
      var rows = memNpcs.length ? memNpcs.map(function (np, i) {
        ...
        return '<div class="mem-npc-row npc-open" data-mact="npcopen" data-val="' + i + '"><span class="mem-npc-ava">' + _nav + '</span>' +
          '<span class="mem-npc-bd"><span class="mem-npc-name">' + escHtml(np.name || '未命名') + '</span><span class="mem-npc-bio">' + escHtml(np.bio || (np.lore ? String(np.lore).split('\n')[0].slice(0, 24) : '暂无完整人设，点开编辑')) + '</span></span>' +
      ...
      html += '<div class="npc-list-head"><button type="button" class="npc-list-back" data-mact="npcback">‹ 角色编辑</button><span class="npc-list-title">NPC 人设库</span></div>';
    ...
    function memberNpcOpen(idx) {
      ...
      if (idx >= 0 && memNpcs[idx]) {
        var x = memNpcs[idx];
        memNpcDraft = { id: x.id, name: x.name || '', bio: x.bio || '', rel: x.rel || '', add: !!x.add, avatar: x.avatar || '', gender: x.gender || '', lore: x.lore || '' };
        memNpcEditIdx = idx;
      } else {
        memNpcDraft = { id: memberUid(), name: '', bio: '', rel: '', add: false, avatar: '', gender: '', lore: '' };
        memNpcEditIdx = -1;
      }
      memberNpcRender();
    }
    ...
    function memberNpcEditBody() {
      ...
      html += '<div class="npc-lore-head"><div class="npc-lore-hint">十段都要写满，每段 200 字上下；写不动的段落交给右上角 AI</div>';
      ...
      html += '<div class="npc-lore-count" id="npcLoreCount">已写 0 字</div>';
      html += '</div>';
    ...
    function memberNpcSave() {
      ...
      memNpcEditIdx = null;
      if (memNpcListView) memberNpcListRender(); else memberRerender();
      var _all = String(obj.lore || '').split('\n').filter(function (x) { return /^(一|二|三|四|五|六|七|八|九|十)、/.test(String(x).trim()); });
      var _tn = npcLoreTotal(_all);
      if (_all.length >= 10 && _tn < 2000) toast('已保存「' + nm + '」，人设目前 ' + _tn + ' 字，建议写到 2000 字左右才够饱满');
      else toast('已保存 NPC「' + nm + '」' + (_tn ? '（人设 ' + _tn + ' 字）' : ''));
    }
    ...
    function npcAiRun(mode, cur, attempt) {
      ...
      memberAiAsk('你是世界观与角色设定助手，只输出 JSON。', user, function (txt, err) {
        ...
        if (err || !txt) {
          ...
          toast('AI 暂时连不上，已按内置模板把人设补齐（共 ' + npcLoreTotal(fb) + ' 字），稍后可再点一次润色');
          return;
        }
        ...
        toast((mode === 'polish' ? '人设已润色' : '人设已写好') + '，共 ' + npcLoreTotal(out) + ' 字');
      });
    }
    // ===== 顶栏 / 返回绑定 =====
    ...
    document.getElementById('memberEditBack').addEventListener('click', function () { if (memNpcEditIdx !== null && memNpcEditIdx !== undefined) { memberNpcBack(); return; } memberDiscard(); });
    ...
    function chatRoleStatusRender(busy) {
      ...
      var src = st.aiTs ? ('AI 按最近对话生成 · ' + new Date(st.aiTs).toLocaleTimeString()) : '按人设与最近对话即时推断';
    ...
    function renderChatMessages() {
      ...
      chatDetailBody.innerHTML = msgs.map(function (m, idx) {
        ...
        /* v195：旁白场景描写 —— 居中灰底小字卡片，配色随情境变化 */
        if (m.fmt === 'narrator') {
          return sepHtml + '<div class="chat-msg-row narrator-row" data-msg-idx="' + idx + '">' +
            '<div class="chat-narrator-card" data-scene="' + chatSceneKey(m.text) + '">' + escHtml(String(m.text || '')) + '</div></div>';
        }
        var mine = m.role === 'me';
    ...
    function aiReply(active) {
      ...
      fetch(base, {
        ...
        /* v195：旁白模式 —— 解析 [[AETHER_NARRATOR]] 场景描写并从正文剥离 */
        var narratorText = '';
        if (s.chatMode === 'narrator') {
          var _nmk = '[[AETHER_NARRATOR]]';
        ...
        /* v195：旁白场景描写排在本轮最前面 */
        if (narratorText) steps.unshift({ narrator: narratorText });
        var pushOne = function () {
          if (step < steps.length) {
            var it = steps[step];
            if (it.narrator) {
              /* v195：旁白场景描写（居中灰底小字，先于正文出现） */
              addChatMsg('other', { type: 'text', text: it.narrator, fmt: 'narrator' }, convRef);
            } else if (it.loc) {
    ...
    function renderChatModeView() {
      ...
      if (s.chatMode === 'narrator') {
        html += '<div class="group-title">旁白字数</div><div class="group-card form-card">' +
          '<div class="chat-setting-switch" id="apNarrRow" style="cursor:pointer"><div style="min-width:0"><div class="sw-label">旁白描写字数</div><div class="sw-desc">控制每条旁白最少 / 最多多少字</div></div><span class="chat-setting-value">' + chatNarrLabel(s) + ' <span style="color:#5ac8fa">&#8250;</span></span></div></div>';
      }
      ...
      chatSettingsBody.querySelectorAll('.ap-mode-card[data-chatmode]').forEach(function (card) {
        card.addEventListener('click', function () {
          ...
          toast('已切换为：' + (key === 'narrator' ? '旁白模式' : '普通线上模式'));
        });
      });
