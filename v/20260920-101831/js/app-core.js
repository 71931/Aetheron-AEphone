  (function () {
    ...
    // ===== MCP 应用（v210：服务卡片 + 小红书配置信息 + 按 Char 授权） =====
    ...
    var mcpXhsOverlay = document.getElementById('mcpXhsOverlay');
    var mcpXhsBody = document.getElementById('mcpXhsBody');
    ...
    var MCP_XHS_TOOLS = ['publish_content', 'publish_with_video', 'post_comment_to_feed', 'reply_comment_in_feed',
      'like_feed', 'favorite_feed', 'search_feeds', 'get_feed_detail', 'get_my_profile', 'user_profile',
      'list_feeds', 'list_notifications', 'reply_notification', 'like_notification', 'get_unread_count',
      'check_login_status', 'get_login_qrcode', 'delete_cookies'];

    var MCP_XHS_INFO = [
      ['服务名', '小红书 MCP（xiaohongshu-mcp）'],
      ['版本', 'v2.5.0'],
      ['服务地址', 'http://localhost:18061/mcp'],
      ['传输', 'MCP over HTTP · JSON-RPC'],
      ['主程序', '/usr/local/bin/xhs-mcp（21.6MB 二进制，原版备份 xhs-mcp.bak）'],
      ['登录态', '/tmp/cookies.json'],
      ['工具数', '18 个']
    ];

    var MCP_XHS_CODES = [
      ['启动（铁律：绝不在源码目录里起）',
        'cd / && COOKIES_PATH=/tmp/cookies.json DISPLAY=:99 \\\n  nohup /usr/local/bin/xhs-mcp -headless=false -port :18061 \\\n  > /tmp/xhs61b.log 2>&1 &'],
      ['配一个屏幕（headless=false 必须有）',
        "pgrep -f 'Xvfb :99' || (nohup Xvfb :99 -screen 0 1280x900x24 >/tmp/xvfb.log 2>&1 &)"],
      ['健康检查（405 = 健康）',
        "pgrep -f 'xhs-mcp' && curl -s -o /dev/null -w '%{http_code}\\n' http://localhost:18061/mcp"],
      ['登录状态 / 取二维码',
        'python3 xhs_call.py check_login_status\npython3 xhs_call.py get_login_qrcode'],
      ['一键启停（门铃含看门狗）',
        'bash start_xhs_doorbell.sh start   # stop / status']
    ];

    var MCP_XHS_CMDS = [
      ['搜内容', 'python3 xhs_do.py search 关键词'],
      ['读笔记+评论', 'python3 xhs_do.py detail <feed_id> <xsec_token>'],
      ['点赞', 'python3 xhs_do.py like <feed_id> <xsec_token>'],
      ['取消赞', 'python3 xhs_do.py unlike <feed_id> <xsec_token>'],
      ['收藏', 'python3 xhs_do.py fav <feed_id> <xsec_token>'],
      ['取消收藏', 'python3 xhs_do.py unfav <feed_id> <xsec_token>'],
      ['发评论', 'python3 xhs_do.py cmt <feed_id> <xsec_token> "内容"'],
      ['看通知', 'python3 xhs_do.py notif mentions'],
      ['通知里回评', 'python3 xhs_do.py notifreply <comment_id> "内容"'],
      ['回楼中楼', 'python3 xhs_do.py reply <feed_id> <xsec_token> <comment_id> <user_id> "内容"'],
      ['未读数', 'python3 xhs_do.py unread'],
      ['首页推荐', 'python3 xhs_do.py feed'],
      ['我的主页', 'python3 xhs_do.py me note'],
      ['发笔记', 'python3 xhs_post.py（加 dry 只预览，不发）']
    ];

    var MCP_XHS_TIPS = [
      '核心门票：几乎所有动作都要 feed_id + xsec_token，token 会过期，隔久了重新拿。',
      '回楼中楼要「四件套」：feed_id + xsec_token + comment_id + user_id，缺一不可（从 mentions 通知里取）。',
      'cookie 路径优先级：COOKIES_PATH ＞ 当前目录/cookies.json ＞ /tmp/cookies.json —— 在源码目录里启动会被空 cookie 顶掉，评论区变「登录后评论」。',
      '没有「删除笔记」工具：发之前务必先给人过目，没过点头不发。',
      '客户端超时设 ≥600 秒（服务端最慢约 5 分钟）；发布点不动就重试，通常第 3 次过。',
      '中文参数别用 python3 -c 传（shell 转义会拆字），写进 .py 文件再跑。',
      'PARSE None 不一定失败，以 raw 原文回执为准；notif 的 tab 只有 mentions / likes / connections。',
      '服务崩（accept4: function not implemented）→ 按上面启动姿势一键复活；看门狗每 30 秒盯 Xvfb / xhs-mcp / 查信守护。'
    ];
    ...
    function mcpGrants() {
      ...
      if (!Array.isArray(o.xhs)) o.xhs = [];
      return o;
    }
    function mcpGrantedXhs() { return mcpGrants().xhs; }
    function mcpSaveGrants(list) { try { dbSet(MCP_GRANTS_KEY, JSON.stringify({ xhs: list })); } catch (e) {} }
    ...
    function mcpRenderServices() {
      var names = mcpGrantedXhs().map(function (x) { return x.name; });
      mcpServiceList.innerHTML =
        '<div class="settings-item mcp-svc" id="mcpSvcXhs">' +
          '<div class="item-icon">' + MCP_SVC_ICON + '</div>' +
          '<div class="item-body">' +
            '<div class="item-title">小红书</div>' +
            '<div class="item-sub">本地 MCP · 18 个工具 · localhost:18061</div>' +
        ...
        '</div>';
      var el = document.getElementById('mcpSvcXhs');
      if (el) el.addEventListener('click', openMcpXhs);
    }

    function mcpRenderXhs() {
      var names = mcpGrantedXhs().map(function (x) { return x.name; });
      var html = '';
      html += '<div class="mcp-topbar" id="mcpXhsCharBar">' +
                '<div class="mcp-topbar-l"><b>选择 Char 使用</b><span>勾上谁，谁就在聊天里有这个能力</span></div>' +
      ...
      MCP_XHS_INFO.forEach(function (kv) {
        html += '<div class="mcp-kv"><span class="mcp-k">' + escHtml(kv[0]) + '</span><span class="mcp-v">' + escHtml(kv[1]) + '</span></div>';
      });
      html += '</div></div>';

      /* v211：通道地址（手机能不能真读到小红书，全看这一栏） */
      var _gwv = xhsGw();
      html += '<div class="mcp-sec"><div class="mcp-sec-t">通道地址（手机读小红书靠它）</div><div class="group-card mcp-gw">' +
        '<div class="mcp-gw-tip">手机网页没法直连 MCP（跨域 + 没登录态），所以在跑 MCP 的那台机器上再开一个小桥，把地址填这里。</div>' +
        '<div class="mcp-gw-bad">这条通道得跑在一台<b>装了小红书 MCP、并且已经登录小红书</b>的电脑上（Windows / Mac / Linux 都行），手机上没法自己起通道；手机在这儿只负责填地址。通道只需跑一次，之后手机随时能读。</div>' +
        '<div class="mcp-gw-tip">注意：本站是 https，浏览器不让它连 http 的通道（本机 localhost 除外）。要让手机真读到，通道地址得是 https——最省事是在那台机器上顺手开个免费隧道（第二条命令），把隧道给的 https 地址填进来。</div>' +
        '<input class="mcp-input" id="mcpGwUrl" type="text" inputmode="url" placeholder="http://192.168.1.5:18901" value="' + mcpAttr(_gwv.url) + '">' +
        '<input class="mcp-input" id="mcpGwToken" type="text" placeholder="令牌（默认 aetheron，可留空）" value="' + mcpAttr(_gwv.token) + '">' +
        '<div class="mcp-gw-row"><button class="mcp-btn" id="mcpGwSave" type="button">保存</button><button class="mcp-btn ghost" id="mcpGwTest" type="button">测连通</button></div>' +
        '<div class="mcp-gw-state" id="mcpGwState"></div>' +
        '<div class="mcp-gw-tip">手机上复制来的短链（xhslink.com/…）也能读了：通道会先把它展开成笔记地址再读，不用你自己转长链。</div>' +
        '<div class="mcp-gw-steps">三步：①在那台电脑上，点下面<b>绿色按钮</b>下载「双击启动」（Windows 存好双击就跑；Mac/Linux 存脚本 + 复制命令）；②跑起来它会印出一行 <b>https://xxxx.trycloudflare.com</b>，那就是通道地址，复制它；③回来粘上、点「保存」，再点「测连通」。</div>' +
        '<a class="mcp-dl win" href="../../tools/start-bridge-windows.bat" download>Windows：下载后双击就能跑（推荐）</a>' +
        '<a class="mcp-dl" href="../../tools/xhs_bridge.py" download>通道脚本 xhs_bridge.py（Mac / Linux 用）</a>' +
        '<a class="mcp-dl2" href="../../tools/bridge-help.html" target="_blank" rel="noopener">手机上也看得懂的三步说明 / 常见报错</a>' +
        '<div class="mcp-gw-tip">Mac / Linux：脚本存到桌面，终端里粘这一行（起桥 + 顺手开 https 隧道，一条就够）：</div>' +
        '<div class="mcp-code" data-copy="' + mcpAttr(XHS_BRIDGE_CMD) + '">' + escHtml(XHS_BRIDGE_CMD) + '<button class="mcp-copy" type="button">复制</button></div>' +
        '<div class="mcp-gw-tip">只想先起桥、隧道自己另开：</div>' +
        '<div class="mcp-code" data-copy="' + mcpAttr(XHS_TUNNEL_CMD) + '">' + escHtml(XHS_TUNNEL_CMD) + '<button class="mcp-copy" type="button">复制</button></div>' +
        '</div></div>';

      html += '<div class="mcp-sec"><div class="mcp-sec-t">起服务 / 登录（点一下复制）</div>';
      MCP_XHS_CODES.forEach(function (it) {
        html += '<div class="mcp-code" data-copy="' + mcpAttr(it[1]) + '"><span class="mcp-code-t">' + escHtml(it[0]) + '</span><br>' + escHtml(it[1]) +
                '<button class="mcp-copy" type="button">复制</button></div>';
      });
      html += '</div>';

      html += '<div class="mcp-sec"><div class="mcp-sec-t">18 个 MCP 工具</div><div class="mcp-chips">';
      MCP_XHS_TOOLS.forEach(function (t) { html += '<span class="mcp-chip">' + escHtml(t) + '</span>'; });
      html += '</div></div>';

      html += '<div class="mcp-sec"><div class="mcp-sec-t">命令速查（点一下复制）</div><div class="group-card">';
      MCP_XHS_CMDS.forEach(function (it) {
        html += '<div class="mcp-cmd" data-copy="' + mcpAttr(it[1]) + '"><span class="mcp-cmd-k">' + escHtml(it[0]) + '</span><span class="mcp-cmd-c">' + escHtml(it[1]) + '</span></div>';
      });
      html += '</div></div>';

      html += '<div class="mcp-sec"><div class="mcp-sec-t">踩坑 & 规矩</div><div class="group-card mcp-tips">';
      MCP_XHS_TIPS.forEach(function (t) { html += '<div class="mcp-tip">' + escHtml(t) + '</div>'; });
      html += '</div></div>';

      mcpXhsBody.innerHTML = html;
      var bar = document.getElementById('mcpXhsCharBar');
      if (bar) bar.addEventListener('click', openMcpCharPick);
      mcpXhsBody.querySelectorAll('[data-copy]').forEach(function (el) {
    ...
    function mcpRenderCharList() {
      ...
      mcpCharList.innerHTML = list.map(function (e) {
        ...
        return '<div class="mcp-char-item' + (on ? ' on' : '') + '" data-cid="' + mcpAttr(String(e.id)) + '">' + av +
               ...
               '<div class="mcp-char-sub">' + (on ? '已授权 · 聊天里能用小红书' : '未授权') + '</div></div>' +
    ...
    function openMcpXhs() {
      mcpRenderXhs();
      mcpXhsBody.scrollTop = 0;
      mcpXhsOverlay.classList.add('open');
    }
    function openMcpCharPick() {
      mcpPickBuf = {};
      mcpGrantedXhs().forEach(function (g) { mcpPickBuf[String(g.id)] = { id: String(g.id), name: g.name || '', avatar: g.avatar || '' }; });
      mcpRenderCharList();
    ...
    if (mcpOverlay) {
      ...
      document.getElementById('mcpXhsBack').addEventListener('click', function () { mcpXhsOverlay.classList.remove('open'); });
      mcpXhsOverlay.addEventListener('click', function (e) { if (e.target === mcpXhsOverlay) mcpXhsOverlay.classList.remove('open'); });
    ...
    /* ===== v211：小红书通道（真读取 / 真分享） ===== */
    var XHS_GW_KEY = 'ins-xhs-gateway';
    var XHS_BRIDGE_CMD = 'python3 xhs_bridge.py --tunnel --mcp http://localhost:18061/mcp --token aetheron';
    var XHS_TOKEN_DEF = 'aetheron';   /* v212：桥脚本默认令牌，输入框直接预填 */
    var XHS_TUNNEL_CMD = 'npx -y cloudflared tunnel --url http://localhost:18901';
    var mcpGwTesting = false;
    var mcpGwState = '';
    var xhsPrefetchTimer = null;
    var xhsInflight = {};

    function xhsGw() {
      var o = null;
      try { o = JSON.parse(dbGet(XHS_GW_KEY) || '{}'); } catch (e) { o = null; }
      if (!o || typeof o !== 'object') o = {};
      return { url: String(o.url || '').replace(/\/+$/, ''), token: String(o.token || '') };
    }
    function xhsGwSave(url, token) {
      dbSet(XHS_GW_KEY, JSON.stringify({ url: String(url || '').trim().replace(/\/+$/, ''), token: String(token || '').trim() }));
    }
    function xhsErrBrief(e) { return String((e && (e.message || e.name)) || e || '未知错误').slice(0, 90); }

    /* ---------- 两种通道：直连 MCP(/mcp 结尾) / 桥接脚本(普通 HTTP) ---------- */
    function xhsMcpText(r) {
      try {
        var c = r && r.content;
        if (!c) return '';
    ...
    function xhsMcpPost(url, headers, payload) {
      return fetch(url, { method: 'POST', headers: headers, body: JSON.stringify(payload) });
    }
    function xhsMcpCall(url, token, tool, args, cb) {
      var h = { 'Content-Type': 'application/json', 'Accept': 'application/json, text/event-stream' };
      if (token) h['Authorization'] = 'Bearer ' + token;
      var sid = '';
      var init = { jsonrpc: '2.0', id: 1, method: 'initialize', params: { protocolVersion: '2024-11-05', capabilities: {}, clientInfo: { name: 'AEphone', version: '211' } } };
      xhsMcpPost(url, h, init).then(function (res) {
        ...
        xhsMcpPost(url, h2, { jsonrpc: '2.0', method: 'notifications/initialized' }).catch(function () {});
        return xhsMcpPost(url, h2, { jsonrpc: '2.0', id: 2, method: 'tools/call', params: { name: tool, arguments: args || {} } }).then(function (r2) { return r2.text(); });
      }).then(function (body) {
        var j = xhsMcpParse(body);
        if (!j) { cb({ ok: false, err: '服务端没回能看懂的 JSON（多半是没开跨域，或者这不是 MCP 地址）' }); return; }
        if (j.error) { cb({ ok: false, err: String(j.error.message || j.error.code || 'MCP 报错') }); return; }
        var r = j.result || {};
        if (r.isError) { cb({ ok: false, err: xhsMcpText(r) || '工具返回错误' }); return; }
        cb({ ok: true, text: xhsMcpText(r), raw: r });
      }).catch(function (e) {
        cb({ ok: false, err: '连不上通道：' + xhsErrBrief(e) + '（浏览器直连 MCP 常被跨域挡，改用桥接地址更稳）' });
      });
    }
    function xhsBridgeCall(url, token, tool, args, cb) {
      var h = { 'Content-Type': 'application/json' };
      if (token) h['X-Xhs-Token'] = token;
      var ctl = null, timer = null;
      try { ctl = new AbortController(); timer = setTimeout(function () { try { ctl.abort(); } catch (e) {} }, 60000); } catch (e2) {}
      fetch(url + '/call', { method: 'POST', headers: h, body: JSON.stringify({ tool: tool, args: args || {} }), signal: ctl ? ctl.signal : undefined })
        .then(function (r) { return r.text().then(function (t) { return { code: r.status, body: t }; }); })
        .then(function (o) {
          var j = null;
          try { j = JSON.parse(o.body); } catch (e) {}
          if (!j) {
            cb({ ok: false, err: (o.code === 404 ? '通道地址不对（404），端口或路径检查一下' : '通道返回的不是 JSON：' + String(o.body || '').slice(0, 80)) });
            return;
          }
          if (j.ok === false) { cb({ ok: false, err: String(j.err || j.error || '通道报错') }); return; }
          cb({ ok: true, text: String(j.text || ''), data: j.data, raw: j });
        })
        .catch(function (e) { cb({ ok: false, err: '连不上通道：' + xhsErrBrief(e) + '（手机要能访问到那台机器）' }); })
        .then(function () { if (timer) clearTimeout(timer); });
    }
    /* 站点是 https（GitHub Pages）时，浏览器不让它请求 http 的通道（混合内容会被直接拦掉）。
       本机 localhost 例外，所以只有「本机自用」能走 http；给外人用必须给通道套 https。 */
    function xhsMixedBlocked(url) {
      try {
    ...
    function xhsCall(tool, args, cb) {
      var g = xhsGw();
      if (!g.url) { cb({ ok: false, err: '还没填通道地址：MCP → 小红书 → 通道地址' }); return; }
      if (xhsMixedBlocked(g.url)) {
        cb({ ok: false, err: '这条通道是 http 的，站点是 https，浏览器不让连（混合内容会被拦）。给通道套一层 https（例：cloudflared tunnel --url http://localhost:18901 拿到的 trycloudflare 地址）再填进来。' });
        return;
      }
      if (/\/mcp\/?$/.test(g.url)) xhsMcpCall(g.url, g.token, tool, args, cb);
      else xhsBridgeCall(g.url, g.token, tool, args, cb);
    }

    /* ---------- 链接识别 ---------- */
    function xhsLinksIn(text) {
      var t = String(text || ''), out = [], m = null;
      var re = /(https?:\/\/)?(www\.)?(xiaohongshu\.com|xhslink\.com)\/[^\s，。、）)】\]"'<>]+/gi;
      while ((m = re.exec(t))) {
        var u = m[0].replace(/[.,;!?，。；！？]+$/, '');
    ...
    function xhsIdOf(url) {
      var m = /(?:explore|discovery\/item|item)\/([0-9a-zA-Z]{16,32})/.exec(String(url || ''));
      return m ? m[1] : '';
    }
    /* v213：手机 App 分享复制出来的多是短链（xhslink.com/a/xxx），本身不带笔记 id */
    function xhsIsShort(url) {
      var u = String(url || '');
      if (!u) return false;
      if (!xhsIdOf(u)) return true;
      return /(^|\.)xhslink\.com\//i.test(u);
    }
    /* v213：让通道把短链跟到真正的笔记地址，回长链 + feed_id + xsec_token */
    function xhsResolveUrl(url, cb) {
      cb = cb || function () {};
      var g = xhsGw();
      if (!g.url) { cb({ ok: false, hard: true, err: '还没填通道地址：MCP → 小红书 → 通道地址' }); return; }
      if (/\/mcp\/?$/.test(g.url)) { cb({ ok: false, hard: true, err: '短链要通道帮忙展开，直连 MCP 展不开。把「通道地址」换成桥接地址（不带 /mcp 的那个）再发一次。' }); return; }
      if (xhsMixedBlocked(g.url)) { cb({ ok: false, hard: true, err: '这条通道是 http 的，站点是 https，浏览器不让连（混合内容会被拦）。给通道套一层 https 再试。' }); return; }
      var q = g.url.replace(/\/+$/, '') + '/resolve?url=' + encodeURIComponent(url) + (g.token ? '&token=' + encodeURIComponent(g.token) : '');
      fetch(q, { method: 'GET' })
        .then(function (r) { return r.text().then(function (t) { return { code: r.status, body: t }; }); })
        .then(function (o) {
          var j = null;
          try { j = JSON.parse(o.body); } catch (e) {}
          if (!j) { cb({ ok: false, err: '通道没回能看懂的解析结果（桥脚本可能是旧版，换成站点根 tools/xhs_bridge.py 最新那份）' }); return; }
          if (j.ok === false) { cb({ ok: false, err: String(j.err || '短链没展开') }); return; }
          cb({ ok: true, url: String(j.url || ''), feed_id: String(j.feed_id || ''), xsec_token: String(j.xsec_token || '') });
        })
        .catch(function (e) { cb({ ok: false, err: '展不开短链：' + xhsErrBrief(e) }); });
    }

    /* ---------- 把工具返回收拾成一条笔记 ---------- */
    function xhsNormNote(text, data) {
      var note = { title: '', author: '', desc: '', likes: '', comments: [], raw: String(text || '') };
      var d = (data && typeof data === 'object') ? (data.data && typeof data.data === 'object' ? data.data : data) : null;
      if (d) {
        note.title = String(d.title || d.name || d.note_title || '').trim();
        note.author = String((d.author && (d.author.nickname || d.author.name)) || d.author || d.nickname || d.user || '').trim();
        note.desc = String(d.desc || d.content || d.text || d.body || '').trim();
        note.likes = String(d.liked_count || d.likes || d.like_count || '').trim();
        var cs = d.comments || d.comment_list || [];
        if (cs && cs.length) {
          for (var i = 0; i < cs.length; i++) {
            var c = cs[i] || {};
            var u = String((c.user_info && c.user_info.nickname) || c.user || c.nickname || '').trim();
            var cx = String(c.content || c.text || '').trim();
            if (cx) note.comments.push({ user: u, text: cx, likes: String(c.like_count || c.likes || '').trim() });
          }
        }
      }
      var t = note.raw;
      note.cmTotal = 0;
      if (t && (!note.title || !note.desc)) {
        var lines = t.split(/\r?\n/);
        var body = [], inCm = false, sawLabel = false;
        for (var k = 0; k < lines.length; k++) {
          var ln = lines[k].replace(/^\s+/, '').replace(/\s+$/, '');
          if (!ln) continue;
          var mTitle = /^(?:标题|title)\s*[:：]\s*(.+)$/i.exec(ln);
          var mAuthor = /^(?:作者|author|昵称)\s*[:：]\s*(.+)$/i.exec(ln);
          var mDesc = /^(?:正文|内容|描述|desc|content)\s*[:：]\s*(.+)$/i.exec(ln);
          var mLike = /^(?:点赞|赞|likes?|liked_count)\s*[:：]\s*(.+)$/i.exec(ln);
          if (/^(?:评论|评论区|全部评论|comments)/i.test(ln)) {
            inCm = true;
            var mc0 = /[（(]\s*(\d+)\s*[）)]/.exec(ln);
            if (mc0) note.cmTotal = parseInt(mc0[1], 10) || 0;
            continue;
          }
          if (mTitle && !note.title) { note.title = mTitle[1].trim(); sawLabel = true; continue; }
          if (mAuthor && !note.author) { note.author = mAuthor[1].trim(); sawLabel = true; continue; }
          if (mLike && !note.likes) { note.likes = String(mLike[1]).trim(); sawLabel = true; continue; }
          if (mDesc) { body.push(mDesc[1].trim()); sawLabel = true; continue; }
          if (inCm) {
            var mc = /^(?:\d+[.、)]|[-*•])\s*(?:([^:：]{1,20})\s*[:：]\s*)?(.+)$/.exec(ln);
            if (mc && mc[2]) { note.comments.push({ user: (mc[1] || '').trim(), text: mc[2].trim(), likes: '' }); continue; }
            note.comments.push({ user: '', text: ln, likes: '' });
            continue;
          }
          body.push(ln);
        }
        if (!note.desc && body.length) note.desc = body.join('\n');
      }
      if (!note.desc && !note.title && !note.comments.length && note.raw) note.desc = note.raw;
      if (note.desc.length > 1200) note.desc = note.desc.slice(0, 1200) + '…';
      if (note.comments.length > 20) note.comments = note.comments.slice(0, 20);
      return note;
    }
    function xhsPickFromSearch(text, data) {
      var d = (data && typeof data === 'object') ? (data.data && typeof data.data === 'object' ? data.data : data) : null;
      var arr = (d && (d.items || d.feeds || d.notes || d.list || d.results)) || null;
      if (arr && arr.length) {
        for (var i = 0; i < arr.length; i++) {
          var it = arr[i] || {};
          var u = String(it.url || it.link || it.note_url || '').trim();
          var id = String(it.id || it.feed_id || it.note_id || '').trim();
          if (!u && id) u = 'https://www.xiaohongshu.com/explore/' + id;
          if (u) return { url: u, title: String(it.title || it.display_title || '').trim(), author: String((it.author && (it.author.nickname || it.author.name)) || it.author || it.nickname || '').trim() };
        }
      }
      var t = String(text || '');
      var m = /https?:\/\/(?:www\.)?xiaohongshu\.com\/[^\s，。、）)】\]"'<>]+/.exec(t);
      if (m) return { url: m[0], title: '', author: '' };
      var m2 = /\b([0-9a-f]{24})\b/.exec(t);
      if (m2) return { url: 'https://www.xiaohongshu.com/explore/' + m2[1], title: '', author: '' };
      return null;
    }

    /* ---------- 卡片 ---------- */
    function xhsPlainCard(title, sub) {
      return '<div class="xhs-card xhs-card-warn"><div class="xhs-card-h">小红书</div>' +
        '<div class="xhs-card-t">' + escHtml(title || '') + '</div>' +
        '<div class="xhs-card-m">' + escHtml(String(sub || '').slice(0, 160)) + '</div></div>';
    }
    function xhsNoteCardHtml(note, tag, cls) {
      var h = '<div class="xhs-card' + (cls ? ' ' + cls : '') + '">';
      h += '<div class="xhs-card-h"><span class="xhs-card-tag">' + escHtml(tag || '小红书') + '</span>';
      if (note.url) h += '<a class="xhs-card-open" href="' + escHtml(note.url).replace(/"/g, '&quot;') + '" target="_blank" rel="noopener">打开</a>';
      h += '</div>';
      h += '<div class="xhs-card-t">' + escHtml(note.title || '（这条笔记没有标题）') + '</div>';
      var meta = [];
      if (note.author) meta.push(note.author);
      if (note.likes) meta.push('赞 ' + note.likes);
      var nCm = (note.comments && note.comments.length) || note.cmTotal || 0;
      meta.push('评论 ' + nCm + ' 条');
      if (note.realUrl && note.realUrl !== note.url) meta.push('短链已展开');
      h += '<div class="xhs-card-m">' + escHtml(meta.join(' · ')) + '</div>';
      if (note.desc) h += '<div class="xhs-card-desc">' + escHtml(note.desc.slice(0, 220)) + (note.desc.length > 220 ? '…' : '') + '</div>';
      if (note.comments && note.comments.length) {
        h += '<details class="xhs-card-cm"><summary>看评论区（' + note.comments.length + ' 条）</summary>';
        for (var i = 0; i < note.comments.length; i++) {
          var c = note.comments[i];
          h += '<div class="xhs-cm"><span class="xhs-cm-u">' + escHtml(c.user || '网友') + '</span>' + escHtml(String(c.text || '').slice(0, 160)) + '</div>';
        }
        h += '</details>';
      }
      h += '</div>';
    ...
    function xhsNotePut(conv, note) {
      if (!conv || !note) return;
      if (!conv.xhsNotes) conv.xhsNotes = [];
      var list = [];
      for (var i = 0; i < conv.xhsNotes.length; i++) {
        if (conv.xhsNotes[i] && conv.xhsNotes[i].url !== note.url) list.push(conv.xhsNotes[i]);
      }
      list.push(note);
    ...
    /* 同一条链接只发一次请求：谁先要谁发起，后来的一起等结果 */
    function xhsFetchNote(conv, url, cb) {
      cb = cb || function () {};
      if (xhsInflight[url]) { xhsInflight[url].push(cb); return; }
      xhsInflight[url] = [cb];
      var finish = function (note, r) {
        var waiters = xhsInflight[url] || [];
        delete xhsInflight[url];
        for (var i = 0; i < waiters.length; i++) { try { waiters[i](note, r); } catch (e) {} }
      };
      var read = function (info) {
        var args = { url: (info && info.url) || url, load_all_comments: true, limit: 20 };
        if (info && info.feed_id) args.feed_id = info.feed_id;
        if (info && info.xsec_token) args.xsec_token = info.xsec_token;
        xhsCall('get_feed_detail', args, function (r) {
          var note = null;
          if (r && r.ok) {
            var n = xhsNormNote(r.text, r.data);
            if (n.title || n.desc) {
              n.url = url; n.ts = Date.now(); n.dir = 'in';
              if (args.url !== url) n.realUrl = args.url;
              note = n; xhsNotePut(conv, n);
            }
          }
          finish(note, r);
        });
      };
      /* v213：短链先展开成笔记地址（带 feed_id / xsec_token），再交给通道去读 */
      if (!xhsIsShort(url)) { read(null); return; }
      xhsResolveUrl(url, function (res) {
        if (res && res.hard) { finish(null, { ok: false, err: res.err }); return; }
        read(res && res.ok ? res : null);
      });
    }
    function xhsNoteCached(conv, url, ms) {
      var hit = null;
      var list = (conv && conv.xhsNotes) || [];
      for (var i = list.length - 1; i >= 0; i--) {
        if (list[i] && list[i].url === url && (Date.now() - (list[i].ts || 0) < (ms || 30 * 60 * 1000))) { hit = list[i]; break; }
      }
      return hit;
    }
    function xhsInsert(conv, role, html) {
      if (!conv) return;
      addChatMsg(role, { type: 'html', text: html }, conv);
    }

    /* ---------- 读一条笔记 ---------- */
    function xhsReadFor(conv, url) {
      if (!conv || !url) return;
      var hit = xhsNoteCached(conv, url);
      if (hit) { xhsInsert(conv, 'me', xhsNoteCardHtml(hit, '已读取')); return; }
      if (!xhsGw().url) {
        xhsInsert(conv, 'me', xhsPlainCard('通道还没接上，这条读不到', '链接我看到了，但读不到内容。去「MCP → 小红书 → 通道地址」把通道接上，我才能真读到这条笔记。'));
        return;
      }
      xhsFetchNote(conv, url, function (note, r) {
        if (!note) {
          xhsInsert(conv, 'me', xhsPlainCard('这条没读成', (r && r.err) || '读回来是空的，可能是登录态过期或这条笔记有权限限制。'));
          return;
        }
        xhsInsert(conv, 'me', xhsNoteCardHtml(note, '已读取'));
        if (typeof toast === 'function') toast('已读到这条笔记，TA 也看到了');
      });
    }

    /* ---------- 让 TA 主动分享一条 ---------- */
    function xhsShareOne(conv, kw) {
      if (!conv) return;
      if (!xhsGw().url) {
        xhsInsert(conv, 'other', xhsPlainCard('通道还没接上', '想给你推一条笔记，但通道没接上，翻不出去。'));
        return;
      }
      xhsCall('search_feeds', { keyword: kw || '今日推荐', limit: 20 }, function (r) {
        if (!r.ok) { xhsInsert(conv, 'other', xhsPlainCard('没搜着', r.err)); return; }
        var pick = xhsPickFromSearch(r.text, r.data);
        if (!pick) { xhsInsert(conv, 'other', xhsPlainCard('没搜着', '这次没翻到能推给你的笔记，等下再试。')); return; }
        xhsCall('get_feed_detail', { url: pick.url, load_all_comments: true, limit: 20 }, function (r2) {
          var note = (r2 && r2.ok) ? xhsNormNote(r2.text, r2.data) : { title: pick.title, author: pick.author, desc: '', comments: [] };
          if ((!note.title && !note.desc) && (pick.title || pick.author)) { note.title = pick.title; note.author = pick.author; }
          note.url = pick.url; note.ts = Date.now(); note.dir = 'share';
          xhsNotePut(conv, note);
          xhsInsert(conv, 'other', xhsNoteCardHtml(note, 'TA 分享', 'xhs-card-ta'));
          if (typeof toast === 'function') toast('TA 给你推了一条小红书');
        });
      });
    }

    /* ---------- 发消息时触发 ---------- */
    function xhsOnSend(text, conv) {
      if (!conv) return;
      var links = xhsLinksIn(text);
      if (links.length) { xhsReadFor(conv, links[0]); return; }
      var t = String(text || '');
      if (/小红书/.test(t) && /(发|推|分享|来一|来条|整一|找一|看看|翻一)/.test(t)) {
        var kw = t.replace(/给我|你|小红书|发一?[条个]?|推一?[条个]?|分享|来一?[条个]?|整一?[条个]?|找一?[条个]?|看看|翻一?[条个]?|一条|笔记|吧|呗|呀|啊|的|了/g, '').trim();
        xhsShareOne(conv, kw);
      }
    }
    /* 输入框里刚粘上链接就先偷偷读回来，等你按发送时卡片就是现成的 */
    function xhsOnInput(text) {
      if (xhsPrefetchTimer) clearTimeout(xhsPrefetchTimer);
      var conv = chatCurrentConv;
      xhsPrefetchTimer = setTimeout(function () {
        var links = xhsLinksIn(text);
        if (!links.length || !conv) return;
        if (!xhsGw().url) return;
        if (xhsNoteCached(conv, links[0])) return;
        xhsFetchNote(conv, links[0], function () {});
      }, 500);
    }
    ...
    function mcpGwWire() {
      ...
      /* v212：令牌留空就默认填脚本同款 aetheron，省得两边对不上 */
      if (t && !String(t.value || '').trim()) t.value = XHS_TOKEN_DEF;
      ...
      if (sv) sv.addEventListener('click', function (e) {
        e.stopPropagation();
        xhsGwSave(u ? u.value : '', t ? t.value : '');
        mcpGwState = xhsGw().url ? ('已保存：' + xhsGw().url) : '通道地址已清空';
      ...
      if (ts) ts.addEventListener('click', function (e) {
        ...
        mcpGwState = '正在测……';
        if (st) st.textContent = mcpGwState;
        xhsCall('check_login_status', {}, function (r) {
          mcpGwTesting = false;
          mcpGwState = r.ok ? ('已连通' + (r.text ? '：' + String(r.text).replace(/\s+/g, ' ').slice(0, 80) : '')) : ('没通：' + r.err);
          if (st) st.textContent = mcpGwState;
        });
      });
    }
    ...
    var _sendWithQuote = function (v) {
      ...
      addChatMsg('me', obj);
      try { xhsOnSend(v, chatCurrentConv); } catch (eXhsSend) {}
    };
    ...
    chatDetailInput.addEventListener('input', function () { try { xhsOnInput(chatDetailInput.value); } catch (eXhsIn) {} });
    chatDetailInput.addEventListener('paste', function () { setTimeout(function () { try { xhsOnInput(chatDetailInput.value); } catch (eXhsPaste) {} }, 60); });
    ...
    function chatBuildSystemPrompt() {
      ...
      /* v210：该角色被授权使用小红书 MCP 时，把能力说明写进提示词（不给它编执行结果的口子） */
      try {
        var _mcList = mcpGrants().xhs || [];
        if (_mcList.length && chatCurrentConv) {
          ...
          if (_mcHit) parts.push('【小红书 MCP 能力】你（' + (chatCurrentConv.name || '该角色') + '）已接入本机的小红书 MCP：18 个工具，含 search_feeds（搜笔记）、get_feed_detail（读笔记+评论）、like_feed / favorite_feed（点赞·收藏）、post_comment_to_feed / reply_comment_in_feed（评论·回楼中楼）、list_notifications / reply_notification（看通知·回评）、get_unread_count、check_login_status 等，服务地址 http://localhost:18061/mcp。对方让你搜/读/赞/收藏/评论/看通知时，按你的口吻自然接活，能做就应下来；但真实动作由本机 MCP 服务执行，禁止编造执行结果（例如「我已经帮你点赞了」），没把握就直说没把握。');
        }
      } catch (e) {}
      /* v211：刚真读到的小红书 → 作为「已读原文」注入，让角色是真看过，不是编 */
      try {
        var _xnAll = (chatCurrentConv && chatCurrentConv.xhsNotes) ? chatCurrentConv.xhsNotes : [];
        var _xnLast = null;
        for (var _xi = _xnAll.length - 1; _xi >= 0; _xi--) {
          if (_xnAll[_xi] && _xnAll[_xi].ts && (Date.now() - _xnAll[_xi].ts < 30 * 60 * 1000)) { _xnLast = _xnAll[_xi]; break; }
        }
        if (_xnLast) {
          var _xTxt = '【小红书笔记原文｜刚刚真实读取，不是编的】';
          if (_xnLast.dir === 'share') _xTxt += '你（' + (chatCurrentConv.name || '该角色') + '）刚主动发给对方一条小红书笔记，内容如下：';
          else _xTxt += '对方刚发来一条小红书链接，你已经真的打开看过了，内容如下：';
          _xTxt += '\n标题：' + (_xnLast.title || '（无标题）');
          if (_xnLast.author) _xTxt += '\n作者：' + _xnLast.author;
          if (_xnLast.likes) _xTxt += '\n点赞：' + _xnLast.likes;
          if (_xnLast.desc) _xTxt += '\n正文：' + String(_xnLast.desc).slice(0, 800);
          if (_xnLast.comments && _xnLast.comments.length) {
            var _xcs = [];
            for (var _xc = 0; _xc < Math.min(_xnLast.comments.length, 15); _xc++) {
              var _xo = _xnLast.comments[_xc];
              _xcs.push('- ' + (_xo.user || '网友') + '：' + String(_xo.text || '').slice(0, 120));
            }
            _xTxt += '\n评论区（真实读取）：\n' + _xcs.join('\n');
          }
          _xTxt += '\n聊天时可以直接引用标题、正文和评论里的内容，像真看过一样自然聊；没读到的部分不许编。';
          parts.push(_xTxt);
        }
      } catch (eXhs) {}
