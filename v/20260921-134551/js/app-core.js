  (function () {
    ...
    function mcpRenderPeek() {
      ...
      html += '<div class="mcp-sec"><div class="mcp-sec-t">通道地址（站点连云端 MCP）</div><div class="group-card mcp-gw">' +
        ...
        '<div class="mcp-gw-tip">地址和 Token 只存在这台手机的浏览器本地（localStorage），不上传、也不写进站点源码。</div>' +
        '</div></div>';
    ...
    function peekCall(tool, args, cb) {
      cb = cb || function () {};
      var g = peekGw();
      if (!g.url) { cb({ ok: false, err: '还没填通道地址：MCP → 掌心窗 → 通道地址' }); return; }
      if (peekMixedBlocked(g.url)) { cb({ ok: false, err: '这条地址是 http 的，站点是 https，浏览器不让连（混合内容会被拦）。云端部署请用 https 的 mcp 地址。' }); return; }
      peekMcpCall(peekMcpUrl(g.url), g.token, tool, args, cb);
    }
