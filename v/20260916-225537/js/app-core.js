  (function () {
    ...
    function memberNpcEditBody() {
      ...
      html += '<div class="npc-prog-wrap" id="npcProgWrap"><div class="npc-prog-bar"><i class="npc-prog-in" id="npcProgIn"></i></div><span class="npc-prog-tx" id="npcProgTx"></span></div>';
      html += '</div>';
    ...
    function npcAiMenuOpen() {
      ...
      pop.innerHTML = '<div class="npc-ai-mask"></div><div class="npc-ai-box">' +
        ...
        '<button type="button" class="npc-ai-opt" data-npcai="write"><span class="npc-ai-opt-n">AI 帮写</span><span class="npc-ai-opt-d">按主角色世界观，直接写满整份人设</span></button>' +
    ...
    function memberNpcAiLore(mode) {
      ...
      npcProgStart();
      toast(mode === 'polish' ? 'AI 正在润色人设…' : 'AI 正在写人设…');
      npcAiRun(mode, cur, 0);
    }
    function npcAiRun(mode, cur, attempt) {
      ...
      var MAX_TRY = 6;
      /* v204：点一次就要把十段写完 —— 一轮只让 AI 写 3 段。
         一轮要十段会被输出长度上限截断、只写出前一两段，用户看到的就是「每次只写一丁点」；
         现在缺的段落自动一轮轮补齐，直到十段齐全。 */
      var targets = [];
      if (mode === 'polish') {
        for (var _pi = 0; _pi < names.length; _pi++) if (String(cur[_pi] || '').trim()) targets.push(_pi);
        if (!targets.length) for (var _pj = 0; _pj < names.length; _pj++) targets.push(_pj);
      } else {
        targets = miss.length ? miss.slice() : [];
        if (!targets.length) for (var _pk = 0; _pk < names.length; _pk++) targets.push(_pk);
      }
      if (targets.length > 3) targets = targets.slice(0, 3);
      ...
      var tip;
      if (mode === 'polish') tip = '下面是这个 NPC 已有的人设草稿。本轮只把这几段扩写到 150~220 字：' + tNames.join('、') + '，保留草稿里的信息与口吻，写得更具体更通顺。';
      else tip = '本轮只写这几段：' + tNames.join('、') + '。';
      ...
      memberAiAsk('你是世界观与角色设定助手，只输出 JSON。', user, function (txt, err) {
        if (err || !txt) {
          npcAiBusy = false;
          npcProgDone();
          toast('AI 暂时连不上，没有写入任何预设内容，稍后再点一次试试');
          return;
        }
        ...
        if (still.length && attempt < MAX_TRY) {
          /* 先把本轮成果写进输入区，再接着补下一批 —— 用户全程只点了一次 */
          try { npcLoreFillToForm(out); } catch (ePF) {}
          npcAiRun(mode, out, attempt + 1);
          return;
        }
        npcAiBusy = false;
        npcLoreFillToForm(npcLoreRealign(out));
        npcProgDone();
        toast(still.length ? ('AI 写了 ' + (names.length - still.length) + ' 段，还剩 ' + still.length + ' 段，再点一次 AI') : (mode === 'polish' ? '人设已润色（十段齐全）' : '人设已写好（十段齐全）'));
      }, { maxTokens: 2400 });
    }
    ...
    function npcProgStart() {
      try {
        ...
        npcProgTimer = setInterval(function () {
          ...
          if (t2) t2.textContent = '正在写人设… ' + Math.round(p) + '%';
        }, 380);
      } catch (e) {}
    }
