  (function () {
    ...
    function memberEditBody(m) {
      ...
      form += '<div class="field"><label>角色ID</label>' +
        ...
        '</div>' +
        '<div class="mem-field-foot"><span>别人扫二维码打开名片页：全部公开 = 资料全显示；部分公开 = 只显示勾选项；仅私密 = 只显示名字，看不到人设细节，但仍可加 TA 聊天；用 AEphone 的「扫一扫 / 从相册选二维码」扫它，能直接把 TA 加为好友</span></div>' +
        '<div class="mem-qr-label">角色二维码（扫一下直接加 TA 为好友）</div>' +
        ...
        '<div class="mem-qr-preview" id="memQrPreview">' + (memQrText ? '<img src="' + memQrUrl(memQrText) + '" alt="角色二维码" data-qzoom="1">' + '<span class="mem-qr-tip">点二维码放大 · 长按图片可存相册 · 别人扫这个码能直接加 TA 为好友</span>' : '<span class="mem-qr-empty">点“生成二维码”，别人扫一下就能加 TA 为好友</span>') + '</div>' +
    ...
    function qrDrawCard(q, cell, img, name, sub) {
      try {
        var n = q.getModuleCount();
        var pad = Math.round(cell * 6);
        var qz = n * cell;
        var nameH = name ? Math.round(cell * 9) : 0;
        var subH = Math.round(cell * 7);
        var W = qz + pad * 2;
        var H = pad + nameH + (name ? Math.round(cell * 3.5) : 0) + qz + Math.round(cell * 3) + subH + pad;
        var cv = document.createElement('canvas');
        cv.width = W; cv.height = H;
        var ctx = cv.getContext('2d');
        ctx.fillStyle = '#ffffff';
        qrRoundRect(ctx, 0, 0, W, H, Math.round(cell * 7)); ctx.fill();
        var y = pad;
        if (name) {
          ctx.fillStyle = '#17171b';
          ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
          qrFitFont(ctx, name, W - pad * 2, Math.round(cell * 5.2), '700 ');
          ctx.fillText(name, W / 2, y + nameH / 2);
          y += nameH + Math.round(cell * 3.5);
        }
        ctx.fillStyle = '#17171b';
        for (var r = 0; r < n; r++) {
          for (var c = 0; c < n; c++) {
            if (q.isDark(r, c)) ctx.fillRect(pad + c * cell, y + r * cell, cell, cell);
          }
        }
        /* 中心角色头像：白底圆 + 圆裁头像；没头像就用渐变底 + 名字首字 */
        var cx = pad + qz / 2, cy = y + qz / 2;
        var rr = qz * 0.16;
        ctx.fillStyle = '#ffffff';
        ...
        ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
        qrFitFont(ctx, sub, W - pad * 2, Math.round(cell * 3.4), '');
        ctx.fillText(sub, W / 2, y + qz + Math.round(cell * 3) + subH / 2);
        return cv.toDataURL('image/png');
      } catch (e) { return ''; }
    }
    ...
    function memberQrSync() {
      ...
      if (!pv) return;
      if (!memQrText) { pv.innerHTML = '<span class="mem-qr-empty">点“生成二维码”，别人扫一下就能加 TA 为好友</span>'; return; }
      ...
      qrPrettyPng(memQrText, { avatar: memAvatar, name: memberQrPreviewName(), cell: 5, sub: '扫一扫 · 加我为好友' }, function (png) {
        ...
        cur.innerHTML = '<img src="' + png + '" alt="角色二维码" data-qzoom="1">' +
          '<span class="mem-qr-tip">点二维码放大 · 长按图片可存相册 · 别人扫这个码能直接加 TA 为好友</span>';
      });
    }
    ...
    function openQrViewer(text, opt) {
      ...
      memberQrBigPng(text, opt, function (png) {
        ...
        sv2.type = 'button'; sv2.className = 'mqv-btn ghost'; sv2.textContent = '复制内容';
        var cls = document.createElement('button');
        cls.type = 'button'; cls.className = 'mqv-btn ghost'; cls.textContent = '关闭';
        bar.appendChild(sv1); bar.appendChild(sv2); bar.appendChild(cls);
        var tip2 = document.createElement('div');
        tip2.className = 'mqv-tip';
        tip2.textContent = '长按二维码图片，选「存储图像 / 保存图片」也能直接存进相册；别人用另一台手机扫它，就能把 TA 加为好友';
        box.appendChild(im); box.appendChild(tip2); box.appendChild(tx); box.appendChild(bar);
        ov.appendChild(box);
        ...
        sv2.addEventListener('click', function (e) {
          e.stopPropagation();
          try {
            if (navigator.clipboard && navigator.clipboard.writeText) {
              navigator.clipboard.writeText(String(text || '')).then(function () { toast('已复制二维码内容'); }, function () { toast('复制失败，长按上方文字手动复制'); });
            } else { toast('当前浏览器不支持自动复制，长按上方文字复制'); }
          } catch (er) { toast('复制失败，长按上方文字手动复制'); }
        });
      });
    }
    /* v187：角色名片链接（扫码打开名片页，按「公开范围」决定能看到什么） */
    function memberCardUrl(src, card) {
      ...
      if (mode === 'secret') {
        ...
        if (f.lore && src.lore) p.bio = String(src.lore).slice(0, 160);
        if (f.look && src.look) p.look = String(src.look).slice(0, 160);
        if (f.voice && src.voiceId) p.voice = String(src.voiceId).slice(0, 60);
        if (f.alts && src.alts && src.alts.length) p.alts = src.alts.slice(0, 6).map(function (x) { return String(x).slice(0, 20); });
        if (f.npcs && src.npcs && src.npcs.length) p.npc = src.npcs.slice(0, 8).map(function (x) { return { n: String(x.name || '').slice(0, 20), r: String(x.rel || '').slice(0, 20) }; });
        var av = String(src.avatar || '');
        if (av && av.length <= 400) p.av = av;
      }
      p.ts = Date.now();
    ...
    function memberQrDefault() {
      ...
      memQrText = memberCardUrl(src, v.card);
      memDirty = true;
      memberQrSync();
    ...
    function afMyQr() {
      ...
      var html = '<div class="af-mem-title">选一个角色看 TA 的二维码（点开可保存到相册，别人扫它就能加 TA）：</div>';
      html += list.map(function (m, i) {
        var nm = memberDispName(m);
      ...
      rs.querySelectorAll('.af-mem-item').forEach(function (it) {
        it.addEventListener('click', function () {
          ...
          var m = list[mi];
          if (!m) return;
          var txt = memberCardUrl(memberQrSrcOf(m), m.card);
          openQrViewer(txt, { avatar: m.avatar, name: nm });
        });
      });
    }
