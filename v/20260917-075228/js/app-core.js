  (function () {
    ...
    function memberNpcEditBody() {
      ...
      html += '<div class="npc-prog-wrap" id="npcProgWrap"><div class="npc-prog-bar"><i class="npc-prog-in" id="npcProgIn"></i></div><span class="npc-prog-tx" id="npcProgTx"></span><button type="button" class="npc-prog-next" id="npcProgNext" style="display:none">继续写下一段</button></div>';
      html += '</div>';
    ...
    /* ⑦ NPC 生成进度条（写人设时给用户一个“正在写”的反馈） */
    var npcProgTimer = null;
    var npcProgBase = 0;
    function npcProgStart(base) {
      try {
        var wrap = memberEditScroll.querySelector('#npcProgWrap');
        var bar = memberEditScroll.querySelector('#npcProgIn');
        var tx = memberEditScroll.querySelector('#npcProgTx');
        var nx = memberEditScroll.querySelector('#npcProgNext');
        if (!wrap || !bar) return;
        wrap.classList.add('on');
        if (nx) nx.style.display = 'none';
        npcProgBase = Math.max(0, Math.min(88, Math.round(base || 0)));
        var p = npcProgBase;
        if (tx) tx.textContent = '正在写这一段…';
        bar.style.width = p + '%';
        if (npcProgTimer) clearInterval(npcProgTimer);
        npcProgTimer = setInterval(function () {
          p = Math.min(npcProgBase + 11, 92, p + Math.random() * 3 + 1.2);
          var b2 = memberEditScroll.querySelector('#npcProgIn');
          if (b2) b2.style.width = p + '%';
          var t2 = memberEditScroll.querySelector('#npcProgTx');
          if (t2) t2.textContent = '正在写这一段… ' + Math.round(p) + '%';
        }, 380);
      } catch (e) {}
    }
    /* v206：一段写完后的进度条 —— 进度＝真实已写段数，右侧给出「继续写第 N 段」按钮 */
    function npcProgSection(done, total, hasNext) {
      try {
        if (npcProgTimer) { clearInterval(npcProgTimer); npcProgTimer = null; }
        var wrap = memberEditScroll.querySelector('#npcProgWrap');
        var bar = memberEditScroll.querySelector('#npcProgIn');
        var tx = memberEditScroll.querySelector('#npcProgTx');
        var nx = memberEditScroll.querySelector('#npcProgNext');
    ...
    function npcProgDone() {
      try {
        ...
        if (nx) nx.style.display = 'none';
        setTimeout(function () {
          var w2 = memberEditScroll.querySelector('#npcProgWrap');
          if (w2) w2.classList.remove('on');
          var b3 = memberEditScroll.querySelector('#npcProgIn');
          if (b3) b3.style.width = '0%';
        }, 900);
      } catch (e) {}
    }
