  (function () {
    ...
    function memberBindEdit() {
      ...
      scroll.onclick = function (e) {
        ...
        if (act === 'albumopen') { openMemberAlbum(memAlbum.length - 1); return; }
    ...
    function memberAlbumHtml() {
      var head = '<div class="mem-album-head"><span class="mem-album-t">角色相册</span><span class="mem-album-c">' + memAlbum.length + ' 张</span>' +
        (memAlbum.length ? '<button type="button" class="mem-album-open" data-mact="albumopen"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M4 9V5.5A1.5 1.5 0 0 1 5.5 4H9M15 4h3.5A1.5 1.5 0 0 1 20 5.5V9M20 15v3.5a1.5 1.5 0 0 1-1.5 1.5H15M9 20H5.5A1.5 1.5 0 0 1 4 18.5V15"/></svg><span>全屏翻阅</span></button>' : '') +
        '</div>';
      if (!memAlbum.length) return head + '<div class="mem-album-empty">点上面「测试生图」，生成的图会自动收进这里</div>';
      return head + '<div class="mem-album-grid">' + memAlbum.map(function (a, i) {
        return '<div class="mem-album-cell" data-mact="albumview" data-val="' + i + '" style="background-image:url(\'' + a.src + '\')"></div>';
      }).join('') + '</div>';
    }
    ...
    function memberGenTest() {
      ...
      generateTestImage(cfg, pos, neg, '', rb, function () {
        ...
        try {
          ...
          if (_gi && _gi.src) memberAlbumAdd(_gi.src, pr ? (pr.name || '') : '');
        } catch (e) {}
