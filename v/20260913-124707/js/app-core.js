  (function () {
    ...
    function memberEditBody(m) {
      ...
      form += '<div class="field"><label>角色ID</label>' +
        ...
        '<div class="mem-qr-label">扫码看名片 · 公开范围</div>' +
        ...
        '</div>' +
        '<div class="mem-field-foot"><span>别人扫二维码打开名片页：全部公开 = 资料全显示；部分公开 = 只显示勾选项；仅私密 = 只显示名字，看不到人设细节，但仍可加 TA 聊天</span></div>' +
        '<div class="mem-qr-label">角色二维码（扫码看名片）</div>' +
        '<div class="mem-qr-row"><input type="text" id="memQrText" value="' + escHtml(memQrText) + '" placeholder="二维码内容，如角色主页 / 身份链接">' +
        '<button type="button" class="mem-ghost" data-mact="qrdef" title="按上面的公开范围生成名片二维码，扫码即可查看 TA 的名片">默认</button>' +
        ...
        '<div class="mem-qr-preview" id="memQrPreview">' + (memQrText ? '<img src="' + memQrUrl(memQrText) + '" alt="角色二维码" data-qzoom="1">' + '<span class="mem-qr-tip">点二维码可放大 / 保存到本地</span>' : '<span class="mem-qr-empty">点“默认”生成含资料的二维码，发给别人扫一扫即可添加 TA 为好友</span>') + '</div>' +
      ...
      if (t) {
        try {
          if (typeof qrcode === 'function') {
            ...
            var build = function (ec) { var q = qrcode(0, ec); q.addData(t); q.make(); return qrToPng(q, 6, 8); };
    ...
    function memberQrSync() {
      ...
      if (pv) pv.innerHTML = memQrText ? '<img src="' + memQrUrl(memQrText) + '" alt="角色二维码" data-qzoom="1">' + '<span class="mem-qr-tip">点二维码可放大 / 保存到本地</span>' : '<span class="mem-qr-empty">填写二维码内容后点“生成”</span>';
    }
    ...
    function memberQrDownload(png, text) {
      ...
      try {
        ...
        toast('已保存：' + fn + '（若没弹出下载，长按图片选\u201c存储图像\u201d）');
      } catch (e) {
    ...
    function openQrViewer(src, text) {
      ...
      memberQrBigPng(text, function (png) {
        ...
        sv1.type = 'button'; sv1.className = 'mqv-btn'; sv1.textContent = '保存到本地';
        ...
        box.appendChild(im); box.appendChild(tx); box.appendChild(bar);
        ov.appendChild(box);
        ...
        sv1.addEventListener('click', function (e) { e.stopPropagation(); memberQrDownload(png, text); });
    ...
    function memberCardUrl(src, card) {
      ...
      if (mode === 'secret') {
        ...
        if (f.lore && src.lore) p.bio = String(src.lore).slice(0, 200);
        if (f.look && src.look) p.look = String(src.look).slice(0, 200);
    ...
    function openAddFriendView() {
      openChatSub('添加好友', '' +
        ...
        '<button type="button" class="mem-ghost ai gen" id="afAlbumBtn">相册扫码</button>' +
        '<button type="button" class="mem-ghost" id="afScanBtn">相机扫码</button>' +
    ...
    function afHandleQrRaw(raw, quiet) {
      raw = String(raw || '').trim();
      if (!raw) return false;
      if (raw.indexOf('aether://') !== 0) {
        afRenderResult('<div class="af-err">认出了二维码，但内容不是好友码：<br>' + escHtml(raw.slice(0, 80)) + '</div>');
        if (!quiet) toast('这张图不是好友码');
        return false;
      }
      var r = parseFriendCode(raw);
      if (r.err) { afRenderResult('<div class="af-err">' + escHtml(r.err) + '</div>'); if (!quiet) toast(r.err); return false; }
      afStopScan();
