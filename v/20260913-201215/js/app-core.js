  (function () {
    ...
    function openRecoPicker() {
      ...
      var items = [{ kind: 'self', name: memberDispName(m), avatar: m.avatar || '', m: m, npc: null }];
      ((m.npcs) || []).forEach(function (n) {
        if (!n || !n.name) return;
        items.push({ kind: 'npc', name: String(n.name), avatar: (n.avatar || m.avatar || ''), m: m, npc: n });
      });
      chatMini('推荐好友 <button type="button" class="tips-q" data-tip="reco">?</button>', '<div class="chat-cfg-tip" style="margin:0 0 8px">让 TA 把谁发给你？</div>' +
        '<div class="chat-mini-list">' + items.map(function (it, i) {
          return '<button class="chat-mini-opt" data-reco="' + i + '">' + escHtml(it.name) + (it.kind === 'npc' ? '（TA 的 ' + escHtml(String((it.npc && it.npc.rel) || 'NPC')) + '）' : '（TA 自己）') + '</button>';
        }).join('') + '</div>', '取消', function () {});
      chatMiniBox.querySelectorAll('[data-reco]').forEach(function (b) {
        b.addEventListener('click', function () {
          var it = items[parseInt(b.getAttribute('data-reco'), 10)];
          chatMiniMask.classList.remove('show');
          if (!it) return;
    ...
    function chatSendImageMenu() {
      ...
      chatMini('发送图片', '<div class="chat-cfg-tip" style="margin:0 0 8px">想给TA发什么？</div><div class="chat-mini-list">' +
        ...
        '<button class="chat-mini-opt" id="imgOptRoleAlbum">TA 用自己的角色相册发我（不消耗生图）</button>' +
      ...
      if (_raB) _raB.addEventListener('click', function () { chatSendFromAlbum(); });
    ...
    function chatMemberOfConv() {
      try {
        var cid = chatCurrentConv ? String(chatCurrentConv.contactId || '') : '';
        if (cid.charAt(0) !== 'm') return null;
        var id = cid.slice(1);
        var list = memberLoad();
    ...
    function chatRoleGenImage() {
      ...
      if (!cfg) { toast('还没有可用的生图配置，先去「设置 → 生图API」添加'); return; }
      var m = chatMemberOfConv();
      chatMini('让 TA 现画一张',
        '<div class="chat-cfg-tip" style="margin:0 0 8px">说说想让 TA 画什么。留空就按' + (m ? 'TA 的外貌与锁定提示词' : '本窗口的生图提示词') + '来画。</div>' +
