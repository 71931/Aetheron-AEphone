  (function () {
    ...
    function memberNorm(m) {
      ...
      var n = {
        ...
        album: Array.isArray(m.album) ? m.album.map(function (x) {
          if (typeof x === 'string') return { id: memberUid(), src: String(x), ts: 0, note: '' };
          x = x || {};
          var _as = String(x.src || '');
          if (!_as) return null;
          return { id: String(x.id || memberUid()), src: _as, ts: Number(x.ts) || 0, note: String(x.note || '').slice(0, 60) };
    ...
    function openMemberEdit(idx) {
      ...
      memAlbum = m ? (Array.isArray(m.album) ? m.album.map(function (x) { return { id: x.id, src: x.src, ts: x.ts, note: x.note }; }) : []) : [];
    ...
    function memberAlbumPersist() {
      ...
      try {
        ...
        l[memEditIdx].album = memAlbum.map(function (a) { return { id: a.id, src: a.src, ts: a.ts, note: a.note }; });
        memberSave(l);
        return true;
    ...
    function memberAlbumHtml() {
      return '<div class="mem-album-entry" data-mact="albumhome" role="button" tabindex="0">' +
        ...
        '<span class="mem-album-entry-txt"><b>角色相册</b><i>' + (memAlbum.length ? (memAlbum.length + ' 张照片 · 点开全屏翻阅、删除，也能当聊天或朋友圈的图发出去') : '还没有照片 · 上方「测试生图」生成的图会自动收进来') + '</i></span>' +
    ...
    function renderMemberAlbumHome(ov) {
      ...
      if (tip) tip.textContent = sel ? '点照片勾选 · 下方可全选 / 删除所选' : '点图看大图 · 右上角 ✕ 单张删除 · 右上角「选择」可批量删';
      if (foot) {
    ...
    function openMemberAlbumHome() {
      ...
      ov.innerHTML = '<div class="mah-head"><button type="button" class="mah-back"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 5l-7 7 7 7"/></svg></button>' +
        '<span class="mah-t">角色相册</span><span class="mah-c"></span>' +
    ...
    function memberAlbumAdd(src, note) {
      if (!src) return;
      ...
      memberAlbumShrink(src, function (out) {
        ...
        memAlbum.push({ id: memberUid(), src: out, ts: Date.now(), note: String(note || '').slice(0, 60) });
        var saved = memberAlbumPersist();
        memDirty = true;
        memberAlbumRefresh();
    ...
    function memberNpcEditBody() {
      ...
      html += '<button type="button" class="primary-btn wb-btn" id="npcAiBtn" style="width:100%;margin-top:8px">AI 生成人设（按十段模板）</button></div>';
      html += '</div>';
    ...
    function memberNpcAiLore() {
      ...
      memberAiAsk('你是世界观与角色设定助手。', user, function (txt, err) {
        if (err || !txt) { toast(err || 'AI 生成失败'); return; }
        if (memNpcDraft) {
          memNpcDraft.lore = String(txt).trim();
          memberNpcSyncForm();
          memberNpcRender();
        }
        toast('已生成 NPC 人设');
    ...
    function renderChatSectionView(sec) {
      ...
      var SEC_META = {
        ...
        sense: { title: '交互感知', items: [
          ...
          { key: 'album', label: '角色相册', desc: 'TA 的相册 · 全屏翻阅 · 长按保存', value: chatConvAlbumLabel() },
          { key: 'roleimg', label: '真实生图', desc: 'TA 主动发图时调生图 API 现画一张；关闭则从 TA 相册挑旧图', value: chatRoleImgSwOn(s) ? '已开启' : '已关闭' },
    ...
    function openConvRoleAlbumHome() {
      try {
        ...
        var role = list[idx];
        if (!(role.album && role.album.length)) { toast('TA 的相册还是空的'); return; }
        memEditIdx = idx;
        memAvatar = role.avatar || '';
        memAlbum = role.album.map(function (x) { return { id: x.id, src: x.src, ts: x.ts, note: x.note }; });
        memDirty = false;
        openMemberAlbumHome();
    ...
    function onChatSetting(key, fromSw) {
      ...
      if (key === 'model') {
        ...
        saveConvs();
        renderChatSettings();
        toast(_rgen.enabled ? '真实生图已开启：TA 主动发图时现画一张' : '真实生图已关闭：TA 发图时从相册挑旧图');
      } else if (key === 'album') {
        openConvRoleAlbum();
