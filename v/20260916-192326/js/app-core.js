  (function () {
    ...
    var NPC_SEC_TIP = [
      '本名 / 年龄 / 身份 / 职业 / 住哪：把身份证式的硬信息写全，别只写一个名字（约 200 字）',
      '身高体型、五官长相、穿衣风格、气质给人什么感觉，最好有一两个能被人记住的特征（约 200 字）',
      '核心性格是什么、说话什么调调、有没有口头禅或小动作，写清他的矛盾之处（约 200 字）',
      '在哪长大、家里什么情况、经历过哪些关键事件、如今处在什么境况（约 200 字）',
      '擅长什么、有什么手艺或特别能力，也可以写他不擅长却硬撑的事（约 200 字）',
      '家人、朋友、同事、在意的人分别是谁，彼此什么关系，有没有过冲突或亏欠（约 200 字）',
      '作息规律、常去的地方、日常爱好和习惯，具体到吃什么、几点睡（约 200 字）',
      '对感情的态度、喜欢什么类型、雷区和底线在哪、亲密时会怎么做（约 200 字）',
      '没告诉过别人的秘密、藏起来的身份或心事，以及被戳破时会怎么反应（约 200 字）',
      '和主角色怎么认识的、如今是什么关系、心里对主角色真实的想法（约 200 字）'
    ];
    ...
    function memberNpcEditBody() {
      ...
      html += '<div class="field"><label>NPC 名字</label><input class="chat-mini-input" id="npcName" maxlength="12" placeholder="例如：林知夏" value="' + escHtml(d.name || '') + '"></div>';
      ...
      html += '<div class="npc-lore-head"><div class="npc-lore-hint">十段都要写满，写不动的段落交给右上角 AI</div>';
    ...
    function npcAiRun(mode, cur, attempt) {
      ...
      var user = '这是主角色的完整人设：\n' + memberNpcMainText() +
        ...
        '只输出一个 JSON 对象，键固定为下列十个段名，值为对应段落文字，不要输出任何解释、不要多余文字：\n' +
      ...
      memberAiAsk('你是世界观与角色设定助手，只输出 JSON。', user, function (txt, err) {
        ...
        if (err || !txt) {
          var fb = cur.slice();
          npcLoreFallbackFill(fb);
          npcLoreTopUp(fb);
          npcAiBusy = false;
          npcLoreFillToForm(fb);
          npcProgDone();
          toast('AI 暂时连不上，已按内置模板把人设补齐，稍后可再点一次润色');
          return;
        }
        ...
        if (still.length) npcLoreFallbackFill(out);
        npcLoreTopUp(out);
        npcAiBusy = false;
        npcLoreFillToForm(out);
        npcProgDone();
        toast(mode === 'polish' ? '人设已润色' : '人设已写好');
      });
    }
    ...
    function renderChatDetailAvatar(conv) {
      try {
        ...
        if (!conv) { el.classList.remove('on'); el.innerHTML = ''; return; }
        var av = '';
        var s = conv.settings || {};
      ...
      chatDetailBody.innerHTML = msgs.map(function (m, idx) {
        ...
        try {
          if (chatCurrentConv && chatCurrentConv.settings && chatCurrentConv.settings.chatMode === 'action' &&
            ...
            var _actPal = ['rgba(124,92,255,.20)', 'rgba(255,138,110,.20)', 'rgba(90,200,250,.20)', 'rgba(255,203,148,.22)', 'rgba(118,182,140,.20)'];
            actBgStyle = ' style="--act-bg:' + _actPal[idx % _actPal.length] + '"';
          }
        } catch (eAB) {}
