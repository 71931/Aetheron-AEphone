...
(function () {
  ...
  var SERVICES = [
    ...
    { key: 'amap', name: '高德地图', desc: '路线 / 周边 / 地名转坐标',
      ...
      steps: '控制台进「应用管理」，新建应用后「添加 Key」，服务平台必须选 Web 服务，复制那串 Key。',
  ...
  function listHtml() {
    ...
    return '<div class="settings-header">' +
      ...
      '<div class="mcp-scroll">' +
        '<div class="mcp-lead">三张卡片，用哪家点哪家。领到 Token 粘进去，点复制，剩下的交给你的客户端。</div>' +
  ...
  function detailHtml(s) {
    ...
    return '<div class="settings-header">' +
      ...
      '<div class="mcp-scroll">' +
        ...
        '</div>' +
        '<div class="mcp-step">' +
          '<div class="mcp-step-title"><span class="mcp-num">1</span>去官方页领 Token</div>' +
        ...
        '</div>' +
        '<div class="mcp-step">' +
          '<div class="mcp-step-title"><span class="mcp-num">3</span>复制配置</div>' +
          '<button class="mcp-primary" id="mcpCopyOne" type="button">复制' + s.name + '配置</button>' +
          '<pre class="mcp-preview" id="mcpPreview"></pre>' +
          '<div class="mcp-hint">粘进客户端后完全退出进程再打开，回列表看到「已连接」就成了。</div>' +
        '</div>' +
        '<div class="mcp-note">' + (on ? '当前已保存一份 Token，重新粘贴会覆盖旧的。' : '填完自动存在这台设备的浏览器里，下次打开还在。') + '</div>' +
      '</div>';
  }
  ...
  function openDetail(key) {
    ...
    inp.addEventListener('input', function () {
      creds[key] = inp.value; saveCreds(creds); renderPreview();
    });
