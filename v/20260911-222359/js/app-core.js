  (function () {
    ...
    function memberEditBody(m) {
      ...
      form += '<div class="field">' + memFieldLabel(6, '角色外貌', '<span class="mem-label-ai-hint">' + memAiBtn('memLookAi', 'AI 从“角色身世”里提取已有外貌描写并润色填充', 'lookai') + '</span>') + '<textarea id="memLook" rows="3" placeholder="外貌描写…可手动写，或点右侧 AI 从身世提取">' + escHtml(m ? m.look : '') + '</textarea></div>';
    ...
    function memberRerender() {
      ...
      var keepIds = { memRealName: 'realName', memNetName: 'netName', memLore: 'lore', memVoiceId: 'voiceId', memVoiceLang: 'voiceLang', memVoiceSpeed: 'voiceSpeed', memLook: 'look', memLookPrompt: 'lookPrompt', memQrText: 'qrText' };
    ...
    function renderChatSettings() {
      ...
      if (chatSettingView === 'voiceinfo') return renderChatVoiceInfoView();
    ...
    function renderChatSectionView(sec) {
      ...
      var SEC_META = {
        ...
        sense: { title: '交互感知', items: [
          { key: 'voiceinfo', label: '语音配置', desc: '音色 · 语言 · 语速（已移步到名单角色）', value: (s.voice && s.voice.enabled) ? '跟随角色' : '未开启' },
    ...
    function chatVoiceApplyMember(conv) {
      try {
        ...
        s.voice.lang = String(v.lang || 'zh-CN');
        s.voice.langLabel = String(v.langLabel || chatLangLabelOf(v.lang || 'zh-CN'));
        s.voice.speed = (v.speed != null && !isNaN(Number(v.speed))) ? Number(v.speed) : 1;
        if (!s.voice.habit || typeof s.voice.habit !== 'object') s.voice.habit = { enabled: true, frequency: '中等', triggers: '', special: '' };
        s.voice.habit.frequency = String(v.freq || '中等');
        return true;
    ...
    function chatVoiceSyncMemberConvs(memberId, voice) {
      try {
        ...
        chatConvs.forEach(function (conv) {
          ...
          s.voice.lang = String(voice.lang || 'zh-CN');
          s.voice.langLabel = String(voice.langLabel || chatLangLabelOf(voice.lang || 'zh-CN'));
          s.voice.speed = (voice.speed != null && !isNaN(Number(voice.speed))) ? Number(voice.speed) : 1;
          if (!s.voice.habit || typeof s.voice.habit !== 'object') s.voice.habit = { enabled: true, frequency: '中等', triggers: '', special: '' };
          s.voice.habit.frequency = String(voice.freq || '中等');
          changed = true;
        });
