  (function () {
    ...
    var NPC_SEC_TIP = ['姓名、年龄、身份、职业这些基本资料', '身高体型、长相、穿着与气质', '核心性格、说话方式、小习惯', '成长经历、关键事件、现在的处境', '擅长的事、特长或特别能力', '家人、朋友、同事、在意的人', '作息、爱好、平时都在做什么', '对感情的态度、喜欢什么、雷区', '没告诉过别人的秘密或隐藏身份', '和你（主角色）怎么认识、现在是什么关系'];
    ...
    function npcSecStatusSync() {
      ...
      for (var i = 0; i < names.length; i++) {
        ...
        st.textContent = ok ? '已填' : '待补';
        st.className = 'npc-sec-st' + (ok ? ' ok' : '');
      }
    }
    ...
    function memberQrDefault() {
      ...
      toast('二维码已生成' + mtxt + '，别人扫码即可加 TA 为好友');
    }
      ...
      memberAiAsk('你是世界观设定助手，输出严格 JSON。', user, function (txt, err) {
        ...
        arr.forEach(function (x) {
          ...
          memNpcs.push({
            ...
            rel: String(x.rel || '').trim(), avatar: '', gender: '', lore: String(x.lore || '').trim()
          });
          _addCnt++;
        });
        memDirty = true;
    ...
    function memberNpcListBody() {
      ...
      }).join('') : '<div class="mem-npc-empty">还没有 NPC，点下方 ＋ 添加</div>';
      var html = '<div class="npc-edit">';
      html += '<div class="npc-list-head"><button type="button" class="npc-list-back" data-mact="npcback">‹ 角色编辑</button><span class="npc-list-title">NPC 名单</span></div>';
      html += '<div class="group-card form-card">';
      html += '<div class="mem-npc-list">' + rows + '</div>';
      /* v200：AI 生成 NPC 的入口收进自建 NPC 编辑页（人设区右上角 AI 键），这里只留一个自建入口 */
      html += '<div class="mem-npc-plus-wrap" id="npcPlusWrap">' +
        '<div class="mem-npc-plus-opts">' +
        ...
        '</div>' +
        '<button type="button" class="mem-npc-plus" id="npcPlusBtn"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 5v14M5 12h14"/></svg><span>添加 NPC</span></button></div>';
      html += '<div class="mem-field-foot"><span>点「添加 NPC」新建自建角色；进编辑页后，人设区右上角的 AI 键可以帮写或润色人设</span></div>';
      html += '</div></div>';
      return html;
    }
    function memberNpcListBind() {
      var pb = document.getElementById('npcPlusBtn');
      if (pb) pb.addEventListener('click', function (e) {
        if (e && e.stopPropagation) e.stopPropagation();
        var w = document.getElementById('npcPlusWrap');
    ...
    function memberNpcEditBody() {
      ...
      var sexChips = [['male', '男'], ['female', '女'], ['secret', '保密']].map(function (x) {
        return '<button type="button" class="npc-chip' + (d.gender === x[0] ? ' on' : '') + '" data-npcact="sex" data-val="' + x[0] + '">' + x[1] + '</button>';
      }).join('');
      var rels = ['朋友', '同事', '同学', '家人', '邻居', '上司', '下属', '宿敌', '青梅竹马'];
      var relHtml = rels.map(function (r) {
        return '<button type="button" class="npc-chip' + (d.rel === r ? ' on' : '') + '" data-npcact="rel" data-val="' + escHtml(r) + '">' + r + '</button>';
      }).join('');
      var html = '<div class="npc-edit">';
      html += '<div class="group-card form-card">';
      ...
      html += '<div class="field"><label>性别</label><div class="npc-chips">' + sexChips + '</div></div>';
      html += '<div class="field"><label>一句话简介</label><input class="chat-mini-input" id="npcBio" maxlength="60" placeholder="例如：楼下咖啡店老板，嘴上嫌弃你其实很照顾你" value="' + escHtml(d.bio || '') + '"></div>';
      html += '<div class="field"><label>与主角色的关系</label><div class="npc-chips">' + relHtml + '</div><input class="chat-mini-input" id="npcRel" style="margin-top:8px" maxlength="30" placeholder="也可以用一句话描述，例如：从小一起长大的邻居" value="' + escHtml(d.rel || '') + '"></div>';
      html += '</div>';
      ...
      html += '<div class="npc-lore-head"><div class="npc-lore-hint">分段填写，写不动的段落交给 AI</div>';
      ...
      for (var si = 0; si < secNames.length; si++) {
        ...
        html += '<textarea class="chat-mini-input npc-sec-ta" id="npcSec' + si + '" rows="2" placeholder="' + escHtml(NPC_SEC_TIP[si] || '') + '">' + escHtml(secVals[si] || '') + '</textarea>';
        html += '</div>';
      }
      html += '</div>';
    ...
    var NPC_SEC_FB = ['{n}，二十出头，有自己的身份和一份安稳的营生。',
      '身形匀称，五官干净，穿着简单，气质偏{ta}。',
      '性子{ta}，说话不爱绕弯，在意什么反倒不肯直说。',
      '在小城里长大，经历过一段难走的路，如今勉强站稳了脚。',
      '擅长和人打交道，手也巧，遇到麻烦总能想出办法。',
      '身边有三两能说上话的人，和你算是{rel}。',
      '作息不算规律，喜欢傍晚一个人待着，偶尔熬夜到很晚。',
      '对感情慢热又认真，最受不了被敷衍，也见不得不告而别。',
      '有件事一直没对任何人讲过，压在心底很久了。',
      '和你是{rel}，从认识到现在，关系比表面看上去要近。'];
    ...
    function npcAiRun(mode, cur, attempt) {
      ...
      var tip;
      if (mode === 'polish') tip = '下面是这个 NPC 现有的十段草稿，请保留草稿里已有的信息与口吻，把每段润色得更具体、更通顺；草稿为空的段落，结合主角色世界观合理补写。';
    ...
    function chatRoleAvatarApply(conv, src) {
      try {
    ...
    function chatRoleMaybeAutoAvatar(conv, imgs) {
      try {
        ...
        var roll = Math.random();
        if (roll < 0.6 && pool.length) return chatRoleAvatarApply(conv, pool[Math.floor(Math.random() * pool.length)]);
        if (roll < 0.85 && alb.length) return chatRoleAvatarApply(conv, alb[Math.floor(Math.random() * alb.length)].src);
        if (mine) {
          chatAvatarCorner(mine, function (url) { if (url) chatRoleAvatarApply(conv, url); });
          return true;
        }
        if (pool.length) return chatRoleAvatarApply(conv, pool[0]);
        return false;
      } catch (e) { return false; }
    }
