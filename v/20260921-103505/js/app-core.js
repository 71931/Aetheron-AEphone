  (function () {
    ...
    function peekOnSend(text, conv) {
      ...
      if (!g.url) {
        peekInsert(conv, 'me', peekWarnCardHtml('通道还没接上', '去「MCP → 掌心窗 → 通道地址」把云端 MCP 地址和 Token 填上，我才能真看到你手机。'));
        return;
      }
      var hit = peekCacheGet(conv);
      if (hit && (Date.now() - hit.ts) < 60 * 1000) {
        peekInsert(conv, 'me', peekStateCardHtml(hit.text, '掌心窗 · 刚读的'));
        return;
      }
      peekRefresh(conv, function (rec, r) {
        if (rec) peekInsert(conv, 'me', peekStateCardHtml(rec.text, '掌心窗'));
        else peekInsert(conv, 'me', peekWarnCardHtml('这次没读到', (r && r.err) || '手机端 App 可能没开着，或者通道没通。'));
      });
    }
