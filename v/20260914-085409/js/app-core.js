  (function () {
    ...
    function memberAlbumHtml() {
      return '<div class="mem-album-entry" data-mact="albumhome" role="button" tabindex="0">' +
        ...
        '<span class="mem-album-entry-txt"><b>相册</b><i>' + (memAlbum.length ? (memAlbum.length + ' 张照片 · 点开全屏翻阅、删除；左下角标着谁存的，长按可设为 TA 的头像') : '还没有照片 · 上方「测试生图」生成的图会自动收进来') + '</i></span>' +
    ...
    function roleAvatarSyncToConv(roleId, src) {
      try {
        ...
        (chatConvs || []).forEach(function (c) {
          ...
          if (String(c.contactId).replace(/^m/, '') !== rid) return;
    ...
    function renderMemberAlbumHome(ov) {
      ...
      if (tip) tip.textContent = sel ? '点照片勾选 · 下方可全选 / 删除所选' : '点图看大图 · 长按可设为 TA 的头像 · 右上角 ✕ 单张删除 · 右上角「选择」可批量删';
      if (foot) {
    ...
    function openMemberAlbumHome() {
      ...
      /* v198：长按相册图 → 把这张设为 TA 的头像（TA 自己也会在发图时自主换头像） */
      ...
      ov.addEventListener('pointerdown', function (e) {
        ...
        _mahLp = setTimeout(function () {
          ...
          if (!window.confirm('把这张照片设为 TA 的头像？')) return;
          ov._lpFired = true;
          if (memberSetAvatarFromAlbum(li)) toast('TA 的头像已换成这张');
    ...
    /* v184：TA 从自己的相册挑一张发给我（不走生图 API，char 也知道自己发的是哪张） */
    /* v194：拆成两条路——① 让 TA 自己挑并主动发（点一下就有图）② 用户从 TA 的相册里挑 */
    function chatRolePickAlbumOne() {
      ...
      if (!list.length) { toast('TA 的相册还是空的：先去「名单 → 角色 → 相册」给 TA 存几张'); return; }
    ...
    function chatPickFromAlbum() {
      ...
      try { if (typeof chatMiniMask !== 'undefined' && chatMiniMask) chatMiniMask.classList.remove('show'); } catch (e2) {}
      openAlbumPicker({
        member: m, multi: false, title: (chatCurrentConv.name || 'TA') + ' 的相册',
    ...
    function memberNpcListBody() {
      ...
      html += '<div class="mem-field-foot"><span>点「添加 NPC」新建，填好名字、与主角色的关系，再补一段完整人设即可</span></div>';
      html += '</div></div>';
      return html;
    }
    ...
    function memberNpcEditBody() {
      ...
      html += '<div class="field"><textarea class="chat-mini-input" id="npcLore" rows="9" style="resize:vertical" placeholder="写这个 NPC 的完整人设：基础档案、外貌特征、性格气质、身世背景、能力特长、人际关系、生活日常、情感观念、隐藏设定、和主角色的关系">' + escHtml(d.lore || '') + '</textarea></div>';
      html += '</div>';
    ...
    var TIP_TEXT = {
      ...
      album: ['角色相册', '<div class="tips-row">TA 的相册。角色生成、收到的图都会存进来，可全屏翻阅、长按保存。</div>']
    };
    ...
    function showChatBubbleBar(row, ev) {
      ...
      if (m.type === 'image' && m.imgs && m.imgs.length) {
        /* v198：聊天里的图（生图/相册发出来的）可以顺手存进 TA 的相册 */
        barBtns += '<button data-act="save2album" style="color:#5ac8fa">存入TA的相册</button>';
      }
      ...
      chatBubbleBar.onclick = function (e) {
        ...
        if (act === 'quote') {
          ...
          copyChatMsg(m);
        } else if (act === 'save2album') {
          chatMsgSaveToRoleAlbum(m);
        }
      };
    ...
    /* v198：把聊天里的图存进 TA 的相册——我发的图标记「我存的」，TA 发的图标记「TA 存的」 */
    function chatMsgSaveToRoleAlbum(m) {
      try {
        if (!chatCurrentConv || !m || !(m.imgs && m.imgs.length)) { toast('这条消息里没有图'); return; }
        var mem = memberByConv(chatCurrentConv);
        if (!mem) { toast('这个聊天没绑定名单角色，存不进 TA 的相册'); return; }
        var list = memberLoad();
        var i = -1;
        for (var k = 0; k < list.length; k++) { if (String(list[k].id) === String(mem.id)) { i = k; break; } }
        if (i < 0) { toast('名单里找不到这个角色'); return; }
        if (!Array.isArray(list[i].album)) list[i].album = [];
        if (list[i].album.length >= MEM_ALBUM_MAX) { toast('TA 的相册已满 ' + MEM_ALBUM_MAX + ' 张，先删掉几张'); return; }
        var src = m.imgs[m.imgs.length - 1];
        var _mine = m.role === 'me';
        list[i].album.push({
          id: memberUid(), src: src, ts: Date.now(),
          note: String((m.picNote || m.text || '') + '').slice(0, 60),
          savedBy: _mine ? 'me' : 'ta'
        });
        memberSave(list);
        ...
        toast('已存入 TA 的相册（' + list[i].album.length + ' 张）');
      } catch (e) { toast('存入相册失败：' + (e && e.message ? e.message : e)); }
    }
    ...
    function chatViewImg(src) { viewImgFull(src); }
    ...
    function renderChatSectionView(sec) {
      ...
      var SEC_META = {
        ...
        sense: { title: '交互感知', items: [
          ...
          { key: 'album', label: '相册', desc: 'TA 的相册 · 点开直接翻看 · 长按某张可设为 TA 的头像', value: chatConvAlbumLabel() },
    ...
    function chatRoleSendOnePhoto() {
      ...
      toast('TA 的相册还是空的，先去「名单」里给 TA 加几张照片');
    }
    ...
    function openConvRoleAlbum() {
      ...
      chatMini('角色相册', '<div class="chat-mini-list">' +
        '<button class="chat-mini-opt" id="cvAlbumView">翻看 TA 的相册（' + (_cnt ? _cnt + ' 张' : '空') + '）</button>' +
