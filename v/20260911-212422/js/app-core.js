  (function () {
    ...
    function memberEditBody(m) {
      ...
      form += '<div class="field">' + memFieldLabel(4, '角色身世') + '<textarea id="memLore" rows="7" placeholder="在这里写下完整角色内容：成长经历、性格、身份、习惯、秘密…">' + escHtml(m ? m.lore : '') + '</textarea>' +
        ...
        '</div></div>';
      var mv = (m && m.voice && typeof m.voice === 'object') ? m.voice : {};
      var mvId = String((m && m.voiceId) ? m.voiceId : (mv.voiceId || ''));
      var mvLang = String(mv.lang || 'zh-CN');
      var mvSpeed = (mv.speed != null && !isNaN(Number(mv.speed))) ? Number(mv.speed) : 1;
      var mvFreq = String(mv.freq || '中等');
      var memLangOpts = chatLangPresets.map(function (l) {
        if (l.sep) return '<hr disabled>';
        return '<option value="' + l.code + '"' + (mvLang === l.code ? ' selected' : '') + '>' + l.label + '</option>';
      }).join('');
      form += '<div class="field">' + memFieldLabel(5, '角色音色与语音') +
        '<input type="text" id="memVoiceId" value="' + escHtml(mvId) + '" placeholder="音色 ID，例如 speech-01-hd / female-soft">' +
        '<div style="margin-top:10px"><span class="mem-voice-lab">语音语言/方言</span>' +
        '<select id="memVoiceLang" class="settings-select" style="width:100%;text-align:left;margin-top:4px">' + memLangOpts + '</select></div>' +
        '<div style="margin-top:10px"><span class="mem-voice-lab">语速 <b id="memVoiceSpeedVal">' + mvSpeed + '</b>x</span>' +
        '<input id="memVoiceSpeed" type="range" min="0.5" max="2" step="0.1" value="' + mvSpeed + '" style="width:100%"></div>' +
        '<div style="margin-top:10px"><span class="mem-voice-lab">主动发语音频率</span>' +
        '<div class="mem-voice-freq" id="memVoiceFreqBox" style="margin-top:4px">' +
          ['低', '中等', '高'].map(function (f, fi) { return (fi ? '<span class="vf-sep">·</span>' : '') + '<button type="button" class="vf-btn' + (mvFreq === f ? ' active' : '') + '" data-mact="voicefreq" data-val="' + f + '">' + f + '</button>'; }).join('') +
        '</div></div>' +
        '<div class="mem-field-foot"><span>语音配置跟着角色走：TA 的语音气泡按这里的音色 / 语言 / 语速发声（语音API在「设置 → 语音」里配置）</span></div>' +
        '</div>';
