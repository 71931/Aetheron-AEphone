  (function () {
    ...
    function npcAiRun(mode, cur, attempt) {
      ...
      var tip;
      if (mode === 'polish') tip = '下面是这个 NPC 现有的十段草稿，请保留草稿里已有的信息与口吻，把每段扩写到 180 字以上、更具体更通顺；草稿为空的段落，结合主角色世界观补写完整，每段同样不少于 180 字。';
      else if (attempt === 0) tip = '请为这个 NPC 写一份完整人设，十段都要写到。';
      else tip = '上一轮有几段没写出来，这次务必补齐这些段：' + miss.map(function (i) { return names[i]; }).join('、') + '；其余段落可沿用前文。';
      /* v203：名字空着就让 AI 顺手取一个；第十段关系必须第三人称写两人怎么认识怎么相处 */
      ...
      var user = '这是主角色的完整人设：\n' + memberNpcMainText() +
        ...
        '\n\n' + tip +
        '\n每段 4~6 句、不少于 180 字，中文，要具体到时间、地点、物件和说话方式，写出血肉与经历，不要空话套话；十段合计不少于 2000 字，贴合主角色世界观与关系。\n' +
        '第十段「' + names[9] + '」必须用第三人称写：直接叙述「' + (memNpcDraft.name || 'TA') + '」和「' + _mainName + '」这两个人是怎么认识的、怎么相处的，禁止用「你」「我」称呼，禁止写成面对面说话的语气。\n' +
        (_needName ? '另外给这个 NPC 起一个贴合世界观的中文名（2~3 字），写在 JSON 的 "npcName" 键里。\n' : '') +
        '只输出一个 JSON 对象，键为下列十个段名' + (_needName ? ' 以及 "npcName"' : '') + '，值为对应段落文字，不要输出任何解释、不要多余文字：\n' +
        npcLoreJsonOf(names, cur);
      memberAiAsk('你是世界观与角色设定助手，只输出 JSON。', user, function (txt, err) {
        /* v200：AI 连不上也不留白——先用内置模板把十段补齐，保证「AI 生成」这件事一定成功 */
        if (err || !txt) {
          /* v203：不再用内置模板兜底写入（那是预设文案），失败就如实说 */
          npcAiBusy = false;
          npcProgDone();
        ...
        var out = cur.slice();
        for (var i = 0; i < names.length; i++) {
          var got = String(arr[i] || '').trim();
          if (!got) continue;
          if (mode === 'polish' || !String(out[i] || '').trim()) out[i] = got;
        }
        try { npcAiFillName(txt); } catch (eNm) {}
        var still = npcLoreMissing(out);
        if (still.length && attempt < 2) { npcAiRun(mode, out, attempt + 1); return; }
        npcAiBusy = false;
        npcLoreFillToForm(out);
        npcProgDone();
        toast(still.length ? ('AI 只写了 ' + (names.length - still.length) + ' 段，剩下几段再点一次 AI') : (mode === 'polish' ? '人设已润色' : '人设已写好'));
      });
    }
    ...
    var CHAT_FUNCS = [
      ...
      { key: 'moment', label: '朋友圈', ico: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><circle cx="12" cy="12" r="4.4"/><path d="M12 3v3M12 18v3M3 12h3M18 12h3"/></svg>' },
      { key: 'reco', label: '推荐好友', ico: '<svg viewBox="0 0 24 24"><path d="M15 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="8.5" cy="7" r="4"/><path d="M19 8v6"/><path d="M22 11h-6"/></svg>' }
    ];
    ...
    function chatRoleStatusLocal(conv) {
      ...
      var ms = (conv && conv.messages) || [];
      var recent = '';
      for (var i = Math.max(0, ms.length - 12); i < ms.length; i++) { var t = ms[i] && ms[i].text; if (t) recent += ' ' + String(t).slice(0, 80); }
    ...
    function chatRoleStatusAi(conv, cb) {
      ...
      try {
        ...
        for (var i = Math.max(0, ms.length - 12); i < ms.length; i++) {
          ...
          recent += (x.role === 'me' ? '我：' : 'TA：') + String(x.text).slice(0, 80) + '\n';
        }
      } catch (e) {}
      var user = '角色人设：\n' + (memberNpcMainText() || '（未填）') +
        '\n角色姓名：' + (conv.name || '未填') +
        '\n现在时间：' + new Date().toLocaleString() +
        '\n最近对话：\n' + (recent || '（暂无）') +
        '\n\n请按上面这些，写出 TA 此刻真实的状态。每项 12~30 字，具体、有画面感，不要写空话套话。' +
    ...
    function chatRoleStatusRefresh() {
      ...
      chatRoleStatusAi(chatCurrentConv, function (err) {
        roleStatusBusy = false;
        if (err) { toast('AI 刷新失败：' + errZh(err)); chatRoleStatusRender(false); return; }
    ...
    function chatRoleStatusRender(busy) {
      ...
      var src = st.aiTs ? ('随对话自动刷新 · 上次重算 ' + new Date(st.aiTs).toLocaleTimeString()) : '随对话自动刷新 · 本地即时推断';
    ...
    function chatRoleStatusOpen() {
      ...
      chatRoleStatusEnsure(chatCurrentConv);
      saveConvs();
      chatRoleStatusRender(false);
    ...
    function chatModePromptLine(s) {
      ...
      if (s.chatMode === 'action') {
        return '【动作描写模式】每条回复拆成两部分：第一行先写动作神态描写，格式严格为：' +
          '[[AETHER_ACTION]]动作神态描写内容（' + nmin + '到' + nmax + '字，写表情、动作、语气、身体反应和细微心理，第三人称，不要加引号）' +
          '。描写写完后换行开始写对话，并且每一句对话都必须单独占一行，一句一行，不要合并成一整段。';
      }
      return '';
    }
    ...
    function onChatFunc(key) {
      if (!chatCurrentConv) return;
      closeChatFuncPanel();
      /* v202：让 TA 把图发到朋友圈（用你刚发的图，或 TA 相册里的图） */
      if (key === 'moment') { chatMomentComposeSend(chatCurrentConv); return; }
      if (key === 'location') {
        locPickOpen();
    ...
    function chatMomentComposeSend(conv) {
      try {
        var _c = conv || chatCurrentConv;
        if (!_c) return;
        var m = null;
        ...
        if (!src) { toast('TA 手里还没有图，先给 TA 发一张'); return; }
        ...
        var post = function (cap) {
          try {
            var data = {
              ...
              content: cap,
              imageContents: [{ image: src, text: '', isManual: false }],
              ipAddress: '', blockedUsers: [], mentionedUsers: []
            };
            var p = { id: generateId(), timestamp: new Date().toISOString(), likes: [], comments: [], type: 'image' };
